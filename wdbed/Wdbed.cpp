/**
	* \file Wdbed.cpp
	* inter-thread exchange object for Wdbe daemon (implementation)
...
void DpchWdbedAck::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchWdbedAck");
...
void DpchWdbedQuit::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchWdbedQuit");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
	xmlTextWriterEndElement(wr);
};
...
void DpchWdbedReg::writeXML(
			xmlTextWriter* wr
			, const ubigint nref
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchWdbedReg");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
...
/******************************************************************************
 class DpchWdbeopdReg
 ******************************************************************************/

DpchWdbeopdReg::DpchWdbeopdReg() :
			DpchWdbe(VecWdbeVDpch::DPCHWDBEOPDREG)
		{
};

bool DpchWdbeopdReg::all(
			const set<uint>& items
		) {
	if (!find(items, PORT)) return false;
	if (!find(items, IXWDBEVOPENGTYPE)) return false;
	if (!find(items, OPPRCN)) return false;

	return true;
};

void DpchWdbeopdReg::readXML(
	...
	string srefIxWdbeVOpengtype;

	bool basefound;

	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchWdbeopdReg");
	else
		basefound = checkXPath(docctx, basexpath);

	if (basefound) {
		if (extractUsmallintUclc(docctx, basexpath, "port", "", port)) add(PORT);
		if (extractStringUclc(docctx, basexpath, "srefIxWdbeVOpengtype", "", srefIxWdbeVOpengtype)) {
			ixWdbeVOpengtype = VecWdbeVOpengtype::getIx(srefIxWdbeVOpengtype);
			add(IXWDBEVOPENGTYPE);
		};
...
/******************************************************************************
 class DpchWdbeopdUnreg
 ******************************************************************************/

DpchWdbeopdUnreg::DpchWdbeopdUnreg() :
			DpchWdbe(VecWdbeVDpch::DPCHWDBEOPDUNREG)
		{
};

bool DpchWdbeopdUnreg::all(
			const set<uint>& items
		) {
...
void DpchWdbeopdUnreg::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchWdbeopdUnreg");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool StgWdbed::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgWdbed");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemWdbed";
...
void StgWdbed::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgWdbed";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemWdbed";
...
DpchEngWdbeAlert* AlrWdbe::prepareAlrAbt(
	...
	if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
		continf.TxtMsg1 = "WhizniumDBE version 1.0.2 released on 11-7-2020";
...
void JobWdbe::addOp(
	...
	// generate squawk
	if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODBSCCTRTD) {
		squawk = SqkWdbeModbsc::getSquawkCtrtd(dbswdbe, (DpchInvWdbeModbscCtrtd*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODBSCFWDCTRTD) {
		squawk = SqkWdbeModbsc::getSquawkFwdctrtd(dbswdbe, (DpchInvWdbeModbscFwdctrtd*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODBSCIMBUFTD) {
		squawk = SqkWdbeModbsc::getSquawkImbuftd(dbswdbe, (DpchInvWdbeModbscImbuftd*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODBSCSYS) {
		squawk = SqkWdbeModbsc::getSquawkSys(dbswdbe, (DpchInvWdbeModbscSys*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODBSCTPLCPY) {
		squawk = SqkWdbeModbsc::getSquawkTplcpy(dbswdbe, (DpchInvWdbeModbscTplcpy*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODBSCUNT) {
		squawk = SqkWdbeModbsc::getSquawkUnt(dbswdbe, (DpchInvWdbeModbscUnt*) inv);
	};
	if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODDETCMDBUS) {
		squawk = SqkWdbeModdet::getSquawkCmdbus(dbswdbe, (DpchInvWdbeModdetCmdbus*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODDETCTRFWDCTR) {
		squawk = SqkWdbeModdet::getSquawkCtrFwdctr(dbswdbe, (DpchInvWdbeModdetCtrFwdctr*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODDETECTR) {
		squawk = SqkWdbeModdet::getSquawkEctr(dbswdbe, (DpchInvWdbeModdetEctr*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODDETEHOSTIF) {
		squawk = SqkWdbeModdet::getSquawkEhostif(dbswdbe, (DpchInvWdbeModdetEhostif*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODDETIMBUF) {
		squawk = SqkWdbeModdet::getSquawkImbuf(dbswdbe, (DpchInvWdbeModdetImbuf*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODDETUNT) {
		squawk = SqkWdbeModdet::getSquawkUnt(dbswdbe, (DpchInvWdbeModdetUnt*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMODDETWIRING) {
		squawk = SqkWdbeModdet::getSquawkWiring(dbswdbe, (DpchInvWdbeModdetWiring*) inv);
	};
	if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMTPMODBSCBU) {
		squawk = SqkWdbeMtpModbscbu::getSquawk(dbswdbe, (DpchInvWdbeMtpModbscbu*) inv);
	};
	if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMTPMODBSCTD) {
		squawk = SqkWdbeMtpModbsctd::getSquawk(dbswdbe, (DpchInvWdbeMtpModbsctd*) inv);
	};
	if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEMTPMODDET) {
		squawk = SqkWdbeMtpModdet::getSquawk(dbswdbe, (DpchInvWdbeMtpModdet*) inv);
	};
	...
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEPLHFPGAEHOSTIF) {
		squawk = SqkWdbePlhfpga::getSquawkEhostif(dbswdbe, (DpchInvWdbePlhfpgaEhostif*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEPLHFPGAFWDCTR) {
		squawk = SqkWdbePlhfpga::getSquawkFwdctr(dbswdbe, (DpchInvWdbePlhfpgaFwdctr*) inv);
	};
	if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEPLHMCUEHOSTIF) {
	...
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEWRDEVSYS) {
		squawk = SqkWdbeWrdev::getSquawkSys(dbswdbe, (DpchInvWdbeWrdevSys*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEWRDEVUNT) {
		squawk = SqkWdbeWrdev::getSquawkUnt(dbswdbe, (DpchInvWdbeWrdevUnt*) inv);
	};
	if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEWRFPGABASE) {
		squawk = SqkWdbeWrfpga::getSquawkBase(dbswdbe, (DpchInvWdbeWrfpgaBase*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEWRFPGACTRFWDCTR) {
		squawk = SqkWdbeWrfpga::getSquawkCtrFwdctr(dbswdbe, (DpchInvWdbeWrfpgaCtrFwdctr*) inv);
	...
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEWRFPGAEHOSTIF) {
		squawk = SqkWdbeWrfpga::getSquawkEhostif(dbswdbe, (DpchInvWdbeWrfpgaEhostif*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEWRFPGAHOSTIF) {
		squawk = SqkWdbeWrfpga::getSquawkHostif(dbswdbe, (DpchInvWdbeWrfpgaHostif*) inv);
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEWRFPGAIPCLR) {
		squawk = SqkWdbeWrfpga::getSquawkIpclr(dbswdbe, (DpchInvWdbeWrfpgaIpclr*) inv);
	...
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEWRMCUCTRFWDCTR) {
		squawk = SqkWdbeWrmcu::getSquawkCtrFwdctr(dbswdbe, (DpchInvWdbeWrmcuCtrFwdctr*) inv);
	...
	} else if (inv->ixWdbeVDpch == VecWdbeVDpch::DPCHINVWDBEWRMCUEHOSTIF) {
		squawk = SqkWdbeWrmcu::getSquawkEhostif(dbswdbe, (DpchInvWdbeWrmcuEhostif*) inv);
...
void StmgrWdbe::commit() {
	for (auto it = stcch->icsVStubNew.begin(); it != stcch->icsVStubNew.end(); it++) {
		if (*it == VecWdbeVStub::STUBWDBEBNKSREF) {
			...
			xchg->addClstnStmgr(VecWdbeVCall::CALLWDBEPRTUPD_REFEQ, jref);
		} else if (*it == VecWdbeVStub::STUBWDBERLSLONG) {
		...
		} else if (*it == VecWdbeVStub::STUBWDBESYSSTD) {
			xchg->addClstnStmgr(VecWdbeVCall::CALLWDBESYSUPD_REFEQ, jref);
		} else if (*it == VecWdbeVStub::STUBWDBETRGSREF) {
			xchg->addClstnStmgr(VecWdbeVCall::CALLWDBETRGUPD_REFEQ, jref);
		} else if (*it == VecWdbeVStub::STUBWDBETRGSTD) {
			xchg->addClstnStmgr(VecWdbeVCall::CALLWDBETRGUPD_REFEQ, jref);
...
NodeWdbe::NodeWdbe(
	...
	for (unsigned int i = 0; i < icsWdbeVOppack.size(); i++) OpengWdbe::getIcsWdbeVDpchByIxWdbeVOppack(icsWdbeVOppack[i], icsWdbeVDpch);
...
/******************************************************************************
 class XchgWdbed::ShrdatJobprc
 ******************************************************************************/

XchgWdbed::ShrdatJobprc::ShrdatJobprc() :
			ShrdatWdbe("XchgWdbe", "ShrdatJobprc")
		{
};

void XchgWdbed::ShrdatJobprc::init(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
		) {
...
void XchgWdbed::ShrdatJobprc::term(
			XchgWdbe* xchg
		) {
...
XchgWdbed::XchgWdbed() :
			cStable("cStable", "XchgWdbed", "XchgWdbed")
			, mLogfile("mLogfile", "XchgWdbed", "XchgWdbed")
			, cOpengcli("cOpengcli", "XchgWdbed", "XchgWdbed")
			, cJobprcs("cJobprcs", "XchgWdbed", "XchgWdbed")
			, cOpprcs("cOpprcs", "XchgWdbed", "XchgWdbed")
			, mReqs("mReqs", "XchgWdbed", "XchgWdbed")
			, orefseq("orefseq")
			, mInvs("mInvs", "XchgWdbed", "XchgWdbed")
			, rwmPresets("rwmPresets", "XchgWdbed", "XchgWdbed")
			, rwmStmgrs("rwmStmgrs", "XchgWdbed", "XchgWdbed")
			, rwmClstns("rwmClstns", "XchgWdbed", "XchgWdbed")
			, rwmDcols("rwmDcols", "XchgWdbed", "XchgWdbed")
			, jrefseq("jrefseq")
			, rwmJobs("rwmJobs", "XchgWdbed", "XchgWdbed")
			, rwmCsjobinfos("rwmCsjobinfos", "XchgWdbed", "XchgWdbed")
			, wrefseq("wrefseq")
			, nrefseq("nrefseq")
			, rwmNodes("rwmNodes", "XchgWdbed", "XchgWdbed")
		{
	// root job
	jrefRoot = 0;
...
void XchgWdbed::startMon() {
	...
	mon.start("WhizniumDBE 1.0.2", stgwdbemonitor.ixDbsVDbstype, stgwdbemonitor.dbspath, stgwdbemonitor.dbsname, stgwdbemonitor.ip, stgwdbemonitor.port, stgwdbemonitor.dbsusername, stgwdbemonitor.dbspassword, stgwdbemonitor.username, stgwdbemonitor.password);
	...
	for (auto it = jobs.begin(); it != jobs.end(); it++) {
		...
		if (dcol) {
			Dcol = true;
			dcol->unlockAccess("XchgWdbed", "startMon");
		} else Dcol = false;

		stmgr = getStmgrByJref(job->jref);
		if (stmgr) {
			Stmgr = true;
			stmgr->unlockAccess("XchgWdbed", "startMon");
		} else Stmgr = false;
	...
	rwmJobs.runlock("XchgWdbed", "startMon");

	rwmClstns.rlock("XchgWdbed", "startMon");
...
void XchgWdbed::appendToLogfile(
	...
	mLogfile.lock("XchgWdbed", "appendToLogfile");
	...
	mLogfile.unlock("XchgWdbed", "appendToLogfile");
};

void XchgWdbed::addReq(
			ReqWdbe* req
		) {
	mReqs.lock("XchgWdbed", "addReq", "jref=" + to_string(req->jref));

	req->ixVState = ReqWdbe::VecVState::WAITPRC;
	reqs.push_back(req);

	mReqs.unlock("XchgWdbed", "addReq", "jref=" + to_string(req->jref));

	cJobprcs.signal("XchgWdbed", "addReq", "jref=" + to_string(req->jref));
};

ReqWdbe* XchgWdbed::pullFirstReq() {
	ReqWdbe* req;

	// get first element from list
	mReqs.lock("XchgWdbed", "pullFirstReq");
	...
	mReqs.unlock("XchgWdbed", "pullFirstReq");

	return req;
};

void XchgWdbed::addInvs(
			DbsWdbe* dbswdbe
			, JobWdbe* job
			, vector<DpchInvWdbe*>& dpchinvs
		) {
	// append to inv list and signal the news (note the double-lock)
	mInvs.lock("XchgWdbed", "addInvs", "jref=" + to_string(job->jref));
	job->mOps.lock("XchgWdbed", "addInvs", "jref=" + to_string(job->jref));
	...
	job->mOps.unlock("XchgWdbed", "addInvs", "jref=" + to_string(job->jref));
	mInvs.unlock("XchgWdbed", "addInvs", "jref=" + to_string(job->jref));

	cOpprcs.broadcast("XchgWdbed", "addInvs", "jref=" + to_string(job->jref));
};

DpchInvWdbe* XchgWdbed::pullFirstInv(
			NodeWdbe* node
		) {
	bool match = false;
	...
	mInvs.lock("XchgWdbed", "pullFirstInv");

	for (auto it = invs.begin(); it != invs.end(); it++) {
		inv = *it;

		if (node->icsWdbeVDpch.find(inv->ixWdbeVDpch) != node->icsWdbeVDpch.end()) {
			// match found
	...
	mInvs.unlock("XchgWdbed", "pullFirstInv");

	if (match) return inv;
	else return NULL;
};

void XchgWdbed::returnInv(
			DpchInvWdbe* dpchinv
		) {
	// un-successful execution of op returns

	// append to inv list and signal the news
	mInvs.lock("XchgWdbed", "returnInv", "oref=" + to_string(dpchinv->oref));
	invs.push_back(dpchinv);
	mInvs.unlock("XchgWdbed", "returnInv", "oref=" + to_string(dpchinv->oref));

	cOpprcs.broadcast("XchgWdbed", "returnInv", "oref=" + to_string(dpchinv->oref));
};

Preset* XchgWdbed::addPreset(
	...
	rwmPresets.wlock("XchgWdbed", "addPreset", "jref=" + to_string(jref));
	...
	rwmPresets.wunlock("XchgWdbed", "addPreset", "jref=" + to_string(jref));

	return(preset);
};

Preset* XchgWdbed::addIxPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
			, const uint ix
		) {
...
Preset* XchgWdbed::addRefsPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
...
Preset* XchgWdbed::addSrefPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
			, const string& sref
		) {
...
Preset* XchgWdbed::addIntvalPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
			, const int intval
		) {
...
Preset* XchgWdbed::addDblvalPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
			, const double dblval
		) {
...
Preset* XchgWdbed::addBoolvalPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
			, const bool boolval
		) {
...
Preset* XchgWdbed::addTxtvalPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
			, const string& txtval
		) {
...
Preset* XchgWdbed::getPresetByPref(
			const presetref_t& pref
		) {
	Preset* preset = NULL;

	rwmPresets.rlock("XchgWdbed", "getPresetByPref");

	auto it = presets.find(pref);
	if (it != presets.end()) preset = it->second;

	rwmPresets.runlock("XchgWdbed", "getPresetByPref");

	return preset;
};

Arg XchgWdbed::getPreset(
			const uint ixWdbeVPreset
			, ubigint jref
		) {
	Arg arg;
	...
	if ( (ixWdbeVPreset == VecWdbeVPreset::PREWDBESYSDATE) || (ixWdbeVPreset == VecWdbeVPreset::PREWDBESYSTIME) || (ixWdbeVPreset == VecWdbeVPreset::PREWDBESYSSTAMP) ) {
		...
		rwmPresets.rlock("XchgWdbed", "getPreset", "jref=" + to_string(jref));
		...
		rwmPresets.runlock("XchgWdbed", "getPreset", "jref=" + to_string(jref));
		rwmJobs.runlock("XchgWdbed", "getPreset", "jref=" + to_string(jref));
	};

	return(arg);
};

uint XchgWdbed::getIxPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
		) {
...
ubigint XchgWdbed::getRefPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixWdbeVPreset, jref);
...
vector<ubigint> XchgWdbed::getRefsPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixWdbeVPreset, jref);
...
string XchgWdbed::getSrefPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixWdbeVPreset, jref);
...
double XchgWdbed::getDblvalPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixWdbeVPreset, jref);
...
bool XchgWdbed::getBoolvalPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixWdbeVPreset, jref);
...
string XchgWdbed::getTxtvalPreset(
			const uint ixWdbeVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixWdbeVPreset, jref);
...
void XchgWdbed::getPresetsByJref(
			const ubigint jref
	...
	rwmPresets.rlock("XchgWdbed", "getPresetsByJref", "jref=" + to_string(jref));
	...
	rwmPresets.runlock("XchgWdbed", "getPresetsByJref", "jref=" + to_string(jref));
};

void XchgWdbed::removePreset(
			const uint ixWdbeVPreset
			, const ubigint jref
		) {
	rwmPresets.wlock("XchgWdbed", "removePreset", "jref=" + to_string(jref) + ",srefIxWdbeVPreset=" + VecWdbeVPreset::getSref(ixWdbeVPreset));
	...
	rwmPresets.wunlock("XchgWdbed", "removePreset", "jref=" + to_string(jref) + ",srefIxWdbeVPreset=" + VecWdbeVPreset::getSref(ixWdbeVPreset));
};

void XchgWdbed::removePresetsByJref(
			const ubigint jref
		) {
	rwmPresets.wlock("XchgWdbed", "removePresetsByJref", "jref=" + to_string(jref));
	...
	rwmPresets.wunlock("XchgWdbed", "removePresetsByJref", "jref=" + to_string(jref));
};

StmgrWdbe* XchgWdbed::addStmgr(
			const ubigint jref
			, const uint ixVNonetype
		) {
	StmgrWdbe* stmgr = NULL;
	...
	if (!stmgr) {
		rwmStmgrs.wlock("XchgWdbed", "addStmgr", "jref=" + to_string(jref));

		stmgr = new StmgrWdbe(this, jref, ixVNonetype);
		stmgrs[jref] = stmgr;

		rwmStmgrs.wunlock("XchgWdbed", "addStmgr", "jref=" + to_string(jref));
...
StmgrWdbe* XchgWdbed::getStmgrByJref(
			const ubigint jref
		) {
	StmgrWdbe* stmgr = NULL;

	rwmStmgrs.rlock("XchgWdbed", "getStmgrByJref", "jref=" + to_string(jref));

	auto it = stmgrs.find(jref);

	if (it != stmgrs.end()) {
		stmgr = it->second;
		stmgr->lockAccess("XchgWdbed", "getStmgrByJref");
	};

	rwmStmgrs.runlock("XchgWdbed", "getStmgrByJref", "jref=" + to_string(jref));

	return(stmgr);
};

void XchgWdbed::removeStmgrByJref(
			const ubigint jref
		) {
	rwmStmgrs.wlock("XchgWdbed", "removeStmgrByJref", "jref=" + to_string(jref));
	...
	rwmStmgrs.wunlock("XchgWdbed", "removeStmgrByJref", "jref=" + to_string(jref));
};

Clstn* XchgWdbed::addClstn(
	...
	rwmClstns.wlock("XchgWdbed", "addClstn", "jref=" + to_string(jref) + ",srefIxWdbeVCall=" + VecWdbeVCall::getSref(ixWdbeVCall));
	...
	rwmClstns.wunlock("XchgWdbed", "addClstn", "jref=" + to_string(jref) + ",srefIxWdbeVCall=" + VecWdbeVCall::getSref(ixWdbeVCall));

	return(clstn);
};

Clstn* XchgWdbed::addIxClstn(
			const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixVJobmask
			, const ubigint jrefTrig
...
Clstn* XchgWdbed::addIxRefSrefClstn(
			const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixVJobmask
			, const ubigint jrefTrig
...
Clstn* XchgWdbed::addClstnStmgr(
	...
	if (!clstn) {
		rwmClstns.wlock("XchgWdbed", "addClstnStmgr", "jref=" + to_string(jref) + ",srefIxWdbeVCall=" + VecWdbeVCall::getSref(ixWdbeVCall));

		clstn = new Clstn(cref, Clstn::VecVJactype::LOCK);
		clstns.insert(pair<clstnref_t,Clstn*>(cref, clstn));
		cref2sClstns.insert(pair<clstnref2_t,clstnref_t>(cref2, cref));

		rwmClstns.wunlock("XchgWdbed", "addClstnStmgr", "jref=" + to_string(jref) + ",srefIxWdbeVCall=" + VecWdbeVCall::getSref(ixWdbeVCall));
...
Clstn* XchgWdbed::getClstnByCref(
			const clstnref_t& cref
		) {
	Clstn* clstn = NULL;

	rwmClstns.rlock("XchgWdbed", "getClstnByCref");

	auto it = clstns.find(cref);
	if (it != clstns.end()) clstn = it->second;

	rwmClstns.runlock("XchgWdbed", "getClstnByCref");

	return clstn;
};

void XchgWdbed::getClstnsByJref(
			const ubigint jref
	...
	rwmClstns.rlock("XchgWdbed", "getClstnsByJref", "jref=" + to_string(jref));
	...
	rwmClstns.runlock("XchgWdbed", "getClstnsByJref", "jref=" + to_string(jref));
};

void XchgWdbed::removeClstns(
			const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixVTarget
		) {
	rwmClstns.wlock("XchgWdbed", "removeClstns", "jref=" + to_string(jref));
	...
	rwmClstns.wunlock("XchgWdbed", "removeClstns", "jref=" + to_string(jref));
};

void XchgWdbed::removeClstnsByJref(
			const ubigint jref
			, const uint ixVTarget
		) {
	rwmClstns.wlock("XchgWdbed", "removeClstnsByJref", "jref=" + to_string(jref));
	...
	rwmClstns.wunlock("XchgWdbed", "removeClstnsByJref", "jref=" + to_string(jref));
};

void XchgWdbed::findJrefsByCall(
			Call* call
	...
	rwmJobs.rlock("XchgWdbed", "findJrefsByCall", "jref=" + to_string(call->jref));
	rwmClstns.rlock("XchgWdbed", "findJrefsByCall", "jref=" + to_string(call->jref));
	...
	rwmClstns.runlock("XchgWdbed", "findJrefsByCall", "jref=" + to_string(call->jref));
	rwmJobs.runlock("XchgWdbed", "findJrefsByCall", "jref=" + to_string(call->jref));
};

void XchgWdbed::triggerCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
	vector<ubigint> jrefTrigs;
	...
	for (unsigned int i = 0; i < jrefs.size(); i++) {
		...
		if (ixVTarget == Clstn::VecVTarget::JOB) {
			...
			if (job) {
				if (ixVJactype != Clstn::VecVJactype::WEAK) {
					if (ixVJactype == Clstn::VecVJactype::LOCK) {
						job->lockAccess("XchgWdbed", "triggerCall");

					} else if (ixVJactype == Clstn::VecVJactype::TRY) {
						if (!job->trylockAccess("XchgWdbed", "triggerCall")) continue;
					};
				};
			...
			if (ixVJactype != Clstn::VecVJactype::WEAK) {
				job = getJobByJref(jref);
				if (job) job->unlockAccess("XchgWdbed", "triggerCall");
			};
			...
			if (stmgr) {
				...
				stmgr->unlockAccess("XchgWdbed", "triggerCall");
			};
...
bool XchgWdbed::triggerArgToArgCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const Arg& argInv
...
bool XchgWdbed::triggerIntvalCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const int intvalInv
...
bool XchgWdbed::triggerBoolvalCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
...
bool XchgWdbed::triggerIxRefCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerIxSrefCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerIxSrefToIxCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerIxIntvalCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerIxDblvalCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerIxTxtvalCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerSrefCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const string& srefInv
...
bool XchgWdbed::triggerToBoolvalCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, bool& boolvalRet
...
bool XchgWdbed::triggerIxToBoolvalCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerRefToSrefCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const ubigint refInv
...
bool XchgWdbed::triggerRefToBoolvalCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const ubigint refInv
...
bool XchgWdbed::triggerIxRefToIxCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerIxRefToRefCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerIxRefSrefCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerIxRefSrefIntvalCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerIxRefSrefIntvalToRefCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgWdbed::triggerRefToIxCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const ubigint refInv
...
bool XchgWdbed::triggerSrefToRefCall(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVCall
			, const ubigint jref
			, const string& srefInv
...
DcolWdbe* XchgWdbed::addDcol(
	...
	rwmJobs.rlock("XchgWdbed", "addDcol", "jref=" + to_string(jref));
	...
	if (job) {
		...
		if (!dcol) {
			rwmDcols.wlock("XchgWdbed", "addDcol", "jref=" + to_string(jref));

			dcol = new DcolWdbe(jref, job->ixWdbeVLocale);
			dcols[jref] = dcol;

			dcol->lockAccess("XchgWdbed", "addDcol");

			rwmDcols.wunlock("XchgWdbed", "addDcol", "jref=" + to_string(jref));

			mon.eventAddDcol(jref);
		};
	};

	rwmJobs.runlock("XchgWdbed", "addDcol", "jref=" + to_string(jref));
...
DcolWdbe* XchgWdbed::getDcolByJref(
			ubigint jref
			, const bool tree
		) {
	Jobinfo* jobinfo = NULL;
	...
	rwmJobs.rlock("XchgWdbed", "getDcolByJref", "jref=" + to_string(jref));
	rwmDcols.rlock("XchgWdbed", "getDcolByJref", "jref=" + to_string(jref));
	...
	while (jobinfo != NULL) {
		...
		if (it != dcols.end()) {
			dcol = it->second;
			dcol->lockAccess("XchgWdbed", "getDcolByJref");
	...
	rwmDcols.runlock("XchgWdbed", "getDcolByJref", "jref=" + to_string(jref));
	rwmJobs.runlock("XchgWdbed", "getDcolByJref", "jref=" + to_string(jref));

	return(dcol);
};

void XchgWdbed::removeDcolByJref(
			const ubigint jref
		) {
	rwmDcols.wlock("XchgWdbed", "removeDcolByJref", "jref=" + to_string(jref));
	...
	rwmDcols.wunlock("XchgWdbed", "removeDcolByJref", "jref=" + to_string(jref));
};

void XchgWdbed::removeDcolsByJref(
			const ubigint jref
		) {
	Jobinfo* jobinfo = NULL;
	...
	rwmJobs.rlock("XchgWdbed", "removeDcolsByJref", "jref=" + to_string(jref));
	rwmDcols.wlock("XchgWdbed", "removeDcolsByJref", "jref=" + to_string(jref));
	...
	rwmDcols.wunlock("XchgWdbed", "removeDcolsByJref", "jref=" + to_string(jref));
	rwmJobs.runlock("XchgWdbed", "removeDcolsByJref", "jref=" + to_string(jref));
};

void XchgWdbed::submitDpch(
			DpchEngWdbe* dpcheng
		) {
	DcolWdbe* dcol = NULL;
	...
	if (dcol) {
		...
		if (dcol->req != NULL) {
			...
			dcol->req = NULL;
			//cout << "XchgWdbed::submitDpch() waiting request for ixWdbeVDpch = " << dpcheng->ixWdbeVDpch << ", jref = " << dpcheng->jref << endl;

			cReady_backup->signal("XchgWdbed", "submitDpch", "jref=" + to_string(dcol->jref));

			dcol->unlockAccess("XchgWdbed", "submitDpch");
			...
			for (auto it = dcol->dpchs.begin(); it != dcol->dpchs.end();) {
				...
				if ((dpchtest->ixWdbeVDpch == dpcheng->ixWdbeVDpch) && (dpchtest->jref == dpcheng->jref)) {
					...
					//cout << "XchgWdbed::submitDpch() merge for ixWdbeVDpch = " << dpcheng->ixWdbeVDpch << ", jref = " << dpcheng->jref << endl;
			...
			if (dpcheng) {
				...
				//cout << "XchgWdbed::submitDpch() append for ixWdbeVDpch = " << dpcheng->ixWdbeVDpch << ", jref = " << dpcheng->jref << endl;
			};
...
DpchEngWdbe* XchgWdbed::pullFirstDpch(
			DcolWdbe* dcol
		) {
...
ubigint XchgWdbed::addJob(
	...
	rwmJobs.wlock("XchgWdbed", "addJob", "jrefSup=" + to_string(jrefSup));
	...
	if (csjobNotJob) {
		rwmCsjobinfos.wlock("XchgWdbed", "addJob", "jref=" + to_string(job->jref));
		...
		rwmCsjobinfos.wunlock("XchgWdbed", "addJob", "jref=" + to_string(job->jref));
	};

	rwmJobs.wunlock("XchgWdbed", "addJob", "jref=" + to_string(job->jref));
...
JobWdbe* XchgWdbed::getJobByJref(
			const ubigint jref
		) {
	JobWdbe* job = NULL;

	rwmJobs.rlock("XchgWdbed", "getJobByJref", "jref=" + to_string(jref));

	auto it = jobs.find(jref);
	if (it != jobs.end()) job = it->second;

	rwmJobs.runlock("XchgWdbed", "getJobByJref", "jref=" + to_string(jref));

	return job;
};

Jobinfo* XchgWdbed::getJobinfoByJref(
			const ubigint jref
		) {
	Jobinfo* jobinfo = NULL;

	rwmJobs.rlock("XchgWdbed", "getJobinfoByJref", "jref=" + to_string(jref));

	auto it = jobinfos.find(jref);
	if (it != jobinfos.end()) jobinfo = it->second;

	rwmJobs.runlock("XchgWdbed", "getJobinfoByJref", "jref=" + to_string(jref));

	return jobinfo;
};

void XchgWdbed::removeJobByJref(
			const ubigint jref
		) {
	JobWdbe* job = NULL;
	Jobinfo* jobinfo = NULL;
	...
	rwmJobs.rlock("XchgWdbed", "removeJobByJref[1]", "jref=" + to_string(jref));
	...
	if (job) {
		...
		if (csjobNotJob) {
			rwmCsjobinfos.rlock("XchgWdbed", "removeJobByJref[1]", "jref=" + to_string(jref));
			...
			rwmCsjobinfos.runlock("XchgWdbed", "removeJobByJref[1]", "jref=" + to_string(jref));

			rwmCsjobinfos.wlock("XchgWdbed", "removeJobByJref[2]", "jref=" + to_string(jref));
			...
			rwmCsjobinfos.wunlock("XchgWdbed", "removeJobByJref[2]", "jref=" + to_string(jref));
		};

		jobinfo = jobinfos[jref];

		rwmJobs.runlock("XchgWdbed", "removeJobByJref[1]", "jref=" + to_string(jref));

		rwmJobs.wlock("XchgWdbed", "removeJobByJref[2]", "jref=" + to_string(jref));

		if (jobinfo->jrefSup != 0) jobinfos[jobinfo->jrefSup]->jrefsSub.erase(jref);

		rwmJobs.wunlock("XchgWdbed", "removeJobByJref[2]", "jref=" + to_string(jref));

		while (jobinfo->jrefsSub.size() > 0) delete jobs[*(jobinfo->jrefsSub.begin())];

		rwmJobs.wlock("XchgWdbed", "removeJobByJref[3]", "jref=" + to_string(jref));
		...
		rwmJobs.wunlock("XchgWdbed", "removeJobByJref[3]", "jref=" + to_string(jref));

		mon.eventRemoveJob(jref);

	} else rwmJobs.wunlock("XchgWdbed", "removeJobByJref[4]", "jref=" + to_string(jref));
};

bool XchgWdbed::getJobSup(
			const ubigint jrefSup
			, ubigint jref
		) {
	bool retval = false;
...
void XchgWdbed::setJobStage(
	...
	rwmJobs.rlock("XchgWdbed", "setJobStage", "jref=" + to_string(job->jref));
	if (csjobNotJob) rwmCsjobinfos.rlock("XchgWdbed", "setJobStage", "jref=" + to_string(job->jref));
	...
	if (csjobNotJob) rwmCsjobinfos.runlock("XchgWdbed", "setJobStage", "jref=" + to_string(job->jref));
	rwmJobs.runlock("XchgWdbed", "setJobStage", "jref=" + to_string(job->jref));

	triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBESGECHG, job->jref, ixVSge);
};

void XchgWdbed::addCsjobClaim(
			DbsWdbe* dbswdbe
			, CsjobWdbe* csjob
			, Claim* claim
		) {
	...
	if (!csjob->srvNotCli && csjob->srv) {
		rwmCsjobinfos.rlock("XchgWdbed", "addCsjobClaim", "jref=" + to_string(csjob->jref));

		csjobinfo = csjobinfos[csjob->ixWdbeVJob];

		rwmCsjobinfos.runlock("XchgWdbed", "addCsjobClaim", "jref=" + to_string(csjob->jref));

		csjob->srv->lockAccess("XchgWdbed", "addCsjobClaim");

		csjobinfo->mClaims.lock("XchgWdbed", "addCsjobClaim", "jref=" + to_string(csjob->jref));
		...
		csjobinfo->mClaims.unlock("XchgWdbed", "addCsjobClaim", "jref=" + to_string(csjob->jref));

		csjob->srv->unlockAccess("XchgWdbed", "addCsjobClaim");

		if (mod) triggerCall(dbswdbe, VecWdbeVCall::CALLWDBECLAIMCHG, csjob->jref);
	};
};

void XchgWdbed::getCsjobClaim(
			CsjobWdbe* csjob
	...
	if (!csjob->srvNotCli && csjob->srv) {
		rwmCsjobinfos.rlock("XchgWdbed", "getCsjobClaim", "jref=" + to_string(csjob->jref));

		csjobinfo = csjobinfos[csjob->ixWdbeVJob];

		csjobinfo->mClaims.lock("XchgWdbed", "getCsjobClaim", "jref=" + to_string(csjob->jref));
		...
		csjobinfo->mClaims.unlock("XchgWdbed", "getCsjobClaim", "jref=" + to_string(csjob->jref));

		rwmCsjobinfos.runlock("XchgWdbed", "getCsjobClaim", "jref=" + to_string(csjob->jref));
	};
};
...
void XchgWdbed::clearCsjobRun(
	...
	rwmCsjobinfos.rlock("XchgWdbed", "clearCsjobRun", "srefIxWdbeVJob=" + VecWdbeVJob::getSref(ixWdbeVJob));

	auto it = csjobinfos.find(ixWdbeVJob);
	if (it != csjobinfos.end()) csjobinfo = it->second;

	rwmCsjobinfos.runlock("XchgWdbed", "clearCsjobRun", "srefIxWdbeVJob=" + VecWdbeVJob::getSref(ixWdbeVJob));

	if (csjobinfo) {
		csjobinfo->mClaims.lock("XchgWdbed", "clearCsjobRun", "srefIxWdbeVJob=" + VecWdbeVJob::getSref(ixWdbeVJob));
		...
		csjobinfo->mClaims.unlock("XchgWdbed", "clearCsjobRun", "srefIxWdbeVJob=" + VecWdbeVJob::getSref(ixWdbeVJob));

		if (mod) triggerCall(dbswdbe, VecWdbeVCall::CALLWDBECLAIMCHG, csjobinfo->jrefSrv);
	};
};

void XchgWdbed::removeCsjobClaim(
			DbsWdbe* dbswdbe
			, CsjobWdbe* csjob
		) {
	Csjobinfo* csjobinfo = NULL;;
	...
	if (!csjob->srvNotCli && csjob->srv) {
		rwmCsjobinfos.rlock("XchgWdbed", "removeCsjobClaim", "jref=" + to_string(csjob->jref));

		csjobinfo = csjobinfos[csjob->ixWdbeVJob];

		rwmCsjobinfos.runlock("XchgWdbed", "removeCsjobClaim", "jref=" + to_string(csjob->jref));

		csjob->srv->lockAccess("XchgWdbed", "removeCsjobClaim");

		csjobinfo->mClaims.lock("XchgWdbed", "removeCsjobClaim", "jref=" + to_string(csjob->jref));
		...
		csjobinfo->mClaims.unlock("XchgWdbed", "removeCsjobClaim", "jref=" + to_string(csjob->jref));

		csjob->srv->unlockAccess("XchgWdbed", "removeCsjobClaim");

		if (mod) triggerCall(dbswdbe, VecWdbeVCall::CALLWDBECLAIMCHG, csjobinfo->jrefSrv);
	};
};

bool XchgWdbed::getCsjobNotJob(
			const uint ixWdbeVJob
		) {
	bool retval;

	rwmCsjobinfos.rlock("XchgWdbed", "getCsjobNotJob", "srefIxWdbeVJob=" + VecWdbeVJob::getSref(ixWdbeVJob));

	retval = (csjobinfos.find(ixWdbeVJob) != csjobinfos.end());

	rwmCsjobinfos.runlock("XchgWdbed", "getCsjobNotJob", "srefIxWdbeVJob=" + VecWdbeVJob::getSref(ixWdbeVJob));

	return retval;
};

set<ubigint> XchgWdbed::getCsjobClisByJref(
			const ubigint jref
		) {
	set<ubigint> retval;

	rwmCsjobinfos.rlock("XchgWdbed", "getCsjobClis", "jref=" + to_string(jref));

	auto it = csjobinfos.find(ixWdbeVJobs[jref]);
	
	if (it != csjobinfos.end()) retval = it->second->jrefsCli;

	rwmCsjobinfos.runlock("XchgWdbed", "getCsjobClis", "jref=" + to_string(jref));
...
ubigint XchgWdbed::addWakeup(
	...
	if (deltat == 0) {
		...
		if (res != 0) cout << "XchgWdbed::addWakeup() error creating timer thread (" << res << ")" << endl;
		res = pthread_detach(timer);
		if (res != 0) cout << "XchgWdbed::addWakeup() error detaching timer thread (" << res << ")" << endl;
	};

	return(wref);
};

void* XchgWdbed::runWakeup(
			void* arg
		) {
...
NodeWdbe* XchgWdbed::addNode(
	...
	rwmNodes.wlock("XchgWdbed", "addNode", "ip:port=" + ip + ":" + to_string(port));

	node = new NodeWdbe(nrefseq.getNewRef(), ip, port, opprcn, ixWdbeVOpengtype);
	nodes[node->nref] = node;

	rwmNodes.wunlock("XchgWdbed", "addNode", "nref=" + to_string(node->nref));

	mon.eventAddNode(node->nref, ip, port, opprcn);

	setNodeState(node, NodeWdbe::VecVState::IDLE);

	triggerCall(NULL, VecWdbeVCall::CALLWDBENODECHG, jrefRoot);

	return(node);
};

NodeWdbe* XchgWdbed::getNodeByNref(
			const ubigint nref
		) {
	NodeWdbe* node = NULL;

	rwmNodes.rlock("XchgWdbed", "getNodeByNref", "nref=" + to_string(nref));

	auto it = nodes.find(nref);
	if (it != nodes.end()) node = it->second;

	rwmNodes.runlock("XchgWdbed", "getNodeByNref", "nref=" + to_string(nref));

	return(node);
};

void XchgWdbed::terminateNodes() {
	rwmNodes.wlock("XchgWdbed", "terminateNodes");

	for (auto it = nodes.begin(); it != nodes.end(); it++) terminateNodeByNref(it->first);

	rwmNodes.wunlock("XchgWdbed", "terminateNodes");
};

void XchgWdbed::terminateNodeByNref(
			const ubigint nref
		) {
	NodeWdbe* node = NULL;

	rwmNodes.wlock("XchgWdbed", "terminateNodeByNref", "nref=" + to_string(nref));

	node = getNodeByNref(nref);
	if (node) setNodeState(node, NodeWdbe::VecVState::TERM);

	rwmNodes.wunlock("XchgWdbed", "terminateNodeByNref", "nref=" + to_string(nref));
};

void XchgWdbed::setNodeState(
			NodeWdbe* node
			, const uint ixVState
		) {
	cOpengcli.lockMutex("XchgWdbed", "setNodeState", "nref=" + to_string(node->nref));
	rwmNodes.wlock("XchgWdbed", "setNodeState", "nref=" + to_string(node->nref));

	if ( ((ixVState == NodeWdbe::VecVState::ERROR) && (node->ixVState == NodeWdbe::VecVState::READY)) || (ixVState == NodeWdbe::VecVState::TERM)) node->ixVState = ixVState;

	rwmNodes.wunlock("XchgWdbed", "setNodeState", "nref=" + to_string(node->nref));
	cOpengcli.unlockMutex("XchgWdbed", "setNodeState", "nref=" + to_string(node->nref));

	cOpengcli.signal("XchgWdbed", "setNodeState", "nref=" + to_string(node->nref));
};
