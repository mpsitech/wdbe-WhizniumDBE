/**
	...
	* API code for job PnlWdbeGenList (implementation)
...
bool PnlWdbeGenList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWdbeGenList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWdbeGenList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWdbeGenList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeGenList::Tag::Tag(
			...
			, TcoMmx(TcoMmx)
		{
	mask = {CPT, TXTFOR, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOSRF, TCOMDL, TCOHTY, TCOWID, TCOMMX};
};

bool PnlWdbeGenList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeGenList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeGenList::DpchAppData::DpchAppData(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEGENLISTDATA, scrJref)
		{
...
void PnlWdbeGenList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeGenListData");
...
PnlWdbeGenList::DpchAppDo::DpchAppDo(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEGENLISTDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void PnlWdbeGenList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeGenListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWdbeGenList::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEGENLISTDATA)
		{
...
void PnlWdbeGenList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeGenListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWdbeGenList::StatApp();
		statshrqry = QryWdbeGenList::StatShr();
