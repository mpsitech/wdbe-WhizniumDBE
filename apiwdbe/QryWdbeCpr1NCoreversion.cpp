/**
	...
	* API code for job QryWdbeCpr1NCoreversion (implementation)
...
QryWdbeCpr1NCoreversion::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeCpr1NCoreversion::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeCpr1NCoreversion");
	else
...
set<uint> QryWdbeCpr1NCoreversion::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeCpr1NCoreversion::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeCpr1NCoreversion::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeCpr1NCoreversion");
	else
		basefound = checkXPath(docctx, basexpath);
