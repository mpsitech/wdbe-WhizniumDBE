/**
	...
	* API code for job QryWdbeUntRef1NCommand (implementation)
...
QryWdbeUntRef1NCommand::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeUntRef1NCommand::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeUntRef1NCommand");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeUntRef1NCommand";
...
set<uint> QryWdbeUntRef1NCommand::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeUntRef1NCommand::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeUntRef1NCommand::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeUntRef1NCommand");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeUntRef1NCommand";
