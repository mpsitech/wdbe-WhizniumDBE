/**
	* \file PnlWdbeRlsHeadbar.cpp
	* API code for job PnlWdbeRlsHeadbar (implementation)
...
PnlWdbeRlsHeadbar::StgInf::StgInf(
...
bool PnlWdbeRlsHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWdbeRlsHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWdbeRlsHeadbar";
...
set<uint> PnlWdbeRlsHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeRlsHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWdbeRlsHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeRlsHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWdbeRlsHeadbar";
...
PnlWdbeRlsHeadbar::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBERLSHEADBARDATA)
		{
};
...
void PnlWdbeRlsHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeRlsHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
