/**
	...
	* API code for job PnlWdbeCvrAIp (implementation)
...
bool PnlWdbeCvrAIp::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWdbeCvrAIp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfWdbeCvrAIp";
...
bool PnlWdbeCvrAIp::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWdbeCvrAIp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppWdbeCvrAIp";
...
set<uint> PnlWdbeCvrAIp::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCvrAIp::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWdbeCvrAIp::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWdbeCvrAIp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrWdbeCvrAIp";
...
PnlWdbeCvrAIp::Tag::Tag(
			...
			, TcoCmt(TcoCmt)
		{
	mask = {CPT, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOSRF, TCOTTY, TCOCMT};
};

bool PnlWdbeCvrAIp::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeCvrAIp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWdbeCvrAIp";
...
PnlWdbeCvrAIp::DpchAppData::DpchAppData(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECVRAIPDATA, scrJref)
		{
...
void PnlWdbeCvrAIp::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeCvrAIpData");
...
PnlWdbeCvrAIp::DpchAppDo::DpchAppDo(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECVRAIPDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void PnlWdbeCvrAIp::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeCvrAIpDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWdbeCvrAIp::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBECVRAIPDATA)
		{
...
void PnlWdbeCvrAIp::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeCvrAIpData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWdbeCvrAIp::StatApp();
		statshrqry = QryWdbeCvrAIp::StatShr();
