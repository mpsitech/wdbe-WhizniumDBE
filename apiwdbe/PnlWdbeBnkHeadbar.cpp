/**
	* \file PnlWdbeBnkHeadbar.cpp
	* API code for job PnlWdbeBnkHeadbar (implementation)
...
PnlWdbeBnkHeadbar::StgInf::StgInf(
...
bool PnlWdbeBnkHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWdbeBnkHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWdbeBnkHeadbar";
...
set<uint> PnlWdbeBnkHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeBnkHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWdbeBnkHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeBnkHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWdbeBnkHeadbar";
...
PnlWdbeBnkHeadbar::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEBNKHEADBARDATA)
		{
};
...
void PnlWdbeBnkHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeBnkHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
