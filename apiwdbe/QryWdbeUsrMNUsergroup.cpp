/**
	...
	* API code for job QryWdbeUsrMNUsergroup (implementation)
...
QryWdbeUsrMNUsergroup::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeUsrMNUsergroup::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeUsrMNUsergroup");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeUsrMNUsergroup";
...
set<uint> QryWdbeUsrMNUsergroup::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeUsrMNUsergroup::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeUsrMNUsergroup::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeUsrMNUsergroup");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeUsrMNUsergroup";
