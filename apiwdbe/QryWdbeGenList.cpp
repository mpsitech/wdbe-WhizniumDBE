/**
	...
	* API code for job QryWdbeGenList (implementation)
...
QryWdbeGenList::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeGenList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeGenList");
	else
...
set<uint> QryWdbeGenList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeGenList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeGenList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeGenList");
	else
		basefound = checkXPath(docctx, basexpath);
