/**
	* \file PnlWdbeCvrAPlh.cpp
	* API code for job PnlWdbeCvrAPlh (implementation)
...
bool PnlWdbeCvrAPlh::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWdbeCvrAPlh");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfWdbeCvrAPlh";
...
PnlWdbeCvrAPlh::StatApp::StatApp(
...
bool PnlWdbeCvrAPlh::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWdbeCvrAPlh");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppWdbeCvrAPlh";
...
set<uint> PnlWdbeCvrAPlh::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCvrAPlh::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWdbeCvrAPlh::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWdbeCvrAPlh");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrWdbeCvrAPlh";
...
PnlWdbeCvrAPlh::Tag::Tag(
			...
			, TcoCmt(TcoCmt)
		{
	mask = {CPT, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOSRF, TCOCMT};
};

bool PnlWdbeCvrAPlh::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeCvrAPlh");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWdbeCvrAPlh";
...
PnlWdbeCvrAPlh::DpchAppData::DpchAppData(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECVRAPLHDATA, scrJref)
		{
...
void PnlWdbeCvrAPlh::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeCvrAPlhData");
...
PnlWdbeCvrAPlh::DpchAppDo::DpchAppDo(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECVRAPLHDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void PnlWdbeCvrAPlh::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeCvrAPlhDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWdbeCvrAPlh::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBECVRAPLHDATA)
		{
...
void PnlWdbeCvrAPlh::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeCvrAPlhData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWdbeCvrAPlh::StatApp();
		statshrqry = QryWdbeCvrAPlh::StatShr();
