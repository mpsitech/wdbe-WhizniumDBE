/**
	* \file CrdWdbePrj.cpp
	* API code for job CrdWdbePrj (implementation)
...
bool CrdWdbePrj::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWdbePrj");
	else
...
bool CrdWdbePrj::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWdbePrj");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> CrdWdbePrj::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdWdbePrj::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdWdbePrj::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWdbePrj");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "scrJrefDlgimpex", scrJrefDlgimpex)) add(SCRJREFDLGIMPEX);
		...
		if (extractBoolAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "MitCrdIpxAvail", MitCrdIpxAvail)) add(MITCRDIPXAVAIL);
	};
...
set<uint> CrdWdbePrj::StatShr::comm(
	...
	if (scrJrefDlgimpex == comp->scrJrefDlgimpex) insert(items, SCRJREFDLGIMPEX);
...
set<uint> CrdWdbePrj::StatShr::diff(
	...
	diffitems = {SCRJREFDLGIMPEX, SCRJREFDLGNEW, SCRJREFHEADBAR, SCRJREFLIST, SCRJREFREC, MSPCRD1AVAIL, MITCRDNEWAVAIL, MITCRDIPXAVAIL};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
bool CrdWdbePrj::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbePrj");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "MitCrdIpx", MitCrdIpx)) add(MITCRDIPX);
	};
...
void CrdWdbePrj::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbePrjDo");
...
CrdWdbePrj::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEPRJDATA)
		{
...
void CrdWdbePrj::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbePrjData");
	else
		basefound = checkXPath(docctx, basexpath);
