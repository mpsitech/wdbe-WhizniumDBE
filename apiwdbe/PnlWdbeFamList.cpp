/**
	...
	* API code for job PnlWdbeFamList (implementation)
...
bool PnlWdbeFamList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWdbeFamList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWdbeFamList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWdbeFamList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeFamList::Tag::Tag(
			...
			, TcoTit(TcoTit)
		{
	mask = {CPT, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOVND, TCOTIT};
};

bool PnlWdbeFamList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeFamList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeFamList::DpchAppData::DpchAppData(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEFAMLISTDATA, scrJref)
		{
...
void PnlWdbeFamList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeFamListData");
...
PnlWdbeFamList::DpchAppDo::DpchAppDo(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEFAMLISTDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void PnlWdbeFamList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeFamListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWdbeFamList::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEFAMLISTDATA)
		{
...
void PnlWdbeFamList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeFamListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWdbeFamList::StatApp();
		statshrqry = QryWdbeFamList::StatShr();
