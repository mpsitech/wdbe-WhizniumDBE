/**
	...
	* API code for job QryWdbePrcMge1NSignal (implementation)
...
QryWdbePrcMge1NSignal::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbePrcMge1NSignal::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbePrcMge1NSignal");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbePrcMge1NSignal";
...
set<uint> QryWdbePrcMge1NSignal::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbePrcMge1NSignal::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbePrcMge1NSignal::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbePrcMge1NSignal");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbePrcMge1NSignal";
