/**
	* \file PnlWdbePrcMge1NSignal.cpp
	* API code for job PnlWdbePrcMge1NSignal (implementation)
...
bool PnlWdbePrcMge1NSignal::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWdbePrcMge1NSignal");
	else
...
PnlWdbePrcMge1NSignal::StatApp::StatApp(
...
bool PnlWdbePrcMge1NSignal::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWdbePrcMge1NSignal");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> PnlWdbePrcMge1NSignal::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbePrcMge1NSignal::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWdbePrcMge1NSignal::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWdbePrcMge1NSignal");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWdbePrcMge1NSignal::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbePrcMge1NSignal");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbePrcMge1NSignal::DpchAppData::DpchAppData(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEPRCMGE1NSIGNALDATA, scrJref)
		{
...
void PnlWdbePrcMge1NSignal::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbePrcMge1NSignalData");
...
PnlWdbePrcMge1NSignal::DpchAppDo::DpchAppDo(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEPRCMGE1NSIGNALDO, scrJref)
		{
...
void PnlWdbePrcMge1NSignal::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbePrcMge1NSignalDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWdbePrcMge1NSignal::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEPRCMGE1NSIGNALDATA)
		{
...
void PnlWdbePrcMge1NSignal::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbePrcMge1NSignalData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWdbePrcMge1NSignal::StatApp();
		statshrqry = QryWdbePrcMge1NSignal::StatShr();
