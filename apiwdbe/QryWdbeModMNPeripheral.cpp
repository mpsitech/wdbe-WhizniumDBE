/**
	...
	* API code for job QryWdbeModMNPeripheral (implementation)
...
QryWdbeModMNPeripheral::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeModMNPeripheral::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeModMNPeripheral");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeModMNPeripheral";
...
set<uint> QryWdbeModMNPeripheral::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeModMNPeripheral::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeModMNPeripheral::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeModMNPeripheral");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeModMNPeripheral";
