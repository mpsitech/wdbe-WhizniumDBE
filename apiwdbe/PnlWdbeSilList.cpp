/**
	* \file PnlWdbeSilList.cpp
	* API code for job PnlWdbeSilList (implementation)
...
bool PnlWdbeSilList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWdbeSilList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWdbeSilList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWdbeSilList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeSilList::Tag::Tag(
			...
			, TcoTch(TcoTch)
		{
	mask = {CPT, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOSRF, TCOTIT, TCOFSR, TCOTYP, TCORET, TCOREU, TCOMDL, TCOPKG, TCOESY, TCOTCH};
};

bool PnlWdbeSilList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeSilList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeSilList::DpchAppData::DpchAppData(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBESILLISTDATA, scrJref)
		{
...
void PnlWdbeSilList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeSilListData");
...
PnlWdbeSilList::DpchAppDo::DpchAppDo(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBESILLISTDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void PnlWdbeSilList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeSilListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWdbeSilList::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBESILLISTDATA)
		{
...
void PnlWdbeSilList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeSilListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWdbeSilList::StatApp();
		statshrqry = QryWdbeSilList::StatShr();
