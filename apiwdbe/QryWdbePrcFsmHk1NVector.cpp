/**
	...
	* API code for job QryWdbePrcFsmHk1NVector (implementation)
...
QryWdbePrcFsmHk1NVector::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbePrcFsmHk1NVector::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbePrcFsmHk1NVector");
	else
...
set<uint> QryWdbePrcFsmHk1NVector::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbePrcFsmHk1NVector::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbePrcFsmHk1NVector::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbePrcFsmHk1NVector");
	else
		basefound = checkXPath(docctx, basexpath);
