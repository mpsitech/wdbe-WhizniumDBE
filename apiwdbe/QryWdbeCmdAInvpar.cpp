/**
	...
	* API code for job QryWdbeCmdAInvpar (implementation)
...
QryWdbeCmdAInvpar::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeCmdAInvpar::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeCmdAInvpar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeCmdAInvpar";
...
set<uint> QryWdbeCmdAInvpar::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeCmdAInvpar::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeCmdAInvpar::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeCmdAInvpar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeCmdAInvpar";
