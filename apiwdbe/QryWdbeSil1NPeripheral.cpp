/**
	...
	* API code for job QryWdbeSil1NPeripheral (implementation)
...
QryWdbeSil1NPeripheral::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeSil1NPeripheral::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeSil1NPeripheral");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeSil1NPeripheral";
...
set<uint> QryWdbeSil1NPeripheral::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeSil1NPeripheral::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeSil1NPeripheral::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeSil1NPeripheral");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeSil1NPeripheral";
