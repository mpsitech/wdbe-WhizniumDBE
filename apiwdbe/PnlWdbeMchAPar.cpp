/**
	* \file PnlWdbeMchAPar.cpp
	* API code for job PnlWdbeMchAPar (implementation)
...
bool PnlWdbeMchAPar::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWdbeMchAPar");
	else
...
PnlWdbeMchAPar::StatApp::StatApp(
...
bool PnlWdbeMchAPar::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWdbeMchAPar");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> PnlWdbeMchAPar::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeMchAPar::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWdbeMchAPar::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWdbeMchAPar");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWdbeMchAPar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeMchAPar");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeMchAPar::DpchAppData::DpchAppData(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEMCHAPARDATA, scrJref)
		{
...
void PnlWdbeMchAPar::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeMchAParData");
...
PnlWdbeMchAPar::DpchAppDo::DpchAppDo(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEMCHAPARDO, scrJref)
		{
...
void PnlWdbeMchAPar::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeMchAParDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWdbeMchAPar::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEMCHAPARDATA)
		{
...
void PnlWdbeMchAPar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeMchAParData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWdbeMchAPar::StatApp();
		statshrqry = QryWdbeMchAPar::StatShr();
