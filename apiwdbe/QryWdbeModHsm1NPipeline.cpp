/**
	...
	* API code for job QryWdbeModHsm1NPipeline (implementation)
...
QryWdbeModHsm1NPipeline::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeModHsm1NPipeline::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeModHsm1NPipeline");
	else
...
set<uint> QryWdbeModHsm1NPipeline::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeModHsm1NPipeline::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeModHsm1NPipeline::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeModHsm1NPipeline");
	else
		basefound = checkXPath(docctx, basexpath);
