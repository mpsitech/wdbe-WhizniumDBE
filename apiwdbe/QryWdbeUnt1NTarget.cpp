/**
	...
	* API code for job QryWdbeUnt1NTarget (implementation)
...
QryWdbeUnt1NTarget::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeUnt1NTarget::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeUnt1NTarget");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeUnt1NTarget";
...
set<uint> QryWdbeUnt1NTarget::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeUnt1NTarget::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeUnt1NTarget::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeUnt1NTarget");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeUnt1NTarget";
