/**
	...
	* API code for job QryWdbeMtpHsm1NPipeline (implementation)
...
QryWdbeMtpHsm1NPipeline::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeMtpHsm1NPipeline::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeMtpHsm1NPipeline");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeMtpHsm1NPipeline";
...
set<uint> QryWdbeMtpHsm1NPipeline::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeMtpHsm1NPipeline::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeMtpHsm1NPipeline::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeMtpHsm1NPipeline");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeMtpHsm1NPipeline";
