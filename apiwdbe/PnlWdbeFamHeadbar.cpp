/**
	* \file PnlWdbeFamHeadbar.cpp
	* API code for job PnlWdbeFamHeadbar (implementation)
...
PnlWdbeFamHeadbar::StgInf::StgInf(
...
bool PnlWdbeFamHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWdbeFamHeadbar");
	else
...
set<uint> PnlWdbeFamHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeFamHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWdbeFamHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeFamHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeFamHeadbar::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEFAMHEADBARDATA)
		{
};
...
void PnlWdbeFamHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeFamHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
