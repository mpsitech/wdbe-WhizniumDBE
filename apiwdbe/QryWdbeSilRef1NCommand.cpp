/**
	...
	* API code for job QryWdbeSilRef1NCommand (implementation)
...
QryWdbeSilRef1NCommand::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeSilRef1NCommand::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeSilRef1NCommand");
	else
...
set<uint> QryWdbeSilRef1NCommand::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeSilRef1NCommand::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeSilRef1NCommand::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeSilRef1NCommand");
	else
		basefound = checkXPath(docctx, basexpath);
