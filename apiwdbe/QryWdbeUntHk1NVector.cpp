/**
	...
	* API code for job QryWdbeUntHk1NVector (implementation)
...
QryWdbeUntHk1NVector::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeUntHk1NVector::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeUntHk1NVector");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeUntHk1NVector";
...
set<uint> QryWdbeUntHk1NVector::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeUntHk1NVector::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeUntHk1NVector::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeUntHk1NVector");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeUntHk1NVector";
