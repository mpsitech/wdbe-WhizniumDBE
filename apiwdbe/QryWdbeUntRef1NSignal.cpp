/**
	...
	* API code for job QryWdbeUntRef1NSignal (implementation)
...
QryWdbeUntRef1NSignal::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeUntRef1NSignal::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeUntRef1NSignal");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeUntRef1NSignal";
...
set<uint> QryWdbeUntRef1NSignal::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeUntRef1NSignal::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeUntRef1NSignal::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeUntRef1NSignal");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeUntRef1NSignal";
