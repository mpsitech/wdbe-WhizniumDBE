/**
	* \file PnlWdbeSysHeadbar.cpp
	* API code for job PnlWdbeSysHeadbar (implementation)
...
PnlWdbeSysHeadbar::StgInf::StgInf(
...
bool PnlWdbeSysHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWdbeSysHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWdbeSysHeadbar";
...
set<uint> PnlWdbeSysHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeSysHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWdbeSysHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeSysHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWdbeSysHeadbar";
...
PnlWdbeSysHeadbar::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBESYSHEADBARDATA)
		{
};
...
void PnlWdbeSysHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeSysHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
