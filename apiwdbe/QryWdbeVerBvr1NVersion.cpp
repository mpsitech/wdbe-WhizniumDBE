/**
	...
	* API code for job QryWdbeVerBvr1NVersion (implementation)
...
QryWdbeVerBvr1NVersion::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeVerBvr1NVersion::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeVerBvr1NVersion");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeVerBvr1NVersion";
...
set<uint> QryWdbeVerBvr1NVersion::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeVerBvr1NVersion::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeVerBvr1NVersion::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeVerBvr1NVersion");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeVerBvr1NVersion";
