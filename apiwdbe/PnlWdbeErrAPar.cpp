/**
	* \file PnlWdbeErrAPar.cpp
	* API code for job PnlWdbeErrAPar (implementation)
...
bool PnlWdbeErrAPar::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWdbeErrAPar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfWdbeErrAPar";
...
PnlWdbeErrAPar::StatApp::StatApp(
...
bool PnlWdbeErrAPar::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWdbeErrAPar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppWdbeErrAPar";
...
set<uint> PnlWdbeErrAPar::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeErrAPar::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWdbeErrAPar::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWdbeErrAPar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrWdbeErrAPar";
...
bool PnlWdbeErrAPar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWdbeErrAPar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWdbeErrAPar";
...
PnlWdbeErrAPar::DpchAppData::DpchAppData(
			...
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEERRAPARDATA, scrJref)
		{
...
void PnlWdbeErrAPar::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeErrAParData");
...
void PnlWdbeErrAPar::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWdbeErrAParDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWdbeErrAPar::DpchEngData::DpchEngData() :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEERRAPARDATA)
		{
...
void PnlWdbeErrAPar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWdbeErrAParData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWdbeErrAPar::StatApp();
		statshrqry = QryWdbeErrAPar::StatShr();
