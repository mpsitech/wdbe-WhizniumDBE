/**
	...
	* API code for job QryWdbeSysSys1NTarget (implementation)
...
QryWdbeSysSys1NTarget::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeSysSys1NTarget::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeSysSys1NTarget");
	else
...
set<uint> QryWdbeSysSys1NTarget::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeSysSys1NTarget::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeSysSys1NTarget::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeSysSys1NTarget");
	else
		basefound = checkXPath(docctx, basexpath);
