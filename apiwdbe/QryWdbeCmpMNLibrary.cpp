/**
	...
	* API code for job QryWdbeCmpMNLibrary (implementation)
...
QryWdbeCmpMNLibrary::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeCmpMNLibrary::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeCmpMNLibrary");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeCmpMNLibrary";
...
set<uint> QryWdbeCmpMNLibrary::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeCmpMNLibrary::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeCmpMNLibrary::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeCmpMNLibrary");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeCmpMNLibrary";
