/**
	...
	* API code for job QryWdbePrsMNCoreproject (implementation)
...
QryWdbePrsMNCoreproject::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbePrsMNCoreproject::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbePrsMNCoreproject");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbePrsMNCoreproject";
...
set<uint> QryWdbePrsMNCoreproject::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbePrsMNCoreproject::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbePrsMNCoreproject::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbePrsMNCoreproject");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbePrsMNCoreproject";
