/**
	...
	* API code for job QryWdbePphAPar (implementation)
...
QryWdbePphAPar::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbePphAPar::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbePphAPar");
	else
...
set<uint> QryWdbePphAPar::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbePphAPar::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbePphAPar::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbePphAPar");
	else
		basefound = checkXPath(docctx, basexpath);
