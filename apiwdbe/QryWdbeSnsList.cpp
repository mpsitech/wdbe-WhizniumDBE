/**
	...
	* API code for job QryWdbeSnsList (implementation)
...
QryWdbeSnsList::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeSnsList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeSnsList");
	else
...
set<uint> QryWdbeSnsList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeSnsList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeSnsList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeSnsList");
	else
		basefound = checkXPath(docctx, basexpath);
