/**
	...
	* API code for job QryWdbeModCtrMNCommand (implementation)
...
QryWdbeModCtrMNCommand::StatApp::StatApp(
			const uint firstcol
...
bool QryWdbeModCtrMNCommand::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWdbeModCtrMNCommand");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWdbeModCtrMNCommand";
...
set<uint> QryWdbeModCtrMNCommand::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeModCtrMNCommand::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWdbeModCtrMNCommand::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWdbeModCtrMNCommand");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWdbeModCtrMNCommand";
