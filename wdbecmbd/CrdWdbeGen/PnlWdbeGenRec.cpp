/**
	...
	* job handler for job PnlWdbeGenRec (implementation)
...
#include "PnlWdbeGenRec_blks.cpp"
#include "PnlWdbeGenRec_evals.cpp"
...
PnlWdbeGenRec::PnlWdbeGenRec(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEGENREC, jrefSup, ixWdbeVLocale)
		{
...
DpchEngWdbe* PnlWdbeGenRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeGenRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWdbe::getStubGenStd(dbswdbe, recGen.ref, ixWdbeVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recGen.ref == 0) statshr.ixWdbeVExpstate = VecWdbeVExpstate::MIND;
	...
	if (statshr.ixWdbeVExpstate == VecWdbeVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWdbeGenDetail(xchg, dbswdbe, jref, ixWdbeVLocale);
	};
...
void PnlWdbeGenRec::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFGEN) {
		...
		if (dbswdbe->tblwdbemgeneric->loadRecByRef(xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFGEN, jref), &_recGen)) {
			recGen = *_recGen;
			delete _recGen;
...
void PnlWdbeGenRec::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEGENRECDO) {
...
void PnlWdbeGenRec::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeGenRec::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeGenRec::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeGenRec::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
	if (call->ixVCall == VecWdbeVCall::CALLWDBEGENUPD_REFEQ) {
...
bool PnlWdbeGenRec::handleCallWdbeGenUpd_refEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool PnlWdbeGenRec::handleCallWdbeGen_cluEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
...
bool PnlWdbeGenRec::handleCallWdbeGen_mdl_inSbs(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWdbeGenRec::handleCallWdbeGen_mdlEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
