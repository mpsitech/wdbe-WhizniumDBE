/**
	...
	* job handler for job PnlWdbePrjList (implementation)
...
#include "PnlWdbePrjList_blks.cpp"
#include "PnlWdbePrjList_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrjList
 ******************************************************************************/

PnlWdbePrjList::PnlWdbePrjList(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRJLIST, jrefSup, ixWdbeVLocale)
		{
	...
	QryWdbePrjList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFSEL, jref, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFPRJ, jref));
	xchg->addIxPreset(VecWdbeVPreset::PREWDBEIXORD, jref, QryWdbePrjList::VecVOrd::SHO);

	qry = new QryWdbePrjList(xchg, dbswdbe, jref, ixWdbeVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWdbeVExpstate = ((xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFPRJ, jref)) ? VecWdbeVExpstate::MIND : VecWdbeVExpstate::REGD);
...
DpchEngWdbe* PnlWdbePrjList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrjList::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbePrjList::regularize(
			DbsWdbe* dbswdbe
			, const bool notif
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjList::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRJLISTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRJLISTDO) {
...
void PnlWdbePrjList::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrjList::handleDpchAppDataContiac(
			DbsWdbe* dbswdbe
			, ContIac* _contiac
			, DpchEngWdbe** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWdbePrjList::VecVOrd::GRP) && (_contiac->numFTos <= QryWdbePrjList::VecVOrd::VER)) {
...
void PnlWdbePrjList::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjList::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrjList::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrjList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrjList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			refresh(dbswdbe, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrjList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFPRJ, refSelNew);
		};
	};
...
void PnlWdbePrjList::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjList::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjList::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjList::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjList::handleDpchAppDoButFilterClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjList::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrjList::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrjList::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
