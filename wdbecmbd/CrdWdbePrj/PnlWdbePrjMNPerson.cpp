/**
	...
	* job handler for job PnlWdbePrjMNPerson (implementation)
...
#include "PnlWdbePrjMNPerson_blks.cpp"
#include "PnlWdbePrjMNPerson_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrjMNPerson
 ******************************************************************************/

PnlWdbePrjMNPerson::PnlWdbePrjMNPerson(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRJMNPERSON, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePrjMNPerson(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePrjMNPerson::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrjMNPerson::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbePrjMNPerson::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbePrjMNPerson::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRJMNPERSONDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRJMNPERSONDO) {
...
void PnlWdbePrjMNPerson::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrjMNPerson::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjMNPerson::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrjMNPerson::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPrjMNPerson* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrjMNPerson::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrjMNPerson::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrjMNPerson::StgIac::JNUM)) {
...
void PnlWdbePrjMNPerson::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbePrjMNPerson::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjMNPerson::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrjMNPerson::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrjMNPerson::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrjMNPerson::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
