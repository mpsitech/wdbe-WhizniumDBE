/**
	...
	* job handler for job PnlWdbeRlsRec (implementation)
...
#include "PnlWdbeRlsRec_blks.cpp"
#include "PnlWdbeRlsRec_evals.cpp"
...
PnlWdbeRlsRec::PnlWdbeRlsRec(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBERLSREC, jrefSup, ixWdbeVLocale)
		{
...
DpchEngWdbe* PnlWdbeRlsRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeRlsRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWdbe::getStubRlsStd(dbswdbe, recRls.ref, ixWdbeVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recRls.ref == 0) statshr.ixWdbeVExpstate = VecWdbeVExpstate::MIND;
	...
	if (statshr.ixWdbeVExpstate == VecWdbeVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWdbeRlsDetail(xchg, dbswdbe, jref, ixWdbeVLocale);
	};
...
void PnlWdbeRlsRec::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFRLS) {
		...
		if (dbswdbe->tblwdbemrelease->loadRecByRef(xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFRLS, jref), &_recRls)) {
			recRls = *_recRls;
			delete _recRls;
...
void PnlWdbeRlsRec::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBERLSRECDO) {
...
void PnlWdbeRlsRec::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeRlsRec::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeRlsRec::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeRlsRec::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
	if (call->ixVCall == VecWdbeVCall::CALLWDBERLS_CMPEQ) {
...
bool PnlWdbeRlsRec::handleCallWdbeRls_cmpEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
...
bool PnlWdbeRlsRec::handleCallWdbeRls_mchEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWdbeRlsRec::handleCallWdbeRlsUpd_refEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
