/**
	...
	* job handler for job PnlWdbePrcRef1NSensitivity (implementation)
...
#include "PnlWdbePrcRef1NSensitivity_blks.cpp"
#include "PnlWdbePrcRef1NSensitivity_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrcRef1NSensitivity
 ******************************************************************************/

PnlWdbePrcRef1NSensitivity::PnlWdbePrcRef1NSensitivity(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRCREF1NSENSITIVITY, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePrcRef1NSensitivity(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePrcRef1NSensitivity::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrcRef1NSensitivity::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbePrcRef1NSensitivity::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbePrcRef1NSensitivity::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRCREF1NSENSITIVITYDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRCREF1NSENSITIVITYDO) {
...
void PnlWdbePrcRef1NSensitivity::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrcRef1NSensitivity::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcRef1NSensitivity::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrcRef1NSensitivity::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPrcRef1NSensitivity* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrcRef1NSensitivity::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrcRef1NSensitivity::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrcRef1NSensitivity::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSns = WdbeMSensitivity();

			if (recSelNew) {
				if (dbswdbe->tblwdbemsensitivity->loadRecByRef(recSelNew->ref, &_recSns)) {
					recSns = *_recSns;
					delete _recSns;
				};
			};
...
void PnlWdbePrcRef1NSensitivity::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSNS, jref)) if (refPrc != 0) {
			sref = "CrdWdbeSns";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFPRC, refPrc, sref, recSns.ref, jrefNew);
		};
...
void PnlWdbePrcRef1NSensitivity::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSNS, jref) & VecWdbeWAccess::EDIT) != 0) if (refPrc != 0) {
			sref = "CrdWdbeSns";
...
void PnlWdbePrcRef1NSensitivity::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcRef1NSensitivity::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrcRef1NSensitivity::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrcRef1NSensitivity::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
