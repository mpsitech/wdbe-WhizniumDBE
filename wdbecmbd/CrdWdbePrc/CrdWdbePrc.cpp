/**
	* \file CrdWdbePrc.cpp
	* job handler for job CrdWdbePrc (implementation)
...
CrdWdbePrc::CrdWdbePrc(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEPRC, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFPRC, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWdbePrcList(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbePrcHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlrec = new PnlWdbePrcRec(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbePrc::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbePrc::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbePrc/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdPrc = StubWdbe::getStubPrcStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFPRC, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbePrc::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, ixWdbeVPreset, xchg->getRefPreset(ixWdbeVPreset, jref), "CrdWdbePrc", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFPRC, jref, ref);

	if (dbswdbe->tblwdbemprocess->loadRecByRef(ref, &rec)) {
		xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFFSM, jref, rec->refWdbeMFsm);

		delete rec;
	};

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFPRC, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFPRC, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbePrc::updatePreset(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWdbePrc::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRCDO) {
...
void CrdWdbePrc::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbePrc::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEPRC);
};

void CrdWdbePrc::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbePrc::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbePrc::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFPRC) {
...
bool CrdWdbePrc::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbePrc::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWdbePrc::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbePrc::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbePrc::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
