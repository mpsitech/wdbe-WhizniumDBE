/**
	...
	* job handler for job PnlWdbePrcMge1NSignal (implementation)
...
#include "PnlWdbePrcMge1NSignal_blks.cpp"
#include "PnlWdbePrcMge1NSignal_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrcMge1NSignal
 ******************************************************************************/

PnlWdbePrcMge1NSignal::PnlWdbePrcMge1NSignal(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRCMGE1NSIGNAL, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePrcMge1NSignal(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePrcMge1NSignal::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrcMge1NSignal::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbePrcMge1NSignal::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbePrcMge1NSignal::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRCMGE1NSIGNALDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRCMGE1NSIGNALDO) {
...
void PnlWdbePrcMge1NSignal::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrcMge1NSignal::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcMge1NSignal::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrcMge1NSignal::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPrcMge1NSignal* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrcMge1NSignal::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrcMge1NSignal::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrcMge1NSignal::StgIac::JNUM)) {
...
void PnlWdbePrcMge1NSignal::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbePrcMge1NSignal::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrcMge1NSignal::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrcMge1NSignal::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
