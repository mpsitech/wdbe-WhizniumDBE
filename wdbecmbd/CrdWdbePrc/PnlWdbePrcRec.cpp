...
void PnlWdbePrcRec::refresh(
	...
	if (statshr.ixWdbeVExpstate == VecWdbeVExpstate::MIND) {
		...
		if (pnlprc1nvariable) {delete pnlprc1nvariable; pnlprc1nvariable = NULL;};
		...
		if (!pnlprc1nvariable) pnlprc1nvariable = new PnlWdbePrcPrc1NVariable(xchg, dbswdbe, jref, ixWdbeVLocale);
	...
	statshr.jrefPrc1NVariable = ((pnlprc1nvariable) ? pnlprc1nvariable->jref : 0);
...
void PnlWdbePrcRec::updatePreset(
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFPRC) {
		...
		if (recPrc.ref != 0) {
			...
			if (pnlprc1nvariable) pnlprc1nvariable->updatePreset(dbswdbe, ixWdbeVPreset, jrefTrig, notif);
