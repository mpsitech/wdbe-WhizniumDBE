/**
	...
	* job handler for job PnlWdbePrcRef1NVariable (implementation)
...
#include "PnlWdbePrcRef1NVariable_blks.cpp"
#include "PnlWdbePrcRef1NVariable_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrcRef1NVariable
 ******************************************************************************/

PnlWdbePrcRef1NVariable::PnlWdbePrcRef1NVariable(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRCREF1NVARIABLE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePrcRef1NVariable(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePrcRef1NVariable::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrcRef1NVariable::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbePrcRef1NVariable::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbePrcRef1NVariable::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRCREF1NVARIABLEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRCREF1NVARIABLEDO) {
...
void PnlWdbePrcRef1NVariable::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrcRef1NVariable::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcRef1NVariable::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrcRef1NVariable::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPrcRef1NVariable* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrcRef1NVariable::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrcRef1NVariable::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrcRef1NVariable::StgIac::JNUM)) {
...
void PnlWdbePrcRef1NVariable::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcRef1NVariable::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcRef1NVariable::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbePrcRef1NVariable::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbePrcRef1NVariable::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcRef1NVariable::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrcRef1NVariable::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrcRef1NVariable::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
