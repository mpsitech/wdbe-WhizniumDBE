/**
	...
	* job handler for job PnlWdbePrcFsmFsm1NFsmstate (implementation)
...
#include "PnlWdbePrcFsmFsm1NFsmstate_blks.cpp"
#include "PnlWdbePrcFsmFsm1NFsmstate_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrcFsmFsm1NFsmstate
 ******************************************************************************/

PnlWdbePrcFsmFsm1NFsmstate::PnlWdbePrcFsmFsm1NFsmstate(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRCFSMFSM1NFSMSTATE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePrcFsmFsm1NFsmstate(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePrcFsmFsm1NFsmstate::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrcFsmFsm1NFsmstate::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbePrcFsmFsm1NFsmstate::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbePrcFsmFsm1NFsmstate::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRCFSMFSM1NFSMSTATEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRCFSMFSM1NFSMSTATEDO) {
...
void PnlWdbePrcFsmFsm1NFsmstate::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrcFsmFsm1NFsmstate::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcFsmFsm1NFsmstate::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrcFsmFsm1NFsmstate::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPrcFsmFsm1NFsmstate* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrcFsmFsm1NFsmstate::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrcFsmFsm1NFsmstate::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrcFsmFsm1NFsmstate::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recFst = WdbeMFsmstate();

			if (recSelNew) {
				if (dbswdbe->tblwdbemfsmstate->loadRecByRef(recSelNew->ref, &_recFst)) {
					recFst = *_recFst;
					delete _recFst;
				};
			};
...
void PnlWdbePrcFsmFsm1NFsmstate::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcFsmFsm1NFsmstate::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcFsmFsm1NFsmstate::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refFsm = xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFFSM, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCFST, jref)) if (refFsm != 0) {
			sref = "CrdWdbeFst";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFFSM, refFsm, sref, recFst.ref, jrefNew);
		};
...
void PnlWdbePrcFsmFsm1NFsmstate::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCFST, jref) & VecWdbeWAccess::EDIT) != 0) if (refFsm != 0) {
			sref = "CrdWdbeFst";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFFSM, refFsm, sref, -1, jrefNew);
		};
...
void PnlWdbePrcFsmFsm1NFsmstate::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrcFsmFsm1NFsmstate::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrcFsmFsm1NFsmstate::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrcFsmFsm1NFsmstate::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
