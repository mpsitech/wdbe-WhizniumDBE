/**
	* \file WdbeMtpWrmcuUarthostif_Easy_v1_1_Mcu.cpp
...
/******************************************************************************
 namespace WdbeMtpWrmcuUarthostif_Easy_v1_1_Mcu
 ******************************************************************************/

DpchRetWdbe* WdbeMtpWrmcuUarthostif_Easy_v1_1_Mcu::run(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
...
// IP cust --- IBEGIN
void WdbeMtpWrmcuUarthostif_Easy_v1_1_Mcu::writeMdlC(
			DbsWdbe* dbswdbe
			, fstream& outfile
	...
	dbswdbe->loadStringBySQL("SELECT TblWdbeMModule.sref FROM TblWdbeMModule, TblWdbeRMModuleMModule WHERE TblWdbeMModule.ref = TblWdbeRMModuleMModule.corRefWdbeMModule AND TblWdbeRMModuleMModule.ctdRefWdbeMModule = "
				+ to_string(mdl->ref) + " AND srefKFunction = 'rx'", Rx);
	Rx = StrMod::cap(Rx);

	dbswdbe->loadStringBySQL("SELECT TblWdbeMModule.sref FROM TblWdbeMModule, TblWdbeRMModuleMModule WHERE TblWdbeMModule.ref = TblWdbeRMModuleMModule.corRefWdbeMModule AND TblWdbeRMModuleMModule.ctdRefWdbeMModule = "
				+ to_string(mdl->ref) + " AND srefKFunction = 'tx'", Tx);
	Tx = StrMod::cap(Tx);
				...
				for (unsigned int j = 0; j < cmds.nodes.size(); j++) {
					...
					if ((cmd->ixVRettype == VecWdbeVMCommandRettype::IMMSNG) || (cmd->ixVRettype == VecWdbeVMCommandRettype::STATSNG)) {
						...
						if (cmd->ixVRettype == VecWdbeVMCommandRettype::IMMSNG) {
							...
							if (ix > 0) {
								outfile << indent << "memcpy(txbuf, (unsigned char*) &shrdat" << StrMod::cap(untmdl->sref) << "." << cmd->sref;
	...
	for (unsigned int i = 0; i < srefsImbRd.size(); i++) {
		...
		outfile << "\t\t\t\tshrdat" << Tx << ".len = (opbuf[IXOPBUF_length] << 8) + opbuf[IXOPBUF_length+1];" << endl;
		outfile << endl;

		outfile << "\t\t\t\tif (shrdat" << Tx << ".len == shrdat" << srefsMgmtImbRd[i] << ".avllen" << srefrootsMgmtImbRd[i] << ") {" << endl;
	...
	for (unsigned int i = 0; i < srefsImbWr.size(); i++) {
		...
		outfile << "\t\t\t\tshrdat" << Rx << ".len = icrc + 2;" << endl;
		outfile << endl;

		outfile << "\t\t\t\tif (icrc == shrdat" << srefsMgmtImbWr[i] << ".avllen" << srefrootsMgmtImbWr[i] << ") {" << endl;
		outfile << "\t\t\t\t\tflags.req" << srefsImbWr[i] << " = 1;" << endl;
		outfile << "\t\t\t\t\tSET_EVT_req" << srefsImbWr[i] << "();" << endl;
		outfile << endl;

		outfile << "\t\t\t\t\tstateOp = stateOpRxbufA;" << endl;
		outfile << "\t\t\t\t};" << endl;
	};
		...
		if (untmdl->ixVBasetype == VecWdbeVMModuleBasetype::ECTR) {
			...
			for (unsigned int j = 0; j < cmds.nodes.size(); j++) {
				...
				if (cmd->ixVRettype == VecWdbeVMCommandRettype::IMMSNG) {
					...
					if (ix > 0) {
						outfile << "\t\t\t\tmemcpy(txbuf, (unsigned char*) &shrdat" << StrMod::cap(untmdl->sref) << "." << cmd->sref;
						if (rpa->ixWdbeVPartype == VecWdbeVPartype::VBLOB) outfile << "Len";
						outfile << StrMod::cap(rpa->sref);
						outfile << ", " << (ix + 1) << ");" << endl;
					};
	...
	if (srefsImbRd.size() > 0) {
		...
		// --- op.txbufA
		outfile << "// IP op.txbufA --- IBEGIN" << endl;

		outfile << "\t\t\t\tif (";

		for (unsigned int i = 0; i < srefsImbRd.size(); i++) {
			if (i != 0) outfile << " ||";
			outfile << " ((opbuf[IXOPBUF_buffer] == VECWBUFFER_" << StrMod::uncap(srefrootsCorImbRd[i]) << ") && IS_SET_EVT_ack" << srefsImbRd[i] << "())";
		};

		outfile << " ) {" << endl;

		outfile << "// IP op.txbufA --- IEND" << endl;
		...
		for (unsigned int i = 0; i < srefsImbRd.size(); i++) {
			outfile << "\t\t\t\t";
			if (i != 0) outfile << "else ";
			outfile << "if (opbuf[IXOPBUF_buffer] == VECWBUFFER_" << StrMod::uncap(srefrootsCorImbRd[i]) << ") shrdat" << Tx << ".d = shrdat" << srefsImbRd[i] << "." << StrMod::uncap(srefrootsMgmtImbRd[i]) << "D;" << endl;
		};
	...
	if (srefsImbWr.size() > 0) {
		...
		// --- op.rxbufA
		outfile << "// IP op.rxbufA --- IBEGIN" << endl;

		outfile << "\t\t\t\tif (";

		for (unsigned int i = 0; i < srefsImbWr.size(); i++) {
			if (i != 0) outfile << " ||";
			outfile << " ((opbuf[IXOPBUF_buffer] == VECWBUFFER_" << StrMod::uncap(srefrootsCorImbWr[i]) << ") && IS_SET_EVT_ack" << srefsImbWr[i] << "())";
		};

		outfile << " ) {" << endl;

		outfile << "// IP op.rxbufA --- IEND" << endl;

		// --- op.rxbufC.strobe
		outfile << "// IP op.rxbufC.strobe --- IBEGIN" << endl;
		...
		for (unsigned int i = 0; i < srefsImbWr.size(); i++) {
			...
			outfile << "\t\t\t\t\tshrdat" << srefrootsMgmtImbWr[i] << "." << StrMod::uncap(srefrootsMgmtImbWr[i]) << "D = shrdat" << Rx << ".d;" << endl;
			outfile << "\t\t\t\t\tSET_EVT_strb" << srefsImbWr[i] << "D();" << endl;
		};

		outfile << "// IP op.rxbufC.strobe --- IEND" << endl;

		// --- op.rxbufC.done
		outfile << "// IP op.rxbufC.done --- IBEGIN" << endl;
		...
		outfile << "// IP op.rxbufC.done --- IEND" << endl;
