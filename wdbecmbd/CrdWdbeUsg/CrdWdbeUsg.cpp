/**
	* \file CrdWdbeUsg.cpp
	* job handler for job CrdWdbeUsg (implementation)
...
CrdWdbeUsg::CrdWdbeUsg(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEUSG, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFUSG, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWdbeUsgRec(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbeUsgHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnllist = new PnlWdbeUsgList(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbeUsg::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeUsg::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeUsg/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdUsg = StubWdbe::getStubUsgStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFUSG, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbeUsg::changeRef(
	...
	xchg->triggerIxRefToIxCall(dbswdbe, VecWdbeVCall::CALLWDBERECACCESS, jref, VecWdbeVCard::CRDWDBEUSG, ref, ixWdbeVRecaccess);

	if (ixWdbeVRecaccess != VecWdbeVRecaccess::NONE) {
		xchg->addIxPreset(VecWdbeVPreset::PREWDBEIXRECACC, jref, ixWdbeVRecaccess);

		if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, VecWdbeVPreset::VOID, 0, "CrdWdbeUsg", ref);
		xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFUSG, jref, ref);

		if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFUSG, jrefTrig, notif);
		if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFUSG, jrefTrig, notif);
...
void CrdWdbeUsg::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUSGDO) {
...
void CrdWdbeUsg::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeUsg::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEUSG);
};

void CrdWdbeUsg::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeUsg::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeUsg::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeUsg::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeUsg::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFUSG) {
...
void CrdWdbeUsg::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeUsg::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeUsg::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
