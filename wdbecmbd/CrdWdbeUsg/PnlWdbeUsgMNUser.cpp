/**
	...
	* job handler for job PnlWdbeUsgMNUser (implementation)
...
#include "PnlWdbeUsgMNUser_blks.cpp"
#include "PnlWdbeUsgMNUser_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUsgMNUser
 ******************************************************************************/

PnlWdbeUsgMNUser::PnlWdbeUsgMNUser(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUSGMNUSER, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUsgMNUser(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUsgMNUser::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUsgMNUser::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUsgMNUser::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUSGMNUSERDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUSGMNUSERDO) {
...
void PnlWdbeUsgMNUser::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUsgMNUser::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsgMNUser::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUsgMNUser::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUsgMNUser::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUsgMNUser::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUsgMNUser::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswdbe->tblwdbemuser->loadRecByRef(recSelNew->mref, &_recUsr)) {
					recUsr = *_recUsr;
					delete _recUsr;
				};
			};
...
void PnlWdbeUsgMNUser::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCUSR, jref)) {
			sref = "CrdWdbeUsr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recUsr.ref, jrefNew);
		};
...
void PnlWdbeUsgMNUser::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsgMNUser::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsgMNUser::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUsgMNUser::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUsgMNUser::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
