/**
	...
	* job handler for job PnlWdbeUsgAAccess (implementation)
...
#include "PnlWdbeUsgAAccess_blks.cpp"
#include "PnlWdbeUsgAAccess_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUsgAAccess
 ******************************************************************************/

PnlWdbeUsgAAccess::PnlWdbeUsgAAccess(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUSGAACCESS, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUsgAAccess(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUsgAAccess::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUsgAAccess::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeUsgAAccess::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFUSG) {
		recUsgAacc = WdbeAMUsergroupAccess();
...
void PnlWdbeUsgAAccess::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUSGAACCESSDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUSGAACCESSDO) {
...
void PnlWdbeUsgAAccess::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUsgAAccess::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsgAAccess::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUsgAAccess::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUsgAAccess* recSelNew = NULL;

	WdbeAMUsergroupAccess* _recUsgAacc = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUsgAAccess::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUsgAAccess::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUsgAAccess::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recUsgAacc = WdbeAMUsergroupAccess();

			if (recSelNew) {
				if (dbswdbe->tblwdbeamusergroupaccess->loadRecByRef(recSelNew->ref, &_recUsgAacc)) {
...
void PnlWdbeUsgAAccess::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsgAAccess::handleDpchAppDoButDuplicateClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsgAAccess::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsgAAccess::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUsgAAccess::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUsgAAccess::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
