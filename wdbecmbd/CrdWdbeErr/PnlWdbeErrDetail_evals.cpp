...
bool PnlWdbeErrDetail::evalButTraViewAvail(
			DbsWdbe* dbswdbe
		) {
	// err.traEq(0)|(pre.ixCrdaccSig()&pre.refUnt())

	vector<bool> args;
	bool a, b;

	a = false; a = (recErr.traRefWdbeMSignal == 0);
	args.push_back(a);
...
bool PnlWdbeErrDetail::evalButTraViewActive(
			DbsWdbe* dbswdbe
		) {
	// !err.traEq(0)
