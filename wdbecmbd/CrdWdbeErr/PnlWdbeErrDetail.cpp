...
void PnlWdbeErrDetail::refreshRecErr(
	...
	statshr.ButTraViewAvail = evalButTraViewAvail(dbswdbe);
	statshr.ButTraViewActive = evalButTraViewActive(dbswdbe);
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEINIT) {
			...
			if (dpchappdo->ixVDo != 0) {
				...
				} else if (dpchappdo->ixVDo == VecVDo::BUTTRAVIEWCLICK) {
					handleDpchAppDoButTraViewClick(dbswdbe, &(req->dpcheng));
				};
...
void PnlWdbeErrDetail::handleDpchAppDoButTraViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButTraViewAvail && statshr.ButTraViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSIG, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFUNT) {
			sref = "CrdWdbeSig";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recErr.traRefWdbeMSignal, jrefNew);
		};
