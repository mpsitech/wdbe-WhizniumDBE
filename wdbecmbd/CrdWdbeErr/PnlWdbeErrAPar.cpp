/**
	...
	* job handler for job PnlWdbeErrAPar (implementation)
...
#include "PnlWdbeErrAPar_blks.cpp"
#include "PnlWdbeErrAPar_evals.cpp"
...
/******************************************************************************
 class PnlWdbeErrAPar
 ******************************************************************************/

PnlWdbeErrAPar::PnlWdbeErrAPar(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEERRAPAR, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeErrAPar(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeErrAPar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeErrAPar::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeErrAPar::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEERRAPARDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEERRAPARDO) {
...
void PnlWdbeErrAPar::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeErrAPar::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeErrAPar::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeErrAPar::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQErrAPar* recSelNew = NULL;

	WdbeAMErrorPar* _recErrApar = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeErrAPar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeErrAPar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeErrAPar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recErrApar = WdbeAMErrorPar();

			if (recSelNew) {
				if (dbswdbe->tblwdbeamerrorpar->loadRecByRef(recSelNew->ref, &_recErrApar)) {
					recErrApar = *_recErrApar;
					delete _recErrApar;
				};
			};
...
void PnlWdbeErrAPar::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeErrAPar::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeErrAPar::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeErrAPar::handleDpchAppDoButDuplicateClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeErrAPar::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeErrAPar::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeErrAPar::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeErrAPar::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
