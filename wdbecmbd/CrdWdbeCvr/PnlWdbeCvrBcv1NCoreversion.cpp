/**
	...
	* job handler for job PnlWdbeCvrBcv1NCoreversion (implementation)
...
#include "PnlWdbeCvrBcv1NCoreversion_blks.cpp"
#include "PnlWdbeCvrBcv1NCoreversion_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCvrBcv1NCoreversion
 ******************************************************************************/

PnlWdbeCvrBcv1NCoreversion::PnlWdbeCvrBcv1NCoreversion(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECVRBCV1NCOREVERSION, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeCvrBcv1NCoreversion(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeCvrBcv1NCoreversion::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCvrBcv1NCoreversion::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeCvrBcv1NCoreversion::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeCvrBcv1NCoreversion::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECVRBCV1NCOREVERSIONDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECVRBCV1NCOREVERSIONDO) {
...
void PnlWdbeCvrBcv1NCoreversion::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCvrBcv1NCoreversion::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCvrBcv1NCoreversion::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCvrBcv1NCoreversion::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQCvrBcv1NCoreversion* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeCvrBcv1NCoreversion::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeCvrBcv1NCoreversion::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeCvrBcv1NCoreversion::StgIac::JNUM)) {
...
void PnlWdbeCvrBcv1NCoreversion::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCVR, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFCPR) {
			sref = "CrdWdbeCvr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recCvr.ref, jrefNew);
		};
		if (jrefNew == 0) {
			if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCVR, jref)) {
				sref = "CrdWdbeCvr";
				xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recCvr.ref, jrefNew);
			};
		};
...
void PnlWdbeCvrBcv1NCoreversion::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCvrBcv1NCoreversion::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCvrBcv1NCoreversion::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
