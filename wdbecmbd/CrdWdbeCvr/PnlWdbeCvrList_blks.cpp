/**
	* \file PnlWdbeCvrList_blks.cpp
	* job handler for job PnlWdbeCvrList (implementation of blocks)
...
bool PnlWdbeCvrList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWdbeCvrList"];}();
...
void PnlWdbeCvrList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
void PnlWdbeCvrList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeCvrList";
...
void PnlWdbeCvrList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWdbeCvrList";
...
void PnlWdbeCvrList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeCvrList";
...
void PnlWdbeCvrList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWdbeCvrList";
...
bool PnlWdbeCvrList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWdbeCvrList"];}();
	...
	if (basefound) {
		...
		if (me.isMember("TcoCprWidth")) {TcoCprWidth = me["TcoCprWidth"].asUInt(); add(TCOCPRWIDTH);};
		if (me.isMember("TcoMajWidth")) {TcoMajWidth = me["TcoMajWidth"].asUInt(); add(TCOMAJWIDTH);};
		if (me.isMember("TcoMinWidth")) {TcoMinWidth = me["TcoMinWidth"].asUInt(); add(TCOMINWIDTH);};
		if (me.isMember("TcoSubWidth")) {TcoSubWidth = me["TcoSubWidth"].asUInt(); add(TCOSUBWIDTH);};
		if (me.isMember("TcoBcvWidth")) {TcoBcvWidth = me["TcoBcvWidth"].asUInt(); add(TCOBCVWIDTH);};
...
void PnlWdbeCvrList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	...
	me["TcoCprWidth"] = (Json::Value::UInt) TcoCprWidth;
	me["TcoMajWidth"] = (Json::Value::UInt) TcoMajWidth;
	me["TcoMinWidth"] = (Json::Value::UInt) TcoMinWidth;
	me["TcoSubWidth"] = (Json::Value::UInt) TcoSubWidth;
	me["TcoBcvWidth"] = (Json::Value::UInt) TcoBcvWidth;
...
void PnlWdbeCvrList::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeCvrList";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
		me["Cpt"] = "Core versions";
		me["TcoCpr"] = "Core project";
		me["TcoMaj"] = "Major";
		me["TcoMin"] = "Minor";
		me["TcoSub"] = "Sub";
		me["TcoBcv"] = "Base core version";
		me["TcoSte"] = "State";
	};
...
void PnlWdbeCvrList::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeCvrList";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Core versions");
			writeStringAttr(wr, itemtag, "sref", "TcoCpr", "Core project");
			writeStringAttr(wr, itemtag, "sref", "TcoMaj", "Major");
			writeStringAttr(wr, itemtag, "sref", "TcoMin", "Minor");
			writeStringAttr(wr, itemtag, "sref", "TcoSub", "Sub");
			writeStringAttr(wr, itemtag, "sref", "TcoBcv", "Base core version");
...
PnlWdbeCvrList::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECVRLISTDATA)
		{
};
...
void PnlWdbeCvrList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCvrListData"];}();
...
void PnlWdbeCvrList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCvrListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeCvrList::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECVRLISTDO)
		{
	ixVDo = 0;
};
...
void PnlWdbeCvrList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCvrListDo"];}();
...
void PnlWdbeCvrList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCvrListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeCvrList::DpchEngData::DpchEngData(
			...
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBECVRLISTDATA, jref)
		{
...
void PnlWdbeCvrList::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeCvrList::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeCvrListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeCvrList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeCvrList::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeCvrListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeCvrList::StatApp::writeXML(wr);
