...
#include "PnlWdbeCvrHk1NModule_blks.cpp"
#include "PnlWdbeCvrHk1NModule_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCvrHk1NModule
 ******************************************************************************/

PnlWdbeCvrHk1NModule::PnlWdbeCvrHk1NModule(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECVRHK1NMODULE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeCvrHk1NModule(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeCvrHk1NModule::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCvrHk1NModule::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECVRHK1NMODULEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECVRHK1NMODULEDO) {
...
void PnlWdbeCvrHk1NModule::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCvrHk1NModule::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCvrHk1NModule::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCvrHk1NModule::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQCvrHk1NModule* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (!has(diffitems, QryWdbeCvrHk1NModule::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeCvrHk1NModule::StgIac::JNUM)) {
...
void PnlWdbeCvrHk1NModule::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeCvrHk1NModule::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCMOD, jref) & VecWdbeWAccess::EDIT) != 0) if (refCvr != 0) {
			sref = "CrdWdbeMod";
...
void PnlWdbeCvrHk1NModule::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCvrHk1NModule::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCvrHk1NModule::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCvrHk1NModule::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
