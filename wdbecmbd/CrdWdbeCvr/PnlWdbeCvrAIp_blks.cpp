...
/******************************************************************************
 class PnlWdbeCvrAIp::VecVDo
 ******************************************************************************/

uint PnlWdbeCvrAIp::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWdbeCvrAIp::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWdbeCvrAIp::ContInf
 ******************************************************************************/

PnlWdbeCvrAIp::ContInf::ContInf(
...
void PnlWdbeCvrAIp::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeCvrAIp";
...
void PnlWdbeCvrAIp::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemInfWdbeCvrAIp";
...
set<uint> PnlWdbeCvrAIp::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCvrAIp::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeCvrAIp::StatApp
 ******************************************************************************/

void PnlWdbeCvrAIp::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
			, const uint ixWdbeVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppWdbeCvrAIp";
...
void PnlWdbeCvrAIp::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixWdbeVExpstate
	...
	else itemtag = "StatitemAppWdbeCvrAIp";
...
/******************************************************************************
 class PnlWdbeCvrAIp::StatShr
 ******************************************************************************/

PnlWdbeCvrAIp::StatShr::StatShr(
...
void PnlWdbeCvrAIp::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeCvrAIp";
...
void PnlWdbeCvrAIp::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrWdbeCvrAIp";
...
set<uint> PnlWdbeCvrAIp::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCvrAIp::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeCvrAIp::StgIac
 ******************************************************************************/

PnlWdbeCvrAIp::StgIac::StgIac(
			...
			, TcoTtyWidth(TcoTtyWidth)
			, TcoCmtWidth(TcoCmtWidth)
		{
	mask = {TCOSRFWIDTH, TCOTTYWIDTH, TCOCMTWIDTH};
};

bool PnlWdbeCvrAIp::StgIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWdbeCvrAIp"];}();

	basefound = (me != Json::nullValue);

	if (basefound) {
		if (me.isMember("TcoSrfWidth")) {TcoSrfWidth = me["TcoSrfWidth"].asUInt(); add(TCOSRFWIDTH);};
		if (me.isMember("TcoTtyWidth")) {TcoTtyWidth = me["TcoTtyWidth"].asUInt(); add(TCOTTYWIDTH);};
...
bool PnlWdbeCvrAIp::StgIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWdbeCvrAIp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacWdbeCvrAIp";
...
void PnlWdbeCvrAIp::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacWdbeCvrAIp";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	me["TcoSrfWidth"] = (Json::Value::UInt) TcoSrfWidth;
	me["TcoTtyWidth"] = (Json::Value::UInt) TcoTtyWidth;
	me["TcoCmtWidth"] = (Json::Value::UInt) TcoCmtWidth;
};

void PnlWdbeCvrAIp::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacWdbeCvrAIp";
...
set<uint> PnlWdbeCvrAIp::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCvrAIp::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {TCOSRFWIDTH, TCOTTYWIDTH, TCOCMTWIDTH};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
/******************************************************************************
 class PnlWdbeCvrAIp::Tag
 ******************************************************************************/

void PnlWdbeCvrAIp::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeCvrAIp";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
		me["Cpt"] = "Insertion points";
...
void PnlWdbeCvrAIp::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeCvrAIp";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Insertion points");
			writeStringAttr(wr, itemtag, "sref", "TcoSrf", "Identifier");
			writeStringAttr(wr, itemtag, "sref", "TcoTty", "Tag type");
...
/******************************************************************************
 class PnlWdbeCvrAIp::DpchAppData
 ******************************************************************************/

PnlWdbeCvrAIp::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECVRAIPDATA)
		{
};

string PnlWdbeCvrAIp::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCvrAIp::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCvrAIpData"];}();
...
void PnlWdbeCvrAIp::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCvrAIpData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeCvrAIp::DpchAppDo
 ******************************************************************************/

PnlWdbeCvrAIp::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECVRAIPDO)
		{
	ixVDo = 0;
};

string PnlWdbeCvrAIp::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCvrAIp::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCvrAIpDo"];}();
...
void PnlWdbeCvrAIp::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCvrAIpDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeCvrAIp::DpchEngData
 ******************************************************************************/

PnlWdbeCvrAIp::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBECVRAIPDATA, jref)
		{
...
string PnlWdbeCvrAIp::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCvrAIp::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeCvrAIp::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeCvrAIpData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeCvrAIp::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeCvrAIp::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeCvrAIpData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeCvrAIp::StatApp::writeXML(wr);
