...
DpchRetWdbe* WdbeGenfstCmdbus::run(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
	...
	if ((refTop || mcuNotFpga) && refCmdbus && refCmdinv && refCmdret) {
		...
		Wdbe::setMpa(dbswdbe, refCmdinv, "maxlenRecv", to_string(maxlenRecv));
		Wdbe::setMpa(dbswdbe, refCmdret, "maxlenSend", to_string(maxlenSend));
