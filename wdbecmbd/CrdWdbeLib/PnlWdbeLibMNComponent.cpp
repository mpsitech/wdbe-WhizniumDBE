/**
	...
	* job handler for job PnlWdbeLibMNComponent (implementation)
...
#include "PnlWdbeLibMNComponent_blks.cpp"
#include "PnlWdbeLibMNComponent_evals.cpp"
...
/******************************************************************************
 class PnlWdbeLibMNComponent
 ******************************************************************************/

PnlWdbeLibMNComponent::PnlWdbeLibMNComponent(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBELIBMNCOMPONENT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeLibMNComponent(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeLibMNComponent::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeLibMNComponent::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeLibMNComponent::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFLIB) {
		recCmpRlib = WdbeRMComponentMLibrary();

		recCmp = WdbeMComponent();
...
void PnlWdbeLibMNComponent::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBELIBMNCOMPONENTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBELIBMNCOMPONENTDO) {
...
void PnlWdbeLibMNComponent::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeLibMNComponent::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeLibMNComponent::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeLibMNComponent::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQLibMNComponent* recSelNew = NULL;

	WdbeRMComponentMLibrary* _recCmpRlib = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeLibMNComponent::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeLibMNComponent::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeLibMNComponent::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswdbe->tblwdbermcomponentmlibrary->loadRecByRef(recSelNew->ref, &_recCmpRlib)) {
					recCmpRlib = *_recCmpRlib;
					delete _recCmpRlib;
				};
				if (dbswdbe->tblwdbemcomponent->loadRecByRef(recSelNew->mref, &_recCmp)) {
					recCmp = *_recCmp;
					delete _recCmp;
				};
			};
...
void PnlWdbeLibMNComponent::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCMP, jref)) if (refVer != 0) {
			sref = "CrdWdbeCmp";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFVER, refVer, sref, recCmp.ref, jrefNew);
		};
		if (jrefNew == 0) {
			if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCMP, jref)) {
				sref = "CrdWdbeCmp";
				xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recCmp.ref, jrefNew);
			};
		};
...
void PnlWdbeLibMNComponent::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeLibMNComponent::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeLibMNComponent::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeLibMNComponent::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeLibMNComponent::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
