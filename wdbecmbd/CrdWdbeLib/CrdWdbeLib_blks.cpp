/**
	* \file CrdWdbeLib_blks.cpp
	* job handler for job CrdWdbeLib (implementation of blocks)
...
void CrdWdbeLib::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdWdbeLib::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeLib";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	me["numFSge"] = numFSge;
	me["MrlAppHlp"] = MrlAppHlp;
	me["MtxCrdLib"] = MtxCrdLib;
};

void CrdWdbeLib::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemInfWdbeLib";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdLib", MtxCrdLib);
...
void CrdWdbeLib::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
	...
	if (difftag.length() == 0) difftag = "StatAppWdbeLib";
...
void CrdWdbeLib::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixWdbeVReqitmode
	...
	else itemtag = "StatitemAppWdbeLib";
...
void CrdWdbeLib::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeLib";
...
void CrdWdbeLib::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrWdbeLib";
...
void CrdWdbeLib::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeLib";
...
CrdWdbeLib::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBELIBDO)
		{
	ixVDo = 0;
};
...
void CrdWdbeLib::DpchAppDo::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeLibDo"];}();
...
void CrdWdbeLib::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeLibDo");
	else
...
CrdWdbeLib::DpchEngData::DpchEngData(
			...
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBELIBDATA, jref)
		{
...
void CrdWdbeLib::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void CrdWdbeLib::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeLibData"] = Json::Value(Json::objectValue);
...
void CrdWdbeLib::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeLibData");
