/**
	...
	* job handler for job PnlWdbeLibAMakefile (implementation)
...
#include "PnlWdbeLibAMakefile_blks.cpp"
#include "PnlWdbeLibAMakefile_evals.cpp"
...
/******************************************************************************
 class PnlWdbeLibAMakefile
 ******************************************************************************/

PnlWdbeLibAMakefile::PnlWdbeLibAMakefile(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBELIBAMAKEFILE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeLibAMakefile(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeLibAMakefile::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeLibAMakefile::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeLibAMakefile::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFLIB) {
		recLibAmkf = WdbeAMLibraryMakefile();
...
void PnlWdbeLibAMakefile::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBELIBAMAKEFILEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBELIBAMAKEFILEDO) {
...
void PnlWdbeLibAMakefile::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeLibAMakefile::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeLibAMakefile::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeLibAMakefile::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQLibAMakefile* recSelNew = NULL;

	WdbeAMLibraryMakefile* _recLibAmkf = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeLibAMakefile::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeLibAMakefile::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeLibAMakefile::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recLibAmkf = WdbeAMLibraryMakefile();

			if (recSelNew) {
				if (dbswdbe->tblwdbeamlibrarymakefile->loadRecByRef(recSelNew->ref, &_recLibAmkf)) {
					recLibAmkf = *_recLibAmkf;
					delete _recLibAmkf;
				};
			};
...
void PnlWdbeLibAMakefile::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeLibAMakefile::handleDpchAppDoButDuplicateClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeLibAMakefile::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeLibAMakefile::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeLibAMakefile::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeLibAMakefile::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
