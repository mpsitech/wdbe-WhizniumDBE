...
#include "PnlWdbeLibRec_blks.cpp"
#include "PnlWdbeLibRec_evals.cpp"
...
PnlWdbeLibRec::PnlWdbeLibRec(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBELIBREC, jrefSup, ixWdbeVLocale)
		{
...
DpchEngWdbe* PnlWdbeLibRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeLibRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWdbe::getStubLibStd(dbswdbe, recLib.ref, ixWdbeVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recLib.ref == 0) statshr.ixWdbeVExpstate = VecWdbeVExpstate::MIND;
...
void PnlWdbeLibRec::updatePreset(
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFLIB) {
		...
		if (dbswdbe->tblwdbemlibrary->loadRecByRef(xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFLIB, jref), &_recLib)) {
			recLib = *_recLib;
			delete _recLib;
...
void PnlWdbeLibRec::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBELIBRECDO) {
...
void PnlWdbeLibRec::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeLibRec::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeLibRec::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
bool PnlWdbeLibRec::handleCallWdbeLibUpd_refEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
