		...
		if (dbswdbe->tblwdbemmodule->loadRecBySQL("SELECT * FROM TblWdbeMModule WHERE hkIxVTbl = " + to_string(mdl->hkIxVTbl) + " AND hkUref = " + to_string(mdl->hkUref) + " AND ixVBasetype = " + to_string(VecWdbeVMModuleBasetype::CMDBUS), &cmdbus)) {
			...
			if (dbswdbe->tblwdbemmodule->loadRecBySQL("SELECT * FROM TblWdbeMModule WHERE hkIxVTbl = " + to_string(mdl->hkIxVTbl) + " AND hkUref = " + to_string(mdl->hkUref) + " AND ixVBasetype = " + to_string(VecWdbeVMModuleBasetype::CMDRET), &cmdret)) {
				...
				dbswdbe->tblwdbemvectoritem->loadRstBySQL("SELECT TblWdbeMVectoritem.* FROM TblWdbeMSignal, TblWdbeMVectoritem WHERE TblWdbeMSignal.refIxVTbl = " + to_string(VecWdbeVMSignalRefTbl::MDL) + " AND TblWdbeMSignal.refUref = "
							+ to_string(top->ref) + " AND TblWdbeMSignal.sref = 'req" + Cmdbussref + "' AND TblWdbeMVectoritem.vecRefWdbeMVector = TblWdbeMSignal.refWdbeMVector AND TblWdbeMVectoritem.sref LIKE '%To" + StrMod::cap(cmdret->sref)
							+ "' ORDER BY TblWdbeMVectoritem.vecNum ASC", false, vits);
