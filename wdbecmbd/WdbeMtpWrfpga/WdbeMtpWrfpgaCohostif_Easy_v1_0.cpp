/**
	* \file WdbeMtpWrfpgaCohostif_Easy_v1_0.cpp
...
DpchRetWdbe* WdbeMtpWrfpgaCohostif_Easy_v1_0::run(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
...
// IP cust --- IBEGIN
void WdbeMtpWrfpgaCohostif_Easy_v1_0::writeMdlVhd(
			DbsWdbe* dbswdbe
			, fstream& outfile
	...
	// stripped down and modified version from WdbeMtpWrfpgaHostif_Easy_v1_0.cpp
		...
		if ((untmdl->ixVBasetype == VecWdbeVMModuleBasetype::ECTR) || (untmdl->ixVBasetype == VecWdbeVMModuleBasetype::EDBGCTR)) {
			...
			for (unsigned int j = 0; j < cmds.nodes.size(); j++) {
				...
				if (first2) {
					...
					outfile << "(tixVController=tixV" << Untsref << "Controller" << StrMod::cap(untmdl->sref) << " and ";
	...
	if (first) outfile << "'0';" << endl;
	else outfile << "\t\t\t\t) else '0';" << endl;
