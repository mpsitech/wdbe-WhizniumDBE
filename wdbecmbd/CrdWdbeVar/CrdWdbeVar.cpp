/**
	* \file CrdWdbeVar.cpp
	* job handler for job CrdWdbeVar (implementation)
...
CrdWdbeVar::CrdWdbeVar(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEVAR, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFVAR, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWdbeVarList(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlrec = new PnlWdbeVarRec(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbeVarHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbeVar::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeVar::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeVar/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdVar = StubWdbe::getStubVarStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFVAR, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbeVar::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, ixWdbeVPreset, xchg->getRefPreset(ixWdbeVPreset, jref), "CrdWdbeVar", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFVAR, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFVAR, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFVAR, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbeVar::updatePreset(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWdbeVar::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVARDO) {
...
void CrdWdbeVar::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeVar::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEVAR);
};

void CrdWdbeVar::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeVar::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeVar::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFVAR) {
...
bool CrdWdbeVar::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeVar::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWdbeVar::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeVar::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeVar::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
