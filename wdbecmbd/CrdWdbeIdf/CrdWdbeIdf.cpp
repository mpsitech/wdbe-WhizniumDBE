/**
	* \file CrdWdbeIdf.cpp
	* job handler for job CrdWdbeIdf (implementation)
...
CrdWdbeIdf::CrdWdbeIdf(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEIDF, jrefSup, ixWdbeVLocale)
		{
	...
	pnlheadbar = new PnlWdbeIdfHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbeIdf::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeIdf::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeIdf/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
...
void CrdWdbeIdf::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEIDFDO) {
...
void CrdWdbeIdf::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeIdf::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEIDF);
};

void CrdWdbeIdf::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeIdf::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeIdf::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWdbeIdf::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeIdf::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeIdf::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
