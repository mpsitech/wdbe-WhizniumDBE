/**
	* \file CrdWdbeFst.cpp
	* job handler for job CrdWdbeFst (implementation)
...
CrdWdbeFst::CrdWdbeFst(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEFST, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFFST, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWdbeFstRec(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbeFstHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnllist = new PnlWdbeFstList(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbeFst::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeFst::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeFst/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdFst = StubWdbe::getStubFstStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFFST, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbeFst::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, ixWdbeVPreset, xchg->getRefPreset(ixWdbeVPreset, jref), "CrdWdbeFst", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFFST, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFFST, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFFST, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbeFst::updatePreset(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWdbeFst::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEFSTDO) {
...
void CrdWdbeFst::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeFst::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEFST);
};

void CrdWdbeFst::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeFst::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeFst::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeFst::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeFst::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFFST) {
...
void CrdWdbeFst::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeFst::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeFst::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
