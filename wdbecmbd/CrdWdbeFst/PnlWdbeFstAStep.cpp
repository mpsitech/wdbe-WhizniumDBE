/**
	...
	* job handler for job PnlWdbeFstAStep (implementation)
...
#include "PnlWdbeFstAStep_blks.cpp"
#include "PnlWdbeFstAStep_evals.cpp"
...
/******************************************************************************
 class PnlWdbeFstAStep
 ******************************************************************************/

PnlWdbeFstAStep::PnlWdbeFstAStep(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEFSTASTEP, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeFstAStep(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeFstAStep::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeFstAStep::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeFstAStep::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEFSTASTEPDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEFSTASTEPDO) {
...
void PnlWdbeFstAStep::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeFstAStep::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFstAStep::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeFstAStep::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQFstAStep* recSelNew = NULL;

	WdbeAMFsmstateStep* _recFstAstp = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeFstAStep::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeFstAStep::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeFstAStep::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recFstAstp = WdbeAMFsmstateStep();

			if (recSelNew) {
				if (dbswdbe->tblwdbeamfsmstatestep->loadRecByRef(recSelNew->ref, &_recFstAstp)) {
					recFstAstp = *_recFstAstp;
					delete _recFstAstp;
				};
			};
...
void PnlWdbeFstAStep::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFstAStep::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFstAStep::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFstAStep::handleDpchAppDoButDuplicateClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFstAStep::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFstAStep::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeFstAStep::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeFstAStep::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
