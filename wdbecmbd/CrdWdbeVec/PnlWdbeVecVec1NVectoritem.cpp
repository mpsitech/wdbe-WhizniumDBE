/**
	...
	* job handler for job PnlWdbeVecVec1NVectoritem (implementation)
...
#include "PnlWdbeVecVec1NVectoritem_blks.cpp"
#include "PnlWdbeVecVec1NVectoritem_evals.cpp"
...
/******************************************************************************
 class PnlWdbeVecVec1NVectoritem
 ******************************************************************************/

PnlWdbeVecVec1NVectoritem::PnlWdbeVecVec1NVectoritem(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEVECVEC1NVECTORITEM, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeVecVec1NVectoritem(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeVecVec1NVectoritem::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeVecVec1NVectoritem::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeVecVec1NVectoritem::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeVecVec1NVectoritem::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVECVEC1NVECTORITEMDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVECVEC1NVECTORITEMDO) {
...
void PnlWdbeVecVec1NVectoritem::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeVecVec1NVectoritem::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVecVec1NVectoritem::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeVecVec1NVectoritem::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQVecVec1NVectoritem* recSelNew = NULL;

	WdbeMVectoritem* _recVit = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeVecVec1NVectoritem::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeVecVec1NVectoritem::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeVecVec1NVectoritem::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recVit = WdbeMVectoritem();

			if (recSelNew) {
				if (dbswdbe->tblwdbemvectoritem->loadRecByRef(recSelNew->ref, &_recVit)) {
					recVit = *_recVit;
					delete _recVit;
				};
			};
...
void PnlWdbeVecVec1NVectoritem::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVecVec1NVectoritem::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVecVec1NVectoritem::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refVec = xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFVEC, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCVIT, jref)) if (refVec != 0) {
			sref = "CrdWdbeVit";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFVEC, refVec, sref, recVit.ref, jrefNew);
		};
...
void PnlWdbeVecVec1NVectoritem::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCVIT, jref) & VecWdbeWAccess::EDIT) != 0) if (refVec != 0) {
			sref = "CrdWdbeVit";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFVEC, refVec, sref, -1, jrefNew);
		};
...
void PnlWdbeVecVec1NVectoritem::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVecVec1NVectoritem::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeVecVec1NVectoritem::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeVecVec1NVectoritem::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
