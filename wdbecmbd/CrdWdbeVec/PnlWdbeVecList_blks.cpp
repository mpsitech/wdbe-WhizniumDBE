/**
	* \file PnlWdbeVecList_blks.cpp
	* job handler for job PnlWdbeVecList (implementation of blocks)
...
/******************************************************************************
 class PnlWdbeVecList::VecVDo
 ******************************************************************************/

uint PnlWdbeVecList::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWdbeVecList::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWdbeVecList::ContIac
 ******************************************************************************/

PnlWdbeVecList::ContIac::ContIac(
			const uint numFTos
		) :
			Block()
		{
...
bool PnlWdbeVecList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWdbeVecList"];}();
...
bool PnlWdbeVecList::ContIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContIacWdbeVecList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemIacWdbeVecList";
...
void PnlWdbeVecList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContIacWdbeVecList";
...
void PnlWdbeVecList::ContIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemIacWdbeVecList";
...
set<uint> PnlWdbeVecList::ContIac::comm(
			const ContIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeVecList::ContIac::diff(
			const ContIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeVecList::ContInf
 ******************************************************************************/

PnlWdbeVecList::ContInf::ContInf(
			const string& TxtFor
			, const string& TxtPre
...
void PnlWdbeVecList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeVecList";
...
void PnlWdbeVecList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWdbeVecList";
...
set<uint> PnlWdbeVecList::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeVecList::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeVecList::StatShr
 ******************************************************************************/

PnlWdbeVecList::StatShr::StatShr(
...
void PnlWdbeVecList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeVecList";
...
void PnlWdbeVecList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWdbeVecList";
...
set<uint> PnlWdbeVecList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeVecList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeVecList::StgIac
 ******************************************************************************/

PnlWdbeVecList::StgIac::StgIac(
	...
	mask = {TCOSRFWIDTH, TCOTYPWIDTH, TCOHKTWIDTH, TCOHKUWIDTH};
};

bool PnlWdbeVecList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWdbeVecList"];}();
...
bool PnlWdbeVecList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWdbeVecList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacWdbeVecList";
...
void PnlWdbeVecList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacWdbeVecList";
...
void PnlWdbeVecList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacWdbeVecList";
...
set<uint> PnlWdbeVecList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeVecList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {TCOSRFWIDTH, TCOTYPWIDTH, TCOHKTWIDTH, TCOHKUWIDTH};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
/******************************************************************************
 class PnlWdbeVecList::Tag
 ******************************************************************************/

void PnlWdbeVecList::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeVecList";
...
void PnlWdbeVecList::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeVecList";
...
/******************************************************************************
 class PnlWdbeVecList::DpchAppData
 ******************************************************************************/

PnlWdbeVecList::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEVECLISTDATA)
		{
};

string PnlWdbeVecList::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeVecList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeVecListData"];}();
	...
	if (basefound) {
		...
		contiac = ContIac();
		stgiac = StgIac();
		stgiacqry = QryWdbeVecList::StgIac();
	};
};

void PnlWdbeVecList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeVecListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeVecList::DpchAppDo
 ******************************************************************************/

PnlWdbeVecList::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEVECLISTDO)
		{
	ixVDo = 0;
};

string PnlWdbeVecList::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeVecList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeVecListDo"];}();
...
void PnlWdbeVecList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeVecListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeVecList::DpchEngData
 ******************************************************************************/

PnlWdbeVecList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, QryWdbeVecList::StatShr* statshrqry
			, QryWdbeVecList::StgIac* stgiacqry
			, const set<uint>& mask
		) :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEVECLISTDATA, jref)
		{
...
string PnlWdbeVecList::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeVecList::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeVecList::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeVecListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeVecList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeVecList::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeVecListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeVecList::StatApp::writeXML(wr);
