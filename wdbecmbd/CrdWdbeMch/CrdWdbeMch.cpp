/**
	* \file CrdWdbeMch.cpp
	* job handler for job CrdWdbeMch (implementation)
...
CrdWdbeMch::CrdWdbeMch(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEMCH, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFMCH, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWdbeMchList(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbeMchHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlrec = new PnlWdbeMchRec(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbeMch::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeMch::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeMch/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdMch = StubWdbe::getStubMchStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFMCH, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbeMch::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, VecWdbeVPreset::VOID, 0, "CrdWdbeMch", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFMCH, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFMCH, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFMCH, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbeMch::updatePreset(
			DbsWdbe* dbswdbe
...
void CrdWdbeMch::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMCHDO) {
...
void CrdWdbeMch::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeMch::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEMCH);
};

void CrdWdbeMch::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeMch::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeMch::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFMCH) {
...
bool CrdWdbeMch::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeMch::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWdbeMch::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeMch::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeMch::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
