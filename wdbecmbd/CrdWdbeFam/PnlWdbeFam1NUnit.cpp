/**
	* \file PnlWdbeFam1NUnit.cpp
	* job handler for job PnlWdbeFam1NUnit (implementation)
...
#include "PnlWdbeFam1NUnit_blks.cpp"
#include "PnlWdbeFam1NUnit_evals.cpp"
...
/******************************************************************************
 class PnlWdbeFam1NUnit
 ******************************************************************************/

PnlWdbeFam1NUnit::PnlWdbeFam1NUnit(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEFAM1NUNIT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeFam1NUnit(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeFam1NUnit::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeFam1NUnit::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeFam1NUnit::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEFAM1NUNITDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEFAM1NUNITDO) {
...
void PnlWdbeFam1NUnit::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeFam1NUnit::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFam1NUnit::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeFam1NUnit::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeFam1NUnit::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeFam1NUnit::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeFam1NUnit::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recUnt = WdbeMUnit();

			if (recSelNew) {
				if (dbswdbe->tblwdbemunit->loadRecByRef(recSelNew->ref, &_recUnt)) {
					recUnt = *_recUnt;
...
void PnlWdbeFam1NUnit::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCUNT, jref)) if (refVer != 0) {
			sref = "CrdWdbeUnt";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFVER, refVer, sref, recUnt.ref, jrefNew);
		};
		if (jrefNew == 0) {
			if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSIL, jref)) if ((ixWSubsetUnt & VecWdbeWMUnitSubset::SBSWDBEBMUNITSIL) != 0) {
				sref = "CrdWdbeSil";
				xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recUnt.ref, jrefNew);
			};
		};
...
void PnlWdbeFam1NUnit::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCUNT, jref) & VecWdbeWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWdbeUnt";
...
void PnlWdbeFam1NUnit::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFam1NUnit::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeFam1NUnit::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeFam1NUnit::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
