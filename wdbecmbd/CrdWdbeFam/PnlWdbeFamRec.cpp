...
#include "PnlWdbeFamRec_blks.cpp"
#include "PnlWdbeFamRec_evals.cpp"
...
PnlWdbeFamRec::PnlWdbeFamRec(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEFAMREC, jrefSup, ixWdbeVLocale)
		{
...
DpchEngWdbe* PnlWdbeFamRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeFamRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWdbe::getStubFamStd(dbswdbe, recFam.ref, ixWdbeVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recFam.ref == 0) statshr.ixWdbeVExpstate = VecWdbeVExpstate::MIND;
	...
	if (statshr.ixWdbeVExpstate == VecWdbeVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWdbeFamDetail(xchg, dbswdbe, jref, ixWdbeVLocale);
		if (!pnl1nunit) pnl1nunit = new PnlWdbeFam1NUnit(xchg, dbswdbe, jref, ixWdbeVLocale);
	};
...
void PnlWdbeFamRec::updatePreset(
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFFAM) {
		...
		if (dbswdbe->tblwdbemfamily->loadRecByRef(xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFFAM, jref), &_recFam)) {
			recFam = *_recFam;
			delete _recFam;
...
void PnlWdbeFamRec::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEFAMRECDO) {
...
void PnlWdbeFamRec::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeFamRec::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFamRec::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
bool PnlWdbeFamRec::handleCallWdbeFamUpd_refEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
