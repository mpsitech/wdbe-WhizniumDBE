/**
	* \file CrdWdbePpl.cpp
	* job handler for job CrdWdbePpl (implementation)
...
CrdWdbePpl::CrdWdbePpl(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEPPL, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFPPL, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWdbePplList(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbePplHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlrec = new PnlWdbePplRec(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbePpl::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbePpl::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbePpl/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdPpl = StubWdbe::getStubPplStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFPPL, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbePpl::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, ixWdbeVPreset, xchg->getRefPreset(ixWdbeVPreset, jref), "CrdWdbePpl", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFPPL, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFPPL, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFPPL, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbePpl::updatePreset(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWdbePpl::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPPLDO) {
...
void CrdWdbePpl::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbePpl::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEPPL);
};

void CrdWdbePpl::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbePpl::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbePpl::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFPPL) {
...
bool CrdWdbePpl::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbePpl::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWdbePpl::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbePpl::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbePpl::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
