/**
	...
	* job handler for job PnlWdbePplPpl1NSegment (implementation)
...
#include "PnlWdbePplPpl1NSegment_blks.cpp"
#include "PnlWdbePplPpl1NSegment_evals.cpp"
...
/******************************************************************************
 class PnlWdbePplPpl1NSegment
 ******************************************************************************/

PnlWdbePplPpl1NSegment::PnlWdbePplPpl1NSegment(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPPLPPL1NSEGMENT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePplPpl1NSegment(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePplPpl1NSegment::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePplPpl1NSegment::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbePplPpl1NSegment::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbePplPpl1NSegment::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPPLPPL1NSEGMENTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPPLPPL1NSEGMENTDO) {
...
void PnlWdbePplPpl1NSegment::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePplPpl1NSegment::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePplPpl1NSegment::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePplPpl1NSegment::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPplPpl1NSegment* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePplPpl1NSegment::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePplPpl1NSegment::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePplPpl1NSegment::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSeg = WdbeMSegment();

			if (recSelNew) {
				if (dbswdbe->tblwdbemsegment->loadRecByRef(recSelNew->ref, &_recSeg)) {
					recSeg = *_recSeg;
					delete _recSeg;
				};
			};
...
void PnlWdbePplPpl1NSegment::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePplPpl1NSegment::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePplPpl1NSegment::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refPpl = xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFPPL, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSEG, jref)) if (refPpl != 0) {
			sref = "CrdWdbeSeg";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFPPL, refPpl, sref, recSeg.ref, jrefNew);
		};
...
void PnlWdbePplPpl1NSegment::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSEG, jref) & VecWdbeWAccess::EDIT) != 0) if (refPpl != 0) {
			sref = "CrdWdbeSeg";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFPPL, refPpl, sref, -1, jrefNew);
		};
...
void PnlWdbePplPpl1NSegment::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePplPpl1NSegment::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePplPpl1NSegment::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePplPpl1NSegment::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
