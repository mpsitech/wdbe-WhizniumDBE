/**
	...
	* job handler for job PnlWdbeUsr1NSession (implementation)
...
#include "PnlWdbeUsr1NSession_blks.cpp"
#include "PnlWdbeUsr1NSession_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUsr1NSession
 ******************************************************************************/

PnlWdbeUsr1NSession::PnlWdbeUsr1NSession(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUSR1NSESSION, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUsr1NSession(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUsr1NSession::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUsr1NSession::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeUsr1NSession::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUsr1NSession::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUSR1NSESSIONDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUSR1NSESSIONDO) {
...
void PnlWdbeUsr1NSession::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUsr1NSession::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsr1NSession::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUsr1NSession::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUsr1NSession* recSelNew = NULL;

	WdbeMSession* _recSes = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUsr1NSession::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUsr1NSession::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUsr1NSession::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSes = WdbeMSession();

			if (recSelNew) {
				if (dbswdbe->tblwdbemsession->loadRecByRef(recSelNew->ref, &_recSes)) {
					recSes = *_recSes;
					delete _recSes;
				};
			};
...
void PnlWdbeUsr1NSession::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsr1NSession::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUsr1NSession::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUsr1NSession::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
