/**
	...
	* job handler for job PnlWdbeUsrMNUsergroup (implementation)
...
#include "PnlWdbeUsrMNUsergroup_blks.cpp"
#include "PnlWdbeUsrMNUsergroup_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUsrMNUsergroup
 ******************************************************************************/

PnlWdbeUsrMNUsergroup::PnlWdbeUsrMNUsergroup(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUSRMNUSERGROUP, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUsrMNUsergroup(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUsrMNUsergroup::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUsrMNUsergroup::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeUsrMNUsergroup::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFUSR) {
		recUsrRusg = WdbeRMUserMUsergroup();

		recUsg = WdbeMUsergroup();
...
void PnlWdbeUsrMNUsergroup::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUSRMNUSERGROUPDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUSRMNUSERGROUPDO) {
...
void PnlWdbeUsrMNUsergroup::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUsrMNUsergroup::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsrMNUsergroup::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUsrMNUsergroup::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUsrMNUsergroup* recSelNew = NULL;

	WdbeRMUserMUsergroup* _recUsrRusg = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUsrMNUsergroup::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUsrMNUsergroup::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUsrMNUsergroup::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswdbe->tblwdbermusermusergroup->loadRecByRef(recSelNew->ref, &_recUsrRusg)) {
					recUsrRusg = *_recUsrRusg;
					delete _recUsrRusg;
				};
				if (dbswdbe->tblwdbemusergroup->loadRecByRef(recSelNew->mref, &_recUsg)) {
					recUsg = *_recUsg;
					delete _recUsg;
				};
			};
...
void PnlWdbeUsrMNUsergroup::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCUSG, jref)) {
			sref = "CrdWdbeUsg";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recUsg.ref, jrefNew);
		};
...
void PnlWdbeUsrMNUsergroup::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsrMNUsergroup::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUsrMNUsergroup::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUsrMNUsergroup::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUsrMNUsergroup::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
