/**
	* \file CrdWdbeUsr_blks.cpp
	* job handler for job CrdWdbeUsr (implementation of blocks)
...
void CrdWdbeUsr::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdWdbeUsr::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeUsr";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	me["numFSge"] = (Json::Value::UInt) numFSge;
	me["MrlAppHlp"] = MrlAppHlp;
	me["MtxCrdUsr"] = MtxCrdUsr;
};

void CrdWdbeUsr::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemInfWdbeUsr";
...
void CrdWdbeUsr::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
	...
	if (difftag.length() == 0) difftag = "StatAppWdbeUsr";
...
void CrdWdbeUsr::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixWdbeVReqitmode
	...
	else itemtag = "StatitemAppWdbeUsr";
...
CrdWdbeUsr::StatShr::StatShr(
			const ubigint jrefDlgmngkeypair
			...
			Block()
		{
	this->jrefDlgmngkeypair = jrefDlgmngkeypair;
	...
	mask = {JREFDLGMNGKEYPAIR, JREFHEADBAR, JREFLIST, JREFREC, MITCRDMKPAVAIL, MITCRDMKPACTIVE};
};

void CrdWdbeUsr::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeUsr";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	me["scrJrefDlgmngkeypair"] = Scr::scramble(jrefDlgmngkeypair);
	me["scrJrefHeadbar"] = Scr::scramble(jrefHeadbar);
	me["scrJrefList"] = Scr::scramble(jrefList);
	me["scrJrefRec"] = Scr::scramble(jrefRec);
	me["MitCrdMkpAvail"] = MitCrdMkpAvail;
	me["MitCrdMkpActive"] = MitCrdMkpActive;
};

void CrdWdbeUsr::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrWdbeUsr";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeStringAttr(wr, itemtag, "sref", "scrJrefDlgmngkeypair", Scr::scramble(jrefDlgmngkeypair));
		...
		writeBoolAttr(wr, itemtag, "sref", "MitCrdMkpAvail", MitCrdMkpAvail);
		writeBoolAttr(wr, itemtag, "sref", "MitCrdMkpActive", MitCrdMkpActive);
	xmlTextWriterEndElement(wr);
};
...
set<uint> CrdWdbeUsr::StatShr::comm(
	...
	if (jrefDlgmngkeypair == comp->jrefDlgmngkeypair) insert(items, JREFDLGMNGKEYPAIR);
...
set<uint> CrdWdbeUsr::StatShr::diff(
	...
	diffitems = {JREFDLGMNGKEYPAIR, JREFHEADBAR, JREFLIST, JREFREC, MITCRDMKPAVAIL, MITCRDMKPACTIVE};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
void CrdWdbeUsr::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeUsr";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
		me["MitCrdMkp"] = "Manage SSH key pair ...";
	};
...
void CrdWdbeUsr::Tag::writeXML(
	...
	else itemtag = "TagitemWdbeUsr";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "MitCrdMkp", "Manage SSH key pair ...");
		};
...
CrdWdbeUsr::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEUSRDO)
		{
	ixVDo = 0;
};
...
void CrdWdbeUsr::DpchAppDo::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeUsrDo"];}();
...
void CrdWdbeUsr::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeUsrDo");
	else
...
CrdWdbeUsr::DpchEngData::DpchEngData(
			...
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEUSRDATA, jref)
		{
...
void CrdWdbeUsr::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void CrdWdbeUsr::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeUsrData"] = Json::Value(Json::objectValue);
...
void CrdWdbeUsr::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeUsrData");
