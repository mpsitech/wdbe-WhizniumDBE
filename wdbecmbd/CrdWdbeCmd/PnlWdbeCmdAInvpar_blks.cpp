/**
	* \file PnlWdbeCmdAInvpar_blks.cpp
	* job handler for job PnlWdbeCmdAInvpar (implementation of blocks)
...
void PnlWdbeCmdAInvpar::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeCmdAInvpar";
...
void PnlWdbeCmdAInvpar::ContInf::writeXML(
	...
	else itemtag = "ContitemInfWdbeCmdAInvpar";
...
void PnlWdbeCmdAInvpar::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
			, const uint ixWdbeVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppWdbeCmdAInvpar";
...
void PnlWdbeCmdAInvpar::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixWdbeVExpstate
	...
	else itemtag = "StatitemAppWdbeCmdAInvpar";
...
void PnlWdbeCmdAInvpar::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeCmdAInvpar";
...
void PnlWdbeCmdAInvpar::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrWdbeCmdAInvpar";
...
void PnlWdbeCmdAInvpar::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
void PnlWdbeCmdAInvpar::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeCmdAInvpar";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
		me["Cpt"] = "Invocation parameters";
		me["TcoSrf"] = "Identifier";
		me["TcoPty"] = "Par. type";
		me["TcoVec"] = "Vector";
		me["TcoLen"] = "Length";
		me["TcoDfv"] = "Default value";
		me["TcoVit"] = "Vector item";
		me["TcoCmt"] = "Comment";
	};
...
void PnlWdbeCmdAInvpar::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeCmdAInvpar";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Invocation parameters");
			...
			writeStringAttr(wr, itemtag, "sref", "TcoDfv", "Default value");
			writeStringAttr(wr, itemtag, "sref", "TcoVit", "Vector item");
...
PnlWdbeCmdAInvpar::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECMDAINVPARDATA)
		{
};
...
void PnlWdbeCmdAInvpar::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCmdAInvparData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeCmdAInvpar::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECMDAINVPARDO)
		{
	ixVDo = 0;
};
...
void PnlWdbeCmdAInvpar::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCmdAInvparDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeCmdAInvpar::DpchEngData::DpchEngData(
			...
			, QryWdbeCmdAInvpar::StatShr* statshrqry
			, QryWdbeCmdAInvpar::StgIac* stgiacqry
			, const set<uint>& mask
		) :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBECMDAINVPARDATA, jref)
		{
...
void PnlWdbeCmdAInvpar::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeCmdAInvpar::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeCmdAInvparData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeCmdAInvpar::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeCmdAInvpar::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeCmdAInvparData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeCmdAInvpar::StatApp::writeXML(wr);
