/**
	...
	* job handler for job PnlWdbeCmdARetpar (implementation)
...
#include "PnlWdbeCmdARetpar_blks.cpp"
#include "PnlWdbeCmdARetpar_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCmdARetpar
 ******************************************************************************/

PnlWdbeCmdARetpar::PnlWdbeCmdARetpar(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECMDARETPAR, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeCmdARetpar(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeCmdARetpar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCmdARetpar::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeCmdARetpar::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFCMD) {
		recCmdArpa = WdbeAMCommandRetpar();
...
void PnlWdbeCmdARetpar::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECMDARETPARDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECMDARETPARDO) {
...
void PnlWdbeCmdARetpar::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCmdARetpar::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdARetpar::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCmdARetpar::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQCmdARetpar* recSelNew = NULL;

	WdbeAMCommandRetpar* _recCmdArpa = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeCmdARetpar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeCmdARetpar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeCmdARetpar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCmdArpa = WdbeAMCommandRetpar();

			if (recSelNew) {
				if (dbswdbe->tblwdbeamcommandretpar->loadRecByRef(recSelNew->ref, &_recCmdArpa)) {
					recCmdArpa = *_recCmdArpa;
					delete _recCmdArpa;
				};
			};
...
void PnlWdbeCmdARetpar::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdARetpar::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdARetpar::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdARetpar::handleDpchAppDoButDuplicateClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdARetpar::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdARetpar::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCmdARetpar::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCmdARetpar::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
