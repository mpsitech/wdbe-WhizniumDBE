/**
	* \file PnlWdbeCmdARetpar_blks.cpp
	* job handler for job PnlWdbeCmdARetpar (implementation of blocks)
...
/******************************************************************************
 class PnlWdbeCmdARetpar::VecVDo
 ******************************************************************************/

uint PnlWdbeCmdARetpar::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWdbeCmdARetpar::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWdbeCmdARetpar::ContInf
 ******************************************************************************/

PnlWdbeCmdARetpar::ContInf::ContInf(
...
void PnlWdbeCmdARetpar::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeCmdARetpar";
...
void PnlWdbeCmdARetpar::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemInfWdbeCmdARetpar";
...
set<uint> PnlWdbeCmdARetpar::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCmdARetpar::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeCmdARetpar::StatApp
 ******************************************************************************/

void PnlWdbeCmdARetpar::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
			, const uint ixWdbeVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppWdbeCmdARetpar";
...
void PnlWdbeCmdARetpar::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixWdbeVExpstate
	...
	else itemtag = "StatitemAppWdbeCmdARetpar";
...
/******************************************************************************
 class PnlWdbeCmdARetpar::StatShr
 ******************************************************************************/

PnlWdbeCmdARetpar::StatShr::StatShr(
...
void PnlWdbeCmdARetpar::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeCmdARetpar";
...
void PnlWdbeCmdARetpar::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrWdbeCmdARetpar";
...
set<uint> PnlWdbeCmdARetpar::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCmdARetpar::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeCmdARetpar::StgIac
 ******************************************************************************/

PnlWdbeCmdARetpar::StgIac::StgIac(
...
bool PnlWdbeCmdARetpar::StgIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWdbeCmdARetpar"];}();

	basefound = (me != Json::nullValue);

	if (basefound) {
		if (me.isMember("TcoSrfWidth")) {TcoSrfWidth = me["TcoSrfWidth"].asUInt(); add(TCOSRFWIDTH);};
		if (me.isMember("TcoPtyWidth")) {TcoPtyWidth = me["TcoPtyWidth"].asUInt(); add(TCOPTYWIDTH);};
		if (me.isMember("TcoVecWidth")) {TcoVecWidth = me["TcoVecWidth"].asUInt(); add(TCOVECWIDTH);};
		if (me.isMember("TcoLenWidth")) {TcoLenWidth = me["TcoLenWidth"].asUInt(); add(TCOLENWIDTH);};
...
bool PnlWdbeCmdARetpar::StgIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWdbeCmdARetpar");
	else
...
void PnlWdbeCmdARetpar::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacWdbeCmdARetpar";
...
void PnlWdbeCmdARetpar::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
...
set<uint> PnlWdbeCmdARetpar::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCmdARetpar::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {TCOSRFWIDTH, TCOPTYWIDTH, TCOVECWIDTH, TCOLENWIDTH, TCOCMTWIDTH};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
/******************************************************************************
 class PnlWdbeCmdARetpar::Tag
 ******************************************************************************/

void PnlWdbeCmdARetpar::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeCmdARetpar";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
		me["Cpt"] = "Return parameters";
...
void PnlWdbeCmdARetpar::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeCmdARetpar";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Return parameters");
			writeStringAttr(wr, itemtag, "sref", "TcoSrf", "Identifier");
			writeStringAttr(wr, itemtag, "sref", "TcoPty", "Par. type");
			writeStringAttr(wr, itemtag, "sref", "TcoVec", "Vector");
			writeStringAttr(wr, itemtag, "sref", "TcoLen", "Length");
...
/******************************************************************************
 class PnlWdbeCmdARetpar::DpchAppData
 ******************************************************************************/

PnlWdbeCmdARetpar::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECMDARETPARDATA)
		{
};

string PnlWdbeCmdARetpar::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCmdARetpar::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCmdARetparData"];}();
	...
	if (basefound) {
		...
		stgiac = StgIac();
		stgiacqry = QryWdbeCmdARetpar::StgIac();
	};
};

void PnlWdbeCmdARetpar::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCmdARetparData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeCmdARetpar::DpchAppDo
 ******************************************************************************/

PnlWdbeCmdARetpar::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECMDARETPARDO)
		{
	ixVDo = 0;
};

string PnlWdbeCmdARetpar::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCmdARetpar::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCmdARetparDo"];}();
...
void PnlWdbeCmdARetpar::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCmdARetparDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeCmdARetpar::DpchEngData
 ******************************************************************************/

PnlWdbeCmdARetpar::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, QryWdbeCmdARetpar::StatShr* statshrqry
			, QryWdbeCmdARetpar::StgIac* stgiacqry
			, const set<uint>& mask
		) :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBECMDARETPARDATA, jref)
		{
...
string PnlWdbeCmdARetpar::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCmdARetpar::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeCmdARetpar::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeCmdARetparData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeCmdARetpar::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeCmdARetpar::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeCmdARetparData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeCmdARetpar::StatApp::writeXML(wr);
