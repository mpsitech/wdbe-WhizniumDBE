/**
	* \file PnlWdbeCmdMNController_evals.cpp
	* job handler for job PnlWdbeCmdMNController (implementation of availability/activation evaluation)
...
bool PnlWdbeCmdMNController::evalButViewAvail(
			DbsWdbe* dbswdbe
		) {
	// !sel()|((pre.ixCrdaccMod()&ctr.mdl.inSbs(mod)&pre.refUnt())|(pre.ixCrdaccMod()&ctr.mdl.inSbs(mod)&pre.refCvr())|(pre.ixCrdaccMtp()&ctr.mdl.inSbs(mtp)))
...
bool PnlWdbeCmdMNController::evalButViewActive(
			DbsWdbe* dbswdbe
		) {
	// sel()
...
bool PnlWdbeCmdMNController::evalButAddAvail(
			DbsWdbe* dbswdbe
		) {
...
bool PnlWdbeCmdMNController::evalButSubAvail(
			DbsWdbe* dbswdbe
		) {
	// pre.ixCrdaccCmdIncl(edit)
...
bool PnlWdbeCmdMNController::evalButSubActive(
			DbsWdbe* dbswdbe
		) {
	// sel()
