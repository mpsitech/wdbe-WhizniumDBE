/**
	...
	* job handler for job PnlWdbeCmdAInvpar (implementation)
...
#include "PnlWdbeCmdAInvpar_blks.cpp"
#include "PnlWdbeCmdAInvpar_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCmdAInvpar
 ******************************************************************************/

PnlWdbeCmdAInvpar::PnlWdbeCmdAInvpar(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECMDAINVPAR, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeCmdAInvpar(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeCmdAInvpar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCmdAInvpar::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeCmdAInvpar::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFCMD) {
		recCmdAipa = WdbeAMCommandInvpar();
...
void PnlWdbeCmdAInvpar::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECMDAINVPARDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECMDAINVPARDO) {
...
void PnlWdbeCmdAInvpar::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCmdAInvpar::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdAInvpar::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCmdAInvpar::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQCmdAInvpar* recSelNew = NULL;

	WdbeAMCommandInvpar* _recCmdAipa = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeCmdAInvpar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeCmdAInvpar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeCmdAInvpar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCmdAipa = WdbeAMCommandInvpar();

			if (recSelNew) {
				if (dbswdbe->tblwdbeamcommandinvpar->loadRecByRef(recSelNew->ref, &_recCmdAipa)) {
					recCmdAipa = *_recCmdAipa;
					delete _recCmdAipa;
				};
			};
...
void PnlWdbeCmdAInvpar::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdAInvpar::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdAInvpar::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdAInvpar::handleDpchAppDoButDuplicateClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdAInvpar::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmdAInvpar::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCmdAInvpar::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCmdAInvpar::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
