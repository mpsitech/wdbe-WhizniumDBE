/**
	...
	* job handler for job PnlWdbePinRec (implementation)
...
#include "PnlWdbePinRec_blks.cpp"
#include "PnlWdbePinRec_evals.cpp"
...
PnlWdbePinRec::PnlWdbePinRec(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPINREC, jrefSup, ixWdbeVLocale)
		{
...
DpchEngWdbe* PnlWdbePinRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePinRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWdbe::getStubPinStd(dbswdbe, recPin.ref, ixWdbeVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recPin.ref == 0) statshr.ixWdbeVExpstate = VecWdbeVExpstate::MIND;
	...
	if (statshr.ixWdbeVExpstate == VecWdbeVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWdbePinDetail(xchg, dbswdbe, jref, ixWdbeVLocale);
		if (!pnlapar) pnlapar = new PnlWdbePinAPar(xchg, dbswdbe, jref, ixWdbeVLocale);
	};
...
void PnlWdbePinRec::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFPIN) {
		...
		if (dbswdbe->tblwdbempin->loadRecByRef(xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFPIN, jref), &_recPin)) {
			recPin = *_recPin;
			delete _recPin;
...
void PnlWdbePinRec::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPINRECDO) {
...
void PnlWdbePinRec::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePinRec::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePinRec::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
