/**
	...
	* job handler for job PnlWdbePphAPar (implementation)
...
#include "PnlWdbePphAPar_blks.cpp"
#include "PnlWdbePphAPar_evals.cpp"
...
/******************************************************************************
 class PnlWdbePphAPar
 ******************************************************************************/

PnlWdbePphAPar::PnlWdbePphAPar(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPPHAPAR, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePphAPar(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePphAPar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePphAPar::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFPPH) {
		recPphApar = WdbeAMPeripheralPar();
...
void PnlWdbePphAPar::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPPHAPARDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPPHAPARDO) {
...
void PnlWdbePphAPar::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePphAPar::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePphAPar::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePphAPar::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPphAPar* recSelNew = NULL;

	WdbeAMPeripheralPar* _recPphApar = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePphAPar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePphAPar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePphAPar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recPphApar = WdbeAMPeripheralPar();

			if (recSelNew) {
				if (dbswdbe->tblwdbeamperipheralpar->loadRecByRef(recSelNew->ref, &_recPphApar)) {
					recPphApar = *_recPphApar;
					delete _recPphApar;
				};
			};
...
void PnlWdbePphAPar::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePphAPar::handleDpchAppDoButDuplicateClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePphAPar::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePphAPar::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePphAPar::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePphAPar::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
