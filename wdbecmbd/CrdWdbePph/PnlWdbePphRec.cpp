/**
	...
	* job handler for job PnlWdbePphRec (implementation)
...
#include "PnlWdbePphRec_blks.cpp"
#include "PnlWdbePphRec_evals.cpp"
...
PnlWdbePphRec::PnlWdbePphRec(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPPHREC, jrefSup, ixWdbeVLocale)
		{
...
DpchEngWdbe* PnlWdbePphRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePphRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWdbe::getStubPphStd(dbswdbe, recPph.ref, ixWdbeVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recPph.ref == 0) statshr.ixWdbeVExpstate = VecWdbeVExpstate::MIND;
	...
	if (statshr.ixWdbeVExpstate == VecWdbeVExpstate::MIND) {
		...
		if (pnlmnmodule) {delete pnlmnmodule; pnlmnmodule = NULL;};
	} else {
		if (!pnldetail) pnldetail = new PnlWdbePphDetail(xchg, dbswdbe, jref, ixWdbeVLocale);
		if (!pnlapar) pnlapar = new PnlWdbePphAPar(xchg, dbswdbe, jref, ixWdbeVLocale);
		if (!pnlmnmodule) pnlmnmodule = new PnlWdbePphMNModule(xchg, dbswdbe, jref, ixWdbeVLocale);
	};

	statshr.jrefDetail = ((pnldetail) ? pnldetail->jref : 0);
	statshr.jrefAPar = ((pnlapar) ? pnlapar->jref : 0);
	statshr.jrefMNModule = ((pnlmnmodule) ? pnlmnmodule->jref : 0);
...
void PnlWdbePphRec::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFPPH) {
		...
		if (dbswdbe->tblwdbemperipheral->loadRecByRef(xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFPPH, jref), &_recPph)) {
			recPph = *_recPph;
			delete _recPph;
		...
		if (recPph.ref != 0) {
			...
			if (pnlmnmodule) pnlmnmodule->updatePreset(dbswdbe, ixWdbeVPreset, jrefTrig, notif);
		};
...
void PnlWdbePphRec::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPPHRECDO) {
...
void PnlWdbePphRec::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePphRec::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePphRec::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePphRec::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
	if (call->ixVCall == VecWdbeVCall::CALLWDBEPPH_UNT_INSBS) {
...
bool PnlWdbePphRec::handleCallWdbePph_unt_inSbs(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWdbePphRec::handleCallWdbePph_untEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
...
bool PnlWdbePphRec::handleCallWdbePphUpd_refEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
