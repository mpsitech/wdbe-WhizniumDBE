/**
	* \file CrdWdbePph.cpp
	* job handler for job CrdWdbePph (implementation)
...
CrdWdbePph::CrdWdbePph(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEPPH, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFPPH, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWdbePphList(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbePphHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlrec = new PnlWdbePphRec(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbePph::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbePph::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbePph/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdPph = StubWdbe::getStubPphStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFPPH, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbePph::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, ixWdbeVPreset, xchg->getRefPreset(ixWdbeVPreset, jref), "CrdWdbePph", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFPPH, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFPPH, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFPPH, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbePph::updatePreset(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWdbePph::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPPHDO) {
...
void CrdWdbePph::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbePph::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEPPH);
};

void CrdWdbePph::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbePph::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbePph::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFPPH) {
...
bool CrdWdbePph::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbePph::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWdbePph::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbePph::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbePph::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
