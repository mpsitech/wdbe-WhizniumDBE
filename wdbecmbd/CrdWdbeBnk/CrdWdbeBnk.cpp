/**
	* \file CrdWdbeBnk.cpp
	* job handler for job CrdWdbeBnk (implementation)
...
CrdWdbeBnk::CrdWdbeBnk(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEBNK, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFBNK, ref);
...
DpchEngWdbe* CrdWdbeBnk::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeBnk::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeBnk/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdBnk = StubWdbe::getStubBnkStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFBNK, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbeBnk::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, ixWdbeVPreset, xchg->getRefPreset(ixWdbeVPreset, jref), "CrdWdbeBnk", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFBNK, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFBNK, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFBNK, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbeBnk::updatePreset(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWdbeBnk::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEBNKDO) {
...
void CrdWdbeBnk::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeBnk::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEBNK);
};

void CrdWdbeBnk::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeBnk::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeBnk::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFBNK) {
...
bool CrdWdbeBnk::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeBnk::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWdbeBnk::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeBnk::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeBnk::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
