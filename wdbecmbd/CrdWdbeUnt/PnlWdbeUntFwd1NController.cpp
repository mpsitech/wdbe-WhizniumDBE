/**
	...
	* job handler for job PnlWdbeUntFwd1NController (implementation)
...
#include "PnlWdbeUntFwd1NController_blks.cpp"
#include "PnlWdbeUntFwd1NController_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUntFwd1NController
 ******************************************************************************/

PnlWdbeUntFwd1NController::PnlWdbeUntFwd1NController(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNTFWD1NCONTROLLER, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUntFwd1NController(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUntFwd1NController::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUntFwd1NController::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeUntFwd1NController::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntFwd1NController::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTFWD1NCONTROLLERDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTFWD1NCONTROLLERDO) {
...
void PnlWdbeUntFwd1NController::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntFwd1NController::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntFwd1NController::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUntFwd1NController::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUntFwd1NController* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUntFwd1NController::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUntFwd1NController::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUntFwd1NController::StgIac::JNUM)) {
...
void PnlWdbeUntFwd1NController::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUntFwd1NController::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUntFwd1NController::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUntFwd1NController::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
