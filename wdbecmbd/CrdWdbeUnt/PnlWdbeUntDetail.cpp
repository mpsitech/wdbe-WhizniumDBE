/**
	...
	* job handler for job PnlWdbeUntDetail (implementation)
...
#include "PnlWdbeUntDetail_blks.cpp"
#include "PnlWdbeUntDetail_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUntDetail
 ******************************************************************************/

PnlWdbeUntDetail::PnlWdbeUntDetail(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNTDETAIL, jrefSup, ixWdbeVLocale)
		{
	jref = xchg->addJob(dbswdbe, this, jrefSup);

	feedFPupPkg.tag = "FeedFPupPkg";
	dbswdbe->fillFeedFromKlst(VecWdbeVKeylist::KLSTWDBEKMUNITPACKAGE, ixWdbeVLocale, feedFPupPkg);
	feedFPupRet.tag = "FeedFPupRet";
	VecWdbeVMUnitRefTbl::fillFeed(ixWdbeVLocale, feedFPupRet);
	feedFPupTch.tag = "FeedFPupTch";
	dbswdbe->fillFeedFromKlst(VecWdbeVKeylist::KLSTWDBEKMUNITTOOLCH, ixWdbeVLocale, feedFPupTch);
	...
	xchg->addIxClstn(VecWdbeVCall::CALLWDBEKLSAKEYMOD_KLSEQ, jref, Clstn::VecVJobmask::ALL, 0, false, VecWdbeVKeylist::KLSTWDBEKMUNITPACKAGE);
	xchg->addIxClstn(VecWdbeVCall::CALLWDBEKLSAKEYMOD_KLSEQ, jref, Clstn::VecVJobmask::ALL, 0, false, VecWdbeVKeylist::KLSTWDBEKMUNITTOOLCH);
...
DpchEngWdbe* PnlWdbeUntDetail::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUntDetail::refreshPupPkg(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeUntDetail::refreshTxfPkg(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
	ContIac oldContiac(contiac);
...
void PnlWdbeUntDetail::refreshPkg(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
	// feedFPupPkg
...
void PnlWdbeUntDetail::refreshPupTch(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
	ContIac oldContiac(contiac);
...
void PnlWdbeUntDetail::refreshTxfTch(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
	ContIac oldContiac(contiac);
...
void PnlWdbeUntDetail::refreshTch(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
	// feedFPupTch
...
void PnlWdbeUntDetail::refreshRecUnt(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
	ContIac oldContiac(contiac);
	...
	continf.TxtSil = StubWdbe::getStubUntStd(dbswdbe, recUnt.silRefWdbeMUnit, ixWdbeVLocale, Stub::VecVNonetype::FULL);
	...
	statshr.TxtSilAvail = evalTxtSilAvail(dbswdbe);
	statshr.TxtSilActive = evalTxtSilActive(dbswdbe);
	statshr.ButSilViewAvail = evalButSilViewAvail(dbswdbe);
...
void PnlWdbeUntDetail::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFUNT) {
		recUnt.ref = xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFUNT, jref);
...
void PnlWdbeUntDetail::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTDETAILDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTDETAILDO) {
...
void PnlWdbeUntDetail::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntDetail::handleDpchAppDataContiac(
			DbsWdbe* dbswdbe
			, ContIac* _contiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntDetail::handleDpchAppDoButSaveClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntDetail::handleDpchAppDoButReuViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUntDetail::handleDpchAppDoButSilViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButSilViewAvail && statshr.ButSilViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSIL, jref)) {
			sref = "CrdWdbeSil";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recUnt.silRefWdbeMUnit, jrefNew);
		};
...
void PnlWdbeUntDetail::handleDpchAppDoButSysViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButSysViewAvail && statshr.ButSysViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSYS, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFVER) {
			sref = "CrdWdbeSys";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recUnt.refWdbeMSystem, jrefNew);
		};
...
void PnlWdbeUntDetail::handleDpchAppDoButMdlViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUntDetail::handleDpchAppDoButPkgEditClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntDetail::handleDpchAppDoButTchEditClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntDetail::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUntDetail::handleCallWdbeUnt_mdlEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
...
bool PnlWdbeUntDetail::handleCallWdbeUnt_retEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWdbeUntDetail::handleCallWdbeUnt_reuEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWdbeUntDetail::handleCallWdbeUnt_silEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWdbeUntDetail::handleCallWdbeUnt_sysEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
