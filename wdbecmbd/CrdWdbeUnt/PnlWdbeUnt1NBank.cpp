/**
	...
	* job handler for job PnlWdbeUnt1NBank (implementation)
...
#include "PnlWdbeUnt1NBank_blks.cpp"
#include "PnlWdbeUnt1NBank_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUnt1NBank
 ******************************************************************************/

PnlWdbeUnt1NBank::PnlWdbeUnt1NBank(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNT1NBANK, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUnt1NBank(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUnt1NBank::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUnt1NBank::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUnt1NBank::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNT1NBANKDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNT1NBANKDO) {
...
void PnlWdbeUnt1NBank::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUnt1NBank::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUnt1NBank::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUnt1NBank::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUnt1NBank::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUnt1NBank::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUnt1NBank::StgIac::JNUM)) {
...
void PnlWdbeUnt1NBank::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUnt1NBank::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUnt1NBank::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUnt1NBank::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUnt1NBank::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUnt1NBank::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
