/**
	...
	* job handler for job PnlWdbeUnt1NPeripheral (implementation)
...
#include "PnlWdbeUnt1NPeripheral_blks.cpp"
#include "PnlWdbeUnt1NPeripheral_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUnt1NPeripheral
 ******************************************************************************/

PnlWdbeUnt1NPeripheral::PnlWdbeUnt1NPeripheral(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNT1NPERIPHERAL, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUnt1NPeripheral(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUnt1NPeripheral::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUnt1NPeripheral::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeUnt1NPeripheral::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUnt1NPeripheral::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNT1NPERIPHERALDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNT1NPERIPHERALDO) {
...
void PnlWdbeUnt1NPeripheral::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUnt1NPeripheral::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUnt1NPeripheral::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUnt1NPeripheral::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUnt1NPeripheral* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUnt1NPeripheral::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUnt1NPeripheral::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUnt1NPeripheral::StgIac::JNUM)) {
...
void PnlWdbeUnt1NPeripheral::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUnt1NPeripheral::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUnt1NPeripheral::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUnt1NPeripheral::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUnt1NPeripheral::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUnt1NPeripheral::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
