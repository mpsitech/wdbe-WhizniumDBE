/**
	...
	* job handler for job PnlWdbeUntHk1NModule (implementation)
...
#include "PnlWdbeUntHk1NModule_blks.cpp"
#include "PnlWdbeUntHk1NModule_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUntHk1NModule
 ******************************************************************************/

PnlWdbeUntHk1NModule::PnlWdbeUntHk1NModule(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNTHK1NMODULE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUntHk1NModule(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUntHk1NModule::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUntHk1NModule::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeUntHk1NModule::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntHk1NModule::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTHK1NMODULEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTHK1NMODULEDO) {
...
void PnlWdbeUntHk1NModule::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntHk1NModule::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntHk1NModule::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUntHk1NModule::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUntHk1NModule* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUntHk1NModule::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUntHk1NModule::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUntHk1NModule::StgIac::JNUM)) {
...
void PnlWdbeUntHk1NModule::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUntHk1NModule::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUntHk1NModule::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntHk1NModule::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUntHk1NModule::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUntHk1NModule::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
