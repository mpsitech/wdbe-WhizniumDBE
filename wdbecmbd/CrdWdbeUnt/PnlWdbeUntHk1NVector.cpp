/**
	...
	* job handler for job PnlWdbeUntHk1NVector (implementation)
...
#include "PnlWdbeUntHk1NVector_blks.cpp"
#include "PnlWdbeUntHk1NVector_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUntHk1NVector
 ******************************************************************************/

PnlWdbeUntHk1NVector::PnlWdbeUntHk1NVector(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNTHK1NVECTOR, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUntHk1NVector(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUntHk1NVector::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUntHk1NVector::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeUntHk1NVector::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntHk1NVector::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTHK1NVECTORDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTHK1NVECTORDO) {
...
void PnlWdbeUntHk1NVector::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntHk1NVector::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntHk1NVector::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUntHk1NVector::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUntHk1NVector* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUntHk1NVector::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUntHk1NVector::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUntHk1NVector::StgIac::JNUM)) {
...
void PnlWdbeUntHk1NVector::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUntHk1NVector::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUntHk1NVector::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntHk1NVector::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUntHk1NVector::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUntHk1NVector::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
