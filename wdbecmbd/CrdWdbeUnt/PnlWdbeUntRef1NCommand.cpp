/**
	...
	* job handler for job PnlWdbeUntRef1NCommand (implementation)
...
#include "PnlWdbeUntRef1NCommand_blks.cpp"
#include "PnlWdbeUntRef1NCommand_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUntRef1NCommand
 ******************************************************************************/

PnlWdbeUntRef1NCommand::PnlWdbeUntRef1NCommand(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNTREF1NCOMMAND, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUntRef1NCommand(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUntRef1NCommand::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUntRef1NCommand::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeUntRef1NCommand::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntRef1NCommand::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTREF1NCOMMANDDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTREF1NCOMMANDDO) {
...
void PnlWdbeUntRef1NCommand::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntRef1NCommand::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntRef1NCommand::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUntRef1NCommand::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUntRef1NCommand* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUntRef1NCommand::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUntRef1NCommand::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUntRef1NCommand::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCmd = WdbeMCommand();

			if (recSelNew) {
				if (dbswdbe->tblwdbemcommand->loadRecByRef(recSelNew->ref, &_recCmd)) {
					recCmd = *_recCmd;
					delete _recCmd;
				};
			};
...
void PnlWdbeUntRef1NCommand::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntRef1NCommand::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntRef1NCommand::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCMD, jref)) if (refUnt != 0) {
			sref = "CrdWdbeCmd";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFUNT, refUnt, sref, recCmd.ref, jrefNew);
		};
...
void PnlWdbeUntRef1NCommand::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCMD, jref) & VecWdbeWAccess::EDIT) != 0) if (refUnt != 0) {
			sref = "CrdWdbeCmd";
...
void PnlWdbeUntRef1NCommand::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntRef1NCommand::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUntRef1NCommand::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUntRef1NCommand::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
