/**
	...
	* job handler for job PnlWdbeUntRef1NSignal (implementation)
...
#include "PnlWdbeUntRef1NSignal_blks.cpp"
#include "PnlWdbeUntRef1NSignal_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUntRef1NSignal
 ******************************************************************************/

PnlWdbeUntRef1NSignal::PnlWdbeUntRef1NSignal(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNTREF1NSIGNAL, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUntRef1NSignal(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUntRef1NSignal::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUntRef1NSignal::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeUntRef1NSignal::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntRef1NSignal::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTREF1NSIGNALDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTREF1NSIGNALDO) {
...
void PnlWdbeUntRef1NSignal::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntRef1NSignal::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntRef1NSignal::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUntRef1NSignal::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUntRef1NSignal* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUntRef1NSignal::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUntRef1NSignal::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUntRef1NSignal::StgIac::JNUM)) {
...
void PnlWdbeUntRef1NSignal::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntRef1NSignal::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntRef1NSignal::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUntRef1NSignal::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSIG, jref) & VecWdbeWAccess::EDIT) != 0) if (refUnt != 0) {
			sref = "CrdWdbeSig";
...
void PnlWdbeUntRef1NSignal::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntRef1NSignal::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUntRef1NSignal::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUntRef1NSignal::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
