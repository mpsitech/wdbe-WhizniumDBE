/**
	...
	* job handler for job PnlWdbeUntList (implementation)
...
#include "PnlWdbeUntList_blks.cpp"
#include "PnlWdbeUntList_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUntList
 ******************************************************************************/

PnlWdbeUntList::PnlWdbeUntList(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNTLIST, jrefSup, ixWdbeVLocale)
		{
	...
	QryWdbeUntList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFSEL, jref, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFUNT, jref));
	xchg->addIxPreset(VecWdbeVPreset::PREWDBEIXORD, jref, QryWdbeUntList::VecVOrd::SRF);

	qry = new QryWdbeUntList(xchg, dbswdbe, jref, ixWdbeVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWdbeVExpstate = ((xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFUNT, jref)) ? VecWdbeVExpstate::MIND : VecWdbeVExpstate::REGD);
...
DpchEngWdbe* PnlWdbeUntList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUntList::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntList::regularize(
			DbsWdbe* dbswdbe
			, const bool notif
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntList::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTLISTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNTLISTDO) {
...
void PnlWdbeUntList::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUntList::handleDpchAppDataContiac(
			DbsWdbe* dbswdbe
			, ContIac* _contiac
			, DpchEngWdbe** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWdbeUntList::VecVOrd::MDL) && (_contiac->numFTos <= QryWdbeUntList::VecVOrd::SRF)) {
			muteRefresh = true;
...
void PnlWdbeUntList::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntList::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUntList::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUntList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUntList::StgIac::JNUM) || (diffitems.size() > 1)) {
			muteRefresh = true;
		...
		if (has(diffitems, QryWdbeUntList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFUNT, refSelNew);
		};
	};
...
void PnlWdbeUntList::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntList::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntList::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntList::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntList::handleDpchAppDoButFilterClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUntList::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUntList::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUntList::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
