...
CrdWdbeUnt::CrdWdbeUnt(
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFUNT, ref);
...
void CrdWdbeUnt::showSubmdls(
	...
	if (_sigs) {
		...
		dbswdbe->tblwdbemsignal->loadRstBySQL("SELECT * FROM TblWdbeMSignal WHERE refIxVTbl = " + to_string(VecWdbeVMSignalRefTbl::MDL) + " AND refUref = " + to_string(refWdbeMModule) + " ORDER BY refNum ASC", false, sigs);
