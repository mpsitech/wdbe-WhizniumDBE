/**
	* \file PnlWdbeUnt1NInterrupt.cpp
	* job handler for job PnlWdbeUnt1NInterrupt (implementation)
...
#include "PnlWdbeUnt1NInterrupt_blks.cpp"
#include "PnlWdbeUnt1NInterrupt_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUnt1NInterrupt
 ******************************************************************************/

PnlWdbeUnt1NInterrupt::PnlWdbeUnt1NInterrupt(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNT1NINTERRUPT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUnt1NInterrupt(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUnt1NInterrupt::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUnt1NInterrupt::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeUnt1NInterrupt::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUnt1NInterrupt::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNT1NINTERRUPTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNT1NINTERRUPTDO) {
...
void PnlWdbeUnt1NInterrupt::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUnt1NInterrupt::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUnt1NInterrupt::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUnt1NInterrupt::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUnt1NInterrupt* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUnt1NInterrupt::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUnt1NInterrupt::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUnt1NInterrupt::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recInt = WdbeMInterrupt();

			if (recSelNew) {
				if (dbswdbe->tblwdbeminterrupt->loadRecByRef(recSelNew->ref, &_recInt)) {
					recInt = *_recInt;
					delete _recInt;
				};
			};
...
void PnlWdbeUnt1NInterrupt::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCINT, jref)) if (refUnt != 0) {
			sref = "CrdWdbeInt";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFUNT, refUnt, sref, recInt.ref, jrefNew);
		};
...
void PnlWdbeUnt1NInterrupt::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCINT, jref) & VecWdbeWAccess::EDIT) != 0) if (refUnt != 0) {
			sref = "CrdWdbeInt";
...
void PnlWdbeUnt1NInterrupt::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUnt1NInterrupt::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUnt1NInterrupt::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUnt1NInterrupt::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
