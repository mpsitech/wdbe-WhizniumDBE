/**
	...
	* job handler for job PnlWdbeUnt1NTarget (implementation)
...
#include "PnlWdbeUnt1NTarget_blks.cpp"
#include "PnlWdbeUnt1NTarget_evals.cpp"
...
/******************************************************************************
 class PnlWdbeUnt1NTarget
 ******************************************************************************/

PnlWdbeUnt1NTarget::PnlWdbeUnt1NTarget(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEUNT1NTARGET, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeUnt1NTarget(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeUnt1NTarget::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeUnt1NTarget::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeUnt1NTarget::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeUnt1NTarget::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNT1NTARGETDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEUNT1NTARGETDO) {
...
void PnlWdbeUnt1NTarget::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeUnt1NTarget::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeUnt1NTarget::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeUnt1NTarget::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQUnt1NTarget* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeUnt1NTarget::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeUnt1NTarget::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeUnt1NTarget::StgIac::JNUM)) {
...
void PnlWdbeUnt1NTarget::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeUnt1NTarget::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeUnt1NTarget::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeUnt1NTarget::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
