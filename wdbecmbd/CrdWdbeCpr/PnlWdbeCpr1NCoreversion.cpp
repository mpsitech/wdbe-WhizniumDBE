/**
	...
	* job handler for job PnlWdbeCpr1NCoreversion (implementation)
...
#include "PnlWdbeCpr1NCoreversion_blks.cpp"
#include "PnlWdbeCpr1NCoreversion_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCpr1NCoreversion
 ******************************************************************************/

PnlWdbeCpr1NCoreversion::PnlWdbeCpr1NCoreversion(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECPR1NCOREVERSION, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeCpr1NCoreversion(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeCpr1NCoreversion::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCpr1NCoreversion::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeCpr1NCoreversion::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeCpr1NCoreversion::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECPR1NCOREVERSIONDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECPR1NCOREVERSIONDO) {
...
void PnlWdbeCpr1NCoreversion::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCpr1NCoreversion::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCpr1NCoreversion::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCpr1NCoreversion::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQCpr1NCoreversion* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeCpr1NCoreversion::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeCpr1NCoreversion::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeCpr1NCoreversion::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCvr = WdbeMCoreversion();

			if (recSelNew) {
				if (dbswdbe->tblwdbemcoreversion->loadRecByRef(recSelNew->ref, &_recCvr)) {
					recCvr = *_recCvr;
					delete _recCvr;
				};
			};
...
void PnlWdbeCpr1NCoreversion::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCVR, jref)) if (refCpr != 0) {
			sref = "CrdWdbeCvr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFCPR, refCpr, sref, recCvr.ref, jrefNew);
		};
...
void PnlWdbeCpr1NCoreversion::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCVR, jref) & VecWdbeWAccess::EDIT) != 0) if (refCpr != 0) {
			sref = "CrdWdbeCvr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFCPR, refCpr, sref, -1, jrefNew);
		};
...
void PnlWdbeCpr1NCoreversion::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCpr1NCoreversion::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCpr1NCoreversion::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCpr1NCoreversion::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
