/**
	...
	* job handler for job PnlWdbeCprMNPerson (implementation)
...
#include "PnlWdbeCprMNPerson_blks.cpp"
#include "PnlWdbeCprMNPerson_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCprMNPerson
 ******************************************************************************/

PnlWdbeCprMNPerson::PnlWdbeCprMNPerson(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECPRMNPERSON, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeCprMNPerson(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeCprMNPerson::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCprMNPerson::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeCprMNPerson::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeCprMNPerson::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECPRMNPERSONDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECPRMNPERSONDO) {
...
void PnlWdbeCprMNPerson::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCprMNPerson::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCprMNPerson::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCprMNPerson::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQCprMNPerson* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeCprMNPerson::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeCprMNPerson::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeCprMNPerson::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswdbe->tblwdbemperson->loadRecByRef(recSelNew->mref, &_recPrs)) {
					recPrs = *_recPrs;
					delete _recPrs;
				};
			};
...
void PnlWdbeCprMNPerson::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPRS, jref)) {
			sref = "CrdWdbePrs";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recPrs.ref, jrefNew);
		};
...
void PnlWdbeCprMNPerson::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCprMNPerson::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCprMNPerson::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCprMNPerson::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCprMNPerson::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
