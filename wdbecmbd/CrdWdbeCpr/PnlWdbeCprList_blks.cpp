/**
	* \file PnlWdbeCprList_blks.cpp
	* job handler for job PnlWdbeCprList (implementation of blocks)
...
/******************************************************************************
 class PnlWdbeCprList::VecVDo
 ******************************************************************************/

uint PnlWdbeCprList::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWdbeCprList::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWdbeCprList::ContIac
 ******************************************************************************/

PnlWdbeCprList::ContIac::ContIac(
			const uint numFTos
		) :
			Block()
			, numFTos(numFTos)
...
bool PnlWdbeCprList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWdbeCprList"];}();
...
bool PnlWdbeCprList::ContIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContIacWdbeCprList");
	else
...
void PnlWdbeCprList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContIacWdbeCprList";
...
void PnlWdbeCprList::ContIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
...
set<uint> PnlWdbeCprList::ContIac::comm(
			const ContIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCprList::ContIac::diff(
			const ContIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeCprList::ContInf
 ******************************************************************************/

PnlWdbeCprList::ContInf::ContInf(
...
void PnlWdbeCprList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeCprList";
...
void PnlWdbeCprList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWdbeCprList";
...
set<uint> PnlWdbeCprList::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCprList::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeCprList::StatShr
 ******************************************************************************/

PnlWdbeCprList::StatShr::StatShr(
...
void PnlWdbeCprList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeCprList";
...
void PnlWdbeCprList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWdbeCprList";
...
set<uint> PnlWdbeCprList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCprList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
PnlWdbeCprList::StgIac::StgIac(
			...
			, TcoCvrWidth(TcoCvrWidth)
			, TcoGrlWidth(TcoGrlWidth)
		{
	mask = {TCOGRPWIDTH, TCOOWNWIDTH, TCOSRFWIDTH, TCOTITWIDTH, TCOTYPWIDTH, TCOCVRWIDTH, TCOGRLWIDTH};
};

bool PnlWdbeCprList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWdbeCprList"];}();
	...
	if (basefound) {
		...
		if (me.isMember("TcoCvrWidth")) {TcoCvrWidth = me["TcoCvrWidth"].asUInt(); add(TCOCVRWIDTH);};
		if (me.isMember("TcoGrlWidth")) {TcoGrlWidth = me["TcoGrlWidth"].asUInt(); add(TCOGRLWIDTH);};
	};

	return basefound;
};

bool PnlWdbeCprList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWdbeCprList");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "TcoCvrWidth", TcoCvrWidth)) add(TCOCVRWIDTH);
...
void PnlWdbeCprList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacWdbeCprList";
	...
	me["TcoCvrWidth"] = (Json::Value::UInt) TcoCvrWidth;
	me["TcoGrlWidth"] = (Json::Value::UInt) TcoGrlWidth;
};

void PnlWdbeCprList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		...
		writeUintAttr(wr, itemtag, "sref", "TcoCvrWidth", TcoCvrWidth);
		writeUintAttr(wr, itemtag, "sref", "TcoGrlWidth", TcoGrlWidth);
	xmlTextWriterEndElement(wr);
};

set<uint> PnlWdbeCprList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
	...
	if (TcoCvrWidth == comp->TcoCvrWidth) insert(items, TCOCVRWIDTH);
	if (TcoGrlWidth == comp->TcoGrlWidth) insert(items, TCOGRLWIDTH);

	return(items);
};

set<uint> PnlWdbeCprList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {TCOGRPWIDTH, TCOOWNWIDTH, TCOSRFWIDTH, TCOTITWIDTH, TCOTYPWIDTH, TCOCVRWIDTH, TCOGRLWIDTH};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
/******************************************************************************
 class PnlWdbeCprList::Tag
 ******************************************************************************/

void PnlWdbeCprList::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeCprList";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
		me["Cpt"] = "Core projects";
		me["TcoSrf"] = "Identifier";
		me["TcoTit"] = "Name";
		me["TcoTyp"] = "Type";
		me["TcoCvr"] = "Current core version";
		me["TcoGrl"] = "Git URL";
	};
...
void PnlWdbeCprList::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeCprList";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Core projects");
			...
			writeStringAttr(wr, itemtag, "sref", "TcoCvr", "Current core version");
...
/******************************************************************************
 class PnlWdbeCprList::DpchAppData
 ******************************************************************************/

PnlWdbeCprList::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECPRLISTDATA)
		{
};

string PnlWdbeCprList::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCprList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCprListData"];}();
	...
	if (basefound) {
		...
		contiac = ContIac();
		stgiac = StgIac();
		stgiacqry = QryWdbeCprList::StgIac();
	};
};

void PnlWdbeCprList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCprListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeCprList::DpchAppDo
 ******************************************************************************/

PnlWdbeCprList::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECPRLISTDO)
		{
	ixVDo = 0;
};

string PnlWdbeCprList::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCprList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCprListDo"];}();
...
void PnlWdbeCprList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCprListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeCprList::DpchEngData
 ******************************************************************************/

PnlWdbeCprList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBECPRLISTDATA, jref)
		{
...
string PnlWdbeCprList::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCprList::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeCprList::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeCprListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeCprList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeCprList::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeCprListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeCprList::StatApp::writeXML(wr);
