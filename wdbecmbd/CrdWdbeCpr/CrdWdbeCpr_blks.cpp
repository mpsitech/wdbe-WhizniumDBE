/**
	* \file CrdWdbeCpr_blks.cpp
	* job handler for job CrdWdbeCpr (implementation of blocks)
...
void CrdWdbeCpr::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdWdbeCpr::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeCpr";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	me["numFSge"] = (Json::Value::UInt) numFSge;
	me["MrlAppHlp"] = MrlAppHlp;
	me["MtxCrdCpr"] = MtxCrdCpr;
};

void CrdWdbeCpr::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemInfWdbeCpr";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdCpr", MtxCrdCpr);
...
void CrdWdbeCpr::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
	...
	if (difftag.length() == 0) difftag = "StatAppWdbeCpr";
...
void CrdWdbeCpr::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixWdbeVReqitmode
	...
	else itemtag = "StatitemAppWdbeCpr";
...
void CrdWdbeCpr::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeCpr";
...
void CrdWdbeCpr::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrWdbeCpr";
...
void CrdWdbeCpr::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeCpr";
...
CrdWdbeCpr::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECPRDO)
		{
	ixVDo = 0;
};
...
void CrdWdbeCpr::DpchAppDo::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCprDo"];}();
...
void CrdWdbeCpr::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCprDo");
	else
...
CrdWdbeCpr::DpchEngData::DpchEngData(
			...
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBECPRDATA, jref)
		{
...
void CrdWdbeCpr::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void CrdWdbeCpr::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeCprData"] = Json::Value(Json::objectValue);
...
void CrdWdbeCpr::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeCprData");
