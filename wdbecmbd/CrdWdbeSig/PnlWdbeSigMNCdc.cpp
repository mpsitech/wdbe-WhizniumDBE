/**
	...
	* job handler for job PnlWdbeSigMNCdc (implementation)
...
#include "PnlWdbeSigMNCdc_blks.cpp"
#include "PnlWdbeSigMNCdc_evals.cpp"
...
/******************************************************************************
 class PnlWdbeSigMNCdc
 ******************************************************************************/

PnlWdbeSigMNCdc::PnlWdbeSigMNCdc(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBESIGMNCDC, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeSigMNCdc(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeSigMNCdc::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeSigMNCdc::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeSigMNCdc::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESIGMNCDCDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESIGMNCDCDO) {
...
void PnlWdbeSigMNCdc::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeSigMNCdc::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSigMNCdc::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeSigMNCdc::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeSigMNCdc::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeSigMNCdc::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeSigMNCdc::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswdbe->tblwdbemcdc->loadRecByRef(recSelNew->mref, &_recCdc)) {
					recCdc = *_recCdc;
					delete _recCdc;
				};
			};
...
void PnlWdbeSigMNCdc::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCDC, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFMOD) {
			sref = "CrdWdbeCdc";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recCdc.ref, jrefNew);
		};
...
void PnlWdbeSigMNCdc::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSigMNCdc::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSigMNCdc::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeSigMNCdc::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeSigMNCdc::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
