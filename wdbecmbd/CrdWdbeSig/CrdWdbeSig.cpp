/**
	* \file CrdWdbeSig.cpp
	* job handler for job CrdWdbeSig (implementation)
...
CrdWdbeSig::CrdWdbeSig(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBESIG, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFSIG, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlheadbar = new PnlWdbeSigHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnllist = new PnlWdbeSigList(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlrec = new PnlWdbeSigRec(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbeSig::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeSig::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeSig/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdSig = StubWdbe::getStubSigStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFSIG, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbeSig::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, ixWdbeVPreset, xchg->getRefPreset(ixWdbeVPreset, jref), "CrdWdbeSig", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFSIG, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFSIG, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFSIG, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbeSig::updatePreset(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWdbeSig::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESIGDO) {
...
void CrdWdbeSig::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeSig::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBESIG);
};

void CrdWdbeSig::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeSig::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeSig::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFSIG) {
...
bool CrdWdbeSig::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeSig::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWdbeSig::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeSig::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeSig::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
