/**
	* \file PnlWdbeSigRec.cpp
	* job handler for job PnlWdbeSigRec (implementation)
...
#include "PnlWdbeSigRec_blks.cpp"
#include "PnlWdbeSigRec_evals.cpp"
...
PnlWdbeSigRec::PnlWdbeSigRec(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBESIGREC, jrefSup, ixWdbeVLocale)
		{
...
DpchEngWdbe* PnlWdbeSigRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeSigRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWdbe::getStubSigStd(dbswdbe, recSig.ref, ixWdbeVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recSig.ref == 0) statshr.ixWdbeVExpstate = VecWdbeVExpstate::MIND;
	...
	if (statshr.ixWdbeVExpstate == VecWdbeVExpstate::MIND) {
		...
		if (pnlmncdc) {delete pnlmncdc; pnlmncdc = NULL;};
	} else {
		if (!pnldetail) pnldetail = new PnlWdbeSigDetail(xchg, dbswdbe, jref, ixWdbeVLocale);
		if (!pnlsrc1nsensitivity) pnlsrc1nsensitivity = new PnlWdbeSigSrc1NSensitivity(xchg, dbswdbe, jref, ixWdbeVLocale);
		if (!pnlmncdc) pnlmncdc = new PnlWdbeSigMNCdc(xchg, dbswdbe, jref, ixWdbeVLocale);
	};

	statshr.jrefDetail = ((pnldetail) ? pnldetail->jref : 0);
	statshr.jrefSrc1NSensitivity = ((pnlsrc1nsensitivity) ? pnlsrc1nsensitivity->jref : 0);
	statshr.jrefMNCdc = ((pnlmncdc) ? pnlmncdc->jref : 0);
...
void PnlWdbeSigRec::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFSIG) {
		...
		if (dbswdbe->tblwdbemsignal->loadRecByRef(xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFSIG, jref), &_recSig)) {
			recSig = *_recSig;
			delete _recSig;
		...
		if (recSig.ref != 0) {
			...
			if (pnlmncdc) pnlmncdc->updatePreset(dbswdbe, ixWdbeVPreset, jrefTrig, notif);
		};
...
void PnlWdbeSigRec::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESIGRECDO) {
...
void PnlWdbeSigRec::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeSigRec::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSigRec::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
bool PnlWdbeSigRec::handleCallWdbeSig_cluEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
...
bool PnlWdbeSigRec::handleCallWdbeSig_drvEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWdbeSigRec::handleCallWdbeSig_mgtEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWdbeSigRec::handleCallWdbeSig_mgu_inSbs(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWdbeSigRec::handleCallWdbeSig_mguEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWdbeSigRec::handleCallWdbeSig_retEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWdbeSigRec::handleCallWdbeSig_reu_inSbs(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWdbeSigRec::handleCallWdbeSig_reuEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWdbeSigRec::handleCallWdbeSig_vecEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWdbeSigRec::handleCallWdbeSigUpd_refEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
