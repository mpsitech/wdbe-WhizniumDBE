/**
	...
	* job handler for job PnlWdbePrtSrc1NSensitivity (implementation)
...
#include "PnlWdbePrtSrc1NSensitivity_blks.cpp"
#include "PnlWdbePrtSrc1NSensitivity_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrtSrc1NSensitivity
 ******************************************************************************/

PnlWdbePrtSrc1NSensitivity::PnlWdbePrtSrc1NSensitivity(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRTSRC1NSENSITIVITY, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePrtSrc1NSensitivity(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePrtSrc1NSensitivity::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrtSrc1NSensitivity::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbePrtSrc1NSensitivity::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbePrtSrc1NSensitivity::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRTSRC1NSENSITIVITYDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRTSRC1NSENSITIVITYDO) {
...
void PnlWdbePrtSrc1NSensitivity::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrtSrc1NSensitivity::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrtSrc1NSensitivity::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrtSrc1NSensitivity::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPrtSrc1NSensitivity* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrtSrc1NSensitivity::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrtSrc1NSensitivity::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrtSrc1NSensitivity::StgIac::JNUM)) {
...
void PnlWdbePrtSrc1NSensitivity::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSNS, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFMOD) {
			sref = "CrdWdbeSns";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recSns.ref, jrefNew);
		};
...
void PnlWdbePrtSrc1NSensitivity::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrtSrc1NSensitivity::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrtSrc1NSensitivity::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
