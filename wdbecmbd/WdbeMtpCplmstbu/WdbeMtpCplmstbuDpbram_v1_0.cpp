...
DpchRetWdbe* WdbeMtpCplmstbuDpbram_v1_0::run(
	...
	for (unsigned int i = 0; i < mdls.nodes.size(); i++) {
		...
		if (mdls.nodes.size() > 1) {
			dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refPrtCDrdA, VecWdbeVMSignalRefTbl::MDL, refWdbeMModule, mdls.nodes.size() + (i+1), VecWdbeVMSignalMgeTbl::VOID, 0, 0, "drdA" + suffix, false, "slvdn", wD, "", "", "", "0", false, 0, "");
			Wdbe::setPrtCsi(dbswdbe, mdl->ref, "DOA", "drdA" + suffix + range);

			dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refPrtCDrdB, VecWdbeVMSignalRefTbl::MDL, refWdbeMModule, 3*mdls.nodes.size() + (i+1), VecWdbeVMSignalMgeTbl::VOID, 0, 0, "drdB" + suffix, false, "slvdn", wD, "", "", "", "0", false, 0, "");
		...
		if (mdls.nodes.size() > 1) {
			dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refPrtCEnA, VecWdbeVMSignalRefTbl::MDL, refWdbeMModule, i+1, VecWdbeVMSignalMgeTbl::VOID, 0, 0, "enA" + suffix, false, "sl", 1, "", "", "", "0", false, 0, "");
			Wdbe::setPrtCsi(dbswdbe, mdl->ref, "ENA", "enA" + suffix);

			dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refPrtCEnB, VecWdbeVMSignalRefTbl::MDL, refWdbeMModule, 2*mdls.nodes.size() + (i+1), VecWdbeVMSignalMgeTbl::VOID, 0, 0, "enB" + suffix, false, "sl", 1, "", "", "", "0", false, 0, "");
