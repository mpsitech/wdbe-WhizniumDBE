/**
	...
	* job handler for job PnlWdbePrsADetail (implementation)
...
#include "PnlWdbePrsADetail_blks.cpp"
#include "PnlWdbePrsADetail_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrsADetail
 ******************************************************************************/

PnlWdbePrsADetail::PnlWdbePrsADetail(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRSADETAIL, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePrsADetail(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePrsADetail::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrsADetail::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbePrsADetail::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbePrsADetail::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRSADETAILDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRSADETAILDO) {
...
void PnlWdbePrsADetail::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrsADetail::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsADetail::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrsADetail::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPrsADetail* recSelNew = NULL;

	WdbeAMPersonDetail* _recPrsAdet = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrsADetail::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrsADetail::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrsADetail::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recPrsAdet = WdbeAMPersonDetail();

			if (recSelNew) {
				if (dbswdbe->tblwdbeampersondetail->loadRecByRef(recSelNew->ref, &_recPrsAdet)) {
...
void PnlWdbePrsADetail::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsADetail::handleDpchAppDoButDuplicateClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsADetail::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsADetail::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrsADetail::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrsADetail::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
