/**
	...
	* job handler for job PnlWdbePrsMNProject (implementation)
...
#include "PnlWdbePrsMNProject_blks.cpp"
#include "PnlWdbePrsMNProject_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrsMNProject
 ******************************************************************************/

PnlWdbePrsMNProject::PnlWdbePrsMNProject(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRSMNPROJECT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePrsMNProject(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePrsMNProject::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrsMNProject::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbePrsMNProject::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFPRS) {
		recPrsRprj = WdbeRMPersonMProject();
...
void PnlWdbePrsMNProject::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRSMNPROJECTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRSMNPROJECTDO) {
...
void PnlWdbePrsMNProject::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrsMNProject::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsMNProject::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrsMNProject::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPrsMNProject* recSelNew = NULL;

	WdbeRMPersonMProject* _recPrsRprj = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrsMNProject::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrsMNProject::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrsMNProject::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswdbe->tblwdbermpersonmproject->loadRecByRef(recSelNew->ref, &_recPrsRprj)) {
					recPrsRprj = *_recPrsRprj;
					delete _recPrsRprj;
				};
				if (dbswdbe->tblwdbemproject->loadRecByRef(recSelNew->mref, &_recPrj)) {
					recPrj = *_recPrj;
					delete _recPrj;
				};
			};
...
void PnlWdbePrsMNProject::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPRJ, jref)) {
			sref = "CrdWdbePrj";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recPrj.ref, jrefNew);
		};
...
void PnlWdbePrsMNProject::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsMNProject::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsMNProject::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrsMNProject::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrsMNProject::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
