/**
	...
	* job handler for job PnlWdbePrsMNCoreproject (implementation)
...
#include "PnlWdbePrsMNCoreproject_blks.cpp"
#include "PnlWdbePrsMNCoreproject_evals.cpp"
...
/******************************************************************************
 class PnlWdbePrsMNCoreproject
 ******************************************************************************/

PnlWdbePrsMNCoreproject::PnlWdbePrsMNCoreproject(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEPRSMNCOREPROJECT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbePrsMNCoreproject(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbePrsMNCoreproject::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbePrsMNCoreproject::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbePrsMNCoreproject::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFPRS) {
		recCprRprs = WdbeRMCoreprojectMPerson();

		recCpr = WdbeMCoreproject();
...
void PnlWdbePrsMNCoreproject::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRSMNCOREPROJECTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEPRSMNCOREPROJECTDO) {
...
void PnlWdbePrsMNCoreproject::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbePrsMNCoreproject::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsMNCoreproject::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbePrsMNCoreproject::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQPrsMNCoreproject* recSelNew = NULL;

	WdbeRMCoreprojectMPerson* _recCprRprs = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbePrsMNCoreproject::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbePrsMNCoreproject::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbePrsMNCoreproject::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswdbe->tblwdbermcoreprojectmperson->loadRecByRef(recSelNew->ref, &_recCprRprs)) {
					recCprRprs = *_recCprRprs;
					delete _recCprRprs;
				};
				if (dbswdbe->tblwdbemcoreproject->loadRecByRef(recSelNew->mref, &_recCpr)) {
					recCpr = *_recCpr;
					delete _recCpr;
				};
			};
...
void PnlWdbePrsMNCoreproject::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCPR, jref)) {
			sref = "CrdWdbeCpr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recCpr.ref, jrefNew);
		};
...
void PnlWdbePrsMNCoreproject::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsMNCoreproject::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbePrsMNCoreproject::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbePrsMNCoreproject::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbePrsMNCoreproject::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
