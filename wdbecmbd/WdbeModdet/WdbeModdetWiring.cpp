/**
	* \file WdbeModdetWiring.cpp
	* Wdbe operation processor - add hierarchical wiring (implementation)
...
DpchRetWdbeModdetWiring* WdbeModdetWiring::run(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
	...
	if (dbswdbe->tblwdbemmodule->loadRecByRef(refWdbeMModule, &mdl)) {
		dbswdbe->loadStringBySQL("SELECT sref FROM TblWdbeMUnit WHERE ref = " + to_string(mdl->hkUref), untsref);
			...
			for (unsigned int i = 0; i < pins.nodes.size(); i++) {
				...
				if (ptr != string::npos) {
					...
					if (pin->sref[pin->sref.length()-1] == 'p') {
						// LVDS pair
						if (srefsPins.find(pin->sref.substr(0, pin->sref.length()-1) + "n") == srefsPins.end()) appendError(xchg->tmppath, logfile, logfi, "wiring error 1: no negative counterpart found for LVDS pin " + pin->sref + "!");
			...
			for (unsigned int j = 0; j < subgens.nodes.size(); j++) {
				...
				if (subgen->srcSrefWdbeMGeneric != "") {
					...
					if (it != srefsGens.end()) {
						...
						if (!match) appendError(xchg->tmppath, logfile, logfi, "wiring error 2: mismatch of generic " + submdl->sref + "." + subgen->sref + " characteristics with superior module " + mdl->sref + "!");
				...
				if (subprt->cpiSrefWdbeMPin != "") {
					...
					if ((mdl->ixVBasetype == VecWdbeVMModuleBasetype::WRP) || (mdl->ixVBasetype == VecWdbeVMModuleBasetype::TOP)) {
						...
						if (it != srefsSigs.end()) {
							...
							if (sig->ixVBasetype == VecWdbeVMSignalBasetype::PSB) {
								...
								if (!match) {
									appendError(xchg->tmppath, logfile, logfi, "wiring error 3: mismatch of port " + submdl->sref + "." + subprt->sref + " characteristics with superior module " + mdl->sref + "!");
								} else {
						...
						if (it != srefsBusprts.end()) {
							...
							if (prt->ref == 0) {
								...
								if ((prt->ixVDir == VecWdbeVMPortDir::OUT) && hasrng && ((to-from+1) < prt->Width)) {
									...
									dbswdbe->tblwdbemsignal->appendNewRecToRst(sigs, NULL, VecWdbeVMSignalBasetype::OPRT, 0, mdl->ref, sigs.nodes.size()+1, VecWdbeVMSignalMgeTbl::VOID, 0, 0, prt->sref, false, prt->srefWdbeKHdltype, prt->Width, "", "", "", "0", false, prt->ref, "");
							...
							if (it != srefsLvdsprts.end()) {
								...
								if (prt->ref == 0) {
									...
									if (prt->ixVDir == VecWdbeVMPortDir::IN) {
										// instantiate IBUFDS (the unorthodox way)
										...
										// instantiate OBUFDS (the unorthodox way)
										dbswdbe->executeQuery("UPDATE TblWdbeMModule SET hkNum = hkNum + 1 WHERE hkIxVTbl = " + to_string(mdl->hkIxVTbl) + " AND hkUref = " + to_string(mdl->hkUref) + " AND hkNum >= " + to_string(hkNum));
									...
									dbswdbe->tblwdbemsignal->appendNewRecToRst(sigs, NULL, VecWdbeVMSignalBasetype::OTH, 0, mdl->ref, sigs.nodes.size()+1, VecWdbeVMSignalMgeTbl::VOID, 0, 0, prt->sref, false, "sl", 1, "", "", "", "", false, 0, "");
									srefsSigs[prt->sref] = sigs.nodes[sigs.nodes.size()-1];
								};
								...
								if (it != srefsPrts.end()) {
									...
									if (!match) {
										appendError(xchg->tmppath, logfile, logfi, "wiring error 4: mismatch of port " + submdl->sref + "." + subprt->sref + " characteristics with superior module " + mdl->sref + "!");
									} else {
					...
					if (!found) {
						...
						if (it != srefsPrts.end()) {
							...
							if (!match) {
								appendError(xchg->tmppath, logfile, logfi, "wiring error 5: mismatch of port " + submdl->sref + "." + subprt->sref + " characteristics with superior module " + mdl->sref + "!");
							} else {
					...
					if (it != srefsPrts.end()) {
						...
						if (!match) appendError(xchg->tmppath, logfile, logfi, "wiring error 6: mismatch of port " + submdl->sref + "." + subprt->sref + " characteristics with superior module " + mdl->sref + "!");

					} else {
						appendError(xchg->tmppath, logfile, logfi, "wiring error 7: port connection of " + submdl->sref + "." + subprt->sref + " port does not exist in superior module " + mdl->sref + "!");
					};
					...
					if (it != srefsSigs.end()) {
						...
						if (!match) appendError(xchg->tmppath, logfile, logfi, "wiring error 8: mismatch of port " + submdl->sref + "." + subprt->sref + " characteristics with superior module " + mdl->sref + "!");
						...
						if (((mdl->ixVBasetype == VecWdbeVMModuleBasetype::TOP) && (subprt->mdlIxVCat != VecWdbeVMPortMdlCat::RTEWRP)) || (mdl->supRefWdbeMModule == 0)) {
							appendError(xchg->tmppath, logfile, logfi, "wiring error 9: signal connection of " + submdl->sref + "." + subprt->sref + " port does not exist in superior module " + mdl->sref + "!");
							...
							for (auto it = srefsPrts.begin(); it != srefsPrts.end(); it++) {
								...
								if (prt->sref == sref) {
									...
									if (!match) appendError(xchg->tmppath, logfile, logfi, "wiring error 10: mismatch of port " + submdl->sref + "." + subprt->sref + " characteristics with superior module " + mdl->sref + "!");
							...
							if (!match) {
								...
								if (it != srefsPrts.end()) {
									...
									if (!match) appendError(xchg->tmppath, logfile, logfi, "wiring error 11: mismatch of port " + submdl->sref + "." + subprt->sref + " characteristics with superior module " + mdl->sref + "!");
			...
			if (sig->ixVBasetype == VecWdbeVMSignalBasetype::CLK) {
				...
				if (sig->drvRefWdbeMPort == 0) {
					...
					if (it != srefsPrts.end()) {
						...
						appendError(xchg->tmppath, logfile, logfi, "wiring error 12: port for clock signal " + mdl->sref + "." + sig->sref + " doesn't exist!");
					};
				};
			};
		};
	...
	return(new DpchRetWdbeModdetWiring("", "", ixOpVOpres, 100, logfile));
};

// IP cust --- IBEGIN
void WdbeModdetWiring::appendError(
