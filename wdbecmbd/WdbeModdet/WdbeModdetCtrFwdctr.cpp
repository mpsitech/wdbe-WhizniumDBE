...
DpchRetWdbe* WdbeModdetCtrFwdctr::run(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
	...
	if (dbswdbe->tblwdbemmodule->loadRecByRef(refWdbeMModule, &mdl)) {
		if (dbswdbe->tblwdbemcontroller->loadRecByRef(mdl->refWdbeMController, &ctr)) {
			...
			if (sigs.nodes.size() > 0) {
				mdlNum = sigs.nodes[sigs.nodes.size()-1]->mdlNum;
				mdlNum++;
			};
			...
			dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "sizeCmdbuf", true, "nat", 0, "", "", "", to_string(sizeCmdbuf), false, 0, "");
			...
			dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "cmdbuf", false, "cmdbuf_t", 0, "", "", "", "", false, 0, "");
			...
			if (!hasSigLenCmdbuf) dbswdbe->tblwdbemvariable->insertNewRec(NULL, refC, prc->ref, prcNum++, "lenCmdbuf", false, false, "nat", 0, "0..sizeCmdbuf", "", "0", false, "");

			if (hasFwderr || hasRecvShrte) {
				refC = dbswdbe->tblwdbecvariable->getNewRef();

				dbswdbe->tblwdbemvariable->insertNewRec(NULL, refC, prc->ref, prcNum++, "i", false, false, "nat", 0, "0..sizeCmdbuf", "", "0", false, "");
				dbswdbe->tblwdbemvariable->insertNewRec(NULL, refC, prc->ref, prcNum++, "j", false, false, "nat", 0, "0..sizeCmdbuf", "", "0", false, "");
				dbswdbe->tblwdbemvariable->insertNewRec(NULL, refC, prc->ref, prcNum++, "x", false, false, "slvdn", 8, "", "", "0", false, "");
			};

			if (hasSend) dbswdbe->tblwdbemvariable->insertNewRec(NULL, 0, prc->ref, prcNum++, "bytecnt", false, false, "nat", 0, "0..sizeCmdbuf", "", "0", false, "");
			...
			if (found) if (dbswdbe->loadRefBySQL("SELECT ref FROM TblWdbeMPort WHERE mdlRefWdbeMModule = " + to_string(mdl->ref) + " AND sref = 'd" + Cmdbussref + "'", ref))
						dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::IOPRT, 0, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "d" + Cmdbussref, false, "slvdn", 8, "", "state(sendA..C)", "", "", false, ref, "");
				...
				if ((it->first == mdl->sref) || (it->second == mdl->sref)) {
					...
					if (it->first == mdl->sref) {
						if (dbswdbe->loadRefBySQL("SELECT ref FROM TblWdbeMPort WHERE mdlRefWdbeMModule = " + to_string(mdl->ref) + " AND sref = 'req" + Cmdbussref + "To" + StrMod::cap(it->second) + "'", ref))
									dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OPRT, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "req" + Cmdbussref + "To" + StrMod::cap(it->second), false, "sl", 1, "", "", "1", "0", false, ref, "");
					} else if (it->second == mdl->sref) {
						if (dbswdbe->loadRefBySQL("SELECT ref FROM TblWdbeMPort WHERE mdlRefWdbeMModule = " + to_string(mdl->ref) + " AND sref = 'rdy" + Cmdbussref + "From" + StrMod::cap(it->first) + "'", ref))
									dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OPRT, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "rdy" + Cmdbussref + "From" + StrMod::cap(it->first), false, "sl", 1, "", "", "1", "0", true, ref, "");
					};
				};
			};
			...
			for (auto it = srefsPrcSigrootCmd.begin(); it != srefsPrcSigrootCmd.end(); it++) {
				if (dbswdbe->tblwdbemprocess->loadRecByRef(it->first, &prc2)) {
					...
					dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc2->ref, 0, "route" + StrMod::cap(it->second), false, "slvdn", 8, "", "", "", "", false, 0, "");
					dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc2->ref, 0, "cref" + StrMod::cap(it->second), false, "slvdn", 32, "", "", "", "", false, 0, "");
					if (it->second == prc2->sref) dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc2->ref, 0, "tixVCommand" + StrMod::cap(it->second), false, "slvdn", 8, "", "", "", "", false, 0, "");
...
// IP cust --- IBEGIN
void WdbeModdetCtrFwdctr::addCmdErrSigs(
			DbsWdbe* dbswdbe
			, WdbeMModule* mdl
			, uint& mdlNum
			, WdbeMProcess* prc
	...
	if (cmd) {
		...
		// tixVCommand...
		dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "tixV" + UntsrefExtcmd + MdlsrefExtcmd + "Command" + Cmdsref, true, "slvdn", 8, "", "", "", to_string(cmd->refNum), false, 0, "");

		// len/maxlenCmdbufInv...
		ref = dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "lenCmdbufInv" + MdlsrefExtcmd + Cmdsref, true, "nat", 0, "", "", "", "10", false, 0, "");
		...
		for (unsigned int k = 0; k < ipas.nodes.size(); k++) {
			...
			dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "ixCmdbufInv" + MdlsrefExtcmd + Cmdsref + StrMod::cap(ipa->sref), true, "nat", 0, "", "", "", to_string(ix), false, 0, "");
		...
		if (cmd->ixVRettype != VecWdbeVMCommandRettype::VOID) {
			...
			ref = dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "lenCmdbuf" + s + MdlsrefExtcmd + Cmdsref, true, "nat", 0, "", "", "", "10", false, 0, "");
			...
			for (unsigned int k = 0; k < rpas.nodes.size(); k++) {
				...
				dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "ixCmdbuf" + s + MdlsrefExtcmd + Cmdsref + StrMod::cap(rpa->sref), true, "nat", 0, "", "", "", to_string(ix), false, 0, "");
		...
		// tixVError...
		dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "tixVError" + StrMod::cap(err->sref), true, "slvdn", 8, "", "", "", to_string(err->refNum), false, 0, "");

		// len/maxlenCmdbufErr...
		ref = dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "lenCmdbufErr" + StrMod::cap(err->sref), true, "nat", 0, "", "", "", "10", false, 0, "");
		...
		for (unsigned int k = 0; k < epas.nodes.size(); k++) {
			...
			dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refC, mdl->ref, mdlNum++, VecWdbeVMSignalMgeTbl::PRC, prc->ref, 0, "ixCmdbufErr" + StrMod::cap(err->sref) + StrMod::cap(epa->sref), true, "nat", 0, "", "", "", to_string(ix), false, 0, "");
...
ubigint WdbeModdetCtrFwdctr::mergeCreateFsts(
			DbsWdbe* dbswdbe
