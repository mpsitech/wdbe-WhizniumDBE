		...
		for (unsigned int i = 0; i < prcs.nodes.size(); i++) {
			...
			if (prc->refWdbeMFsm != 0) if (dbswdbe->tblwdbemfsm->loadRecByRef(prc->refWdbeMFsm, &fsm)) {
				if (fsm->ixVDbgtaptype != VecWdbeVMFsmDbgtaptype::VOID) {
					...
					dbswdbe->tblwdbemsignal->insertNewRec(NULL, VecWdbeVMSignalBasetype::OTH, refCTopsig, VecWdbeVMSignalRefTbl::MDL, mdl->supRefWdbeMModule, refNumSig++, VecWdbeVMSignalMgeTbl::MDL, refWdbeMModule, 0, sref2, false, "slvdn", 8, "", "", "", "", 0, "");
				};
...
void WdbeGenAux::genCdc(
	...
	if (imax != "0") dbswdbe->tblwdbemvariable->insertNewRec(NULL, 0, VecWdbeVMVariableRefTbl::PRC, refPrc, refNumVar++, "imax", true, false, "nat", 0, "", "", imax, "");
	...
	for (unsigned int j = 0; j < crss.nodes.size(); j++) {
		...
		if (crs->ixVDir == ixVDir) {
			if (dbswdbe->tblwdbemsignal->loadRecByRef(crs->refWdbeMSignal, &sig)) {
				if (imax != "0") dbswdbe->tblwdbemvariable->insertNewRec(NULL, refCVar, VecWdbeVMVariableRefTbl::PRC, refPrc, refNumVar++, sig->sref + "_i", false, false, "nat", 0, "0..imax", "", "0", "");
	...
	for (unsigned int j = 0; j < crss.nodes.size(); j++) {
		...
		if (crs->ixVDir == ixVDir) {
			if (dbswdbe->tblwdbemsignal->loadRecByRef(crs->refWdbeMSignal, &sig)) {
				...
				if (sig->ixVBasetype == VecWdbeVMSignalBasetype::STRB) dbswdbe->tblwdbemvariable->insertNewRec(NULL, refCVar, VecWdbeVMVariableRefTbl::PRC, refPrc, refNumVar++, sig->sref + "_wait", false, false, "_bool", 0, "", "", "false", "");
