/**
	...
	* job handler for job PnlWdbeModMdl1NPort (implementation)
...
#include "PnlWdbeModMdl1NPort_blks.cpp"
#include "PnlWdbeModMdl1NPort_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModMdl1NPort
 ******************************************************************************/

PnlWdbeModMdl1NPort::PnlWdbeModMdl1NPort(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODMDL1NPORT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModMdl1NPort(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModMdl1NPort::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModMdl1NPort::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeModMdl1NPort::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeModMdl1NPort::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODMDL1NPORTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODMDL1NPORTDO) {
...
void PnlWdbeModMdl1NPort::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModMdl1NPort::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMdl1NPort::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModMdl1NPort::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModMdl1NPort* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModMdl1NPort::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModMdl1NPort::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModMdl1NPort::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recPrt = WdbeMPort();

			if (recSelNew) {
				if (dbswdbe->tblwdbemport->loadRecByRef(recSelNew->ref, &_recPrt)) {
					recPrt = *_recPrt;
					delete _recPrt;
				};
			};
...
void PnlWdbeModMdl1NPort::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMdl1NPort::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMdl1NPort::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPRT, jref)) if (refMod != 0) {
			sref = "CrdWdbePrt";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFMOD, refMod, sref, recPrt.ref, jrefNew);
		};
...
void PnlWdbeModMdl1NPort::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPRT, jref) & VecWdbeWAccess::EDIT) != 0) if (refMod != 0) {
			sref = "CrdWdbePrt";
...
void PnlWdbeModMdl1NPort::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMdl1NPort::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModMdl1NPort::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModMdl1NPort::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
