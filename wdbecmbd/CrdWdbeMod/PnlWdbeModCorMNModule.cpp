/**
	...
	* job handler for job PnlWdbeModCorMNModule (implementation)
...
#include "PnlWdbeModCorMNModule_blks.cpp"
#include "PnlWdbeModCorMNModule_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModCorMNModule
 ******************************************************************************/

PnlWdbeModCorMNModule::PnlWdbeModCorMNModule(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODCORMNMODULE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModCorMNModule(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModCorMNModule::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModCorMNModule::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeModCorMNModule::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFMOD) {
		recMdlRmdl = WdbeRMModuleMModule();
...
void PnlWdbeModCorMNModule::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODCORMNMODULEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODCORMNMODULEDO) {
...
void PnlWdbeModCorMNModule::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModCorMNModule::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCorMNModule::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModCorMNModule::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModCorMNModule* recSelNew = NULL;

	WdbeRMModuleMModule* _recMdlRmdl = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModCorMNModule::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModCorMNModule::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModCorMNModule::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswdbe->tblwdbermmodulemmodule->loadRecByRef(recSelNew->ref, &_recMdlRmdl)) {
					recMdlRmdl = *_recMdlRmdl;
					delete _recMdlRmdl;
				};
				if (dbswdbe->tblwdbemmodule->loadRecByRef(recSelNew->mref, &_recMdl)) {
					recMdl = *_recMdl;
...
void PnlWdbeModCorMNModule::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCMOD, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFUNT) {
			sref = "CrdWdbeMod";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recMdl.ref, jrefNew);
		};
		if (jrefNew == 0) {
...
void PnlWdbeModCorMNModule::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCorMNModule::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCorMNModule::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModCorMNModule::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModCorMNModule::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
