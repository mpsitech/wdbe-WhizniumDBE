/**
	...
	* job handler for job PnlWdbeMod1NPeripheral (implementation)
...
#include "PnlWdbeMod1NPeripheral_blks.cpp"
#include "PnlWdbeMod1NPeripheral_evals.cpp"
...
/******************************************************************************
 class PnlWdbeMod1NPeripheral
 ******************************************************************************/

PnlWdbeMod1NPeripheral::PnlWdbeMod1NPeripheral(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMOD1NPERIPHERAL, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeMod1NPeripheral(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeMod1NPeripheral::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeMod1NPeripheral::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeMod1NPeripheral::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeMod1NPeripheral::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMOD1NPERIPHERALDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMOD1NPERIPHERALDO) {
...
void PnlWdbeMod1NPeripheral::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeMod1NPeripheral::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMod1NPeripheral::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeMod1NPeripheral::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQMod1NPeripheral* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeMod1NPeripheral::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeMod1NPeripheral::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeMod1NPeripheral::StgIac::JNUM)) {
...
void PnlWdbeMod1NPeripheral::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeMod1NPeripheral::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeMod1NPeripheral::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeMod1NPeripheral::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
