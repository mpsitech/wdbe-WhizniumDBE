/**
	...
	* job handler for job PnlWdbeModHsm1NPipeline (implementation)
...
#include "PnlWdbeModHsm1NPipeline_blks.cpp"
#include "PnlWdbeModHsm1NPipeline_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModHsm1NPipeline
 ******************************************************************************/

PnlWdbeModHsm1NPipeline::PnlWdbeModHsm1NPipeline(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODHSM1NPIPELINE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModHsm1NPipeline(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModHsm1NPipeline::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModHsm1NPipeline::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeModHsm1NPipeline::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeModHsm1NPipeline::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODHSM1NPIPELINEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODHSM1NPIPELINEDO) {
...
void PnlWdbeModHsm1NPipeline::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModHsm1NPipeline::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModHsm1NPipeline::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModHsm1NPipeline::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModHsm1NPipeline* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModHsm1NPipeline::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModHsm1NPipeline::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModHsm1NPipeline::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recPpl = WdbeMPipeline();

			if (recSelNew) {
				if (dbswdbe->tblwdbempipeline->loadRecByRef(recSelNew->ref, &_recPpl)) {
					recPpl = *_recPpl;
					delete _recPpl;
				};
			};
...
void PnlWdbeModHsm1NPipeline::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPPL, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFUNT) {
			sref = "CrdWdbePpl";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recPpl.ref, jrefNew);
		};
...
void PnlWdbeModHsm1NPipeline::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPPL, jref) & VecWdbeWAccess::EDIT) != 0) if (ixPre == VecWdbeVPreset::PREWDBEREFUNT) {
			sref = "CrdWdbePpl";
...
void PnlWdbeModHsm1NPipeline::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModHsm1NPipeline::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModHsm1NPipeline::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModHsm1NPipeline::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
