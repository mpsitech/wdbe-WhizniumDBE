/**
	* \file PnlWdbeModList_blks.cpp
	* job handler for job PnlWdbeModList (implementation of blocks)
...
bool PnlWdbeModList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWdbeModList"];}();
...
void PnlWdbeModList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
void PnlWdbeModList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeModList";
...
void PnlWdbeModList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWdbeModList";
...
void PnlWdbeModList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeModList";
...
void PnlWdbeModList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWdbeModList";
...
bool PnlWdbeModList::StgIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWdbeModList"];}();
	...
	if (basefound) {
		...
		if (me.isMember("TcoTplWidth")) {TcoTplWidth = me["TcoTplWidth"].asUInt(); add(TCOTPLWIDTH);};
	};
...
void PnlWdbeModList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	...
	me["TcoTplWidth"] = (Json::Value::UInt) TcoTplWidth;
};
...
void PnlWdbeModList::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeModList";
...
void PnlWdbeModList::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeModList";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "TcoSup", "Super module");
			writeStringAttr(wr, itemtag, "sref", "TcoTpl", "Template");
		};
...
PnlWdbeModList::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEMODLISTDATA)
		{
};
...
void PnlWdbeModList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeModListData"];}();
...
void PnlWdbeModList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeModListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeModList::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBEMODLISTDO)
		{
	ixVDo = 0;
};
...
void PnlWdbeModList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeModListDo"];}();
...
void PnlWdbeModList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeModListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeModList::DpchEngData::DpchEngData(
			...
			, QryWdbeModList::StatShr* statshrqry
			, QryWdbeModList::StgIac* stgiacqry
			, const set<uint>& mask
		) :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBEMODLISTDATA, jref)
		{
...
void PnlWdbeModList::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeModList::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeModListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeModList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeModList::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeModListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeModList::StatApp::writeXML(wr);
