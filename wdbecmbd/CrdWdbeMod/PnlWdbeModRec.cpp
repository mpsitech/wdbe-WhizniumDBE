...
void PnlWdbeModRec::refresh(
	...
	if (statshr.ixWdbeVExpstate == VecWdbeVExpstate::MIND) {
		...
		if (pnl1ncdc) {delete pnl1ncdc; pnl1ncdc = NULL;};
		...
		if (!pnl1ncdc) pnl1ncdc = new PnlWdbeMod1NCdc(xchg, dbswdbe, jref, ixWdbeVLocale);
	...
	statshr.jref1NCdc = ((pnl1ncdc) ? pnl1ncdc->jref : 0);
...
void PnlWdbeModRec::updatePreset(
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFMOD) {
		...
		if (recMdl.ref != 0) {
			...
			if (pnl1ncdc) pnl1ncdc->updatePreset(dbswdbe, ixWdbeVPreset, jrefTrig, notif);
