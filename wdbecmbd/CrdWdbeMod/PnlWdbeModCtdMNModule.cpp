/**
	...
	* job handler for job PnlWdbeModCtdMNModule (implementation)
...
#include "PnlWdbeModCtdMNModule_blks.cpp"
#include "PnlWdbeModCtdMNModule_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModCtdMNModule
 ******************************************************************************/

PnlWdbeModCtdMNModule::PnlWdbeModCtdMNModule(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODCTDMNMODULE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModCtdMNModule(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModCtdMNModule::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModCtdMNModule::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeModCtdMNModule::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeModCtdMNModule::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODCTDMNMODULEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODCTDMNMODULEDO) {
...
void PnlWdbeModCtdMNModule::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModCtdMNModule::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtdMNModule::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModCtdMNModule::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModCtdMNModule* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModCtdMNModule::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModCtdMNModule::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModCtdMNModule::StgIac::JNUM)) {
...
void PnlWdbeModCtdMNModule::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeModCtdMNModule::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtdMNModule::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtdMNModule::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModCtdMNModule::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModCtdMNModule::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
