/**
	...
	* job handler for job PnlWdbeModRef1NSensitivity (implementation)
...
#include "PnlWdbeModRef1NSensitivity_blks.cpp"
#include "PnlWdbeModRef1NSensitivity_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModRef1NSensitivity
 ******************************************************************************/

PnlWdbeModRef1NSensitivity::PnlWdbeModRef1NSensitivity(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODREF1NSENSITIVITY, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModRef1NSensitivity(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModRef1NSensitivity::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModRef1NSensitivity::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeModRef1NSensitivity::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeModRef1NSensitivity::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODREF1NSENSITIVITYDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODREF1NSENSITIVITYDO) {
...
void PnlWdbeModRef1NSensitivity::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModRef1NSensitivity::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModRef1NSensitivity::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModRef1NSensitivity::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModRef1NSensitivity* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModRef1NSensitivity::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModRef1NSensitivity::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModRef1NSensitivity::StgIac::JNUM)) {
...
void PnlWdbeModRef1NSensitivity::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSNS, jref)) if (refMod != 0) {
			sref = "CrdWdbeSns";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFMOD, refMod, sref, recSns.ref, jrefNew);
		};
...
void PnlWdbeModRef1NSensitivity::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSNS, jref) & VecWdbeWAccess::EDIT) != 0) if (refMod != 0) {
			sref = "CrdWdbeSns";
...
void PnlWdbeModRef1NSensitivity::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModRef1NSensitivity::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModRef1NSensitivity::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModRef1NSensitivity::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
