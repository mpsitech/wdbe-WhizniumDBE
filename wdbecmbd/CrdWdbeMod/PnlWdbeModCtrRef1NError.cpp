/**
	...
	* job handler for job PnlWdbeModCtrRef1NError (implementation)
...
#include "PnlWdbeModCtrRef1NError_blks.cpp"
#include "PnlWdbeModCtrRef1NError_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModCtrRef1NError
 ******************************************************************************/

PnlWdbeModCtrRef1NError::PnlWdbeModCtrRef1NError(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODCTRREF1NERROR, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModCtrRef1NError(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModCtrRef1NError::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModCtrRef1NError::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeModCtrRef1NError::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeModCtrRef1NError::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODCTRREF1NERRORDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODCTRREF1NERRORDO) {
...
void PnlWdbeModCtrRef1NError::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModCtrRef1NError::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtrRef1NError::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModCtrRef1NError::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModCtrRef1NError* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModCtrRef1NError::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModCtrRef1NError::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModCtrRef1NError::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recErr = WdbeMError();

			if (recSelNew) {
				if (dbswdbe->tblwdbemerror->loadRecByRef(recSelNew->ref, &_recErr)) {
					recErr = *_recErr;
					delete _recErr;
				};
			};
...
void PnlWdbeModCtrRef1NError::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtrRef1NError::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtrRef1NError::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCERR, jref)) if (refCtr != 0) {
			sref = "CrdWdbeErr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFCTR, refCtr, sref, recErr.ref, jrefNew);
		};
...
void PnlWdbeModCtrRef1NError::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCERR, jref) & VecWdbeWAccess::EDIT) != 0) if (refCtr != 0) {
			sref = "CrdWdbeErr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFCTR, refCtr, sref, -1, jrefNew);
		};
...
void PnlWdbeModCtrRef1NError::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtrRef1NError::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModCtrRef1NError::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModCtrRef1NError::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
