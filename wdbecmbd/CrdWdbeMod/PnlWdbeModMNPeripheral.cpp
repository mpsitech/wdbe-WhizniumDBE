/**
	* \file PnlWdbeModMNPeripheral.cpp
	* job handler for job PnlWdbeModMNPeripheral (implementation)
...
#include "PnlWdbeModMNPeripheral_blks.cpp"
#include "PnlWdbeModMNPeripheral_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModMNPeripheral
 ******************************************************************************/

PnlWdbeModMNPeripheral::PnlWdbeModMNPeripheral(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODMNPERIPHERAL, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModMNPeripheral(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModMNPeripheral::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModMNPeripheral::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeModMNPeripheral::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFMOD) {
		recMdlRpph = WdbeRMModuleMPeripheral();
...
void PnlWdbeModMNPeripheral::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODMNPERIPHERALDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODMNPERIPHERALDO) {
...
void PnlWdbeModMNPeripheral::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModMNPeripheral::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMNPeripheral::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModMNPeripheral::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModMNPeripheral* recSelNew = NULL;

	WdbeRMModuleMPeripheral* _recMdlRpph = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModMNPeripheral::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModMNPeripheral::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModMNPeripheral::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswdbe->tblwdbermmodulemperipheral->loadRecByRef(recSelNew->ref, &_recMdlRpph)) {
					recMdlRpph = *_recMdlRpph;
					delete _recMdlRpph;
				};
				if (dbswdbe->tblwdbemperipheral->loadRecByRef(recSelNew->mref, &_recPph)) {
					recPph = *_recPph;
					delete _recPph;
				};
			};
...
void PnlWdbeModMNPeripheral::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPPH, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFUNT) {
			sref = "CrdWdbePph";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recPph.ref, jrefNew);
		};
...
void PnlWdbeModMNPeripheral::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMNPeripheral::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMNPeripheral::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModMNPeripheral::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModMNPeripheral::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
