/**
	...
	* job handler for job PnlWdbeModMdl1NGeneric (implementation)
...
#include "PnlWdbeModMdl1NGeneric_blks.cpp"
#include "PnlWdbeModMdl1NGeneric_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModMdl1NGeneric
 ******************************************************************************/

PnlWdbeModMdl1NGeneric::PnlWdbeModMdl1NGeneric(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODMDL1NGENERIC, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModMdl1NGeneric(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModMdl1NGeneric::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModMdl1NGeneric::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeModMdl1NGeneric::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeModMdl1NGeneric::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODMDL1NGENERICDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODMDL1NGENERICDO) {
...
void PnlWdbeModMdl1NGeneric::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModMdl1NGeneric::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMdl1NGeneric::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModMdl1NGeneric::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModMdl1NGeneric* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModMdl1NGeneric::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModMdl1NGeneric::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModMdl1NGeneric::StgIac::JNUM)) {
...
void PnlWdbeModMdl1NGeneric::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMdl1NGeneric::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMdl1NGeneric::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCGEN, jref)) if (refMod != 0) {
			sref = "CrdWdbeGen";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFMOD, refMod, sref, recGen.ref, jrefNew);
		};
...
void PnlWdbeModMdl1NGeneric::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCGEN, jref) & VecWdbeWAccess::EDIT) != 0) if (refMod != 0) {
			sref = "CrdWdbeGen";
...
void PnlWdbeModMdl1NGeneric::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMdl1NGeneric::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModMdl1NGeneric::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModMdl1NGeneric::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
