/**
	...
	* job handler for job PnlWdbeMod1NCdc (implementation)
...
#include "PnlWdbeMod1NCdc_blks.cpp"
#include "PnlWdbeMod1NCdc_evals.cpp"
...
/******************************************************************************
 class PnlWdbeMod1NCdc
 ******************************************************************************/

PnlWdbeMod1NCdc::PnlWdbeMod1NCdc(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMOD1NCDC, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeMod1NCdc(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeMod1NCdc::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeMod1NCdc::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeMod1NCdc::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMOD1NCDCDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMOD1NCDCDO) {
...
void PnlWdbeMod1NCdc::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeMod1NCdc::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMod1NCdc::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeMod1NCdc::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeMod1NCdc::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeMod1NCdc::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeMod1NCdc::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCdc = WdbeMCdc();

			if (recSelNew) {
				if (dbswdbe->tblwdbemcdc->loadRecByRef(recSelNew->ref, &_recCdc)) {
					recCdc = *_recCdc;
					delete _recCdc;
				};
			};
...
void PnlWdbeMod1NCdc::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCDC, jref)) if (refMod != 0) {
			sref = "CrdWdbeCdc";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFMOD, refMod, sref, recCdc.ref, jrefNew);
		};
...
void PnlWdbeMod1NCdc::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCDC, jref) & VecWdbeWAccess::EDIT) != 0) if (refMod != 0) {
			sref = "CrdWdbeCdc";
...
void PnlWdbeMod1NCdc::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMod1NCdc::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeMod1NCdc::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeMod1NCdc::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
