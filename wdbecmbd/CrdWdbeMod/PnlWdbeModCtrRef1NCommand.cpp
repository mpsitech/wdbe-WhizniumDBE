/**
	...
	* job handler for job PnlWdbeModCtrRef1NCommand (implementation)
...
#include "PnlWdbeModCtrRef1NCommand_blks.cpp"
#include "PnlWdbeModCtrRef1NCommand_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModCtrRef1NCommand
 ******************************************************************************/

PnlWdbeModCtrRef1NCommand::PnlWdbeModCtrRef1NCommand(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODCTRREF1NCOMMAND, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModCtrRef1NCommand(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModCtrRef1NCommand::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModCtrRef1NCommand::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeModCtrRef1NCommand::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeModCtrRef1NCommand::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODCTRREF1NCOMMANDDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODCTRREF1NCOMMANDDO) {
...
void PnlWdbeModCtrRef1NCommand::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModCtrRef1NCommand::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtrRef1NCommand::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModCtrRef1NCommand::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModCtrRef1NCommand* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModCtrRef1NCommand::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModCtrRef1NCommand::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModCtrRef1NCommand::StgIac::JNUM)) {
...
void PnlWdbeModCtrRef1NCommand::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtrRef1NCommand::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtrRef1NCommand::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeModCtrRef1NCommand::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCMD, jref) & VecWdbeWAccess::EDIT) != 0) if (refCtr != 0) {
			sref = "CrdWdbeCmd";
...
void PnlWdbeModCtrRef1NCommand::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModCtrRef1NCommand::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModCtrRef1NCommand::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModCtrRef1NCommand::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
