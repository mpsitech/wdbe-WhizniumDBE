/**
	...
	* job handler for job PnlWdbeModMge1NSignal (implementation)
...
#include "PnlWdbeModMge1NSignal_blks.cpp"
#include "PnlWdbeModMge1NSignal_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModMge1NSignal
 ******************************************************************************/

PnlWdbeModMge1NSignal::PnlWdbeModMge1NSignal(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODMGE1NSIGNAL, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModMge1NSignal(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModMge1NSignal::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModMge1NSignal::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeModMge1NSignal::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeModMge1NSignal::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODMGE1NSIGNALDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODMGE1NSIGNALDO) {
...
void PnlWdbeModMge1NSignal::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModMge1NSignal::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModMge1NSignal::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModMge1NSignal::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModMge1NSignal* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModMge1NSignal::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModMge1NSignal::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModMge1NSignal::StgIac::JNUM)) {
...
void PnlWdbeModMge1NSignal::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeModMge1NSignal::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModMge1NSignal::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModMge1NSignal::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
