/**
	...
	* job handler for job PnlWdbeModAPar (implementation)
...
#include "PnlWdbeModAPar_blks.cpp"
#include "PnlWdbeModAPar_evals.cpp"
...
/******************************************************************************
 class PnlWdbeModAPar
 ******************************************************************************/

PnlWdbeModAPar::PnlWdbeModAPar(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMODAPAR, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeModAPar(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeModAPar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeModAPar::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeModAPar::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODAPARDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMODAPARDO) {
...
void PnlWdbeModAPar::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeModAPar::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModAPar::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeModAPar::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQModAPar* recSelNew = NULL;

	WdbeAMModulePar* _recMdlApar = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeModAPar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeModAPar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeModAPar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recMdlApar = WdbeAMModulePar();

			if (recSelNew) {
				if (dbswdbe->tblwdbeammodulepar->loadRecByRef(recSelNew->ref, &_recMdlApar)) {
					recMdlApar = *_recMdlApar;
					delete _recMdlApar;
				};
			};
...
void PnlWdbeModAPar::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModAPar::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModAPar::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModAPar::handleDpchAppDoButDuplicateClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModAPar::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeModAPar::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeModAPar::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeModAPar::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
