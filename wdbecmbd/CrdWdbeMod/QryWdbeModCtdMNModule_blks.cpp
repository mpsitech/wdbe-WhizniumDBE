/**
	* \file QryWdbeModCtdMNModule_blks.cpp
	* job handler for job QryWdbeModCtdMNModule (implementation of blocks)
...
void QryWdbeModCtdMNModule::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
			, const uint firstcol
	...
	if (difftag.length() == 0) difftag = "StatAppQryWdbeModCtdMNModule";
...
void QryWdbeModCtdMNModule::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint firstcol
	...
	else itemtag = "StatitemAppQryWdbeModCtdMNModule";
...
void QryWdbeModCtdMNModule::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrQryWdbeModCtdMNModule";
...
void QryWdbeModCtdMNModule::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrQryWdbeModCtdMNModule";
...
bool QryWdbeModCtdMNModule::StgIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacQryWdbeModCtdMNModule"];}();
...
void QryWdbeModCtdMNModule::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
