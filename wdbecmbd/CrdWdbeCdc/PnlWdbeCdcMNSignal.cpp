/**
	...
	* job handler for job PnlWdbeCdcMNSignal (implementation)
...
#include "PnlWdbeCdcMNSignal_blks.cpp"
#include "PnlWdbeCdcMNSignal_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCdcMNSignal
 ******************************************************************************/

PnlWdbeCdcMNSignal::PnlWdbeCdcMNSignal(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECDCMNSIGNAL, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeCdcMNSignal(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeCdcMNSignal::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCdcMNSignal::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeCdcMNSignal::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeCdcMNSignal::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECDCMNSIGNALDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECDCMNSIGNALDO) {
...
void PnlWdbeCdcMNSignal::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCdcMNSignal::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcMNSignal::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCdcMNSignal::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQCdcMNSignal* recSelNew = NULL;

	WdbeRMCdcMSignal* _recCdcRsig = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeCdcMNSignal::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeCdcMNSignal::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeCdcMNSignal::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswdbe->tblwdbermcdcmsignal->loadRecByRef(recSelNew->ref, &_recCdcRsig)) {
					recCdcRsig = *_recCdcRsig;
					delete _recCdcRsig;
				};
				if (dbswdbe->tblwdbemsignal->loadRecByRef(recSelNew->mref, &_recSig)) {
					recSig = *_recSig;
					delete _recSig;
				};
			};
...
void PnlWdbeCdcMNSignal::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeCdcMNSignal::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcMNSignal::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcMNSignal::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCdcMNSignal::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCdcMNSignal::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
