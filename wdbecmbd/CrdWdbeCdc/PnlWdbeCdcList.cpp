/**
	...
	* job handler for job PnlWdbeCdcList (implementation)
...
#include "PnlWdbeCdcList_blks.cpp"
#include "PnlWdbeCdcList_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCdcList
 ******************************************************************************/

PnlWdbeCdcList::PnlWdbeCdcList(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECDCLIST, jrefSup, ixWdbeVLocale)
		{
	...
	QryWdbeCdcList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFSEL, jref, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFCDC, jref));

	qry = new QryWdbeCdcList(xchg, dbswdbe, jref, ixWdbeVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWdbeVExpstate = ((xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFCDC, jref)) ? VecWdbeVExpstate::MIND : VecWdbeVExpstate::REGD);
...
DpchEngWdbe* PnlWdbeCdcList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCdcList::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeCdcList::regularize(
			DbsWdbe* dbswdbe
			, const bool notif
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcList::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECDCLISTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECDCLISTDO) {
...
void PnlWdbeCdcList::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCdcList::handleDpchAppDataContiac(
			DbsWdbe* dbswdbe
			, ContIac* _contiac
			, DpchEngWdbe** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWdbeCdcList::VecVOrd::MDL) && (_contiac->numFTos <= QryWdbeCdcList::VecVOrd::MDL)) {
			muteRefresh = true;
...
void PnlWdbeCdcList::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcList::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCdcList::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeCdcList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeCdcList::StgIac::JNUM) || (diffitems.size() > 1)) {
			muteRefresh = true;
		...
		if (has(diffitems, QryWdbeCdcList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFCDC, refSelNew);
		};
	};
...
void PnlWdbeCdcList::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcList::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcList::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcList::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcList::handleDpchAppDoButFilterClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCdcList::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCdcList::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCdcList::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
