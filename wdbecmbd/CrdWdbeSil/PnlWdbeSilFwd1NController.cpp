/**
	* \file PnlWdbeSilFwd1NController.cpp
	* job handler for job PnlWdbeSilFwd1NController (implementation)
...
#include "PnlWdbeSilFwd1NController_blks.cpp"
#include "PnlWdbeSilFwd1NController_evals.cpp"
...
/******************************************************************************
 class PnlWdbeSilFwd1NController
 ******************************************************************************/

PnlWdbeSilFwd1NController::PnlWdbeSilFwd1NController(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBESILFWD1NCONTROLLER, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeSilFwd1NController(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeSilFwd1NController::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeSilFwd1NController::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeSilFwd1NController::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeSilFwd1NController::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESILFWD1NCONTROLLERDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESILFWD1NCONTROLLERDO) {
...
void PnlWdbeSilFwd1NController::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeSilFwd1NController::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSilFwd1NController::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeSilFwd1NController::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQSilFwd1NController* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeSilFwd1NController::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeSilFwd1NController::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeSilFwd1NController::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCtr = WdbeMController();

			if (recSelNew) {
				if (dbswdbe->tblwdbemcontroller->loadRecByRef(recSelNew->ref, &_recCtr)) {
					recCtr = *_recCtr;
					delete _recCtr;
				};
			};
...
void PnlWdbeSilFwd1NController::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCMOD, jref)) if ((dbswdbe->getIxWSubsetByRefWdbeMModule(recCtr.refWdbeMModule) & VecWdbeWMModuleSubset::SBSWDBEBMMODULEMOD) != 0) if (refUnt != 0) {
			sref = "CrdWdbeMod";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFUNT, refUnt, sref, [&](){ubigint ref = 0; dbswdbe->loadRefBySQL("SELECT ref FROM TblWdbeMModule WHERE refWdbeMController = " + to_string(recCtr.ref), ref); return ref;}(), jrefNew);
		};
		if (jrefNew == 0) {
...
void PnlWdbeSilFwd1NController::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeSilFwd1NController::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeSilFwd1NController::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
