/**
	...
	* job handler for job PnlWdbeSil1NBank (implementation)
...
#include "PnlWdbeSil1NBank_blks.cpp"
#include "PnlWdbeSil1NBank_evals.cpp"
...
/******************************************************************************
 class PnlWdbeSil1NBank
 ******************************************************************************/

PnlWdbeSil1NBank::PnlWdbeSil1NBank(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBESIL1NBANK, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeSil1NBank(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeSil1NBank::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeSil1NBank::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeSil1NBank::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESIL1NBANKDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESIL1NBANKDO) {
...
void PnlWdbeSil1NBank::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeSil1NBank::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSil1NBank::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeSil1NBank::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeSil1NBank::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeSil1NBank::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeSil1NBank::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recBnk = WdbeMBank();

			if (recSelNew) {
				if (dbswdbe->tblwdbembank->loadRecByRef(recSelNew->ref, &_recBnk)) {
					recBnk = *_recBnk;
					delete _recBnk;
				};
			};
...
void PnlWdbeSil1NBank::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCBNK, jref)) if (refUnt != 0) {
			sref = "CrdWdbeBnk";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFUNT, refUnt, sref, recBnk.ref, jrefNew);
		};
...
void PnlWdbeSil1NBank::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCBNK, jref) & VecWdbeWAccess::EDIT) != 0) if (refUnt != 0) {
			sref = "CrdWdbeBnk";
...
void PnlWdbeSil1NBank::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSil1NBank::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeSil1NBank::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeSil1NBank::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
