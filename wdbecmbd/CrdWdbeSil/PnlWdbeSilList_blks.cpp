/**
	* \file PnlWdbeSilList_blks.cpp
	* job handler for job PnlWdbeSilList (implementation of blocks)
...
/******************************************************************************
 class PnlWdbeSilList::VecVDo
 ******************************************************************************/

uint PnlWdbeSilList::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWdbeSilList::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWdbeSilList::ContIac
 ******************************************************************************/

PnlWdbeSilList::ContIac::ContIac(
			const uint numFTos
		) :
			Block()
			, numFTos(numFTos)
...
bool PnlWdbeSilList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWdbeSilList"];}();
...
bool PnlWdbeSilList::ContIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContIacWdbeSilList");
	else
...
void PnlWdbeSilList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContIacWdbeSilList";
...
void PnlWdbeSilList::ContIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
...
set<uint> PnlWdbeSilList::ContIac::comm(
			const ContIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeSilList::ContIac::diff(
			const ContIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeSilList::ContInf
 ******************************************************************************/

PnlWdbeSilList::ContInf::ContInf(
...
void PnlWdbeSilList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeSilList";
...
void PnlWdbeSilList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWdbeSilList";
...
set<uint> PnlWdbeSilList::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeSilList::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeSilList::StatShr
 ******************************************************************************/

PnlWdbeSilList::StatShr::StatShr(
...
void PnlWdbeSilList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeSilList";
...
void PnlWdbeSilList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWdbeSilList";
...
set<uint> PnlWdbeSilList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeSilList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeSilList::StgIac
 ******************************************************************************/

PnlWdbeSilList::StgIac::StgIac(
			...
			, TcoPkgWidth(TcoPkgWidth)
			, TcoEsyWidth(TcoEsyWidth)
			, TcoTchWidth(TcoTchWidth)
		{
...
bool PnlWdbeSilList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWdbeSilList"];}();
	...
	if (basefound) {
		...
		if (me.isMember("TcoTchWidth")) {TcoTchWidth = me["TcoTchWidth"].asUInt(); add(TCOTCHWIDTH);};
	};

	return basefound;
};

bool PnlWdbeSilList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWdbeSilList");
	else
		basefound = checkXPath(docctx, basexpath);
...
void PnlWdbeSilList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacWdbeSilList";
...
void PnlWdbeSilList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
...
set<uint> PnlWdbeSilList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeSilList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeSilList::Tag
 ******************************************************************************/

void PnlWdbeSilList::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeSilList";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
		me["Cpt"] = "Silicon devices";
		me["TcoSrf"] = "Identifier";
		me["TcoTit"] = "Name";
		me["TcoFsr"] = "Full identifier";
		me["TcoTyp"] = "Type";
...
void PnlWdbeSilList::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeSilList";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Silicon devices");
			writeStringAttr(wr, itemtag, "sref", "TcoSrf", "Identifier");
			writeStringAttr(wr, itemtag, "sref", "TcoTit", "Name");
			writeStringAttr(wr, itemtag, "sref", "TcoFsr", "Full identifier");
...
/******************************************************************************
 class PnlWdbeSilList::DpchAppData
 ******************************************************************************/

PnlWdbeSilList::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBESILLISTDATA)
		{
};

string PnlWdbeSilList::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeSilList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeSilListData"];}();
...
void PnlWdbeSilList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeSilListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeSilList::DpchAppDo
 ******************************************************************************/

PnlWdbeSilList::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBESILLISTDO)
		{
	ixVDo = 0;
};

string PnlWdbeSilList::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeSilList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeSilListDo"];}();
...
void PnlWdbeSilList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeSilListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeSilList::DpchEngData
 ******************************************************************************/

PnlWdbeSilList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBESILLISTDATA, jref)
		{
...
string PnlWdbeSilList::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeSilList::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeSilList::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeSilListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeSilList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeSilList::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeSilListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeSilList::StatApp::writeXML(wr);
