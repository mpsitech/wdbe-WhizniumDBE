/**
	...
	* job handler for job PnlWdbeSilHk1NModule (implementation)
...
#include "PnlWdbeSilHk1NModule_blks.cpp"
#include "PnlWdbeSilHk1NModule_evals.cpp"
...
/******************************************************************************
 class PnlWdbeSilHk1NModule
 ******************************************************************************/

PnlWdbeSilHk1NModule::PnlWdbeSilHk1NModule(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBESILHK1NMODULE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeSilHk1NModule(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeSilHk1NModule::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeSilHk1NModule::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeSilHk1NModule::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeSilHk1NModule::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESILHK1NMODULEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESILHK1NMODULEDO) {
...
void PnlWdbeSilHk1NModule::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeSilHk1NModule::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSilHk1NModule::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeSilHk1NModule::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQSilHk1NModule* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeSilHk1NModule::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeSilHk1NModule::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeSilHk1NModule::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recMdl = WdbeMModule();

			if (recSelNew) {
				if (dbswdbe->tblwdbemmodule->loadRecByRef(recSelNew->ref, &_recMdl)) {
					recMdl = *_recMdl;
...
void PnlWdbeSilHk1NModule::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCMOD, jref)) if (refUnt != 0) {
			sref = "CrdWdbeMod";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFUNT, refUnt, sref, recMdl.ref, jrefNew);
		};
...
void PnlWdbeSilHk1NModule::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCMOD, jref) & VecWdbeWAccess::EDIT) != 0) if (refUnt != 0) {
			sref = "CrdWdbeMod";
...
void PnlWdbeSilHk1NModule::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSilHk1NModule::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeSilHk1NModule::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeSilHk1NModule::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
