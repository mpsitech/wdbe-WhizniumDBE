/**
	...
	* job handler for job PnlWdbeSil1NInterrupt (implementation)
...
#include "PnlWdbeSil1NInterrupt_blks.cpp"
#include "PnlWdbeSil1NInterrupt_evals.cpp"
...
/******************************************************************************
 class PnlWdbeSil1NInterrupt
 ******************************************************************************/

PnlWdbeSil1NInterrupt::PnlWdbeSil1NInterrupt(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBESIL1NINTERRUPT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeSil1NInterrupt(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeSil1NInterrupt::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeSil1NInterrupt::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeSil1NInterrupt::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeSil1NInterrupt::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESIL1NINTERRUPTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESIL1NINTERRUPTDO) {
...
void PnlWdbeSil1NInterrupt::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeSil1NInterrupt::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSil1NInterrupt::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeSil1NInterrupt::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQSil1NInterrupt* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeSil1NInterrupt::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeSil1NInterrupt::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeSil1NInterrupt::StgIac::JNUM)) {
...
void PnlWdbeSil1NInterrupt::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeSil1NInterrupt::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeSil1NInterrupt::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSil1NInterrupt::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeSil1NInterrupt::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeSil1NInterrupt::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
