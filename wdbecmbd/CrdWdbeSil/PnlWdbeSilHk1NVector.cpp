/**
	...
	* job handler for job PnlWdbeSilHk1NVector (implementation)
...
#include "PnlWdbeSilHk1NVector_blks.cpp"
#include "PnlWdbeSilHk1NVector_evals.cpp"
...
/******************************************************************************
 class PnlWdbeSilHk1NVector
 ******************************************************************************/

PnlWdbeSilHk1NVector::PnlWdbeSilHk1NVector(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBESILHK1NVECTOR, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeSilHk1NVector(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeSilHk1NVector::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeSilHk1NVector::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeSilHk1NVector::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeSilHk1NVector::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESILHK1NVECTORDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESILHK1NVECTORDO) {
...
void PnlWdbeSilHk1NVector::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeSilHk1NVector::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSilHk1NVector::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeSilHk1NVector::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQSilHk1NVector* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeSilHk1NVector::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeSilHk1NVector::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeSilHk1NVector::StgIac::JNUM)) {
...
void PnlWdbeSilHk1NVector::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeSilHk1NVector::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeSilHk1NVector::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSilHk1NVector::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeSilHk1NVector::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeSilHk1NVector::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
