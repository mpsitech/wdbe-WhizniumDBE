/**
	...
	* job handler for job PnlWdbeVer1NComponent (implementation)
...
#include "PnlWdbeVer1NComponent_blks.cpp"
#include "PnlWdbeVer1NComponent_evals.cpp"
...
/******************************************************************************
 class PnlWdbeVer1NComponent
 ******************************************************************************/

PnlWdbeVer1NComponent::PnlWdbeVer1NComponent(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEVER1NCOMPONENT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeVer1NComponent(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeVer1NComponent::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeVer1NComponent::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeVer1NComponent::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeVer1NComponent::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVER1NCOMPONENTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVER1NCOMPONENTDO) {
...
void PnlWdbeVer1NComponent::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeVer1NComponent::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVer1NComponent::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeVer1NComponent::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQVer1NComponent* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeVer1NComponent::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeVer1NComponent::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeVer1NComponent::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCmp = WdbeMComponent();

			if (recSelNew) {
				if (dbswdbe->tblwdbemcomponent->loadRecByRef(recSelNew->ref, &_recCmp)) {
					recCmp = *_recCmp;
					delete _recCmp;
				};
			};
...
void PnlWdbeVer1NComponent::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeVer1NComponent::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCCMP, jref) & VecWdbeWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWdbeCmp";
...
void PnlWdbeVer1NComponent::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVer1NComponent::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeVer1NComponent::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeVer1NComponent::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
