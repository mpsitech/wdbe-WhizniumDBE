/**
	...
	* job handler for job PnlWdbeVer1NUnit (implementation)
...
#include "PnlWdbeVer1NUnit_blks.cpp"
#include "PnlWdbeVer1NUnit_evals.cpp"
...
/******************************************************************************
 class PnlWdbeVer1NUnit
 ******************************************************************************/

PnlWdbeVer1NUnit::PnlWdbeVer1NUnit(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEVER1NUNIT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeVer1NUnit(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeVer1NUnit::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeVer1NUnit::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeVer1NUnit::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVER1NUNITDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVER1NUNITDO) {
...
void PnlWdbeVer1NUnit::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeVer1NUnit::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVer1NUnit::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeVer1NUnit::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeVer1NUnit::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeVer1NUnit::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeVer1NUnit::StgIac::JNUM)) {
...
void PnlWdbeVer1NUnit::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeVer1NUnit::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeVer1NUnit::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVer1NUnit::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeVer1NUnit::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeVer1NUnit::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
