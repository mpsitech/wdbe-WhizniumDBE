/**
	...
	* job handler for job PnlWdbeVerBvr1NVersion (implementation)
...
#include "PnlWdbeVerBvr1NVersion_blks.cpp"
#include "PnlWdbeVerBvr1NVersion_evals.cpp"
...
/******************************************************************************
 class PnlWdbeVerBvr1NVersion
 ******************************************************************************/

PnlWdbeVerBvr1NVersion::PnlWdbeVerBvr1NVersion(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEVERBVR1NVERSION, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeVerBvr1NVersion(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeVerBvr1NVersion::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeVerBvr1NVersion::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeVerBvr1NVersion::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeVerBvr1NVersion::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVERBVR1NVERSIONDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVERBVR1NVERSIONDO) {
...
void PnlWdbeVerBvr1NVersion::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeVerBvr1NVersion::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVerBvr1NVersion::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeVerBvr1NVersion::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQVerBvr1NVersion* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeVerBvr1NVersion::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeVerBvr1NVersion::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeVerBvr1NVersion::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recVer = WdbeMVersion();

			if (recSelNew) {
				if (dbswdbe->tblwdbemversion->loadRecByRef(recSelNew->ref, &_recVer)) {
					recVer = *_recVer;
					delete _recVer;
				};
			};
...
void PnlWdbeVerBvr1NVersion::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCVER, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFPRJ) {
			sref = "CrdWdbeVer";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recVer.ref, jrefNew);
		};
		if (jrefNew == 0) {
...
void PnlWdbeVerBvr1NVersion::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeVerBvr1NVersion::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeVerBvr1NVersion::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
