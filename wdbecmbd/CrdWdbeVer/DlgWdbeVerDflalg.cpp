...
uint DlgWdbeVerDflalg::enterSgeDone(
	...
	Filename = "IexWdbeDal_" + StrMod::lc(Prjshort);

	dbswdbe->loadUintBySQL("SELECT COUNT(ref) FROM TblWdbeMFile WHERE refIxVTbl = " + to_string(VecWdbeVMFileRefTbl::VER) + " AND refUref = " + to_string(refWdbeMVersion) + " AND Filename LIKE 'IexWdbeDal%'", cnt);
