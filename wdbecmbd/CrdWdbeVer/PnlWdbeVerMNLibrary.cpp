/**
	...
	* job handler for job PnlWdbeVerMNLibrary (implementation)
...
#include "PnlWdbeVerMNLibrary_blks.cpp"
#include "PnlWdbeVerMNLibrary_evals.cpp"
...
/******************************************************************************
 class PnlWdbeVerMNLibrary
 ******************************************************************************/

PnlWdbeVerMNLibrary::PnlWdbeVerMNLibrary(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEVERMNLIBRARY, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeVerMNLibrary(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeVerMNLibrary::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeVerMNLibrary::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeVerMNLibrary::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeVerMNLibrary::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVERMNLIBRARYDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVERMNLIBRARYDO) {
...
void PnlWdbeVerMNLibrary::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeVerMNLibrary::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVerMNLibrary::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeVerMNLibrary::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQVerMNLibrary* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeVerMNLibrary::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeVerMNLibrary::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeVerMNLibrary::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswdbe->tblwdbemlibrary->loadRecByRef(recSelNew->mref, &_recLib)) {
					recLib = *_recLib;
					delete _recLib;
				};
			};
...
void PnlWdbeVerMNLibrary::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCLIB, jref)) {
			sref = "CrdWdbeLib";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, 0, 0, sref, recLib.ref, jrefNew);
		};
...
void PnlWdbeVerMNLibrary::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVerMNLibrary::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVerMNLibrary::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeVerMNLibrary::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeVerMNLibrary::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
