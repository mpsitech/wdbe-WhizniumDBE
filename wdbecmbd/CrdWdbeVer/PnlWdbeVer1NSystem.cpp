/**
	...
	* job handler for job PnlWdbeVer1NSystem (implementation)
...
#include "PnlWdbeVer1NSystem_blks.cpp"
#include "PnlWdbeVer1NSystem_evals.cpp"
...
/******************************************************************************
 class PnlWdbeVer1NSystem
 ******************************************************************************/

PnlWdbeVer1NSystem::PnlWdbeVer1NSystem(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEVER1NSYSTEM, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeVer1NSystem(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeVer1NSystem::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeVer1NSystem::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeVer1NSystem::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeVer1NSystem::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVER1NSYSTEMDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVER1NSYSTEMDO) {
...
void PnlWdbeVer1NSystem::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeVer1NSystem::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVer1NSystem::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeVer1NSystem::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQVer1NSystem* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeVer1NSystem::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeVer1NSystem::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeVer1NSystem::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSys = WdbeMSystem();

			if (recSelNew) {
				if (dbswdbe->tblwdbemsystem->loadRecByRef(recSelNew->ref, &_recSys)) {
					recSys = *_recSys;
					delete _recSys;
				};
			};
...
void PnlWdbeVer1NSystem::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSYS, jref)) if (refVer != 0) {
			sref = "CrdWdbeSys";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFVER, refVer, sref, recSys.ref, jrefNew);
		};
...
void PnlWdbeVer1NSystem::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSYS, jref) & VecWdbeWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWdbeSys";
...
void PnlWdbeVer1NSystem::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVer1NSystem::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeVer1NSystem::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeVer1NSystem::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
