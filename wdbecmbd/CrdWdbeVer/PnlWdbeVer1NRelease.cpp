/**
	...
	* job handler for job PnlWdbeVer1NRelease (implementation)
...
#include "PnlWdbeVer1NRelease_blks.cpp"
#include "PnlWdbeVer1NRelease_evals.cpp"
...
/******************************************************************************
 class PnlWdbeVer1NRelease
 ******************************************************************************/

PnlWdbeVer1NRelease::PnlWdbeVer1NRelease(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEVER1NRELEASE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeVer1NRelease(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeVer1NRelease::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeVer1NRelease::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeVer1NRelease::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeVer1NRelease::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVER1NRELEASEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVER1NRELEASEDO) {
...
void PnlWdbeVer1NRelease::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeVer1NRelease::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVer1NRelease::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeVer1NRelease::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQVer1NRelease* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeVer1NRelease::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeVer1NRelease::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeVer1NRelease::StgIac::JNUM)) {
...
void PnlWdbeVer1NRelease::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeVer1NRelease::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCRLS, jref) & VecWdbeWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWdbeRls";
...
void PnlWdbeVer1NRelease::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeVer1NRelease::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeVer1NRelease::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeVer1NRelease::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
