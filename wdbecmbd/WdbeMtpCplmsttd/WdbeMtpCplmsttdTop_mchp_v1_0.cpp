/**
	* \file WdbeMtpCplmsttdTop_mchp_v1_0.cpp
	* Wdbe operation processor - infer clock primitives and port identifiers (implementation)
...
DpchRetWdbe* WdbeMtpCplmsttdTop_mchp_v1_0::run(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
	...
	if (dbswdbe->tblwdbemmodule->loadRecByRef(refWdbeMModule, &mdl)) {
		...
		// exact copy from MtpCplmsttdTop_xlnx_v1_0
		...
		if ((fExtclk == 0.0) || (fMclk == 0.0) || (fExtclk == fMclk)) {
			...
			if (!extclkDiffNotSng) {
				...
				dbswdbe->tblwdbemmodule->insertNewRec(NULL, VecWdbeVMModuleBasetype::MNFPRIM, mdl->hkIxVTbl, mdl->hkUref, mdl->hkNum + 1, mdl->ref, Wdbe::getRefMtp(dbswdbe, "CLKBUF"), 0, 0, "clkbufExtclk", "", "", "");

			} else {
				dbswdbe->executeQuery("UPDATE TblWdbeMModule SET hkNum = hkNum + 1 WHERE hkIxVTbl = " + to_string(mdl->hkIxVTbl) + " AND hkUref = " + to_string(mdl->hkUref) + " AND hkNum > " + to_string(mdl->hkNum));

				dbswdbe->tblwdbemmodule->insertNewRec(NULL, VecWdbeVMModuleBasetype::MNFPRIM, mdl->hkIxVTbl, mdl->hkUref, mdl->hkNum + 1, mdl->ref, Wdbe::getRefMtp(dbswdbe, "INBUF_DIFF"), 0, 0, "inbuf_diffExtclk", "", "", "");
			};
			...
			if (!extclkDiffNotSng) {
				...
				ref = dbswdbe->tblwdbemmodule->insertNewRec(NULL, VecWdbeVMModuleBasetype::MNFCORE, mdl->hkIxVTbl, mdl->hkUref, mdl->hkNum+2, mdl->ref, Wdbe::getRefMtp(dbswdbe, "ccc_mchp_v1_0"), 0, 0, "cccMclk", "", "", "");
