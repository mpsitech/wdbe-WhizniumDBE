...
DpchRetWdbe* WdbeMtpCplmsttdGpio_Easy_v1_0::run(
	...
	if (Wdbe::getMpa(dbswdbe, refWdbeMModule, "portNotPin", s)) portNotPin = (s == "true");
	...
	if (dbswdbe->tblwdbemmodule->loadRecByRef(refWdbeMModule, &mdl)) {
		...
		if (refMtp) {
			...
			refC = 0; dbswdbe->loadRefBySQL("SELECT refWdbeCPort FROM TblWdbeMPort WHERE mdlRefWdbeMModule = " + to_string(refWdbeMModule) + " AND sref = 'bitsIn'", refC);
			if (refC != 0) dbswdbe->executeQuery("DELETE FROM TblWdbeMPort WHERE refWdbeCPort = " + to_string(refC));

		} else {
			dbswdbe->executeQuery("DELETE FROM TblWdbeMPort WHERE mdlRefWdbeMModule = " + to_string(refWdbeMModule) + " AND sref = 'bitsIo'");
			if (!bidirNotUnidir && !inNotOut) dbswdbe->executeQuery("DELETE FROM TblWdbeMPort WHERE mdlRefWdbeMModule = " + to_string(refWdbeMModule) + " AND sref = 'bitsIn'");
			if (!bidirNotUnidir && inNotOut) dbswdbe->executeQuery("DELETE FROM TblWdbeMPort WHERE mdlRefWdbeMModule = " + to_string(refWdbeMModule) + " AND sref = 'bitsOut'");
			if (!bidirNotUnidir) dbswdbe->executeQuery("DELETE FROM TblWdbeMPort WHERE mdlRefWdbeMModule = " + to_string(refWdbeMModule) + " AND sref = 'bitsDir'");
		};
