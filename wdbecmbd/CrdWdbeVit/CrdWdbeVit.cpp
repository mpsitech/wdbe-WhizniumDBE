/**
	* \file CrdWdbeVit.cpp
	* job handler for job CrdWdbeVit (implementation)
...
CrdWdbeVit::CrdWdbeVit(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEVIT, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFVIT, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWdbeVitList(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlrec = new PnlWdbeVitRec(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbeVitHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbeVit::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeVit::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeVit/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdVit = StubWdbe::getStubVitStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFVIT, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbeVit::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, ixWdbeVPreset, xchg->getRefPreset(ixWdbeVPreset, jref), "CrdWdbeVit", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFVIT, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFVIT, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFVIT, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbeVit::updatePreset(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWdbeVit::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEVITDO) {
...
void CrdWdbeVit::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeVit::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEVIT);
};

void CrdWdbeVit::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeVit::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeVit::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFVIT) {
...
bool CrdWdbeVit::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeVit::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWdbeVit::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeVit::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeVit::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
