/**
	* \file CrdWdbeCmp.cpp
	* job handler for job CrdWdbeCmp (implementation)
...
CrdWdbeCmp::CrdWdbeCmp(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBECMP, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFCMP, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWdbeCmpRec(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbeCmpHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnllist = new PnlWdbeCmpList(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbeCmp::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeCmp::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeCmp/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdCmp = StubWdbe::getStubCmpStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFCMP, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbeCmp::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, ixWdbeVPreset, xchg->getRefPreset(ixWdbeVPreset, jref), "CrdWdbeCmp", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFCMP, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFCMP, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFCMP, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbeCmp::updatePreset(
			DbsWdbe* dbswdbe
			, const uint ixWdbeVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWdbeCmp::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECMPDO) {
...
void CrdWdbeCmp::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeCmp::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBECMP);
};

void CrdWdbeCmp::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeCmp::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeCmp::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeCmp::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeCmp::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFCMP) {
...
void CrdWdbeCmp::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeCmp::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeCmp::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
