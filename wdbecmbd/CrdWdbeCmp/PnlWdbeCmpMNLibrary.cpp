/**
	...
	* job handler for job PnlWdbeCmpMNLibrary (implementation)
...
#include "PnlWdbeCmpMNLibrary_blks.cpp"
#include "PnlWdbeCmpMNLibrary_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCmpMNLibrary
 ******************************************************************************/

PnlWdbeCmpMNLibrary::PnlWdbeCmpMNLibrary(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECMPMNLIBRARY, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeCmpMNLibrary(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeCmpMNLibrary::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCmpMNLibrary::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeCmpMNLibrary::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeCmpMNLibrary::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECMPMNLIBRARYDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECMPMNLIBRARYDO) {
...
void PnlWdbeCmpMNLibrary::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCmpMNLibrary::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmpMNLibrary::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCmpMNLibrary::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQCmpMNLibrary* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeCmpMNLibrary::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeCmpMNLibrary::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeCmpMNLibrary::StgIac::JNUM)) {
...
void PnlWdbeCmpMNLibrary::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWdbeCmpMNLibrary::handleDpchAppDoButAddClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmpMNLibrary::handleDpchAppDoButSubClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmpMNLibrary::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCmpMNLibrary::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCmpMNLibrary::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
