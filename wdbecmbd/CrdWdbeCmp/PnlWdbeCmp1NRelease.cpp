/**
	...
	* job handler for job PnlWdbeCmp1NRelease (implementation)
...
#include "PnlWdbeCmp1NRelease_blks.cpp"
#include "PnlWdbeCmp1NRelease_evals.cpp"
...
/******************************************************************************
 class PnlWdbeCmp1NRelease
 ******************************************************************************/

PnlWdbeCmp1NRelease::PnlWdbeCmp1NRelease(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBECMP1NRELEASE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeCmp1NRelease(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeCmp1NRelease::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeCmp1NRelease::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeCmp1NRelease::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeCmp1NRelease::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECMP1NRELEASEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBECMP1NRELEASEDO) {
...
void PnlWdbeCmp1NRelease::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeCmp1NRelease::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmp1NRelease::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeCmp1NRelease::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQCmp1NRelease* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeCmp1NRelease::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeCmp1NRelease::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeCmp1NRelease::StgIac::JNUM)) {
...
void PnlWdbeCmp1NRelease::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCRLS, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFVER) {
			sref = "CrdWdbeRls";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recRls.ref, jrefNew);
		};
...
void PnlWdbeCmp1NRelease::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCRLS, jref) & VecWdbeWAccess::EDIT) != 0) if (ixPre == VecWdbeVPreset::PREWDBEREFVER) {
			sref = "CrdWdbeRls";
...
void PnlWdbeCmp1NRelease::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeCmp1NRelease::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeCmp1NRelease::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeCmp1NRelease::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
