/**
	* \file PnlWdbeCmpList_blks.cpp
	* job handler for job PnlWdbeCmpList (implementation of blocks)
...
/******************************************************************************
 class PnlWdbeCmpList::VecVDo
 ******************************************************************************/

uint PnlWdbeCmpList::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWdbeCmpList::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWdbeCmpList::ContIac
 ******************************************************************************/

PnlWdbeCmpList::ContIac::ContIac(
			const uint numFTos
		) :
			Block()
		{
...
bool PnlWdbeCmpList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWdbeCmpList"];}();
...
bool PnlWdbeCmpList::ContIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContIacWdbeCmpList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemIacWdbeCmpList";
...
void PnlWdbeCmpList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContIacWdbeCmpList";
...
void PnlWdbeCmpList::ContIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemIacWdbeCmpList";
...
set<uint> PnlWdbeCmpList::ContIac::comm(
			const ContIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCmpList::ContIac::diff(
			const ContIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeCmpList::ContInf
 ******************************************************************************/

PnlWdbeCmpList::ContInf::ContInf(
			const string& TxtFor
			, const string& TxtPre
...
void PnlWdbeCmpList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeCmpList";
...
void PnlWdbeCmpList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWdbeCmpList";
...
set<uint> PnlWdbeCmpList::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCmpList::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeCmpList::StatShr
 ******************************************************************************/

PnlWdbeCmpList::StatShr::StatShr(
...
void PnlWdbeCmpList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeCmpList";
...
void PnlWdbeCmpList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWdbeCmpList";
...
set<uint> PnlWdbeCmpList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCmpList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWdbeCmpList::StgIac
 ******************************************************************************/

PnlWdbeCmpList::StgIac::StgIac(
	...
	mask = {TCOSRFWIDTH, TCOTYPWIDTH, TCOVERWIDTH};
};

bool PnlWdbeCmpList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWdbeCmpList"];}();
...
bool PnlWdbeCmpList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWdbeCmpList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacWdbeCmpList";
...
void PnlWdbeCmpList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacWdbeCmpList";
...
void PnlWdbeCmpList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacWdbeCmpList";
...
set<uint> PnlWdbeCmpList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWdbeCmpList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {TCOSRFWIDTH, TCOTYPWIDTH, TCOVERWIDTH};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
/******************************************************************************
 class PnlWdbeCmpList::Tag
 ******************************************************************************/

void PnlWdbeCmpList::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeCmpList";
...
void PnlWdbeCmpList::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeCmpList";
...
/******************************************************************************
 class PnlWdbeCmpList::DpchAppData
 ******************************************************************************/

PnlWdbeCmpList::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECMPLISTDATA)
		{
};

string PnlWdbeCmpList::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCmpList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCmpListData"];}();
	...
	if (basefound) {
		...
		contiac = ContIac();
		stgiac = StgIac();
		stgiacqry = QryWdbeCmpList::StgIac();
	};
};

void PnlWdbeCmpList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCmpListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeCmpList::DpchAppDo
 ******************************************************************************/

PnlWdbeCmpList::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBECMPLISTDO)
		{
	ixVDo = 0;
};

string PnlWdbeCmpList::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCmpList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeCmpListDo"];}();
...
void PnlWdbeCmpList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeCmpListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWdbeCmpList::DpchEngData
 ******************************************************************************/

PnlWdbeCmpList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, QryWdbeCmpList::StatShr* statshrqry
			, QryWdbeCmpList::StgIac* stgiacqry
			, const set<uint>& mask
		) :
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBECMPLISTDATA, jref)
		{
...
string PnlWdbeCmpList::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWdbeCmpList::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeCmpList::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeCmpListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeCmpList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeCmpList::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeCmpListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeCmpList::StatApp::writeXML(wr);
