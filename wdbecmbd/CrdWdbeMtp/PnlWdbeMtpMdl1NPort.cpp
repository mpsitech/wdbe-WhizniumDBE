/**
	...
	* job handler for job PnlWdbeMtpMdl1NPort (implementation)
...
#include "PnlWdbeMtpMdl1NPort_blks.cpp"
#include "PnlWdbeMtpMdl1NPort_evals.cpp"
...
/******************************************************************************
 class PnlWdbeMtpMdl1NPort
 ******************************************************************************/

PnlWdbeMtpMdl1NPort::PnlWdbeMtpMdl1NPort(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMTPMDL1NPORT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeMtpMdl1NPort(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeMtpMdl1NPort::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeMtpMdl1NPort::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeMtpMdl1NPort::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeMtpMdl1NPort::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMTPMDL1NPORTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMTPMDL1NPORTDO) {
...
void PnlWdbeMtpMdl1NPort::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeMtpMdl1NPort::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMtpMdl1NPort::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeMtpMdl1NPort::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQMtpMdl1NPort* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeMtpMdl1NPort::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeMtpMdl1NPort::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeMtpMdl1NPort::StgIac::JNUM)) {
...
void PnlWdbeMtpMdl1NPort::handleDpchAppDoButUpClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMtpMdl1NPort::handleDpchAppDoButDownClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMtpMdl1NPort::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPRT, jref)) if (refMtp != 0) {
			sref = "CrdWdbePrt";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFMTP, refMtp, sref, recPrt.ref, jrefNew);
		};
...
void PnlWdbeMtpMdl1NPort::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPRT, jref) & VecWdbeWAccess::EDIT) != 0) if (refMtp != 0) {
			sref = "CrdWdbePrt";
...
void PnlWdbeMtpMdl1NPort::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMtpMdl1NPort::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeMtpMdl1NPort::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeMtpMdl1NPort::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
