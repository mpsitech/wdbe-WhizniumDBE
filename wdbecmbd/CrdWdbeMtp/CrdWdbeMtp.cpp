/**
	* \file CrdWdbeMtp.cpp
	* job handler for job CrdWdbeMtp (implementation)
...
CrdWdbeMtp::CrdWdbeMtp(
			...
			JobWdbe(xchg, VecWdbeVJob::CRDWDBEMTP, jrefSup, ixWdbeVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFMTP, ref);

	// initialize according to ref
	changeRef(dbswdbe, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWdbeMtpRec(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnlheadbar = new PnlWdbeMtpHeadbar(xchg, dbswdbe, jref, ixWdbeVLocale);
	pnllist = new PnlWdbeMtpList(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* CrdWdbeMtp::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWdbeMtp::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWdbeMtp/" + VecWdbeVLocale::getSref(ixWdbeVLocale);
	continf.MtxCrdMtp = StubWdbe::getStubMtpStd(dbswdbe, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFMTP, jref), ixWdbeVLocale, Stub::VecVNonetype::FULL);
...
void CrdWdbeMtp::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswdbe, VecWdbeVCall::CALLWDBELOG, jref, VecWdbeVPreset::VOID, 0, "CrdWdbeMtp", ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFMDL, jref, ref);
	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFMTP, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFMTP, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswdbe, VecWdbeVPreset::PREWDBEREFMTP, jrefTrig, notif);

	refresh(dbswdbe, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWdbeMtp::updatePreset(
			DbsWdbe* dbswdbe
...
void CrdWdbeMtp::handleRequest(
	...
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMTPDO) {
...
void CrdWdbeMtp::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void CrdWdbeMtp::handleDpchAppDoClose(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWdbeConfirm(true, jref, "");
	xchg->triggerIxCall(dbswdbe, VecWdbeVCall::CALLWDBECRDCLOSE, jref, VecWdbeVCard::CRDWDBEMTP);
};

void CrdWdbeMtp::handleDpchAppDoMitAppAbtClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void CrdWdbeMtp::handleDpchAppWdbeAlert(
			DbsWdbe* dbswdbe
...
bool CrdWdbeMtp::handleCallWdbeDlgClose(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeMtp::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWdbeMtp::handleCallWdbeRefPreSet(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWdbeVPreset::PREWDBEREFMTP) {
...
void CrdWdbeMtp::leaveSgeIdle(
			DbsWdbe* dbswdbe
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWdbeMtp::enterSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
			, const bool reenter
		) {
...
void CrdWdbeMtp::leaveSgeAlrwdbeabt(
			DbsWdbe* dbswdbe
		) {
