/**
	...
	* job handler for job PnlWdbeMtpMge1NSignal (implementation)
...
#include "PnlWdbeMtpMge1NSignal_blks.cpp"
#include "PnlWdbeMtpMge1NSignal_evals.cpp"
...
/******************************************************************************
 class PnlWdbeMtpMge1NSignal
 ******************************************************************************/

PnlWdbeMtpMge1NSignal::PnlWdbeMtpMge1NSignal(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMTPMGE1NSIGNAL, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeMtpMge1NSignal(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeMtpMge1NSignal::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeMtpMge1NSignal::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
		) {
...
void PnlWdbeMtpMge1NSignal::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeMtpMge1NSignal::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMTPMGE1NSIGNALDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMTPMGE1NSIGNALDO) {
...
void PnlWdbeMtpMge1NSignal::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeMtpMge1NSignal::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMtpMge1NSignal::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeMtpMge1NSignal::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQMtpMge1NSignal* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeMtpMge1NSignal::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeMtpMge1NSignal::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeMtpMge1NSignal::StgIac::JNUM)) {
...
void PnlWdbeMtpMge1NSignal::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSIG, jref)) if (refMtp != 0) {
			sref = "CrdWdbeSig";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFMTP, refMtp, sref, recSig.ref, jrefNew);
		};
...
void PnlWdbeMtpMge1NSignal::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeMtpMge1NSignal::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeMtpMge1NSignal::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
