/**
	...
	* job handler for job PnlWdbeMtpHsm1NPipeline (implementation)
...
#include "PnlWdbeMtpHsm1NPipeline_blks.cpp"
#include "PnlWdbeMtpHsm1NPipeline_evals.cpp"
...
/******************************************************************************
 class PnlWdbeMtpHsm1NPipeline
 ******************************************************************************/

PnlWdbeMtpHsm1NPipeline::PnlWdbeMtpHsm1NPipeline(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMTPHSM1NPIPELINE, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeMtpHsm1NPipeline(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeMtpHsm1NPipeline::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeMtpHsm1NPipeline::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeMtpHsm1NPipeline::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeMtpHsm1NPipeline::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMTPHSM1NPIPELINEDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMTPHSM1NPIPELINEDO) {
...
void PnlWdbeMtpHsm1NPipeline::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeMtpHsm1NPipeline::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMtpHsm1NPipeline::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeMtpHsm1NPipeline::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQMtpHsm1NPipeline* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeMtpHsm1NPipeline::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeMtpHsm1NPipeline::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeMtpHsm1NPipeline::StgIac::JNUM)) {
...
void PnlWdbeMtpHsm1NPipeline::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPPL, jref)) if (refUnt != 0) {
			sref = "CrdWdbePpl";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, VecWdbeVPreset::PREWDBEREFUNT, refUnt, sref, recPpl.ref, jrefNew);
		};
...
void PnlWdbeMtpHsm1NPipeline::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCPPL, jref) & VecWdbeWAccess::EDIT) != 0) if (refUnt != 0) {
			sref = "CrdWdbePpl";
...
void PnlWdbeMtpHsm1NPipeline::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMtpHsm1NPipeline::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeMtpHsm1NPipeline::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeMtpHsm1NPipeline::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
