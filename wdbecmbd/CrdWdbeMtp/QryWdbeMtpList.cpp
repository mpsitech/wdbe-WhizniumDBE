...
void QryWdbeMtpList::rerun(
	...
	sqlstr = "INSERT INTO TblWdbeQMtpList(jref, jnum, ref, sref, ixVBasetype, hkIxVTbl, hkUref, Srefrule)";
	sqlstr += " SELECT " + to_string(jref) + ", 0, TblWdbeMModule.ref, TblWdbeMModule.sref, TblWdbeMModule.ixVBasetype, TblWdbeMModule.hkIxVTbl, TblWdbeMModule.hkUref, TblWdbeMModule.Srefrule";
	sqlstr += " FROM TblWdbeMModule";
	sqlstr += " WHERE (TblWdbeMModule.hkIxVTbl = 1 AND (TblWdbeMModule.supRefWdbeMModule = 0 AND TblWdbeMModule.tplRefWdbeMModule = 0))";
	rerun_filtSQL(sqlstr, preSrf, preTyp, preHkt, preHku, false);
...
void QryWdbeMtpList::rerun_orderSQL(
	...
	if (preIxOrd == VecVOrd::HKU) sqlstr += " ORDER BY TblWdbeMModule.hkUref ASC";
...
void QryWdbeMtpList::handleCall(
	...
	if (call->ixVCall == VecWdbeVCall::CALLWDBEMDLMOD) {
