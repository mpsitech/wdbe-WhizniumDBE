/**
	...
	* job handler for job PnlWdbeMtpRef1NSensitivity (implementation)
...
#include "PnlWdbeMtpRef1NSensitivity_blks.cpp"
#include "PnlWdbeMtpRef1NSensitivity_evals.cpp"
...
/******************************************************************************
 class PnlWdbeMtpRef1NSensitivity
 ******************************************************************************/

PnlWdbeMtpRef1NSensitivity::PnlWdbeMtpRef1NSensitivity(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEMTPREF1NSENSITIVITY, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeMtpRef1NSensitivity(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeMtpRef1NSensitivity::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeMtpRef1NSensitivity::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeMtpRef1NSensitivity::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeMtpRef1NSensitivity::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMTPREF1NSENSITIVITYDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEMTPREF1NSENSITIVITYDO) {
...
void PnlWdbeMtpRef1NSensitivity::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeMtpRef1NSensitivity::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMtpRef1NSensitivity::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeMtpRef1NSensitivity::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQMtpRef1NSensitivity* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeMtpRef1NSensitivity::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeMtpRef1NSensitivity::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeMtpRef1NSensitivity::StgIac::JNUM)) {
...
void PnlWdbeMtpRef1NSensitivity::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeMtpRef1NSensitivity::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeMtpRef1NSensitivity::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeMtpRef1NSensitivity::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
