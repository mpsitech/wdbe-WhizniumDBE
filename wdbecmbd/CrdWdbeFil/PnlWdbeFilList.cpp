/**
	...
	* job handler for job PnlWdbeFilList (implementation)
...
#include "PnlWdbeFilList_blks.cpp"
#include "PnlWdbeFilList_evals.cpp"
...
/******************************************************************************
 class PnlWdbeFilList
 ******************************************************************************/

PnlWdbeFilList::PnlWdbeFilList(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBEFILLIST, jrefSup, ixWdbeVLocale)
		{
	...
	QryWdbeFilList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWdbeVPreset::PREWDBEREFSEL, jref, xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFFIL, jref));
	xchg->addIxPreset(VecWdbeVPreset::PREWDBEIXORD, jref, QryWdbeFilList::VecVOrd::FNM);

	qry = new QryWdbeFilList(xchg, dbswdbe, jref, ixWdbeVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWdbeVExpstate = ((xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFFIL, jref)) ? VecWdbeVExpstate::MIND : VecWdbeVExpstate::REGD);
...
DpchEngWdbe* PnlWdbeFilList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeFilList::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeFilList::regularize(
			DbsWdbe* dbswdbe
			, const bool notif
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFilList::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEFILLISTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBEFILLISTDO) {
...
void PnlWdbeFilList::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeFilList::handleDpchAppDataContiac(
			DbsWdbe* dbswdbe
			, ContIac* _contiac
			, DpchEngWdbe** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWdbeFilList::VecVOrd::REU) && (_contiac->numFTos <= QryWdbeFilList::VecVOrd::GRP)) {
			muteRefresh = true;
...
void PnlWdbeFilList::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFilList::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeFilList::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeFilList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeFilList::StgIac::JNUM) || (diffitems.size() > 1)) {
			muteRefresh = true;
		...
		if (has(diffitems, QryWdbeFilList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswdbe, VecWdbeVCall::CALLWDBEREFPRESET, jref, VecWdbeVPreset::PREWDBEREFFIL, refSelNew);
		};
	};
...
void PnlWdbeFilList::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFilList::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFilList::handleDpchAppDoButNewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFilList::handleDpchAppDoButDeleteClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFilList::handleDpchAppDoButFilterClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeFilList::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeFilList::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeFilList::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
