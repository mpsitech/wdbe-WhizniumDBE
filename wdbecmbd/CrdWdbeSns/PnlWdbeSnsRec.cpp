/**
	...
	* job handler for job PnlWdbeSnsRec (implementation)
...
#include "PnlWdbeSnsRec_blks.cpp"
#include "PnlWdbeSnsRec_evals.cpp"
...
PnlWdbeSnsRec::PnlWdbeSnsRec(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBESNSREC, jrefSup, ixWdbeVLocale)
		{
...
DpchEngWdbe* PnlWdbeSnsRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeSnsRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWdbe::getStubSnsStd(dbswdbe, recSns.ref, ixWdbeVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recSns.ref == 0) statshr.ixWdbeVExpstate = VecWdbeVExpstate::MIND;
	...
	if (statshr.ixWdbeVExpstate == VecWdbeVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWdbeSnsDetail(xchg, dbswdbe, jref, ixWdbeVLocale);
	};
...
void PnlWdbeSnsRec::updatePreset(
			DbsWdbe* dbswdbe
	...
	if (ixWdbeVPreset == VecWdbeVPreset::PREWDBEREFSNS) {
		...
		if (dbswdbe->tblwdbemsensitivity->loadRecByRef(xchg->getRefPreset(VecWdbeVPreset::PREWDBEREFSNS, jref), &_recSns)) {
			recSns = *_recSns;
			delete _recSns;
...
void PnlWdbeSnsRec::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESNSRECDO) {
...
void PnlWdbeSnsRec::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeSnsRec::handleDpchAppDoButMinimizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSnsRec::handleDpchAppDoButRegularizeClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
...
bool PnlWdbeSnsRec::handleCallWdbeSns_srtEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWdbeSnsRec::handleCallWdbeSns_reu_inSbs(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWdbeSnsRec::handleCallWdbeSns_sruEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
...
bool PnlWdbeSnsRec::handleCallWdbeSns_reuEq(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
