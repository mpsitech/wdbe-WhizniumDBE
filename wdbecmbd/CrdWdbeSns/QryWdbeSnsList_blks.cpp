/**
	* \file QryWdbeSnsList_blks.cpp
	* job handler for job QryWdbeSnsList (implementation of blocks)
...
/******************************************************************************
 class QryWdbeSnsList::VecVOrd
 ******************************************************************************/

uint QryWdbeSnsList::VecVOrd::getIx(
			const string& sref
		) {
...
void QryWdbeSnsList::VecVOrd::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
/******************************************************************************
 class QryWdbeSnsList::StatApp
 ******************************************************************************/

void QryWdbeSnsList::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
			, const uint firstcol
	...
	if (difftag.length() == 0) difftag = "StatAppQryWdbeSnsList";
...
void QryWdbeSnsList::StatApp::writeXML(
	...
	else itemtag = "StatitemAppQryWdbeSnsList";
...
/******************************************************************************
 class QryWdbeSnsList::StatShr
 ******************************************************************************/

QryWdbeSnsList::StatShr::StatShr(
			const uint ntot
...
void QryWdbeSnsList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrQryWdbeSnsList";
...
void QryWdbeSnsList::StatShr::writeXML(
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrQryWdbeSnsList";
...
set<uint> QryWdbeSnsList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeSnsList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryWdbeSnsList::StgIac
 ******************************************************************************/

QryWdbeSnsList::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) :
...
bool QryWdbeSnsList::StgIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryWdbeSnsList");
	else
...
void QryWdbeSnsList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacQryWdbeSnsList";
...
set<uint> QryWdbeSnsList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryWdbeSnsList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
