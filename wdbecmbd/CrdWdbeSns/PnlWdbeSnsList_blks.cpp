/**
	* \file PnlWdbeSnsList_blks.cpp
	* job handler for job PnlWdbeSnsList (implementation of blocks)
...
bool PnlWdbeSnsList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWdbeSnsList"];}();
...
void PnlWdbeSnsList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
void PnlWdbeSnsList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWdbeSnsList";
...
void PnlWdbeSnsList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWdbeSnsList";
...
void PnlWdbeSnsList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWdbeSnsList";
...
void PnlWdbeSnsList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWdbeSnsList";
...
bool PnlWdbeSnsList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWdbeSnsList"];}();
	...
	if (basefound) {
		...
		if (me.isMember("TcoSrtWidth")) {TcoSrtWidth = me["TcoSrtWidth"].asUInt(); add(TCOSRTWIDTH);};
		if (me.isMember("TcoSruWidth")) {TcoSruWidth = me["TcoSruWidth"].asUInt(); add(TCOSRUWIDTH);};
	};
...
void PnlWdbeSnsList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	...
	me["TcoSrtWidth"] = (Json::Value::UInt) TcoSrtWidth;
	me["TcoSruWidth"] = (Json::Value::UInt) TcoSruWidth;
};
...
void PnlWdbeSnsList::Tag::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWdbeSnsList";
...
void PnlWdbeSnsList::Tag::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWdbeSnsList";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWdbeVLocale == VecWdbeVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "TcoSrt", "Source");
			writeStringAttr(wr, itemtag, "sref", "TcoSru", "Source");
		};
...
PnlWdbeSnsList::DpchAppData::DpchAppData() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBESNSLISTDATA)
		{
};
...
void PnlWdbeSnsList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeSnsListData"];}();
...
void PnlWdbeSnsList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeSnsListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeSnsList::DpchAppDo::DpchAppDo() :
			DpchAppWdbe(VecWdbeVDpch::DPCHAPPWDBESNSLISTDO)
		{
	ixVDo = 0;
};
...
void PnlWdbeSnsList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWdbeSnsListDo"];}();
...
void PnlWdbeSnsList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWdbeSnsListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWdbeSnsList::DpchEngData::DpchEngData(
			...
			DpchEngWdbe(VecWdbeVDpch::DPCHENGWDBESNSLISTDATA, jref)
		{
...
void PnlWdbeSnsList::DpchEngData::merge(
			DpchEngWdbe* dpcheng
		) {
...
void PnlWdbeSnsList::DpchEngData::writeJSON(
			const uint ixWdbeVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWdbeSnsListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWdbeSnsList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWdbeSnsList::DpchEngData::writeXML(
			const uint ixWdbeVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWdbeSnsListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wdbe");
		...
		if (has(STATAPPQRY)) QryWdbeSnsList::StatApp::writeXML(wr);
