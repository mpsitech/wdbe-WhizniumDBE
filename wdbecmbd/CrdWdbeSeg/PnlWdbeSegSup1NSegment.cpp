/**
	...
	* job handler for job PnlWdbeSegSup1NSegment (implementation)
...
#include "PnlWdbeSegSup1NSegment_blks.cpp"
#include "PnlWdbeSegSup1NSegment_evals.cpp"
...
/******************************************************************************
 class PnlWdbeSegSup1NSegment
 ******************************************************************************/

PnlWdbeSegSup1NSegment::PnlWdbeSegSup1NSegment(
			XchgWdbe* xchg
			, DbsWdbe* dbswdbe
			, const ubigint jrefSup
			, const uint ixWdbeVLocale
		) :
			JobWdbe(xchg, VecWdbeVJob::PNLWDBESEGSUP1NSEGMENT, jrefSup, ixWdbeVLocale)
		{
	...
	qry = new QryWdbeSegSup1NSegment(xchg, dbswdbe, jref, ixWdbeVLocale);
...
DpchEngWdbe* PnlWdbeSegSup1NSegment::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWdbeSegSup1NSegment::refresh(
			DbsWdbe* dbswdbe
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWdbeSegSup1NSegment::updatePreset(
			DbsWdbe* dbswdbe
...
void PnlWdbeSegSup1NSegment::handleRequest(
			DbsWdbe* dbswdbe
			, ReqWdbe* req
		) {
	if (req->ixVBasetype == ReqWdbe::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESEGSUP1NSEGMENTDATA) {
		...
		} else if (req->dpchapp->ixWdbeVDpch == VecWdbeVDpch::DPCHAPPWDBESEGSUP1NSEGMENTDO) {
...
void PnlWdbeSegSup1NSegment::handleDpchAppWdbeInit(
			DbsWdbe* dbswdbe
...
void PnlWdbeSegSup1NSegment::handleDpchAppDataStgiac(
			DbsWdbe* dbswdbe
			, StgIac* _stgiac
			, DpchEngWdbe** dpcheng
		) {
...
void PnlWdbeSegSup1NSegment::handleDpchAppDataStgiacqry(
			DbsWdbe* dbswdbe
			, QryWdbeSegSup1NSegment::StgIac* _stgiacqry
			, DpchEngWdbe** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WdbeQSegSup1NSegment* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWdbeSegSup1NSegment::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWdbeSegSup1NSegment::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswdbe);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWdbeSegSup1NSegment::StgIac::JNUM)) {
...
void PnlWdbeSegSup1NSegment::handleDpchAppDoButViewClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWdbeVPreset::PREWDBEIXCRDACCSEG, jref)) if (ixPre == VecWdbeVPreset::PREWDBEREFPPL) {
			sref = "CrdWdbeSeg";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswdbe, VecWdbeVCall::CALLWDBECRDOPEN, jref, ixPre, refPre, sref, recSeg.ref, jrefNew);
		};
		if (jrefNew == 0) {
...
void PnlWdbeSegSup1NSegment::handleDpchAppDoButRefreshClick(
			DbsWdbe* dbswdbe
			, DpchEngWdbe** dpcheng
		) {
	set<uint> moditems;
...
void PnlWdbeSegSup1NSegment::handleCall(
			DbsWdbe* dbswdbe
			, Call* call
		) {
...
bool PnlWdbeSegSup1NSegment::handleCallWdbeStatChg(
			DbsWdbe* dbswdbe
			, const ubigint jrefTrig
		) {
	bool retval = false;
