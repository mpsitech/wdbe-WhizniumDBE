/**
	* \file JobWdbeIexDcd_blks.cpp
	* job handler for job JobWdbeIexDcd (implementation of blocks)
...
/******************************************************************************
 class JobWdbeIexDcd::VecVSge
 ******************************************************************************/

uint JobWdbeIexDcd::VecVSge::getIx(
			const string& sref
		) {
...
string JobWdbeIexDcd::VecVSge::getSref(
			const uint ix
		) {
...
void JobWdbeIexDcd::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
