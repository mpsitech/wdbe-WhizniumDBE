/**
	* \file JobWdbeIexDal_blks.cpp
	* job handler for job JobWdbeIexDal (implementation of blocks)
...
/******************************************************************************
 class JobWdbeIexDal::VecVSge
 ******************************************************************************/

uint JobWdbeIexDal::VecVSge::getIx(
			const string& sref
		) {
...
string JobWdbeIexDal::VecVSge::getSref(
			const uint ix
		) {
...
void JobWdbeIexDal::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
