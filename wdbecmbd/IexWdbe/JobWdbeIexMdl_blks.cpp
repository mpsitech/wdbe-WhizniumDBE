/**
	* \file JobWdbeIexMdl_blks.cpp
	* job handler for job JobWdbeIexMdl (implementation of blocks)
...
/******************************************************************************
 class JobWdbeIexMdl::VecVSge
 ******************************************************************************/

uint JobWdbeIexMdl::VecVSge::getIx(
			const string& sref
		) {
...
string JobWdbeIexMdl::VecVSge::getSref(
			const uint ix
		) {
...
void JobWdbeIexMdl::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
