...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbePrcList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbePrcListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbePrcList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbePrcList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbePrcListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbePrcListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbePrcList) this.contiac = dpcheng.ContIacWdbePrcList;
				if (dpcheng.ContInfWdbePrcList) this.continf = dpcheng.ContInfWdbePrcList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbePrcList) this.statshr = dpcheng.StatShrWdbePrcList;
				if (dpcheng.StgIacWdbePrcList) this.stgiac = dpcheng.StgIacWdbePrcList;
				if (dpcheng.TagWdbePrcList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbePrcList);
					this.tag = dpcheng.TagWdbePrcList;
				}
				if (dpcheng.ListWdbeQPrcList) this.rst = dpcheng.ListWdbeQPrcList;
				if (dpcheng.StatAppQryWdbePrcList) this.statappqry = dpcheng.StatAppQryWdbePrcList;
				if (dpcheng.StatShrQryWdbePrcList) this.statshrqry = dpcheng.StatShrQryWdbePrcList;
				if (dpcheng.StgIacQryWdbePrcList) this.stgiacqry = dpcheng.StgIacQryWdbePrcList;
				if (dpcheng.ContInfWdbePrcList) {
				...
				*/
				...
				if (dpcheng.TagWdbePrcList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoMdl", text: this.tag.TcoMdl}, {value: "TcoFal", text: this.tag.TcoFal});
					/*
					*/
				...
				if (dpcheng.ListWdbeQPrcList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoMdl: rec["mdl"], TcoFal: rec["fal"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbePrcListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbePrcListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
