...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeVecList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeVecListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeVecList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeVecList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeVecListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeVecListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeVecList) this.contiac = dpcheng.ContIacWdbeVecList;
				if (dpcheng.ContInfWdbeVecList) this.continf = dpcheng.ContInfWdbeVecList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeVecList) this.statshr = dpcheng.StatShrWdbeVecList;
				if (dpcheng.StgIacWdbeVecList) this.stgiac = dpcheng.StgIacWdbeVecList;
				if (dpcheng.TagWdbeVecList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeVecList);
					this.tag = dpcheng.TagWdbeVecList;
				}
				if (dpcheng.ListWdbeQVecList) this.rst = dpcheng.ListWdbeQVecList;
				if (dpcheng.StatAppQryWdbeVecList) this.statappqry = dpcheng.StatAppQryWdbeVecList;
				if (dpcheng.StatShrQryWdbeVecList) this.statshrqry = dpcheng.StatShrQryWdbeVecList;
				if (dpcheng.StgIacQryWdbeVecList) this.stgiacqry = dpcheng.StgIacQryWdbeVecList;
				if (dpcheng.ContInfWdbeVecList) {
				...
				*/
				...
				if (dpcheng.TagWdbeVecList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTyp", text: this.tag.TcoTyp}, {value: "TcoHkt", text: this.tag.TcoHkt}, {value: "TcoHku", text: this.tag.TcoHku});
					/*
					*/
				...
				if (dpcheng.ListWdbeQVecList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTyp: rec["typ2"], TcoHkt: rec["hkt2"], TcoHku: rec["hku"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeVecListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeVecListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
