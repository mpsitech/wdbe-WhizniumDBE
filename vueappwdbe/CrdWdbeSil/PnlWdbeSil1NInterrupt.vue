...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeSil1NInterrupt.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeSil1NInterruptData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeSil1NInterrupt": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeSil1NInterrupt"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeSil1NInterruptData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeSil1NInterruptData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeSil1NInterrupt) this.continf = dpcheng.ContInfWdbeSil1NInterrupt;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeSil1NInterrupt) this.statapp = dpcheng.StatAppWdbeSil1NInterrupt;
				if (dpcheng.StatShrWdbeSil1NInterrupt) this.statshr = dpcheng.StatShrWdbeSil1NInterrupt;
				if (dpcheng.StgIacWdbeSil1NInterrupt) this.stgiac = dpcheng.StgIacWdbeSil1NInterrupt;
				if (dpcheng.TagWdbeSil1NInterrupt) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeSil1NInterrupt);
					this.tag = dpcheng.TagWdbeSil1NInterrupt;
				}
				if (dpcheng.ListWdbeQSil1NInterrupt) this.rst = dpcheng.ListWdbeQSil1NInterrupt;
				if (dpcheng.StatAppQryWdbeSil1NInterrupt) this.statappqry = dpcheng.StatAppQryWdbeSil1NInterrupt;
				if (dpcheng.StatShrQryWdbeSil1NInterrupt) this.statshrqry = dpcheng.StatShrQryWdbeSil1NInterrupt;
				if (dpcheng.StgIacQryWdbeSil1NInterrupt) this.stgiacqry = dpcheng.StgIacQryWdbeSil1NInterrupt;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQSil1NInterrupt) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeSil1NInterruptData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeSil1NInterruptData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
