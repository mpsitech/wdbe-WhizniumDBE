...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeSilList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeSilListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeSilList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeSilList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeSilListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeSilListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeSilList) this.contiac = dpcheng.ContIacWdbeSilList;
				if (dpcheng.ContInfWdbeSilList) this.continf = dpcheng.ContInfWdbeSilList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeSilList) this.statshr = dpcheng.StatShrWdbeSilList;
				if (dpcheng.StgIacWdbeSilList) this.stgiac = dpcheng.StgIacWdbeSilList;
				if (dpcheng.TagWdbeSilList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeSilList);
					this.tag = dpcheng.TagWdbeSilList;
				}
				if (dpcheng.ListWdbeQSilList) this.rst = dpcheng.ListWdbeQSilList;
				if (dpcheng.StatAppQryWdbeSilList) this.statappqry = dpcheng.StatAppQryWdbeSilList;
				if (dpcheng.StatShrQryWdbeSilList) this.statshrqry = dpcheng.StatShrQryWdbeSilList;
				if (dpcheng.StgIacQryWdbeSilList) this.stgiacqry = dpcheng.StgIacQryWdbeSilList;
				if (dpcheng.ContInfWdbeSilList) {
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeSilListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeSilListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
