...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeSil1NBank.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeSil1NBankData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeSil1NBank": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeSil1NBank"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeSil1NBankData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeSil1NBankData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeSil1NBank) this.continf = dpcheng.ContInfWdbeSil1NBank;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeSil1NBank) this.statapp = dpcheng.StatAppWdbeSil1NBank;
				if (dpcheng.StatShrWdbeSil1NBank) this.statshr = dpcheng.StatShrWdbeSil1NBank;
				if (dpcheng.StgIacWdbeSil1NBank) this.stgiac = dpcheng.StgIacWdbeSil1NBank;
				if (dpcheng.TagWdbeSil1NBank) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeSil1NBank);
					this.tag = dpcheng.TagWdbeSil1NBank;
				}
				if (dpcheng.ListWdbeQSil1NBank) this.rst = dpcheng.ListWdbeQSil1NBank;
				if (dpcheng.StatAppQryWdbeSil1NBank) this.statappqry = dpcheng.StatAppQryWdbeSil1NBank;
				if (dpcheng.StatShrQryWdbeSil1NBank) this.statshrqry = dpcheng.StatShrQryWdbeSil1NBank;
				if (dpcheng.StgIacQryWdbeSil1NBank) this.stgiacqry = dpcheng.StgIacQryWdbeSil1NBank;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeSil1NBankData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeSil1NBankData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
