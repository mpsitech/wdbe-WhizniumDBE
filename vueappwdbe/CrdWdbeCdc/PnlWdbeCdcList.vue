...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeCdcList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeCdcListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeCdcList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeCdcList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeCdcListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeCdcListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeCdcList) this.contiac = dpcheng.ContIacWdbeCdcList;
				if (dpcheng.ContInfWdbeCdcList) this.continf = dpcheng.ContInfWdbeCdcList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeCdcList) this.statshr = dpcheng.StatShrWdbeCdcList;
				if (dpcheng.StgIacWdbeCdcList) this.stgiac = dpcheng.StgIacWdbeCdcList;
				if (dpcheng.TagWdbeCdcList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeCdcList);
					this.tag = dpcheng.TagWdbeCdcList;
				}
				if (dpcheng.ListWdbeQCdcList) this.rst = dpcheng.ListWdbeQCdcList;
				if (dpcheng.StatAppQryWdbeCdcList) this.statappqry = dpcheng.StatAppQryWdbeCdcList;
				if (dpcheng.StatShrQryWdbeCdcList) this.statshrqry = dpcheng.StatShrQryWdbeCdcList;
				if (dpcheng.StgIacQryWdbeCdcList) this.stgiacqry = dpcheng.StgIacQryWdbeCdcList;
				if (dpcheng.ContInfWdbeCdcList) {
				...
				*/
				...
				if (dpcheng.TagWdbeCdcList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMdl", text: this.tag.TcoMdl});
					/*
					*/
				...
				if (dpcheng.ListWdbeQCdcList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMdl: rec["mdl"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeCdcListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeCdcListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
