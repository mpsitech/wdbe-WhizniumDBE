...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeCdcMNSignal.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeCdcMNSignalData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeCdcMNSignal": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeCdcMNSignal"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeCdcMNSignalData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeCdcMNSignalData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeCdcMNSignal) this.continf = dpcheng.ContInfWdbeCdcMNSignal;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeCdcMNSignal) this.statapp = dpcheng.StatAppWdbeCdcMNSignal;
				if (dpcheng.StatShrWdbeCdcMNSignal) this.statshr = dpcheng.StatShrWdbeCdcMNSignal;
				if (dpcheng.StgIacWdbeCdcMNSignal) this.stgiac = dpcheng.StgIacWdbeCdcMNSignal;
				if (dpcheng.TagWdbeCdcMNSignal) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeCdcMNSignal);
					this.tag = dpcheng.TagWdbeCdcMNSignal;
				}
				if (dpcheng.ListWdbeQCdcMNSignal) this.rst = dpcheng.ListWdbeQCdcMNSignal;
				if (dpcheng.StatAppQryWdbeCdcMNSignal) this.statappqry = dpcheng.StatAppQryWdbeCdcMNSignal;
				if (dpcheng.StatShrQryWdbeCdcMNSignal) this.statshrqry = dpcheng.StatShrQryWdbeCdcMNSignal;
				if (dpcheng.StgIacQryWdbeCdcMNSignal) this.stgiacqry = dpcheng.StgIacQryWdbeCdcMNSignal;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQCdcMNSignal) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeCdcMNSignalData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeCdcMNSignalData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
