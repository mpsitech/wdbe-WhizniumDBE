...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeLibMNVersion.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeLibMNVersionData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeLibMNVersion": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeLibMNVersion"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeLibMNVersionData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeLibMNVersionData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeLibMNVersion) this.continf = dpcheng.ContInfWdbeLibMNVersion;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeLibMNVersion) this.statapp = dpcheng.StatAppWdbeLibMNVersion;
				if (dpcheng.StatShrWdbeLibMNVersion) this.statshr = dpcheng.StatShrWdbeLibMNVersion;
				if (dpcheng.StgIacWdbeLibMNVersion) this.stgiac = dpcheng.StgIacWdbeLibMNVersion;
				if (dpcheng.TagWdbeLibMNVersion) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeLibMNVersion);
					this.tag = dpcheng.TagWdbeLibMNVersion;
				}
				if (dpcheng.ListWdbeQLibMNVersion) this.rst = dpcheng.ListWdbeQLibMNVersion;
				if (dpcheng.StatAppQryWdbeLibMNVersion) this.statappqry = dpcheng.StatAppQryWdbeLibMNVersion;
				if (dpcheng.StatShrQryWdbeLibMNVersion) this.statshrqry = dpcheng.StatShrQryWdbeLibMNVersion;
				if (dpcheng.StgIacQryWdbeLibMNVersion) this.stgiacqry = dpcheng.StgIacQryWdbeLibMNVersion;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQLibMNVersion) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeLibMNVersionData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeLibMNVersionData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
