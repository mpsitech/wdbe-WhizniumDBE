...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeLibList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeLibListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeLibList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeLibList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeLibListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeLibListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeLibList) this.contiac = dpcheng.ContIacWdbeLibList;
				if (dpcheng.ContInfWdbeLibList) this.continf = dpcheng.ContInfWdbeLibList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeLibList) this.statshr = dpcheng.StatShrWdbeLibList;
				if (dpcheng.StgIacWdbeLibList) this.stgiac = dpcheng.StgIacWdbeLibList;
				if (dpcheng.TagWdbeLibList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeLibList);
					this.tag = dpcheng.TagWdbeLibList;
				}
				if (dpcheng.ListWdbeQLibList) this.rst = dpcheng.ListWdbeQLibList;
				if (dpcheng.StatAppQryWdbeLibList) this.statappqry = dpcheng.StatAppQryWdbeLibList;
				if (dpcheng.StatShrQryWdbeLibList) this.statshrqry = dpcheng.StatShrQryWdbeLibList;
				if (dpcheng.StgIacQryWdbeLibList) this.stgiacqry = dpcheng.StgIacQryWdbeLibList;
				if (dpcheng.ContInfWdbeLibList) {
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeLibListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeLibListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
