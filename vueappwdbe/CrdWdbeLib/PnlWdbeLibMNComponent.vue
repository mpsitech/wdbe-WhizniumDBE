...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeLibMNComponent.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeLibMNComponentData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeLibMNComponent": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeLibMNComponent"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeLibMNComponentData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeLibMNComponentData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeLibMNComponent) this.continf = dpcheng.ContInfWdbeLibMNComponent;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeLibMNComponent) this.statapp = dpcheng.StatAppWdbeLibMNComponent;
				if (dpcheng.StatShrWdbeLibMNComponent) this.statshr = dpcheng.StatShrWdbeLibMNComponent;
				if (dpcheng.StgIacWdbeLibMNComponent) this.stgiac = dpcheng.StgIacWdbeLibMNComponent;
				if (dpcheng.TagWdbeLibMNComponent) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeLibMNComponent);
					this.tag = dpcheng.TagWdbeLibMNComponent;
				}
				if (dpcheng.ListWdbeQLibMNComponent) this.rst = dpcheng.ListWdbeQLibMNComponent;
				if (dpcheng.StatAppQryWdbeLibMNComponent) this.statappqry = dpcheng.StatAppQryWdbeLibMNComponent;
				if (dpcheng.StatShrQryWdbeLibMNComponent) this.statshrqry = dpcheng.StatShrQryWdbeLibMNComponent;
				if (dpcheng.StgIacQryWdbeLibMNComponent) this.stgiacqry = dpcheng.StgIacQryWdbeLibMNComponent;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQLibMNComponent) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeLibMNComponentData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeLibMNComponentData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
