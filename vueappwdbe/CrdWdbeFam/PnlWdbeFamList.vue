...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeFamList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeFamListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeFamList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeFamList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeFamListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeFamListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeFamList) this.contiac = dpcheng.ContIacWdbeFamList;
				if (dpcheng.ContInfWdbeFamList) this.continf = dpcheng.ContInfWdbeFamList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeFamList) this.statshr = dpcheng.StatShrWdbeFamList;
				if (dpcheng.StgIacWdbeFamList) this.stgiac = dpcheng.StgIacWdbeFamList;
				if (dpcheng.TagWdbeFamList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeFamList);
					this.tag = dpcheng.TagWdbeFamList;
				}
				if (dpcheng.ListWdbeQFamList) this.rst = dpcheng.ListWdbeQFamList;
				if (dpcheng.StatAppQryWdbeFamList) this.statappqry = dpcheng.StatAppQryWdbeFamList;
				if (dpcheng.StatShrQryWdbeFamList) this.statshrqry = dpcheng.StatShrQryWdbeFamList;
				if (dpcheng.StgIacQryWdbeFamList) this.stgiacqry = dpcheng.StgIacQryWdbeFamList;
				if (dpcheng.ContInfWdbeFamList) {
				...
				*/
				...
				if (dpcheng.TagWdbeFamList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoVnd", text: this.tag.TcoVnd}, {value: "TcoTit", text: this.tag.TcoTit});
					/*
					*/
				...
				if (dpcheng.ListWdbeQFamList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoVnd: rec["vnd2"], TcoTit: rec["tit"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeFamListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeFamListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
