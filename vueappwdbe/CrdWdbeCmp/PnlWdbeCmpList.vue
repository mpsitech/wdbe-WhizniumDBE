...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeCmpList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeCmpListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeCmpList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeCmpList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeCmpListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeCmpListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeCmpList) this.contiac = dpcheng.ContIacWdbeCmpList;
				if (dpcheng.ContInfWdbeCmpList) this.continf = dpcheng.ContInfWdbeCmpList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeCmpList) this.statshr = dpcheng.StatShrWdbeCmpList;
				if (dpcheng.StgIacWdbeCmpList) this.stgiac = dpcheng.StgIacWdbeCmpList;
				if (dpcheng.TagWdbeCmpList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeCmpList);
					this.tag = dpcheng.TagWdbeCmpList;
				}
				if (dpcheng.ListWdbeQCmpList) this.rst = dpcheng.ListWdbeQCmpList;
				if (dpcheng.StatAppQryWdbeCmpList) this.statappqry = dpcheng.StatAppQryWdbeCmpList;
				if (dpcheng.StatShrQryWdbeCmpList) this.statshrqry = dpcheng.StatShrQryWdbeCmpList;
				if (dpcheng.StgIacQryWdbeCmpList) this.stgiacqry = dpcheng.StgIacQryWdbeCmpList;
				if (dpcheng.ContInfWdbeCmpList) {
				...
				*/
				...
				if (dpcheng.TagWdbeCmpList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTyp", text: this.tag.TcoTyp}, {value: "TcoVer", text: this.tag.TcoVer});
					/*
					*/
				...
				if (dpcheng.ListWdbeQCmpList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTyp: rec["typ2"], TcoVer: rec["ver"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeCmpListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeCmpListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
