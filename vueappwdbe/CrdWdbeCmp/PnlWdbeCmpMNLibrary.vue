...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeCmpMNLibrary.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeCmpMNLibraryData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeCmpMNLibrary": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeCmpMNLibrary"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeCmpMNLibraryData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeCmpMNLibraryData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeCmpMNLibrary) this.continf = dpcheng.ContInfWdbeCmpMNLibrary;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeCmpMNLibrary) this.statapp = dpcheng.StatAppWdbeCmpMNLibrary;
				if (dpcheng.StatShrWdbeCmpMNLibrary) this.statshr = dpcheng.StatShrWdbeCmpMNLibrary;
				if (dpcheng.StgIacWdbeCmpMNLibrary) this.stgiac = dpcheng.StgIacWdbeCmpMNLibrary;
				if (dpcheng.TagWdbeCmpMNLibrary) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeCmpMNLibrary);
					this.tag = dpcheng.TagWdbeCmpMNLibrary;
				}
				if (dpcheng.ListWdbeQCmpMNLibrary) this.rst = dpcheng.ListWdbeQCmpMNLibrary;
				if (dpcheng.StatAppQryWdbeCmpMNLibrary) this.statappqry = dpcheng.StatAppQryWdbeCmpMNLibrary;
				if (dpcheng.StatShrQryWdbeCmpMNLibrary) this.statshrqry = dpcheng.StatShrQryWdbeCmpMNLibrary;
				if (dpcheng.StgIacQryWdbeCmpMNLibrary) this.stgiacqry = dpcheng.StgIacQryWdbeCmpMNLibrary;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQCmpMNLibrary) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeCmpMNLibraryData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeCmpMNLibraryData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
