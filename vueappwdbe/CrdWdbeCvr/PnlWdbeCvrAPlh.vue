...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeCvrAPlh.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeCvrAPlhData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeCvrAPlh": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeCvrAPlh"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeCvrAPlhData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeCvrAPlhData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeCvrAPlh) this.continf = dpcheng.ContInfWdbeCvrAPlh;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeCvrAPlh) this.statapp = dpcheng.StatAppWdbeCvrAPlh;
				if (dpcheng.StatShrWdbeCvrAPlh) this.statshr = dpcheng.StatShrWdbeCvrAPlh;
				if (dpcheng.StgIacWdbeCvrAPlh) this.stgiac = dpcheng.StgIacWdbeCvrAPlh;
				if (dpcheng.TagWdbeCvrAPlh) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeCvrAPlh);
					this.tag = dpcheng.TagWdbeCvrAPlh;
				}
				if (dpcheng.ListWdbeQCvrAPlh) this.rst = dpcheng.ListWdbeQCvrAPlh;
				if (dpcheng.StatAppQryWdbeCvrAPlh) this.statappqry = dpcheng.StatAppQryWdbeCvrAPlh;
				if (dpcheng.StatShrQryWdbeCvrAPlh) this.statshrqry = dpcheng.StatShrQryWdbeCvrAPlh;
				if (dpcheng.StgIacQryWdbeCvrAPlh) this.stgiacqry = dpcheng.StgIacQryWdbeCvrAPlh;
				/*
				...
				*/
				...
				if (dpcheng.TagWdbeCvrAPlh) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoCmt", text: this.tag.TcoCmt});
					/*
					*/
				...
				if (dpcheng.ListWdbeQCvrAPlh) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoCmt: rec["cmt"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeCvrAPlhData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeCvrAPlhData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
