...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeCvrAIp.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeCvrAIpData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeCvrAIp": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeCvrAIp"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeCvrAIpData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeCvrAIpData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeCvrAIp) this.continf = dpcheng.ContInfWdbeCvrAIp;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeCvrAIp) this.statapp = dpcheng.StatAppWdbeCvrAIp;
				if (dpcheng.StatShrWdbeCvrAIp) this.statshr = dpcheng.StatShrWdbeCvrAIp;
				if (dpcheng.StgIacWdbeCvrAIp) this.stgiac = dpcheng.StgIacWdbeCvrAIp;
				if (dpcheng.TagWdbeCvrAIp) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeCvrAIp);
					this.tag = dpcheng.TagWdbeCvrAIp;
				}
				if (dpcheng.ListWdbeQCvrAIp) this.rst = dpcheng.ListWdbeQCvrAIp;
				if (dpcheng.StatAppQryWdbeCvrAIp) this.statappqry = dpcheng.StatAppQryWdbeCvrAIp;
				if (dpcheng.StatShrQryWdbeCvrAIp) this.statshrqry = dpcheng.StatShrQryWdbeCvrAIp;
				if (dpcheng.StgIacQryWdbeCvrAIp) this.stgiacqry = dpcheng.StgIacQryWdbeCvrAIp;
				/*
				...
				*/
				...
				if (dpcheng.TagWdbeCvrAIp) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTty", text: this.tag.TcoTty}, {value: "TcoCmt", text: this.tag.TcoCmt});
					/*
					*/
				...
				if (dpcheng.ListWdbeQCvrAIp) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTty: rec["tty"], TcoCmt: rec["cmt"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeCvrAIpData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeCvrAIpData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
