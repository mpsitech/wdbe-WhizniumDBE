...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbePplList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbePplListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbePplList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbePplList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbePplListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbePplListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbePplList) this.contiac = dpcheng.ContIacWdbePplList;
				if (dpcheng.ContInfWdbePplList) this.continf = dpcheng.ContInfWdbePplList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbePplList) this.statshr = dpcheng.StatShrWdbePplList;
				if (dpcheng.StgIacWdbePplList) this.stgiac = dpcheng.StgIacWdbePplList;
				if (dpcheng.TagWdbePplList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbePplList);
					this.tag = dpcheng.TagWdbePplList;
				}
				if (dpcheng.ListWdbeQPplList) this.rst = dpcheng.ListWdbeQPplList;
				if (dpcheng.StatAppQryWdbePplList) this.statappqry = dpcheng.StatAppQryWdbePplList;
				if (dpcheng.StatShrQryWdbePplList) this.statshrqry = dpcheng.StatShrQryWdbePplList;
				if (dpcheng.StgIacQryWdbePplList) this.stgiacqry = dpcheng.StgIacQryWdbePplList;
				if (dpcheng.ContInfWdbePplList) {
				...
				*/
				...
				if (dpcheng.TagWdbePplList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoHsm", text: this.tag.TcoHsm}, {value: "TcoDpt", text: this.tag.TcoDpt});
					/*
					*/
				...
				if (dpcheng.ListWdbeQPplList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoHsm: rec["hsm"], TcoDpt: rec["dpt"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbePplListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbePplListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
