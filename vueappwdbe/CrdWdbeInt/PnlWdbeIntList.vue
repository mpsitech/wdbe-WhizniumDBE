...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeIntList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeIntListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeIntList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeIntList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeIntListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeIntListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeIntList) this.contiac = dpcheng.ContIacWdbeIntList;
				if (dpcheng.ContInfWdbeIntList) this.continf = dpcheng.ContInfWdbeIntList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeIntList) this.statshr = dpcheng.StatShrWdbeIntList;
				if (dpcheng.StgIacWdbeIntList) this.stgiac = dpcheng.StgIacWdbeIntList;
				if (dpcheng.TagWdbeIntList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeIntList);
					this.tag = dpcheng.TagWdbeIntList;
				}
				if (dpcheng.ListWdbeQIntList) this.rst = dpcheng.ListWdbeQIntList;
				if (dpcheng.StatAppQryWdbeIntList) this.statappqry = dpcheng.StatAppQryWdbeIntList;
				if (dpcheng.StatShrQryWdbeIntList) this.statshrqry = dpcheng.StatShrQryWdbeIntList;
				if (dpcheng.StgIacQryWdbeIntList) this.stgiacqry = dpcheng.StgIacQryWdbeIntList;
				if (dpcheng.ContInfWdbeIntList) {
				...
				*/
				...
				if (dpcheng.TagWdbeIntList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoUnt", text: this.tag.TcoUnt});
					/*
					*/
				...
				if (dpcheng.ListWdbeQIntList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoUnt: rec["unt"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeIntListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeIntListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
