...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeSysList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeSysListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeSysList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeSysList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeSysListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeSysListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeSysList) this.contiac = dpcheng.ContIacWdbeSysList;
				if (dpcheng.ContInfWdbeSysList) this.continf = dpcheng.ContInfWdbeSysList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeSysList) this.statshr = dpcheng.StatShrWdbeSysList;
				if (dpcheng.StgIacWdbeSysList) this.stgiac = dpcheng.StgIacWdbeSysList;
				if (dpcheng.TagWdbeSysList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeSysList);
					this.tag = dpcheng.TagWdbeSysList;
				}
				if (dpcheng.ListWdbeQSysList) this.rst = dpcheng.ListWdbeQSysList;
				if (dpcheng.StatAppQryWdbeSysList) this.statappqry = dpcheng.StatAppQryWdbeSysList;
				if (dpcheng.StatShrQryWdbeSysList) this.statshrqry = dpcheng.StatShrQryWdbeSysList;
				if (dpcheng.StgIacQryWdbeSysList) this.stgiacqry = dpcheng.StgIacQryWdbeSysList;
				if (dpcheng.ContInfWdbeSysList) {
				...
				*/
				...
				if (dpcheng.TagWdbeSysList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoVer", text: this.tag.TcoVer}, {value: "TcoUnt", text: this.tag.TcoUnt});
					/*
					*/
				...
				if (dpcheng.ListWdbeQSysList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoVer: rec["ver"], TcoUnt: rec["unt"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeSysListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeSysListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
