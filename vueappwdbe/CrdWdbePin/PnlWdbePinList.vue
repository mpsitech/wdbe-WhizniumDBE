...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbePinList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbePinListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbePinList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbePinList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbePinListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbePinListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbePinList) this.contiac = dpcheng.ContIacWdbePinList;
				if (dpcheng.ContInfWdbePinList) this.continf = dpcheng.ContInfWdbePinList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbePinList) this.statshr = dpcheng.StatShrWdbePinList;
				if (dpcheng.StgIacWdbePinList) this.stgiac = dpcheng.StgIacWdbePinList;
				if (dpcheng.TagWdbePinList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbePinList);
					this.tag = dpcheng.TagWdbePinList;
				}
				if (dpcheng.ListWdbeQPinList) this.rst = dpcheng.ListWdbeQPinList;
				if (dpcheng.StatAppQryWdbePinList) this.statappqry = dpcheng.StatAppQryWdbePinList;
				if (dpcheng.StatShrQryWdbePinList) this.statshrqry = dpcheng.StatShrQryWdbePinList;
				if (dpcheng.StgIacQryWdbePinList) this.stgiacqry = dpcheng.StgIacQryWdbePinList;
				if (dpcheng.ContInfWdbePinList) {
				...
				*/
				...
				if (dpcheng.TagWdbePinList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoBnk", text: this.tag.TcoBnk}, {value: "TcoLoc", text: this.tag.TcoLoc});
					/*
					*/
				...
				if (dpcheng.ListWdbeQPinList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoBnk: rec["bnk"], TcoLoc: rec["loc"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbePinListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbePinListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
