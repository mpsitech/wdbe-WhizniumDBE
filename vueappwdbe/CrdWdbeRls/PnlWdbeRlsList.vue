...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeRlsList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeRlsListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeRlsList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeRlsList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeRlsListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeRlsListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeRlsList) this.contiac = dpcheng.ContIacWdbeRlsList;
				if (dpcheng.ContInfWdbeRlsList) this.continf = dpcheng.ContInfWdbeRlsList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeRlsList) this.statshr = dpcheng.StatShrWdbeRlsList;
				if (dpcheng.StgIacWdbeRlsList) this.stgiac = dpcheng.StgIacWdbeRlsList;
				if (dpcheng.TagWdbeRlsList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeRlsList);
					this.tag = dpcheng.TagWdbeRlsList;
				}
				if (dpcheng.ListWdbeQRlsList) this.rst = dpcheng.ListWdbeQRlsList;
				if (dpcheng.StatAppQryWdbeRlsList) this.statappqry = dpcheng.StatAppQryWdbeRlsList;
				if (dpcheng.StatShrQryWdbeRlsList) this.statshrqry = dpcheng.StatShrQryWdbeRlsList;
				if (dpcheng.StgIacQryWdbeRlsList) this.stgiacqry = dpcheng.StgIacQryWdbeRlsList;
				if (dpcheng.ContInfWdbeRlsList) {
				...
				*/
				...
				if (dpcheng.TagWdbeRlsList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTyp", text: this.tag.TcoTyp}, {value: "TcoVer", text: this.tag.TcoVer}, {value: "TcoMch", text: this.tag.TcoMch});
					/*
					*/
				...
				if (dpcheng.ListWdbeQRlsList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTyp: rec["typ2"], TcoVer: rec["ver"], TcoMch: rec["mch"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeRlsListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeRlsListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
