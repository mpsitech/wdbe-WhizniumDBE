...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeModCtrMNCommand.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeModCtrMNCommandData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeModCtrMNCommand": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeModCtrMNCommand"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeModCtrMNCommandData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeModCtrMNCommandData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeModCtrMNCommand) this.continf = dpcheng.ContInfWdbeModCtrMNCommand;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeModCtrMNCommand) this.statapp = dpcheng.StatAppWdbeModCtrMNCommand;
				if (dpcheng.StatShrWdbeModCtrMNCommand) this.statshr = dpcheng.StatShrWdbeModCtrMNCommand;
				if (dpcheng.StgIacWdbeModCtrMNCommand) this.stgiac = dpcheng.StgIacWdbeModCtrMNCommand;
				if (dpcheng.TagWdbeModCtrMNCommand) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeModCtrMNCommand);
					this.tag = dpcheng.TagWdbeModCtrMNCommand;
				}
				if (dpcheng.ListWdbeQModCtrMNCommand) this.rst = dpcheng.ListWdbeQModCtrMNCommand;
				if (dpcheng.StatAppQryWdbeModCtrMNCommand) this.statappqry = dpcheng.StatAppQryWdbeModCtrMNCommand;
				if (dpcheng.StatShrQryWdbeModCtrMNCommand) this.statshrqry = dpcheng.StatShrQryWdbeModCtrMNCommand;
				if (dpcheng.StgIacQryWdbeModCtrMNCommand) this.stgiacqry = dpcheng.StgIacQryWdbeModCtrMNCommand;
				/*
				...
				*/
				...
				if (dpcheng.TagWdbeModCtrMNCommand) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMref", text: this.tag.TcoMref}, {value: "TcoIvr", text: this.tag.TcoIvr}, {value: "TcoRvr", text: this.tag.TcoRvr});
					/*
					*/
				...
				if (dpcheng.ListWdbeQModCtrMNCommand) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMref: rec["mref"], TcoIvr: rec["ivr"], TcoRvr: rec["rvr"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeModCtrMNCommandData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeModCtrMNCommandData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
