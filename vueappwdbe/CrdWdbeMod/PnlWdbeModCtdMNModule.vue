...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeModCtdMNModule.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeModCtdMNModuleData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeModCtdMNModule": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeModCtdMNModule"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeModCtdMNModuleData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeModCtdMNModuleData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeModCtdMNModule) this.continf = dpcheng.ContInfWdbeModCtdMNModule;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeModCtdMNModule) this.statapp = dpcheng.StatAppWdbeModCtdMNModule;
				if (dpcheng.StatShrWdbeModCtdMNModule) this.statshr = dpcheng.StatShrWdbeModCtdMNModule;
				if (dpcheng.StgIacWdbeModCtdMNModule) this.stgiac = dpcheng.StgIacWdbeModCtdMNModule;
				if (dpcheng.TagWdbeModCtdMNModule) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeModCtdMNModule);
					this.tag = dpcheng.TagWdbeModCtdMNModule;
				}
				if (dpcheng.ListWdbeQModCtdMNModule) this.rst = dpcheng.ListWdbeQModCtdMNModule;
				if (dpcheng.StatAppQryWdbeModCtdMNModule) this.statappqry = dpcheng.StatAppQryWdbeModCtdMNModule;
				if (dpcheng.StatShrQryWdbeModCtdMNModule) this.statshrqry = dpcheng.StatShrQryWdbeModCtdMNModule;
				if (dpcheng.StgIacQryWdbeModCtdMNModule) this.stgiacqry = dpcheng.StgIacQryWdbeModCtdMNModule;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQModCtdMNModule) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeModCtdMNModuleData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeModCtdMNModuleData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
