...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeModCtrRef1NError.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeModCtrRef1NErrorData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeModCtrRef1NError": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeModCtrRef1NError"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeModCtrRef1NErrorData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeModCtrRef1NErrorData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeModCtrRef1NError) this.continf = dpcheng.ContInfWdbeModCtrRef1NError;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeModCtrRef1NError) this.statapp = dpcheng.StatAppWdbeModCtrRef1NError;
				if (dpcheng.StatShrWdbeModCtrRef1NError) this.statshr = dpcheng.StatShrWdbeModCtrRef1NError;
				if (dpcheng.StgIacWdbeModCtrRef1NError) this.stgiac = dpcheng.StgIacWdbeModCtrRef1NError;
				if (dpcheng.TagWdbeModCtrRef1NError) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeModCtrRef1NError);
					this.tag = dpcheng.TagWdbeModCtrRef1NError;
				}
				if (dpcheng.ListWdbeQModCtrRef1NError) this.rst = dpcheng.ListWdbeQModCtrRef1NError;
				if (dpcheng.StatAppQryWdbeModCtrRef1NError) this.statappqry = dpcheng.StatAppQryWdbeModCtrRef1NError;
				if (dpcheng.StatShrQryWdbeModCtrRef1NError) this.statshrqry = dpcheng.StatShrQryWdbeModCtrRef1NError;
				if (dpcheng.StgIacQryWdbeModCtrRef1NError) this.stgiacqry = dpcheng.StgIacQryWdbeModCtrRef1NError;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQModCtrRef1NError) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeModCtrRef1NErrorData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeModCtrRef1NErrorData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
