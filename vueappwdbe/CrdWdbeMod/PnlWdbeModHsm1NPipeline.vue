...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeModHsm1NPipeline.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeModHsm1NPipelineData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeModHsm1NPipeline": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeModHsm1NPipeline"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeModHsm1NPipelineData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeModHsm1NPipelineData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeModHsm1NPipeline) this.continf = dpcheng.ContInfWdbeModHsm1NPipeline;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeModHsm1NPipeline) this.statapp = dpcheng.StatAppWdbeModHsm1NPipeline;
				if (dpcheng.StatShrWdbeModHsm1NPipeline) this.statshr = dpcheng.StatShrWdbeModHsm1NPipeline;
				if (dpcheng.StgIacWdbeModHsm1NPipeline) this.stgiac = dpcheng.StgIacWdbeModHsm1NPipeline;
				if (dpcheng.TagWdbeModHsm1NPipeline) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeModHsm1NPipeline);
					this.tag = dpcheng.TagWdbeModHsm1NPipeline;
				}
				if (dpcheng.ListWdbeQModHsm1NPipeline) this.rst = dpcheng.ListWdbeQModHsm1NPipeline;
				if (dpcheng.StatAppQryWdbeModHsm1NPipeline) this.statappqry = dpcheng.StatAppQryWdbeModHsm1NPipeline;
				if (dpcheng.StatShrQryWdbeModHsm1NPipeline) this.statshrqry = dpcheng.StatShrQryWdbeModHsm1NPipeline;
				if (dpcheng.StgIacQryWdbeModHsm1NPipeline) this.stgiacqry = dpcheng.StgIacQryWdbeModHsm1NPipeline;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQModHsm1NPipeline) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeModHsm1NPipelineData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeModHsm1NPipelineData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
