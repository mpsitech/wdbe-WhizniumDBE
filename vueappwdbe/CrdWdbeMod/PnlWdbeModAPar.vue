...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeModAPar.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeModAParData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeModAPar": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeModAPar"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeModAParData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeModAParData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeModAPar) this.continf = dpcheng.ContInfWdbeModAPar;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeModAPar) this.statapp = dpcheng.StatAppWdbeModAPar;
				if (dpcheng.StatShrWdbeModAPar) this.statshr = dpcheng.StatShrWdbeModAPar;
				if (dpcheng.StgIacWdbeModAPar) this.stgiac = dpcheng.StgIacWdbeModAPar;
				if (dpcheng.TagWdbeModAPar) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeModAPar);
					this.tag = dpcheng.TagWdbeModAPar;
				}
				if (dpcheng.ListWdbeQModAPar) this.rst = dpcheng.ListWdbeQModAPar;
				if (dpcheng.StatAppQryWdbeModAPar) this.statappqry = dpcheng.StatAppQryWdbeModAPar;
				if (dpcheng.StatShrQryWdbeModAPar) this.statshrqry = dpcheng.StatShrQryWdbeModAPar;
				if (dpcheng.StgIacQryWdbeModAPar) this.stgiacqry = dpcheng.StgIacQryWdbeModAPar;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeModAParData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeModAParData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
