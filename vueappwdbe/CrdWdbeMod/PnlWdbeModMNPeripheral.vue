...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeModMNPeripheral.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeModMNPeripheralData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeModMNPeripheral": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeModMNPeripheral"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeModMNPeripheralData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeModMNPeripheralData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeModMNPeripheral) this.continf = dpcheng.ContInfWdbeModMNPeripheral;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeModMNPeripheral) this.statapp = dpcheng.StatAppWdbeModMNPeripheral;
				if (dpcheng.StatShrWdbeModMNPeripheral) this.statshr = dpcheng.StatShrWdbeModMNPeripheral;
				if (dpcheng.StgIacWdbeModMNPeripheral) this.stgiac = dpcheng.StgIacWdbeModMNPeripheral;
				if (dpcheng.TagWdbeModMNPeripheral) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeModMNPeripheral);
					this.tag = dpcheng.TagWdbeModMNPeripheral;
				}
				if (dpcheng.ListWdbeQModMNPeripheral) this.rst = dpcheng.ListWdbeQModMNPeripheral;
				if (dpcheng.StatAppQryWdbeModMNPeripheral) this.statappqry = dpcheng.StatAppQryWdbeModMNPeripheral;
				if (dpcheng.StatShrQryWdbeModMNPeripheral) this.statshrqry = dpcheng.StatShrQryWdbeModMNPeripheral;
				if (dpcheng.StgIacQryWdbeModMNPeripheral) this.stgiacqry = dpcheng.StgIacQryWdbeModMNPeripheral;
				/*
				...
				*/
				...
				if (dpcheng.TagWdbeModMNPeripheral) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMref", text: this.tag.TcoMref});
					/*
					*/
				...
				if (dpcheng.ListWdbeQModMNPeripheral) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMref: rec["mref"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeModMNPeripheralData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeModMNPeripheralData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
