...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeFstList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeFstListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeFstList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeFstList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeFstListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeFstListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeFstList) this.contiac = dpcheng.ContIacWdbeFstList;
				if (dpcheng.ContInfWdbeFstList) this.continf = dpcheng.ContInfWdbeFstList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeFstList) this.statshr = dpcheng.StatShrWdbeFstList;
				if (dpcheng.StgIacWdbeFstList) this.stgiac = dpcheng.StgIacWdbeFstList;
				if (dpcheng.TagWdbeFstList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeFstList);
					this.tag = dpcheng.TagWdbeFstList;
				}
				if (dpcheng.ListWdbeQFstList) this.rst = dpcheng.ListWdbeQFstList;
				if (dpcheng.StatAppQryWdbeFstList) this.statappqry = dpcheng.StatAppQryWdbeFstList;
				if (dpcheng.StatShrQryWdbeFstList) this.statshrqry = dpcheng.StatShrQryWdbeFstList;
				if (dpcheng.StgIacQryWdbeFstList) this.stgiacqry = dpcheng.StgIacQryWdbeFstList;
				if (dpcheng.ContInfWdbeFstList) {
				...
				*/
				...
				if (dpcheng.TagWdbeFstList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoFsm", text: this.tag.TcoFsm});
					/*
					*/
				...
				if (dpcheng.ListWdbeQFstList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoFsm: rec["fsm"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeFstListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeFstListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
