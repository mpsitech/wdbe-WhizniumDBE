...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeErrList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeErrListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeErrList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeErrList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeErrListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeErrListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeErrList) this.contiac = dpcheng.ContIacWdbeErrList;
				if (dpcheng.ContInfWdbeErrList) this.continf = dpcheng.ContInfWdbeErrList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeErrList) this.statshr = dpcheng.StatShrWdbeErrList;
				if (dpcheng.StgIacWdbeErrList) this.stgiac = dpcheng.StgIacWdbeErrList;
				if (dpcheng.TagWdbeErrList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeErrList);
					this.tag = dpcheng.TagWdbeErrList;
				}
				if (dpcheng.ListWdbeQErrList) this.rst = dpcheng.ListWdbeQErrList;
				if (dpcheng.StatAppQryWdbeErrList) this.statappqry = dpcheng.StatAppQryWdbeErrList;
				if (dpcheng.StatShrQryWdbeErrList) this.statshrqry = dpcheng.StatShrQryWdbeErrList;
				if (dpcheng.StgIacQryWdbeErrList) this.stgiacqry = dpcheng.StgIacQryWdbeErrList;
				if (dpcheng.ContInfWdbeErrList) {
				...
				*/
				...
				if (dpcheng.TagWdbeErrList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoFsr", text: this.tag.TcoFsr}, {value: "TcoRet", text: this.tag.TcoRet}, {value: "TcoReu", text: this.tag.TcoReu});
					/*
					*/
				...
				if (dpcheng.ListWdbeQErrList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoFsr: rec["fsr"], TcoRet: rec["ret2"], TcoReu: rec["reu"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeErrListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeErrListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
