...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeErrAPar.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeErrAParData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeErrAPar": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeErrAPar"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeErrAParData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeErrAParData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeErrAPar) this.continf = dpcheng.ContInfWdbeErrAPar;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeErrAPar) this.statapp = dpcheng.StatAppWdbeErrAPar;
				if (dpcheng.StatShrWdbeErrAPar) this.statshr = dpcheng.StatShrWdbeErrAPar;
				if (dpcheng.StgIacWdbeErrAPar) this.stgiac = dpcheng.StgIacWdbeErrAPar;
				if (dpcheng.TagWdbeErrAPar) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeErrAPar);
					this.tag = dpcheng.TagWdbeErrAPar;
				}
				if (dpcheng.ListWdbeQErrAPar) this.rst = dpcheng.ListWdbeQErrAPar;
				if (dpcheng.StatAppQryWdbeErrAPar) this.statappqry = dpcheng.StatAppQryWdbeErrAPar;
				if (dpcheng.StatShrQryWdbeErrAPar) this.statshrqry = dpcheng.StatShrQryWdbeErrAPar;
				if (dpcheng.StgIacQryWdbeErrAPar) this.stgiacqry = dpcheng.StgIacQryWdbeErrAPar;
				/*
				...
				*/
				...
				if (dpcheng.TagWdbeErrAPar) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoPty", text: this.tag.TcoPty}, {value: "TcoVec", text: this.tag.TcoVec}, {value: "TcoLen", text: this.tag.TcoLen}, {value: "TcoCmt", text: this.tag.TcoCmt});
					/*
					*/
				...
				if (dpcheng.ListWdbeQErrAPar) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoPty: rec["pty"], TcoVec: rec["vec"], TcoLen: rec["len"], TcoCmt: rec["cmt"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeErrAParData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeErrAParData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
