...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeTrgList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeTrgListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeTrgList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeTrgList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeTrgListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeTrgListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeTrgList) this.contiac = dpcheng.ContIacWdbeTrgList;
				if (dpcheng.ContInfWdbeTrgList) this.continf = dpcheng.ContInfWdbeTrgList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeTrgList) this.statshr = dpcheng.StatShrWdbeTrgList;
				if (dpcheng.StgIacWdbeTrgList) this.stgiac = dpcheng.StgIacWdbeTrgList;
				if (dpcheng.TagWdbeTrgList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeTrgList);
					this.tag = dpcheng.TagWdbeTrgList;
				}
				if (dpcheng.ListWdbeQTrgList) this.rst = dpcheng.ListWdbeQTrgList;
				if (dpcheng.StatAppQryWdbeTrgList) this.statappqry = dpcheng.StatAppQryWdbeTrgList;
				if (dpcheng.StatShrQryWdbeTrgList) this.statshrqry = dpcheng.StatShrQryWdbeTrgList;
				if (dpcheng.StgIacQryWdbeTrgList) this.stgiacqry = dpcheng.StgIacQryWdbeTrgList;
				if (dpcheng.ContInfWdbeTrgList) {
				...
				*/
				...
				if (dpcheng.TagWdbeTrgList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoSys", text: this.tag.TcoSys}, {value: "TcoUnt", text: this.tag.TcoUnt});
					/*
					*/
				...
				if (dpcheng.ListWdbeQTrgList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoSys: rec["sys"], TcoUnt: rec["unt"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeTrgListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeTrgListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
