...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeUsrList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeUsrListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeUsrList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeUsrList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeUsrListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeUsrListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbeUsrList) this.contiac = dpcheng.ContIacWdbeUsrList;
				if (dpcheng.ContInfWdbeUsrList) this.continf = dpcheng.ContInfWdbeUsrList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbeUsrList) this.statshr = dpcheng.StatShrWdbeUsrList;
				if (dpcheng.StgIacWdbeUsrList) this.stgiac = dpcheng.StgIacWdbeUsrList;
				if (dpcheng.TagWdbeUsrList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeUsrList);
					this.tag = dpcheng.TagWdbeUsrList;
				}
				if (dpcheng.ListWdbeQUsrList) this.rst = dpcheng.ListWdbeQUsrList;
				if (dpcheng.StatAppQryWdbeUsrList) this.statappqry = dpcheng.StatAppQryWdbeUsrList;
				if (dpcheng.StatShrQryWdbeUsrList) this.statshrqry = dpcheng.StatShrQryWdbeUsrList;
				if (dpcheng.StgIacQryWdbeUsrList) this.stgiacqry = dpcheng.StgIacQryWdbeUsrList;
				if (dpcheng.ContInfWdbeUsrList) {
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeUsrListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeUsrListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
