...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeUsrMNUsergroup.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeUsrMNUsergroupData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeUsrMNUsergroup": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeUsrMNUsergroup"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeUsrMNUsergroupData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeUsrMNUsergroupData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeUsrMNUsergroup) this.continf = dpcheng.ContInfWdbeUsrMNUsergroup;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeUsrMNUsergroup) this.statapp = dpcheng.StatAppWdbeUsrMNUsergroup;
				if (dpcheng.StatShrWdbeUsrMNUsergroup) this.statshr = dpcheng.StatShrWdbeUsrMNUsergroup;
				if (dpcheng.StgIacWdbeUsrMNUsergroup) this.stgiac = dpcheng.StgIacWdbeUsrMNUsergroup;
				if (dpcheng.TagWdbeUsrMNUsergroup) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeUsrMNUsergroup);
					this.tag = dpcheng.TagWdbeUsrMNUsergroup;
				}
				if (dpcheng.ListWdbeQUsrMNUsergroup) this.rst = dpcheng.ListWdbeQUsrMNUsergroup;
				if (dpcheng.StatAppQryWdbeUsrMNUsergroup) this.statappqry = dpcheng.StatAppQryWdbeUsrMNUsergroup;
				if (dpcheng.StatShrQryWdbeUsrMNUsergroup) this.statshrqry = dpcheng.StatShrQryWdbeUsrMNUsergroup;
				if (dpcheng.StgIacQryWdbeUsrMNUsergroup) this.stgiacqry = dpcheng.StgIacQryWdbeUsrMNUsergroup;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQUsrMNUsergroup) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeUsrMNUsergroupData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeUsrMNUsergroupData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
