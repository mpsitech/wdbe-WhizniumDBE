...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeUsrAAccess.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeUsrAAccessData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeUsrAAccess": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeUsrAAccess"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeUsrAAccessData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeUsrAAccessData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeUsrAAccess) this.continf = dpcheng.ContInfWdbeUsrAAccess;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeUsrAAccess) this.statapp = dpcheng.StatAppWdbeUsrAAccess;
				if (dpcheng.StatShrWdbeUsrAAccess) this.statshr = dpcheng.StatShrWdbeUsrAAccess;
				if (dpcheng.StgIacWdbeUsrAAccess) this.stgiac = dpcheng.StgIacWdbeUsrAAccess;
				if (dpcheng.TagWdbeUsrAAccess) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeUsrAAccess);
					this.tag = dpcheng.TagWdbeUsrAAccess;
				}
				if (dpcheng.ListWdbeQUsrAAccess) this.rst = dpcheng.ListWdbeQUsrAAccess;
				if (dpcheng.StatAppQryWdbeUsrAAccess) this.statappqry = dpcheng.StatAppQryWdbeUsrAAccess;
				if (dpcheng.StatShrQryWdbeUsrAAccess) this.statshrqry = dpcheng.StatShrQryWdbeUsrAAccess;
				if (dpcheng.StgIacQryWdbeUsrAAccess) this.stgiacqry = dpcheng.StgIacQryWdbeUsrAAccess;
				/*
				...
				*/
				...
				if (dpcheng.ListWdbeQUsrAAccess) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeUsrAAccessData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeUsrAAccessData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
