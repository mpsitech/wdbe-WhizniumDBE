...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbeMchAPar.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbeMchAParData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbeMchAPar": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbeMchAPar"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbeMchAParData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbeMchAParData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbeMchAPar) this.continf = dpcheng.ContInfWdbeMchAPar;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbeMchAPar) this.statapp = dpcheng.StatAppWdbeMchAPar;
				if (dpcheng.StatShrWdbeMchAPar) this.statshr = dpcheng.StatShrWdbeMchAPar;
				if (dpcheng.StgIacWdbeMchAPar) this.stgiac = dpcheng.StgIacWdbeMchAPar;
				if (dpcheng.TagWdbeMchAPar) {
					Wdbe.unescapeBlock(dpcheng.TagWdbeMchAPar);
					this.tag = dpcheng.TagWdbeMchAPar;
				}
				if (dpcheng.ListWdbeQMchAPar) this.rst = dpcheng.ListWdbeQMchAPar;
				if (dpcheng.StatAppQryWdbeMchAPar) this.statappqry = dpcheng.StatAppQryWdbeMchAPar;
				if (dpcheng.StatShrQryWdbeMchAPar) this.statshrqry = dpcheng.StatShrQryWdbeMchAPar;
				if (dpcheng.StgIacQryWdbeMchAPar) this.stgiacqry = dpcheng.StgIacQryWdbeMchAPar;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbeMchAParData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbeMchAParData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
