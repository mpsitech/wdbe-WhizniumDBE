...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbePphAPar.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbePphAParData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbePphAPar": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbePphAPar"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbePphAParData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbePphAParData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWdbePphAPar) this.continf = dpcheng.ContInfWdbePphAPar;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWdbePphAPar) this.statapp = dpcheng.StatAppWdbePphAPar;
				if (dpcheng.StatShrWdbePphAPar) this.statshr = dpcheng.StatShrWdbePphAPar;
				if (dpcheng.StgIacWdbePphAPar) this.stgiac = dpcheng.StgIacWdbePphAPar;
				if (dpcheng.TagWdbePphAPar) {
					Wdbe.unescapeBlock(dpcheng.TagWdbePphAPar);
					this.tag = dpcheng.TagWdbePphAPar;
				}
				if (dpcheng.ListWdbeQPphAPar) this.rst = dpcheng.ListWdbeQPphAPar;
				if (dpcheng.StatAppQryWdbePphAPar) this.statappqry = dpcheng.StatAppQryWdbePphAPar;
				if (dpcheng.StatShrQryWdbePphAPar) this.statshrqry = dpcheng.StatShrQryWdbePphAPar;
				if (dpcheng.StgIacQryWdbePphAPar) this.stgiacqry = dpcheng.StgIacQryWdbePphAPar;
				/*
				...
				*/
				...
				if (dpcheng.TagWdbePphAPar) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoKey", text: this.tag.TcoKey}, {value: "TcoVal", text: this.tag.TcoVal});
					/*
					*/
				...
				if (dpcheng.ListWdbeQPphAPar) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoKey: rec["key"], TcoVal: rec["val"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbePphAParData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbePphAParData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
