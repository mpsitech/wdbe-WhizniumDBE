...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWdbePphList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWdbePphListData": {
						"scrJref": this.scrJref,
						"StgIacQryWdbePphList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWdbePphList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWdbePphListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWdbePphListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWdbePphList) this.contiac = dpcheng.ContIacWdbePphList;
				if (dpcheng.ContInfWdbePphList) this.continf = dpcheng.ContInfWdbePphList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWdbePphList) this.statshr = dpcheng.StatShrWdbePphList;
				if (dpcheng.StgIacWdbePphList) this.stgiac = dpcheng.StgIacWdbePphList;
				if (dpcheng.TagWdbePphList) {
					Wdbe.unescapeBlock(dpcheng.TagWdbePphList);
					this.tag = dpcheng.TagWdbePphList;
				}
				if (dpcheng.ListWdbeQPphList) this.rst = dpcheng.ListWdbeQPphList;
				if (dpcheng.StatAppQryWdbePphList) this.statappqry = dpcheng.StatAppQryWdbePphList;
				if (dpcheng.StatShrQryWdbePphList) this.statshrqry = dpcheng.StatShrQryWdbePphList;
				if (dpcheng.StgIacQryWdbePphList) this.stgiacqry = dpcheng.StgIacQryWdbePphList;
				if (dpcheng.ContInfWdbePphList) {
			...
			handleDpchAppDataDoReply: function(srefIxWdbeVDpch, dpcheng) {
				if (srefIxWdbeVDpch == "DpchEngWdbePphListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWdbeVDpch == "DpchEngWdbePphListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
