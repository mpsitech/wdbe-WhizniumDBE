/**
  * \file CrdWdbeFam.js
  * web client functionality for card CrdWdbeFam
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWdbeFam", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWdbeFam", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWdbeFam", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWdbeFam", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWdbeFam", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWdbeFamHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWdbeFamList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWdbeFamRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWdbeFam", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWdbeFam", "shortMenu", short);
	setSi(srcdoc, "StatAppWdbeFam", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWdbeFam", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWdbeFam", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWdbeFam", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWdbeFam", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdFam"), retrieveCi(srcdoc, "ContInfWdbeFam", "MtxCrdFam"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWdbeFam", "MtxCrdFam") + " - WhizniumDBE v1.0.3";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeFamDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeFamDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWdbeFam", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeFamData", "ContInfWdbeFam", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeFamData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWdbeFamData", "StatAppWdbeFam", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeFamData", "StatShrWdbeFam", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeFamData", "TagWdbeFam", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeFamData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWdbeFam", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWdbeFam", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWdbeFam", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWdbeFamData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeFam", "srefIxWdbeVReqitmode");

	if (srefIxWdbeVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWdbeVReqitmode = "notify";
		setSi(srcdoc, "StatAppWdbeFam", "srefIxWdbeVReqitmode", srefIxWdbeVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeFam", "srefIxWdbeVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWdbeFam", "latency"));
