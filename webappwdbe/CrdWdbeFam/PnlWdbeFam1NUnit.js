/**
  * \file PnlWdbeFam1NUnit.js
  * web client functionality for panel PnlWdbeFam1NUnit
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeFam1NUnit", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeFam1NUnit", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NUnit_side").src = "./PnlWdbeFam1NUnit_aside.html";
	doc.getElementById("1NUnit_cont").src = "./PnlWdbeFam1NUnit_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeFam1NUnit", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeFam1NUnit", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NUnit_side").src = "./PnlWdbeFam1NUnit_bside.html";
	doc.getElementById("1NUnit_cont").src = "./PnlWdbeFam1NUnit_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeFam1NUnit", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeFam1NUnit", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeFam1NUnit", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeFam1NUnit", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeFam1NUnit", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeFam1NUnit", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFam1NUnit", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFam1NUnit", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeFam1NUnit", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeFam1NUnit", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeFam1NUnit", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFam1NUnit", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFam1NUnit", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeFam1NUnit", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeFam1NUnit", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeFam1NUnit", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQFam1NUnit
		rownode = getRownode(srcdoc, "ListWdbeQFam1NUnit", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeFam1NUnit", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeFam1NUnit", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeFam1NUnit", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeFam1NUnit", "ListWdbeQFam1NUnit", retrieveTi(srcdoc, "TagWdbeFam1NUnit", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeFam1NUnit", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeFam1NUnit", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFam1NUnit", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeFam1NUnit", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeFam1NUnit", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeFam1NUnit", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeFam1NUnitData", scrJref, "StgIacQryWdbeFam1NUnit");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeFam1NUnit", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFam1NUnit", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeFam1NUnit", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeFam1NUnit", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeFam1NUnit", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeFam1NUnitDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "ContInfWdbeFam1NUnit", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "StatAppWdbeFam1NUnit", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "StatShrWdbeFam1NUnit", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "StgIacWdbeFam1NUnit", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "TagWdbeFam1NUnit", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "ListWdbeQFam1NUnit", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "StatAppQryWdbeFam1NUnit", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "StatShrQryWdbeFam1NUnit", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeFam1NUnitData", "StgIacQryWdbeFam1NUnit", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeFam1NUnitData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeFam1NUnitData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeFam1NUnitData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
