/**
  * \file CrdWdbeUsr.js
  * web client functionality for card CrdWdbeUsr
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWdbeUsr", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWdbeUsr", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWdbeUsr", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWdbeUsr", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWdbeUsr", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWdbeUsrHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWdbeUsrList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWdbeUsrRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWdbeUsr", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWdbeUsr", "shortMenu", short);
	setSi(srcdoc, "StatAppWdbeUsr", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWdbeUsr", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWdbeUsr", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWdbeUsr", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWdbeUsr", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MitCrdMkpAvail = (retrieveSi(srcdoc, "StatShrWdbeUsr", "MitCrdMkpAvail") == "true");
	MitCrdMkpActive = (retrieveSi(srcdoc, "StatShrWdbeUsr", "MitCrdMkpActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWdbeUsr", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdUsr"), retrieveCi(srcdoc, "ContInfWdbeUsr", "MtxCrdUsr"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdMkp"), retrieveTi(srcdoc, "TagWdbeUsr", "MitCrdMkp"));
	setMitActive("MitCrdMkp", MitCrdMkpActive);

	height -= setMitMspAvail("MitCrdMkp", MitCrdMkpAvail, 20);
...
function refresh() {
	var scrJrefDlgmngkeypair = retrieveSi(srcdoc, "StatShrWdbeUsr", "scrJrefDlgmngkeypair");

	if (scrJrefDlgmngkeypair != "") {
		if (scrJrefDlg != scrJrefDlgmngkeypair) showDlg("DlgWdbeUsrMngkeypair", scrJrefDlgmngkeypair);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfWdbeUsr", "MtxCrdUsr") + " - WhizniumDBE v1.0.3";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUsrDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUsrDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWdbeUsr", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUsrData", "ContInfWdbeUsr", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUsrData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWdbeUsrData", "StatAppWdbeUsr", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUsrData", "StatShrWdbeUsr", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUsrData", "TagWdbeUsr", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUsrData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWdbeUsr", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWdbeUsr", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWdbeUsr", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWdbeUsrData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeUsr", "srefIxWdbeVReqitmode");

	if (srefIxWdbeVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWdbeVReqitmode = "notify";
		setSi(srcdoc, "StatAppWdbeUsr", "srefIxWdbeVReqitmode", srefIxWdbeVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeUsr", "srefIxWdbeVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWdbeUsr", "latency"));
