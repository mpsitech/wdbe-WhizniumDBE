/**
  * \file PnlWdbeUsr1NSession.js
  * web client functionality for panel PnlWdbeUsr1NSession
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUsr1NSession", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUsr1NSession", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NSession", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NSession_side").setAttribute("height", "30");
	doc.getElementById("1NSession_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NSession_side").src = "./PnlWdbeUsr1NSession_aside.html";
	doc.getElementById("1NSession_cont").src = "./PnlWdbeUsr1NSession_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUsr1NSession", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUsr1NSession", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NSession", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NSession_side").setAttribute("height", "255");
	doc.getElementById("1NSession_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NSession_side").src = "./PnlWdbeUsr1NSession_bside.html";
	doc.getElementById("1NSession_cont").src = "./PnlWdbeUsr1NSession_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NSession_side").contentDocument;
	contdoc = doc.getElementById("1NSession_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NSession_side").contentDocument;
	contdoc = doc.getElementById("1NSession_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NSession_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NSession_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NSession_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUsr1NSession", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUsr1NSession", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUsr1NSession", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUsr1NSession", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUsr1NSession", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsr1NSession", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsr1NSession", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsr1NSession", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsr1NSession", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsr1NSession", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsr1NSession", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsr1NSession", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsr1NSession", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsr1NSession", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUsr1NSession", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUsr1NSession", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUsr1NSession
		rownode = getRownode(srcdoc, "ListWdbeQUsr1NSession", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUsr1NSession", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUsr1NSession", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUsr1NSession", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NSession", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSession_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSession_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NSession_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NSession_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUsr1NSession", "ListWdbeQUsr1NSession", retrieveTi(srcdoc, "TagWdbeUsr1NSession", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUsr1NSession", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsr1NSession", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsr1NSession", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsr1NSession", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUsr1NSession", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUsr1NSession", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUsr1NSessionData", scrJref, "StgIacQryWdbeUsr1NSession");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUsr1NSession", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsr1NSession", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUsr1NSession", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUsr1NSession", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsr1NSession", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUsr1NSessionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "ContInfWdbeUsr1NSession", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "StatAppWdbeUsr1NSession", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "StatShrWdbeUsr1NSession", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "StgIacWdbeUsr1NSession", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "TagWdbeUsr1NSession", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "ListWdbeQUsr1NSession", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "StatAppQryWdbeUsr1NSession", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "StatShrQryWdbeUsr1NSession", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUsr1NSessionData", "StgIacQryWdbeUsr1NSession", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUsr1NSessionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUsr1NSessionData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NSession");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUsr1NSessionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
