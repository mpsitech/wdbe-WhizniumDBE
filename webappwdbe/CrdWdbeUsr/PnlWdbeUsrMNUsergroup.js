/**
  * \file PnlWdbeUsrMNUsergroup.js
  * web client functionality for panel PnlWdbeUsrMNUsergroup
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUsrMNUsergroup", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUsrMNUsergroup", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNUsergroup_side").src = "./PnlWdbeUsrMNUsergroup_aside.html";
	doc.getElementById("MNUsergroup_cont").src = "./PnlWdbeUsrMNUsergroup_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUsrMNUsergroup", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUsrMNUsergroup", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNUsergroup_side").src = "./PnlWdbeUsrMNUsergroup_bside.html";
	doc.getElementById("MNUsergroup_cont").src = "./PnlWdbeUsrMNUsergroup_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUsrMNUsergroup", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUsrMNUsergroup", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUsrMNUsergroup", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUsrMNUsergroup", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUsrMNUsergroup", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsrMNUsergroup", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrMNUsergroup", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrMNUsergroup", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsrMNUsergroup", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsrMNUsergroup", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsrMNUsergroup", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrMNUsergroup", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrMNUsergroup", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrMNUsergroup", "TcoMrefWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrMNUsergroup", "TcoUlvWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUsrMNUsergroup", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUsrMNUsergroup", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUsrMNUsergroup
		rownode = getRownode(srcdoc, "ListWdbeQUsrMNUsergroup", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUsrMNUsergroup", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUsrMNUsergroup", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUsrMNUsergroup", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUsrMNUsergroup", "ListWdbeQUsrMNUsergroup", retrieveTi(srcdoc, "TagWdbeUsrMNUsergroup", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUsrMNUsergroup", "TxtRecord2"),
				["Mref","Ulv",""],
				["mref","ulv","ulv2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsrMNUsergroup", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrMNUsergroup", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsrMNUsergroup", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUsrMNUsergroup", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUsrMNUsergroup", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUsrMNUsergroupData", scrJref, "StgIacQryWdbeUsrMNUsergroup");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUsrMNUsergroup", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrMNUsergroup", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUsrMNUsergroup", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUsrMNUsergroup", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsrMNUsergroup", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUsrMNUsergroupDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "ContInfWdbeUsrMNUsergroup", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "StatAppWdbeUsrMNUsergroup", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "StatShrWdbeUsrMNUsergroup", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "StgIacWdbeUsrMNUsergroup", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "TagWdbeUsrMNUsergroup", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "ListWdbeQUsrMNUsergroup", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "StatAppQryWdbeUsrMNUsergroup", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "StatShrQryWdbeUsrMNUsergroup", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUsrMNUsergroupData", "StgIacQryWdbeUsrMNUsergroup", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUsrMNUsergroupData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUsrMNUsergroupData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUsrMNUsergroupData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
