/**
  * \file PnlWdbeUsrList.js
  * web client functionality for panel PnlWdbeUsrList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeUsrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeUsrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeUsrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeUsrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUsrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUsrList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeUsrList", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUsrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUsrList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsrList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrList", "TcoOwnWidth"));
		var TcoPrsWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrList", "TcoPrsWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrList", "TcoSrfWidth"));
		var TcoUsgWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrList", "TcoUsgWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrList", "TcoSteWidth"));
		var TcoLclWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrList", "TcoLclWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsrList", "TcoUlvWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeUsrList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoPrs"); doc.widths.push(TcoPrsWidth); doc.fs.push("prs"); doc.tos.push(4); wtot += TcoPrsWidth;
			} else if (i == 4) {
				doc.cols.push("TcoSrf"); doc.widths.push(TcoSrfWidth); doc.fs.push("srf"); doc.tos.push(3); wtot += TcoSrfWidth;
			} else if (i == 5) {
				doc.cols.push("TcoUsg"); doc.widths.push(TcoUsgWidth); doc.fs.push("usg"); doc.tos.push(1); wtot += TcoUsgWidth;
			} else if (i == 6) {
				doc.cols.push("TcoSte"); doc.widths.push(TcoSteWidth); doc.fs.push("ste2"); doc.tos.push(0); wtot += TcoSteWidth;
			} else if (i == 7) {
				doc.cols.push("TcoLcl"); doc.widths.push(TcoLclWidth); doc.fs.push("lcl2"); doc.tos.push(0); wtot += TcoLclWidth;
			} else if (i == 8) {
				doc.cols.push("TcoUlv"); doc.widths.push(TcoUlvWidth); doc.fs.push("ulv2"); doc.tos.push(0); wtot += TcoUlvWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeUsrList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUsrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUsrList
		rownode = getRownode(srcdoc, "ListWdbeQUsrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUsrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUsrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUsrList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUsrList", "ListWdbeQUsrList", retrieveTi(srcdoc, "TagWdbeUsrList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUsrList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUsrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUsrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUsrListData", scrJref, "StgIacQryWdbeUsrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsrList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUsrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUsrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeUsrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeUsrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUsrListData", scrJref, "ContIacWdbeUsrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "ContIacWdbeUsrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "ContInfWdbeUsrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "StatShrWdbeUsrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "StgIacWdbeUsrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "TagWdbeUsrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "ListWdbeQUsrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "StatAppQryWdbeUsrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "StatShrQryWdbeUsrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUsrListData", "StgIacQryWdbeUsrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeUsrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUsrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUsrListData") {
				mask = mergeDpchEngData(dom);
