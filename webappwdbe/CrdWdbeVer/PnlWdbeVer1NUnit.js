/**
  * \file PnlWdbeVer1NUnit.js
  * web client functionality for panel PnlWdbeVer1NUnit
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeVer1NUnit", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeVer1NUnit", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NUnit", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NUnit_side").setAttribute("height", "30");
	doc.getElementById("1NUnit_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NUnit_side").src = "./PnlWdbeVer1NUnit_aside.html";
	doc.getElementById("1NUnit_cont").src = "./PnlWdbeVer1NUnit_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeVer1NUnit", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeVer1NUnit", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NUnit", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NUnit_side").setAttribute("height", "255");
	doc.getElementById("1NUnit_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NUnit_side").src = "./PnlWdbeVer1NUnit_bside.html";
	doc.getElementById("1NUnit_cont").src = "./PnlWdbeVer1NUnit_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NUnit_side").contentDocument;
	contdoc = doc.getElementById("1NUnit_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NUnit_side").contentDocument;
	contdoc = doc.getElementById("1NUnit_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NUnit_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NUnit_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NUnit_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVer1NUnit", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeVer1NUnit", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeVer1NUnit", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVer1NUnit", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeVer1NUnit", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVer1NUnit", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NUnit", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NUnit", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NUnit", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NUnit", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NUnit", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NUnit", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NUnit", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVer1NUnit", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeVer1NUnit", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeVer1NUnit", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQVer1NUnit
		rownode = getRownode(srcdoc, "ListWdbeQVer1NUnit", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeVer1NUnit", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeVer1NUnit", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeVer1NUnit", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NUnit", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NUnit_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NUnit_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NUnit_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NUnit_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeVer1NUnit", "ListWdbeQVer1NUnit", retrieveTi(srcdoc, "TagWdbeVer1NUnit", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeVer1NUnit", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVer1NUnit", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NUnit", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NUnit", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeVer1NUnit", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeVer1NUnit", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeVer1NUnitData", scrJref, "StgIacQryWdbeVer1NUnit");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeVer1NUnit", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NUnit", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeVer1NUnit", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeVer1NUnit", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVer1NUnit", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeVer1NUnitDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "ContInfWdbeVer1NUnit", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "StatAppWdbeVer1NUnit", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "StatShrWdbeVer1NUnit", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "StgIacWdbeVer1NUnit", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "TagWdbeVer1NUnit", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "ListWdbeQVer1NUnit", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "StatAppQryWdbeVer1NUnit", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "StatShrQryWdbeVer1NUnit", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NUnitData", "StgIacQryWdbeVer1NUnit", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeVer1NUnitData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeVer1NUnitData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NUnit");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeVer1NUnitData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
