/**
  * \file PnlWdbeVerRef1NFile.js
  * web client functionality for panel PnlWdbeVerRef1NFile
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeVerRef1NFile", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeVerRef1NFile", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlWdbeVerRef1NFile_aside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlWdbeVerRef1NFile_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeVerRef1NFile", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeVerRef1NFile", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlWdbeVerRef1NFile_bside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlWdbeVerRef1NFile_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVerRef1NFile", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeVerRef1NFile", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeVerRef1NFile", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVerRef1NFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeVerRef1NFile", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVerRef1NFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerRef1NFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerRef1NFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerRef1NFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerRef1NFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerRef1NFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerRef1NFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerRef1NFile", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerRef1NFile", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeVerRef1NFile", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeVerRef1NFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQVerRef1NFile
		rownode = getRownode(srcdoc, "ListWdbeQVerRef1NFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeVerRef1NFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeVerRef1NFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeVerRef1NFile", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeVerRef1NFile", "ListWdbeQVerRef1NFile", retrieveTi(srcdoc, "TagWdbeVerRef1NFile", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeVerRef1NFile", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVerRef1NFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerRef1NFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerRef1NFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeVerRef1NFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeVerRef1NFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeVerRef1NFileData", scrJref, "StgIacQryWdbeVerRef1NFile");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeVerRef1NFile", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerRef1NFile", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeVerRef1NFile", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeVerRef1NFile", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVerRef1NFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeVerRef1NFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "ContInfWdbeVerRef1NFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "StatAppWdbeVerRef1NFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "StatShrWdbeVerRef1NFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "StgIacWdbeVerRef1NFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "TagWdbeVerRef1NFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "ListWdbeQVerRef1NFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "StatAppQryWdbeVerRef1NFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "StatShrQryWdbeVerRef1NFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeVerRef1NFileData", "StgIacQryWdbeVerRef1NFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeVerRef1NFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeVerRef1NFileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeVerRef1NFileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
