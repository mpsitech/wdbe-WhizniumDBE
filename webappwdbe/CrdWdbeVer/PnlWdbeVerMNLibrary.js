/**
  * \file PnlWdbeVerMNLibrary.js
  * web client functionality for panel PnlWdbeVerMNLibrary
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeVerMNLibrary", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeVerMNLibrary", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("MNLibrary", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNLibrary_side").setAttribute("height", "30");
	doc.getElementById("MNLibrary_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNLibrary_side").src = "./PnlWdbeVerMNLibrary_aside.html";
	doc.getElementById("MNLibrary_cont").src = "./PnlWdbeVerMNLibrary_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeVerMNLibrary", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeVerMNLibrary", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("MNLibrary", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNLibrary_side").setAttribute("height", "255");
	doc.getElementById("MNLibrary_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNLibrary_side").src = "./PnlWdbeVerMNLibrary_bside.html";
	doc.getElementById("MNLibrary_cont").src = "./PnlWdbeVerMNLibrary_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNLibrary_side").contentDocument;
	contdoc = doc.getElementById("MNLibrary_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNLibrary_side").contentDocument;
	contdoc = doc.getElementById("MNLibrary_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNLibrary_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNLibrary_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNLibrary_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVerMNLibrary", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeVerMNLibrary", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeVerMNLibrary", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVerMNLibrary", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeVerMNLibrary", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVerMNLibrary", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerMNLibrary", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerMNLibrary", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerMNLibrary", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerMNLibrary", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerMNLibrary", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerMNLibrary", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerMNLibrary", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerMNLibrary", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeVerMNLibrary", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeVerMNLibrary", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQVerMNLibrary
		rownode = getRownode(srcdoc, "ListWdbeQVerMNLibrary", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeVerMNLibrary", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeVerMNLibrary", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeVerMNLibrary", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNLibrary", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNLibrary_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNLibrary_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNLibrary_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNLibrary_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeVerMNLibrary", "ListWdbeQVerMNLibrary", retrieveTi(srcdoc, "TagWdbeVerMNLibrary", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeVerMNLibrary", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVerMNLibrary", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerMNLibrary", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerMNLibrary", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeVerMNLibrary", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeVerMNLibrary", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeVerMNLibraryData", scrJref, "StgIacQryWdbeVerMNLibrary");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeVerMNLibrary", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerMNLibrary", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeVerMNLibrary", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeVerMNLibrary", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVerMNLibrary", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeVerMNLibraryDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "ContInfWdbeVerMNLibrary", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "StatAppWdbeVerMNLibrary", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "StatShrWdbeVerMNLibrary", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "StgIacWdbeVerMNLibrary", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "TagWdbeVerMNLibrary", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "ListWdbeQVerMNLibrary", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "StatAppQryWdbeVerMNLibrary", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "StatShrQryWdbeVerMNLibrary", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeVerMNLibraryData", "StgIacQryWdbeVerMNLibrary", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeVerMNLibraryData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeVerMNLibraryData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNLibrary");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeVerMNLibraryData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
