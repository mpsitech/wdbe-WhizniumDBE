/**
  * \file PnlWdbeVer1NRelease.js
  * web client functionality for panel PnlWdbeVer1NRelease
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeVer1NRelease", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeVer1NRelease", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NRelease_side").src = "./PnlWdbeVer1NRelease_aside.html";
	doc.getElementById("1NRelease_cont").src = "./PnlWdbeVer1NRelease_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeVer1NRelease", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeVer1NRelease", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NRelease_side").src = "./PnlWdbeVer1NRelease_bside.html";
	doc.getElementById("1NRelease_cont").src = "./PnlWdbeVer1NRelease_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVer1NRelease", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeVer1NRelease", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeVer1NRelease", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVer1NRelease", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeVer1NRelease", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVer1NRelease", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NRelease", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NRelease", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NRelease", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NRelease", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NRelease", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NRelease", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NRelease", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVer1NRelease", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeVer1NRelease", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeVer1NRelease", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQVer1NRelease
		rownode = getRownode(srcdoc, "ListWdbeQVer1NRelease", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeVer1NRelease", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeVer1NRelease", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeVer1NRelease", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeVer1NRelease", "ListWdbeQVer1NRelease", retrieveTi(srcdoc, "TagWdbeVer1NRelease", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeVer1NRelease", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVer1NRelease", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NRelease", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NRelease", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeVer1NRelease", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeVer1NRelease", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeVer1NReleaseData", scrJref, "StgIacQryWdbeVer1NRelease");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeVer1NRelease", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NRelease", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeVer1NRelease", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeVer1NRelease", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVer1NRelease", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeVer1NReleaseDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "ContInfWdbeVer1NRelease", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "StatAppWdbeVer1NRelease", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "StatShrWdbeVer1NRelease", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "StgIacWdbeVer1NRelease", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "TagWdbeVer1NRelease", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "ListWdbeQVer1NRelease", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "StatAppQryWdbeVer1NRelease", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "StatShrQryWdbeVer1NRelease", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NReleaseData", "StgIacQryWdbeVer1NRelease", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeVer1NReleaseData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeVer1NReleaseData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeVer1NReleaseData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
