/**
  * \file PnlWdbeVer1NSystem.js
  * web client functionality for panel PnlWdbeVer1NSystem
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeVer1NSystem", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeVer1NSystem", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NSystem", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NSystem_side").setAttribute("height", "30");
	doc.getElementById("1NSystem_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NSystem_side").src = "./PnlWdbeVer1NSystem_aside.html";
	doc.getElementById("1NSystem_cont").src = "./PnlWdbeVer1NSystem_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeVer1NSystem", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeVer1NSystem", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NSystem", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NSystem_side").setAttribute("height", "255");
	doc.getElementById("1NSystem_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NSystem_side").src = "./PnlWdbeVer1NSystem_bside.html";
	doc.getElementById("1NSystem_cont").src = "./PnlWdbeVer1NSystem_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NSystem_side").contentDocument;
	contdoc = doc.getElementById("1NSystem_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NSystem_side").contentDocument;
	contdoc = doc.getElementById("1NSystem_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NSystem_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NSystem_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NSystem_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVer1NSystem", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeVer1NSystem", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeVer1NSystem", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVer1NSystem", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeVer1NSystem", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVer1NSystem", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NSystem", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NSystem", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NSystem", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NSystem", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NSystem", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NSystem", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NSystem", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVer1NSystem", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeVer1NSystem", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeVer1NSystem", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQVer1NSystem
		rownode = getRownode(srcdoc, "ListWdbeQVer1NSystem", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeVer1NSystem", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeVer1NSystem", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeVer1NSystem", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NSystem", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSystem_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSystem_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NSystem_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NSystem_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeVer1NSystem", "ListWdbeQVer1NSystem", retrieveTi(srcdoc, "TagWdbeVer1NSystem", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeVer1NSystem", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVer1NSystem", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NSystem", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVer1NSystem", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeVer1NSystem", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeVer1NSystem", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeVer1NSystemData", scrJref, "StgIacQryWdbeVer1NSystem");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeVer1NSystem", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVer1NSystem", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeVer1NSystem", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeVer1NSystem", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVer1NSystem", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeVer1NSystemDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "ContInfWdbeVer1NSystem", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "StatAppWdbeVer1NSystem", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "StatShrWdbeVer1NSystem", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "StgIacWdbeVer1NSystem", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "TagWdbeVer1NSystem", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "ListWdbeQVer1NSystem", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "StatAppQryWdbeVer1NSystem", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "StatShrQryWdbeVer1NSystem", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeVer1NSystemData", "StgIacQryWdbeVer1NSystem", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeVer1NSystemData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeVer1NSystemData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NSystem");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeVer1NSystemData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
