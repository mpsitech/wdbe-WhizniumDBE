...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeVerList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeVerList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeVerList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeVerList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVerList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeVerList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeVerList", "srefIxWdbeVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWdbeVerList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeVerList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVerList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeVerList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVerList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeVerList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeVerList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVerList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeVerList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeVerList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerList", "TcoOwnWidth"));
		var TcoPrjWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerList", "TcoPrjWidth"));
		var TcoMajWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerList", "TcoMajWidth"));
		var TcoMinWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerList", "TcoMinWidth"));
		var TcoSubWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerList", "TcoSubWidth"));
		var TcoBvrWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerList", "TcoBvrWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVerList", "TcoSteWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeVerList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoGrp"); doc.widths.push(TcoGrpWidth); doc.fs.push("grp"); doc.tos.push(4); wtot += TcoGrpWidth;
			} else if (i == 2) {
				doc.cols.push("TcoOwn"); doc.widths.push(TcoOwnWidth); doc.fs.push("own"); doc.tos.push(5); wtot += TcoOwnWidth;
			} else if (i == 3) {
				doc.cols.push("TcoPrj"); doc.widths.push(TcoPrjWidth); doc.fs.push("prj"); doc.tos.push(1); wtot += TcoPrjWidth;
			} else if (i == 4) {
				...
				doc.cols.push("TcoBvr"); doc.widths.push(TcoBvrWidth); doc.fs.push("bvr"); doc.tos.push(2); wtot += TcoBvrWidth;
			} else if (i == 8) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeVerList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeVerList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQVerList
		rownode = getRownode(srcdoc, "ListWdbeQVerList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeVerList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeVerList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeVerList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeVerList", "ListWdbeQVerList", retrieveTi(srcdoc, "TagWdbeVerList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeVerList", "TxtRecord2"),
				["Grp","Own","Prj","Maj","Min","Sub","Bvr","","Ste"],
				["grp","own","prj","maj","min","sub","bvr","ste","ste2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVerList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVerList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeVerList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeVerList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeVerListData", scrJref, "StgIacQryWdbeVerList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVerList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeVerList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVerList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeVerListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeVerList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeVerList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeVerListData", scrJref, "ContIacWdbeVerList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "ContIacWdbeVerList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "ContInfWdbeVerList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "StatShrWdbeVerList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "StgIacWdbeVerList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "TagWdbeVerList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "ListWdbeQVerList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "StatAppQryWdbeVerList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "StatShrQryWdbeVerList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeVerListData", "StgIacQryWdbeVerList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeVerListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeVerListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeVerListData") {
				mask = mergeDpchEngData(dom);
