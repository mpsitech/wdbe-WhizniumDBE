/**
  * \file PnlWdbeSilRef1NError.js
  * web client functionality for panel PnlWdbeSilRef1NError
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeSilRef1NError", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeSilRef1NError", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NError_side").src = "./PnlWdbeSilRef1NError_aside.html";
	doc.getElementById("Ref1NError_cont").src = "./PnlWdbeSilRef1NError_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeSilRef1NError", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeSilRef1NError", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NError_side").src = "./PnlWdbeSilRef1NError_bside.html";
	doc.getElementById("Ref1NError_cont").src = "./PnlWdbeSilRef1NError_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSilRef1NError", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSilRef1NError", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeSilRef1NError", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSilRef1NError", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NError", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSilRef1NError", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NError", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NError", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NError", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NError", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NError", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NError", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NError", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSilRef1NError", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeSilRef1NError", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSilRef1NError", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSilRef1NError
		rownode = getRownode(srcdoc, "ListWdbeQSilRef1NError", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSilRef1NError", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSilRef1NError", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSilRef1NError", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSilRef1NError", "ListWdbeQSilRef1NError", retrieveTi(srcdoc, "TagWdbeSilRef1NError", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSilRef1NError", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSilRef1NError", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NError", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NError", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSilRef1NError", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSilRef1NError", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSilRef1NErrorData", scrJref, "StgIacQryWdbeSilRef1NError");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeSilRef1NError", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NError", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSilRef1NError", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeSilRef1NError", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSilRef1NError", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSilRef1NErrorDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "ContInfWdbeSilRef1NError", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "StatAppWdbeSilRef1NError", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "StatShrWdbeSilRef1NError", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "StgIacWdbeSilRef1NError", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "TagWdbeSilRef1NError", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "ListWdbeQSilRef1NError", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "StatAppQryWdbeSilRef1NError", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "StatShrQryWdbeSilRef1NError", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NErrorData", "StgIacQryWdbeSilRef1NError", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeSilRef1NErrorData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSilRef1NErrorData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSilRef1NErrorData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
