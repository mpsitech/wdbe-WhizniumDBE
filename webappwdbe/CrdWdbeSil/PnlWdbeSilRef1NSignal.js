...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeSilRef1NSignal", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeSilRef1NSignal", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NSignal_side").src = "./PnlWdbeSilRef1NSignal_aside.html";
	doc.getElementById("Ref1NSignal_cont").src = "./PnlWdbeSilRef1NSignal_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeSilRef1NSignal", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeSilRef1NSignal", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NSignal_side").src = "./PnlWdbeSilRef1NSignal_bside.html";
	doc.getElementById("Ref1NSignal_cont").src = "./PnlWdbeSilRef1NSignal_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSilRef1NSignal", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSilRef1NSignal", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeSilRef1NSignal", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSilRef1NSignal", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NSignal", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSilRef1NSignal", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NSignal", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NSignal", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NSignal", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NSignal", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NSignal", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NSignal", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NSignal", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSilRef1NSignal", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeSilRef1NSignal", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSilRef1NSignal", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSilRef1NSignal
		rownode = getRownode(srcdoc, "ListWdbeQSilRef1NSignal", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSilRef1NSignal", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSilRef1NSignal", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSilRef1NSignal", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSilRef1NSignal", "ListWdbeQSilRef1NSignal", retrieveTi(srcdoc, "TagWdbeSilRef1NSignal", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSilRef1NSignal", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSilRef1NSignal", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NSignal", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NSignal", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSilRef1NSignal", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSilRef1NSignal", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSilRef1NSignalData", scrJref, "StgIacQryWdbeSilRef1NSignal");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeSilRef1NSignal", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NSignal", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSilRef1NSignal", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeSilRef1NSignal", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSilRef1NSignal", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSilRef1NSignalDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "ContInfWdbeSilRef1NSignal", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "StatAppWdbeSilRef1NSignal", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "StatShrWdbeSilRef1NSignal", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "StgIacWdbeSilRef1NSignal", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "TagWdbeSilRef1NSignal", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "ListWdbeQSilRef1NSignal", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "StatAppQryWdbeSilRef1NSignal", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "StatShrQryWdbeSilRef1NSignal", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NSignalData", "StgIacQryWdbeSilRef1NSignal", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeSilRef1NSignalData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSilRef1NSignalData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSilRef1NSignalData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
