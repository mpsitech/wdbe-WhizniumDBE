/**
  * \file PnlWdbeSilRef1NCommand.js
  * web client functionality for panel PnlWdbeSilRef1NCommand
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeSilRef1NCommand", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeSilRef1NCommand", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NCommand", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NCommand_side").setAttribute("height", "30");
	doc.getElementById("Ref1NCommand_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NCommand_side").src = "./PnlWdbeSilRef1NCommand_aside.html";
	doc.getElementById("Ref1NCommand_cont").src = "./PnlWdbeSilRef1NCommand_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeSilRef1NCommand", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeSilRef1NCommand", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NCommand", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NCommand_side").setAttribute("height", "255");
	doc.getElementById("Ref1NCommand_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NCommand_side").src = "./PnlWdbeSilRef1NCommand_bside.html";
	doc.getElementById("Ref1NCommand_cont").src = "./PnlWdbeSilRef1NCommand_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NCommand_side").contentDocument;
	contdoc = doc.getElementById("Ref1NCommand_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NCommand_side").contentDocument;
	contdoc = doc.getElementById("Ref1NCommand_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NCommand_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NCommand_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NCommand_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSilRef1NCommand", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSilRef1NCommand", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeSilRef1NCommand", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSilRef1NCommand", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NCommand", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSilRef1NCommand", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NCommand", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NCommand", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NCommand", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NCommand", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NCommand", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NCommand", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NCommand", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSilRef1NCommand", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeSilRef1NCommand", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSilRef1NCommand", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSilRef1NCommand
		rownode = getRownode(srcdoc, "ListWdbeQSilRef1NCommand", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSilRef1NCommand", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSilRef1NCommand", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSilRef1NCommand", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NCommand", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NCommand_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NCommand_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NCommand_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NCommand_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSilRef1NCommand", "ListWdbeQSilRef1NCommand", retrieveTi(srcdoc, "TagWdbeSilRef1NCommand", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSilRef1NCommand", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSilRef1NCommand", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NCommand", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSilRef1NCommand", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSilRef1NCommand", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSilRef1NCommand", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSilRef1NCommandData", scrJref, "StgIacQryWdbeSilRef1NCommand");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeSilRef1NCommand", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSilRef1NCommand", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSilRef1NCommand", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeSilRef1NCommand", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSilRef1NCommand", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSilRef1NCommandDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "ContInfWdbeSilRef1NCommand", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "StatAppWdbeSilRef1NCommand", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "StatShrWdbeSilRef1NCommand", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "StgIacWdbeSilRef1NCommand", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "TagWdbeSilRef1NCommand", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "ListWdbeQSilRef1NCommand", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "StatAppQryWdbeSilRef1NCommand", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "StatShrQryWdbeSilRef1NCommand", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSilRef1NCommandData", "StgIacQryWdbeSilRef1NCommand", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeSilRef1NCommandData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSilRef1NCommandData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NCommand");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSilRef1NCommandData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
