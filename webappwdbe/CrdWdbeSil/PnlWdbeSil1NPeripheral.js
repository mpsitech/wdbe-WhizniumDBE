...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWdbeSil1NPeripheral", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NPeripheral_side").src = "./PnlWdbeSil1NPeripheral_aside.html";
	doc.getElementById("1NPeripheral_cont").src = "./PnlWdbeSil1NPeripheral_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeSil1NPeripheral", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeSil1NPeripheral", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NPeripheral_side").src = "./PnlWdbeSil1NPeripheral_bside.html";
	doc.getElementById("1NPeripheral_cont").src = "./PnlWdbeSil1NPeripheral_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSil1NPeripheral", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSil1NPeripheral", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeSil1NPeripheral", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSil1NPeripheral", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSil1NPeripheral", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NPeripheral", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NPeripheral", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NPeripheral", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NPeripheral", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NPeripheral", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NPeripheral", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NPeripheral", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NPeripheral", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSil1NPeripheral", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeSil1NPeripheral", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSil1NPeripheral", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSil1NPeripheral
		rownode = getRownode(srcdoc, "ListWdbeQSil1NPeripheral", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSil1NPeripheral", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSil1NPeripheral", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSil1NPeripheral", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSil1NPeripheral", "ListWdbeQSil1NPeripheral", retrieveTi(srcdoc, "TagWdbeSil1NPeripheral", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSil1NPeripheral", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NPeripheral", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NPeripheral", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NPeripheral", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSil1NPeripheral", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSil1NPeripheral", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSil1NPeripheralData", scrJref, "StgIacQryWdbeSil1NPeripheral");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeSil1NPeripheral", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NPeripheral", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSil1NPeripheral", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeSil1NPeripheral", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NPeripheral", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSil1NPeripheralDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "ContInfWdbeSil1NPeripheral", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "StatAppWdbeSil1NPeripheral", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "StatShrWdbeSil1NPeripheral", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "StgIacWdbeSil1NPeripheral", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "TagWdbeSil1NPeripheral", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "ListWdbeQSil1NPeripheral", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "StatAppQryWdbeSil1NPeripheral", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "StatShrQryWdbeSil1NPeripheral", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NPeripheralData", "StgIacQryWdbeSil1NPeripheral", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeSil1NPeripheralData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSil1NPeripheralData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSil1NPeripheralData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
