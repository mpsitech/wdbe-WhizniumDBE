/**
  * \file PnlWdbeSil1NBank.js
  * web client functionality for panel PnlWdbeSil1NBank
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeSil1NBank", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeSil1NBank", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NBank_side").src = "./PnlWdbeSil1NBank_aside.html";
	doc.getElementById("1NBank_cont").src = "./PnlWdbeSil1NBank_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeSil1NBank", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeSil1NBank", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NBank_side").src = "./PnlWdbeSil1NBank_bside.html";
	doc.getElementById("1NBank_cont").src = "./PnlWdbeSil1NBank_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSil1NBank", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSil1NBank", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeSil1NBank", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSil1NBank", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSil1NBank", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NBank", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NBank", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NBank", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NBank", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NBank", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NBank", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NBank", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NBank", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSil1NBank", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeSil1NBank", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSil1NBank", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSil1NBank
		rownode = getRownode(srcdoc, "ListWdbeQSil1NBank", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSil1NBank", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSil1NBank", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSil1NBank", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSil1NBank", "ListWdbeQSil1NBank", retrieveTi(srcdoc, "TagWdbeSil1NBank", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSil1NBank", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NBank", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NBank", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NBank", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSil1NBank", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSil1NBank", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSil1NBankData", scrJref, "StgIacQryWdbeSil1NBank");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeSil1NBank", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NBank", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSil1NBank", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeSil1NBank", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NBank", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSil1NBankDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "ContInfWdbeSil1NBank", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "StatAppWdbeSil1NBank", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "StatShrWdbeSil1NBank", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "StgIacWdbeSil1NBank", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "TagWdbeSil1NBank", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "ListWdbeQSil1NBank", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "StatAppQryWdbeSil1NBank", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "StatShrQryWdbeSil1NBank", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NBankData", "StgIacQryWdbeSil1NBank", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeSil1NBankData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSil1NBankData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSil1NBankData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
