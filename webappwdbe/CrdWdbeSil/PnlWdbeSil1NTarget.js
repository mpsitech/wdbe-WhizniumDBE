/**
  * \file PnlWdbeSil1NTarget.js
  * web client functionality for panel PnlWdbeSil1NTarget
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeSil1NTarget", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeSil1NTarget", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NTarget", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NTarget_side").setAttribute("height", "30");
	doc.getElementById("1NTarget_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NTarget_side").src = "./PnlWdbeSil1NTarget_aside.html";
	doc.getElementById("1NTarget_cont").src = "./PnlWdbeSil1NTarget_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeSil1NTarget", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeSil1NTarget", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NTarget", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NTarget_side").setAttribute("height", "255");
	doc.getElementById("1NTarget_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NTarget_side").src = "./PnlWdbeSil1NTarget_bside.html";
	doc.getElementById("1NTarget_cont").src = "./PnlWdbeSil1NTarget_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NTarget_side").contentDocument;
	contdoc = doc.getElementById("1NTarget_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NTarget_side").contentDocument;
	contdoc = doc.getElementById("1NTarget_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NTarget_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NTarget_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NTarget_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSil1NTarget", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSil1NTarget", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeSil1NTarget", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSil1NTarget", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSil1NTarget", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NTarget", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NTarget", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NTarget", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NTarget", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NTarget", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NTarget", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NTarget", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NTarget", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSil1NTarget", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeSil1NTarget", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSil1NTarget", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSil1NTarget
		rownode = getRownode(srcdoc, "ListWdbeQSil1NTarget", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSil1NTarget", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSil1NTarget", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSil1NTarget", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NTarget", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NTarget_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NTarget_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NTarget_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NTarget_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSil1NTarget", "ListWdbeQSil1NTarget", retrieveTi(srcdoc, "TagWdbeSil1NTarget", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSil1NTarget", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NTarget", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NTarget", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NTarget", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSil1NTarget", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSil1NTarget", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSil1NTargetData", scrJref, "StgIacQryWdbeSil1NTarget");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeSil1NTarget", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NTarget", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSil1NTarget", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeSil1NTarget", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NTarget", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSil1NTargetDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "ContInfWdbeSil1NTarget", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "StatAppWdbeSil1NTarget", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "StatShrWdbeSil1NTarget", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "StgIacWdbeSil1NTarget", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "TagWdbeSil1NTarget", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "ListWdbeQSil1NTarget", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "StatAppQryWdbeSil1NTarget", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "StatShrQryWdbeSil1NTarget", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NTargetData", "StgIacQryWdbeSil1NTarget", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeSil1NTargetData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSil1NTargetData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NTarget");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSil1NTargetData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
