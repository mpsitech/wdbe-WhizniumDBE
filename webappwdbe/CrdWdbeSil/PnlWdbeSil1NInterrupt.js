...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeSil1NInterrupt", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeSil1NInterrupt", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NInterrupt", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NInterrupt_side").setAttribute("height", "30");
	doc.getElementById("1NInterrupt_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NInterrupt_side").src = "./PnlWdbeSil1NInterrupt_aside.html";
	doc.getElementById("1NInterrupt_cont").src = "./PnlWdbeSil1NInterrupt_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeSil1NInterrupt", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeSil1NInterrupt", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NInterrupt", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NInterrupt_side").setAttribute("height", "255");
	doc.getElementById("1NInterrupt_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NInterrupt_side").src = "./PnlWdbeSil1NInterrupt_bside.html";
	doc.getElementById("1NInterrupt_cont").src = "./PnlWdbeSil1NInterrupt_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NInterrupt_side").contentDocument;
	contdoc = doc.getElementById("1NInterrupt_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NInterrupt_side").contentDocument;
	contdoc = doc.getElementById("1NInterrupt_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NInterrupt_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NInterrupt_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NInterrupt_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSil1NInterrupt", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSil1NInterrupt", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeSil1NInterrupt", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSil1NInterrupt", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSil1NInterrupt", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NInterrupt", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NInterrupt", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NInterrupt", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NInterrupt", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NInterrupt", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NInterrupt", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NInterrupt", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NInterrupt", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSil1NInterrupt", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeSil1NInterrupt", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSil1NInterrupt", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSil1NInterrupt
		rownode = getRownode(srcdoc, "ListWdbeQSil1NInterrupt", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSil1NInterrupt", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSil1NInterrupt", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSil1NInterrupt", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NInterrupt", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NInterrupt_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NInterrupt_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NInterrupt_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NInterrupt_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSil1NInterrupt", "ListWdbeQSil1NInterrupt", retrieveTi(srcdoc, "TagWdbeSil1NInterrupt", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSil1NInterrupt", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NInterrupt", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NInterrupt", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSil1NInterrupt", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSil1NInterrupt", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSil1NInterrupt", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSil1NInterruptData", scrJref, "StgIacQryWdbeSil1NInterrupt");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeSil1NInterrupt", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSil1NInterrupt", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSil1NInterrupt", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeSil1NInterrupt", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSil1NInterrupt", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSil1NInterruptDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "ContInfWdbeSil1NInterrupt", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "StatAppWdbeSil1NInterrupt", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "StatShrWdbeSil1NInterrupt", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "StgIacWdbeSil1NInterrupt", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "TagWdbeSil1NInterrupt", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "ListWdbeQSil1NInterrupt", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "StatAppQryWdbeSil1NInterrupt", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "StatShrQryWdbeSil1NInterrupt", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSil1NInterruptData", "StgIacQryWdbeSil1NInterrupt", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeSil1NInterruptData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSil1NInterruptData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NInterrupt");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSil1NInterruptData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
