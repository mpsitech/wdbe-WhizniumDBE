...
function refreshB(chgcol) {
	...
	if (chgcol) {
		...
		var TcoSysWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSilList", "TcoSysWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoReu"); doc.widths.push(TcoReuWidth); doc.fs.push("reu"); doc.tos.push(6); wtot += TcoReuWidth;
			} else if (i == 7) {
				doc.cols.push("TcoSys"); doc.widths.push(TcoSysWidth); doc.fs.push("sys"); doc.tos.push(7); wtot += TcoSysWidth;
			} else if (i == 8) {
