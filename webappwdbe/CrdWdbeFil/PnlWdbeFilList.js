/**
  * \file PnlWdbeFilList.js
  * web client functionality for panel PnlWdbeFilList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeFilList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeFilList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeFilList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeFilList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeFilList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeFilList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeFilList", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeFilList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeFilList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeFilList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFilList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFilList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeFilList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeFilList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeFilList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFilList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFilList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeFilList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeFilList", "TcoOwnWidth"));
		var TcoFnmWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeFilList", "TcoFnmWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeFilList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeFilList", "TcoReuWidth"));
		var TcoCntWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeFilList", "TcoCntWidth"));
		var TcoMimWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeFilList", "TcoMimWidth"));
		var TcoSizWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeFilList", "TcoSizWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeFilList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoFnm"); doc.widths.push(TcoFnmWidth); doc.fs.push("fnm"); doc.tos.push(2); wtot += TcoFnmWidth;
			} else if (i == 4) {
				...
				doc.cols.push("TcoCnt"); doc.widths.push(TcoCntWidth); doc.fs.push("cnt2"); doc.tos.push(0); wtot += TcoCntWidth;
			} else if (i == 7) {
				doc.cols.push("TcoMim"); doc.widths.push(TcoMimWidth); doc.fs.push("mim2"); doc.tos.push(0); wtot += TcoMimWidth;
			} else if (i == 8) {
				doc.cols.push("TcoSiz"); doc.widths.push(TcoSizWidth); doc.fs.push("siz"); doc.tos.push(0); wtot += TcoSizWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeFilList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeFilList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQFilList
		rownode = getRownode(srcdoc, "ListWdbeQFilList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeFilList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeFilList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeFilList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeFilList", "ListWdbeQFilList", retrieveTi(srcdoc, "TagWdbeFilList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeFilList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeFilList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFilList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeFilList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeFilList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeFilList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeFilListData", scrJref, "StgIacQryWdbeFilList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeFilList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeFilList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeFilList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeFilListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeFilList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeFilList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeFilListData", scrJref, "ContIacWdbeFilList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "ContIacWdbeFilList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "ContInfWdbeFilList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "StatShrWdbeFilList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "StgIacWdbeFilList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "TagWdbeFilList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "ListWdbeQFilList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "StatAppQryWdbeFilList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "StatShrQryWdbeFilList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeFilListData", "StgIacQryWdbeFilList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeFilListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeFilListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeFilListData") {
				mask = mergeDpchEngData(dom);
