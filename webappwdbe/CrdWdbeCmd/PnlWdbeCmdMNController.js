/**
  * \file PnlWdbeCmdMNController.js
  * web client functionality for panel PnlWdbeCmdMNController
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeCmdMNController", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeCmdMNController", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("MNController", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNController_side").setAttribute("height", "30");
	doc.getElementById("MNController_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNController_side").src = "./PnlWdbeCmdMNController_aside.html";
	doc.getElementById("MNController_cont").src = "./PnlWdbeCmdMNController_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeCmdMNController", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeCmdMNController", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("MNController", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNController_side").setAttribute("height", "255");
	doc.getElementById("MNController_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNController_side").src = "./PnlWdbeCmdMNController_bside.html";
	doc.getElementById("MNController_cont").src = "./PnlWdbeCmdMNController_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNController_side").contentDocument;
	contdoc = doc.getElementById("MNController_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNController_side").contentDocument;
	contdoc = doc.getElementById("MNController_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNController_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNController_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNController_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCmdMNController", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCmdMNController", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeCmdMNController", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCmdMNController", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCmdMNController", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmdMNController", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdMNController", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdMNController", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdMNController", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdMNController", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdMNController", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdMNController", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdMNController", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdMNController", "TcoMrefWidth"));
		var TcoIvrWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdMNController", "TcoIvrWidth"));
		var TcoRvrWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdMNController", "TcoRvrWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoIvr"); doc.widths.push(TcoIvrWidth); doc.fs.push("ivr"); wtot += TcoIvrWidth;
			} else if (i == 3) {
				doc.cols.push("TcoRvr"); doc.widths.push(TcoRvrWidth); doc.fs.push("rvr"); wtot += TcoRvrWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeCmdMNController", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCmdMNController", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCmdMNController
		rownode = getRownode(srcdoc, "ListWdbeQCmdMNController", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCmdMNController", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCmdMNController", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCmdMNController", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNController", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNController_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNController_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNController_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNController_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCmdMNController", "ListWdbeQCmdMNController", retrieveTi(srcdoc, "TagWdbeCmdMNController", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCmdMNController", "TxtRecord2"),
				["Mref","Ivr","Rvr"],
				["mref","ivr","rvr"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmdMNController", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdMNController", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdMNController", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCmdMNController", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCmdMNController", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCmdMNControllerData", scrJref, "StgIacQryWdbeCmdMNController");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeCmdMNController", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdMNController", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCmdMNController", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeCmdMNController", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmdMNController", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCmdMNControllerDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "ContInfWdbeCmdMNController", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "StatAppWdbeCmdMNController", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "StatShrWdbeCmdMNController", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "StgIacWdbeCmdMNController", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "TagWdbeCmdMNController", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "ListWdbeQCmdMNController", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "StatAppQryWdbeCmdMNController", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "StatShrQryWdbeCmdMNController", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCmdMNControllerData", "StgIacQryWdbeCmdMNController", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeCmdMNControllerData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCmdMNControllerData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNController");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCmdMNControllerData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
