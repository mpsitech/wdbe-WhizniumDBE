/**
  * \file PnlWdbeCmdARetpar.js
  * web client functionality for panel PnlWdbeCmdARetpar
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeCmdARetpar", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeCmdARetpar", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("ARetpar", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ARetpar_side").setAttribute("height", "30");
	doc.getElementById("ARetpar_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ARetpar_side").src = "./PnlWdbeCmdARetpar_aside.html";
	doc.getElementById("ARetpar_cont").src = "./PnlWdbeCmdARetpar_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeCmdARetpar", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeCmdARetpar", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("ARetpar", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ARetpar_side").setAttribute("height", "255");
	doc.getElementById("ARetpar_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ARetpar_side").src = "./PnlWdbeCmdARetpar_bside.html";
	doc.getElementById("ARetpar_cont").src = "./PnlWdbeCmdARetpar_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ARetpar_side").contentDocument;
	contdoc = doc.getElementById("ARetpar_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("ARetpar_side").contentDocument;
	contdoc = doc.getElementById("ARetpar_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ARetpar_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ARetpar_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ARetpar_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCmdARetpar", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCmdARetpar", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeCmdARetpar", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCmdARetpar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCmdARetpar", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmdARetpar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdARetpar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdARetpar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdARetpar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdARetpar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdARetpar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdARetpar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdARetpar", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdARetpar", "TcoSrfWidth"));
		var TcoPtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdARetpar", "TcoPtyWidth"));
		var TcoVecWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdARetpar", "TcoVecWidth"));
		var TcoLenWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdARetpar", "TcoLenWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdARetpar", "TcoCmtWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeCmdARetpar", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCmdARetpar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCmdARetpar
		rownode = getRownode(srcdoc, "ListWdbeQCmdARetpar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCmdARetpar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCmdARetpar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCmdARetpar", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ARetpar", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ARetpar_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ARetpar_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ARetpar_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ARetpar_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCmdARetpar", "ListWdbeQCmdARetpar", retrieveTi(srcdoc, "TagWdbeCmdARetpar", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCmdARetpar", "TxtRecord2"),
				["Srf","Pty","","Vec","Len","Cmt"],
				["srf","pty","pty2","vec","len","cmt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmdARetpar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdARetpar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdARetpar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCmdARetpar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCmdARetpar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCmdARetparData", scrJref, "StgIacQryWdbeCmdARetpar");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeCmdARetpar", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdARetpar", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCmdARetpar", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeCmdARetpar", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmdARetpar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCmdARetparDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "ContInfWdbeCmdARetpar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "StatAppWdbeCmdARetpar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "StatShrWdbeCmdARetpar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "StgIacWdbeCmdARetpar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "TagWdbeCmdARetpar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "ListWdbeQCmdARetpar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "StatAppQryWdbeCmdARetpar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "StatShrQryWdbeCmdARetpar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCmdARetparData", "StgIacQryWdbeCmdARetpar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeCmdARetparData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCmdARetparData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ARetpar");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCmdARetparData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
