/**
  * \file PnlWdbeCmdAInvpar.js
  * web client functionality for panel PnlWdbeCmdAInvpar
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeCmdAInvpar", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeCmdAInvpar", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("AInvpar", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("AInvpar_side").setAttribute("height", "30");
	doc.getElementById("AInvpar_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("AInvpar_side").src = "./PnlWdbeCmdAInvpar_aside.html";
	doc.getElementById("AInvpar_cont").src = "./PnlWdbeCmdAInvpar_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeCmdAInvpar", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeCmdAInvpar", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("AInvpar", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("AInvpar_side").setAttribute("height", "255");
	doc.getElementById("AInvpar_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("AInvpar_side").src = "./PnlWdbeCmdAInvpar_bside.html";
	doc.getElementById("AInvpar_cont").src = "./PnlWdbeCmdAInvpar_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AInvpar_side").contentDocument;
	contdoc = doc.getElementById("AInvpar_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("AInvpar_side").contentDocument;
	contdoc = doc.getElementById("AInvpar_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AInvpar_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AInvpar_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AInvpar_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCmdAInvpar", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCmdAInvpar", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeCmdAInvpar", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCmdAInvpar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCmdAInvpar", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmdAInvpar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdAInvpar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdAInvpar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdAInvpar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdAInvpar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdAInvpar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdAInvpar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdAInvpar", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdAInvpar", "TcoSrfWidth"));
		var TcoPtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdAInvpar", "TcoPtyWidth"));
		var TcoVecWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdAInvpar", "TcoVecWidth"));
		var TcoLenWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdAInvpar", "TcoLenWidth"));
		var TcoDfvWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdAInvpar", "TcoDfvWidth"));
		var TcoVitWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdAInvpar", "TcoVitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmdAInvpar", "TcoCmtWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoPty"); doc.widths.push(TcoPtyWidth); doc.fs.push("pty"); wtot += TcoPtyWidth;
			} else if (i == 3) {
				doc.cols.push("TcoVec"); doc.widths.push(TcoVecWidth); doc.fs.push("vec"); wtot += TcoVecWidth;
			} else if (i == 4) {
				doc.cols.push("TcoLen"); doc.widths.push(TcoLenWidth); doc.fs.push("len"); wtot += TcoLenWidth;
			} else if (i == 5) {
				doc.cols.push("TcoDfv"); doc.widths.push(TcoDfvWidth); doc.fs.push("dfv"); wtot += TcoDfvWidth;
			} else if (i == 6) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeCmdAInvpar", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCmdAInvpar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCmdAInvpar
		rownode = getRownode(srcdoc, "ListWdbeQCmdAInvpar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCmdAInvpar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCmdAInvpar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCmdAInvpar", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AInvpar", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AInvpar_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AInvpar_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AInvpar_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AInvpar_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCmdAInvpar", "ListWdbeQCmdAInvpar", retrieveTi(srcdoc, "TagWdbeCmdAInvpar", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCmdAInvpar", "TxtRecord2"),
				["Srf","Pty","","Vec","Len","Dfv","Vit","Cmt"],
				["srf","pty","pty2","vec","len","dfv","vit","cmt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmdAInvpar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdAInvpar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmdAInvpar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCmdAInvpar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCmdAInvpar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCmdAInvparData", scrJref, "StgIacQryWdbeCmdAInvpar");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeCmdAInvpar", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmdAInvpar", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCmdAInvpar", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeCmdAInvpar", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmdAInvpar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCmdAInvparDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "ContInfWdbeCmdAInvpar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "StatAppWdbeCmdAInvpar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "StatShrWdbeCmdAInvpar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "StgIacWdbeCmdAInvpar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "TagWdbeCmdAInvpar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "ListWdbeQCmdAInvpar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "StatAppQryWdbeCmdAInvpar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "StatShrQryWdbeCmdAInvpar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCmdAInvparData", "StgIacQryWdbeCmdAInvpar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeCmdAInvparData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCmdAInvparData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AInvpar");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCmdAInvparData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
