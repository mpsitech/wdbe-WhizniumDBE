...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePplPpl1NSegment", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePplPpl1NSegment", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Ppl1NSegment", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ppl1NSegment_side").setAttribute("height", "30");
	doc.getElementById("Ppl1NSegment_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ppl1NSegment_side").src = "./PnlWdbePplPpl1NSegment_aside.html";
	doc.getElementById("Ppl1NSegment_cont").src = "./PnlWdbePplPpl1NSegment_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePplPpl1NSegment", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePplPpl1NSegment", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Ppl1NSegment", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ppl1NSegment_side").setAttribute("height", "255");
	doc.getElementById("Ppl1NSegment_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ppl1NSegment_side").src = "./PnlWdbePplPpl1NSegment_bside.html";
	doc.getElementById("Ppl1NSegment_cont").src = "./PnlWdbePplPpl1NSegment_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ppl1NSegment_side").contentDocument;
	contdoc = doc.getElementById("Ppl1NSegment_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ppl1NSegment_side").contentDocument;
	contdoc = doc.getElementById("Ppl1NSegment_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ppl1NSegment_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ppl1NSegment_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ppl1NSegment_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePplPpl1NSegment", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePplPpl1NSegment", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePplPpl1NSegment", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePplPpl1NSegment", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePplPpl1NSegment", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePplPpl1NSegment", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePplPpl1NSegment", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePplPpl1NSegment", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePplPpl1NSegment", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePplPpl1NSegment", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePplPpl1NSegment", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePplPpl1NSegment", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePplPpl1NSegment", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePplPpl1NSegment", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePplPpl1NSegment", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePplPpl1NSegment", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPplPpl1NSegment
		rownode = getRownode(srcdoc, "ListWdbeQPplPpl1NSegment", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePplPpl1NSegment", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePplPpl1NSegment", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePplPpl1NSegment", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ppl1NSegment", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ppl1NSegment_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ppl1NSegment_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ppl1NSegment_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ppl1NSegment_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePplPpl1NSegment", "ListWdbeQPplPpl1NSegment", retrieveTi(srcdoc, "TagWdbePplPpl1NSegment", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePplPpl1NSegment", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePplPpl1NSegment", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePplPpl1NSegment", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePplPpl1NSegment", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePplPpl1NSegment", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePplPpl1NSegment", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePplPpl1NSegmentData", scrJref, "StgIacQryWdbePplPpl1NSegment");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePplPpl1NSegment", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePplPpl1NSegment", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePplPpl1NSegment", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePplPpl1NSegment", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePplPpl1NSegment", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePplPpl1NSegmentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "ContInfWdbePplPpl1NSegment", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "StatAppWdbePplPpl1NSegment", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "StatShrWdbePplPpl1NSegment", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "StgIacWdbePplPpl1NSegment", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "TagWdbePplPpl1NSegment", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "ListWdbeQPplPpl1NSegment", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "StatAppQryWdbePplPpl1NSegment", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "StatShrQryWdbePplPpl1NSegment", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePplPpl1NSegmentData", "StgIacQryWdbePplPpl1NSegment", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePplPpl1NSegmentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePplPpl1NSegmentData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ppl1NSegment");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePplPpl1NSegmentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
