/**
  * \file CrdWdbeUtl.js
  * web client functionality for card CrdWdbeUtl
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWdbeUtl", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWdbeUtl", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWdbeUtl", "initdoneHeadbar") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWdbeUtlHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWdbeUtl", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWdbeUtl", "shortMenu", short);
	setSi(srcdoc, "StatAppWdbeUtl", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWdbeUtl", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWdbeUtl", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWdbeUtl", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWdbeUtl", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MitCrdCif"), retrieveTi(srcdoc, "TagWdbeUtl", "MitCrdCif"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdXip"), retrieveTi(srcdoc, "TagWdbeUtl", "MitCrdXip"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdMip"), retrieveTi(srcdoc, "TagWdbeUtl", "MitCrdMip"));
};
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgWdbeUtlIexconv") || (sref == "DlgWdbeUtlExtrip") || (sref == "DlgWdbeUtlMrgip")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgextrip = retrieveSi(srcdoc, "StatShrWdbeUtl", "scrJrefDlgextrip");
	var scrJrefDlgiexconv = retrieveSi(srcdoc, "StatShrWdbeUtl", "scrJrefDlgiexconv");
	var scrJrefDlgmrgip = retrieveSi(srcdoc, "StatShrWdbeUtl", "scrJrefDlgmrgip");

	if (scrJrefDlgextrip != "") {
		if (scrJrefDlg != scrJrefDlgextrip) showDlg("DlgWdbeUtlExtrip", scrJrefDlgextrip);
	} else if (scrJrefDlgiexconv != "") {
		if (scrJrefDlg != scrJrefDlgiexconv) showDlg("DlgWdbeUtlIexconv", scrJrefDlgiexconv);
	} else if (scrJrefDlgmrgip != "") {
		if (scrJrefDlg != scrJrefDlgmrgip) showDlg("DlgWdbeUtlMrgip", scrJrefDlgmrgip);
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUtlDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUtlDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWdbeUtl", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUtlData", "ContInfWdbeUtl", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUtlData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWdbeUtlData", "StatAppWdbeUtl", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUtlData", "StatShrWdbeUtl", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUtlData", "TagWdbeUtl", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUtlData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWdbeUtl", "scrJrefHeadbar");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWdbeUtlData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeUtl", "srefIxWdbeVReqitmode");

	if (srefIxWdbeVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWdbeVReqitmode = "notify";
		setSi(srcdoc, "StatAppWdbeUtl", "srefIxWdbeVReqitmode", srefIxWdbeVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeUtl", "srefIxWdbeVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWdbeUtl", "latency"));
