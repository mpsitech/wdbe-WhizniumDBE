/**
  * \file CrdWdbeErr.js
  * web client functionality for card CrdWdbeErr
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWdbeErr", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWdbeErr", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWdbeErr", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWdbeErr", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWdbeErr", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWdbeErrHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWdbeErrList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWdbeErrRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWdbeErr", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWdbeErr", "shortMenu", short);
	setSi(srcdoc, "StatAppWdbeErr", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWdbeErr", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWdbeErr", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWdbeErr", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWdbeErr", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdErr"), retrieveCi(srcdoc, "ContInfWdbeErr", "MtxCrdErr"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWdbeErr", "MtxCrdErr") + " - WhizniumDBE v1.0.3";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeErrDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeErrDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWdbeErr", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeErrData", "ContInfWdbeErr", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeErrData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWdbeErrData", "StatAppWdbeErr", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeErrData", "StatShrWdbeErr", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeErrData", "TagWdbeErr", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeErrData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWdbeErr", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWdbeErr", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWdbeErr", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWdbeErrData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeErr", "srefIxWdbeVReqitmode");

	if (srefIxWdbeVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWdbeVReqitmode = "notify";
		setSi(srcdoc, "StatAppWdbeErr", "srefIxWdbeVReqitmode", srefIxWdbeVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeErr", "srefIxWdbeVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWdbeErr", "latency"));
