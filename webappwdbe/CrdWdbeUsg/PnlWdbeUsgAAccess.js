...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWdbeUsgAAccess", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlWdbeUsgAAccess_aside.html";
	doc.getElementById("AAccess_cont").src = "./PnlWdbeUsgAAccess_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUsgAAccess", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUsgAAccess", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlWdbeUsgAAccess_bside.html";
	doc.getElementById("AAccess_cont").src = "./PnlWdbeUsgAAccess_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUsgAAccess", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUsgAAccess", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUsgAAccess", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUsgAAccess", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUsgAAccess", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsgAAccess", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsgAAccess", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsgAAccess", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsgAAccess", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsgAAccess", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsgAAccess", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsgAAccess", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsgAAccess", "ncol"));

		var TcoFegWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsgAAccess", "TcoFegWidth"));
		var TcoFeaWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsgAAccess", "TcoFeaWidth"));
		var TcoAccWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUsgAAccess", "TcoAccWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUsgAAccess", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUsgAAccess", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUsgAAccess
		rownode = getRownode(srcdoc, "ListWdbeQUsgAAccess", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUsgAAccess", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUsgAAccess", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUsgAAccess", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUsgAAccess", "ListWdbeQUsgAAccess", retrieveTi(srcdoc, "TagWdbeUsgAAccess", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUsgAAccess", "TxtRecord2"),
				["Feg","","Fea","Acc",""],
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsgAAccess", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsgAAccess", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUsgAAccess", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUsgAAccess", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUsgAAccess", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUsgAAccessData", scrJref, "StgIacQryWdbeUsgAAccess");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUsgAAccess", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUsgAAccess", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUsgAAccess", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUsgAAccess", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUsgAAccess", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUsgAAccessDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "ContInfWdbeUsgAAccess", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "StatAppWdbeUsgAAccess", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "StatShrWdbeUsgAAccess", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "StgIacWdbeUsgAAccess", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "TagWdbeUsgAAccess", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "ListWdbeQUsgAAccess", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "StatAppQryWdbeUsgAAccess", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "StatShrQryWdbeUsgAAccess", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUsgAAccessData", "StgIacQryWdbeUsgAAccess", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUsgAAccessData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUsgAAccessData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUsgAAccessData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
