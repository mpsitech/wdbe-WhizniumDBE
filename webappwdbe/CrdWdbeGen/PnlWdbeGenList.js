/**
  * \file PnlWdbeGenList.js
  * web client functionality for panel PnlWdbeGenList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeGenList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeGenList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeGenList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeGenList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeGenList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeGenList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeGenList", "srefIxWdbeVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWdbeGenList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeGenList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeGenList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeGenList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeGenList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeGenList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeGenList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeGenList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeGenList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeGenList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeGenList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeGenList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeGenList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeGenList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeGenList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeGenList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeGenList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeGenList", "TcoSrfWidth"));
		var TcoMdlWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeGenList", "TcoMdlWidth"));
		var TcoHtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeGenList", "TcoHtyWidth"));
		var TcoWidWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeGenList", "TcoWidWidth"));
		var TcoMmxWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeGenList", "TcoMmxWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeGenList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeGenList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeGenList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQGenList
		rownode = getRownode(srcdoc, "ListWdbeQGenList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeGenList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeGenList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeGenList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeGenList", "ListWdbeQGenList", retrieveTi(srcdoc, "TagWdbeGenList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeGenList", "TxtRecord2"),
				["Srf","Mdl","","Hty","Wid","Mmx"],
				["srf","mdl","hty","hty2","wid","mmx"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeGenList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeGenList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeGenList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeGenList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeGenList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeGenListData", scrJref, "StgIacQryWdbeGenList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeGenList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeGenList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeGenList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeGenListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeGenList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeGenList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeGenListData", scrJref, "ContIacWdbeGenList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "ContIacWdbeGenList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "ContInfWdbeGenList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "StatShrWdbeGenList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "StgIacWdbeGenList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "TagWdbeGenList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "ListWdbeQGenList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "StatAppQryWdbeGenList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "StatShrQryWdbeGenList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeGenListData", "StgIacQryWdbeGenList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeGenListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeGenListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeGenListData") {
				mask = mergeDpchEngData(dom);
