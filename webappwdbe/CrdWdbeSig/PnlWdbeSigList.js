...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeSigList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeSigList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeSigList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeSigList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSigList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSigList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeSigList", "srefIxWdbeVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWdbeSigList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeSigList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSigList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSigList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSigList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeSigList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSigList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSigList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSigList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSigList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSigList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSigList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSigList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeSigList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeSigList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSigList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSigList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoSrfWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoTypWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoReuWidth"));
		var TcoMgtWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoMgtWidth"));
		var TcoMguWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoMguWidth"));
		var TcoVecWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoVecWidth"));
		var TcoConWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoConWidth"));
		var TcoHtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoHtyWidth"));
		var TcoWidWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoWidWidth"));
		var TcoMmxWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSigList", "TcoMmxWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeSigList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoReu"); doc.widths.push(TcoReuWidth); doc.fs.push("reu"); doc.tos.push(4); wtot += TcoReuWidth;
			} else if (i == 5) {
				doc.cols.push("TcoMgt"); doc.widths.push(TcoMgtWidth); doc.fs.push("mgt2"); doc.tos.push(0); wtot += TcoMgtWidth;
			} else if (i == 6) {
				doc.cols.push("TcoMgu"); doc.widths.push(TcoMguWidth); doc.fs.push("mgu"); doc.tos.push(6); wtot += TcoMguWidth;
			} else if (i == 7) {
				doc.cols.push("TcoVec"); doc.widths.push(TcoVecWidth); doc.fs.push("vec"); doc.tos.push(7); wtot += TcoVecWidth;
			} else if (i == 8) {
				doc.cols.push("TcoCon"); doc.widths.push(TcoConWidth); doc.fs.push("con"); doc.tos.push(8); wtot += TcoConWidth;
			} else if (i == 9) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeSigList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSigList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSigList
		rownode = getRownode(srcdoc, "ListWdbeQSigList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSigList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSigList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSigList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSigList", "ListWdbeQSigList", retrieveTi(srcdoc, "TagWdbeSigList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSigList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSigList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSigList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSigList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSigList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSigList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSigListData", scrJref, "StgIacQryWdbeSigList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSigList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSigList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSigList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSigListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeSigList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeSigList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSigListData", scrJref, "ContIacWdbeSigList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "ContIacWdbeSigList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "ContInfWdbeSigList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "StatShrWdbeSigList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "StgIacWdbeSigList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "TagWdbeSigList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "ListWdbeQSigList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "StatAppQryWdbeSigList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "StatShrQryWdbeSigList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSigListData", "StgIacQryWdbeSigList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeSigListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSigListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSigListData") {
				mask = mergeDpchEngData(dom);
