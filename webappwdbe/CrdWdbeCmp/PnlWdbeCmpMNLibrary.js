...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeCmpMNLibrary", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeCmpMNLibrary", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNLibrary_side").src = "./PnlWdbeCmpMNLibrary_aside.html";
	doc.getElementById("MNLibrary_cont").src = "./PnlWdbeCmpMNLibrary_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeCmpMNLibrary", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeCmpMNLibrary", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNLibrary_side").src = "./PnlWdbeCmpMNLibrary_bside.html";
	doc.getElementById("MNLibrary_cont").src = "./PnlWdbeCmpMNLibrary_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCmpMNLibrary", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCmpMNLibrary", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeCmpMNLibrary", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCmpMNLibrary", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCmpMNLibrary", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmpMNLibrary", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmpMNLibrary", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmpMNLibrary", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmpMNLibrary", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmpMNLibrary", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmpMNLibrary", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmpMNLibrary", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmpMNLibrary", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCmpMNLibrary", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeCmpMNLibrary", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCmpMNLibrary", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCmpMNLibrary
		rownode = getRownode(srcdoc, "ListWdbeQCmpMNLibrary", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCmpMNLibrary", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCmpMNLibrary", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCmpMNLibrary", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCmpMNLibrary", "ListWdbeQCmpMNLibrary", retrieveTi(srcdoc, "TagWdbeCmpMNLibrary", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCmpMNLibrary", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmpMNLibrary", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmpMNLibrary", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCmpMNLibrary", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCmpMNLibrary", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCmpMNLibrary", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCmpMNLibraryData", scrJref, "StgIacQryWdbeCmpMNLibrary");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeCmpMNLibrary", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCmpMNLibrary", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCmpMNLibrary", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeCmpMNLibrary", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCmpMNLibrary", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCmpMNLibraryDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "ContInfWdbeCmpMNLibrary", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "StatAppWdbeCmpMNLibrary", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "StatShrWdbeCmpMNLibrary", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "StgIacWdbeCmpMNLibrary", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "TagWdbeCmpMNLibrary", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "ListWdbeQCmpMNLibrary", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "StatAppQryWdbeCmpMNLibrary", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "StatShrQryWdbeCmpMNLibrary", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCmpMNLibraryData", "StgIacQryWdbeCmpMNLibrary", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeCmpMNLibraryData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCmpMNLibraryData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCmpMNLibraryData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
