/**
  * \file PnlWdbePrjList.js
  * web client functionality for panel PnlWdbePrjList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbePrjList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbePrjList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbePrjList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbePrjList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrjList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrjList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbePrjList", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrjList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrjList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrjList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrjList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrjList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrjList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrjList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrjList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrjList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrjList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrjList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrjList", "TcoOwnWidth"));
		var TcoShoWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrjList", "TcoShoWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrjList", "TcoTitWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrjList", "TcoVerWidth"));
		var TcoEsyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrjList", "TcoEsyWidth"));
		var TcoGrlWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrjList", "TcoGrlWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbePrjList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoSho"); doc.widths.push(TcoShoWidth); doc.fs.push("sho"); doc.tos.push(3); wtot += TcoShoWidth;
			} else if (i == 4) {
				doc.cols.push("TcoTit"); doc.widths.push(TcoTitWidth); doc.fs.push("tit"); doc.tos.push(4); wtot += TcoTitWidth;
			} else if (i == 5) {
				doc.cols.push("TcoVer"); doc.widths.push(TcoVerWidth); doc.fs.push("ver"); doc.tos.push(5); wtot += TcoVerWidth;
			} else if (i == 6) {
				doc.cols.push("TcoEsy"); doc.widths.push(TcoEsyWidth); doc.fs.push("esy"); doc.tos.push(0); wtot += TcoEsyWidth;
			} else if (i == 7) {
				doc.cols.push("TcoGrl"); doc.widths.push(TcoGrlWidth); doc.fs.push("grl"); doc.tos.push(0); wtot += TcoGrlWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbePrjList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrjList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrjList
		rownode = getRownode(srcdoc, "ListWdbeQPrjList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrjList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrjList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrjList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrjList", "ListWdbeQPrjList", retrieveTi(srcdoc, "TagWdbePrjList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrjList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrjList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrjList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrjList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrjList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrjList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrjListData", scrJref, "StgIacQryWdbePrjList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrjList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrjList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrjList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrjListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbePrjList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbePrjList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrjListData", scrJref, "ContIacWdbePrjList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "ContIacWdbePrjList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "ContInfWdbePrjList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "StatShrWdbePrjList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "StgIacWdbePrjList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "TagWdbePrjList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "ListWdbeQPrjList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "StatAppQryWdbePrjList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "StatShrQryWdbePrjList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrjListData", "StgIacQryWdbePrjList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbePrjListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrjListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrjListData") {
				mask = mergeDpchEngData(dom);
