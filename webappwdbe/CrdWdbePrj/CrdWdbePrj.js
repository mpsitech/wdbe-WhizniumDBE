/**
  * \file CrdWdbePrj.js
  * web client functionality for card CrdWdbePrj
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWdbePrj", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWdbePrj", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWdbePrj", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWdbePrj", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWdbePrj", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWdbePrjHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWdbePrjList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWdbePrjRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWdbePrj", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWdbePrj", "shortMenu", short);
	setSi(srcdoc, "StatAppWdbePrj", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWdbePrj", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWdbePrj", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWdbePrj", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWdbePrj", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrWdbePrj", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrWdbePrj", "MitCrdNewAvail") == "true");
	MitCrdIpxAvail = (retrieveSi(srcdoc, "StatShrWdbePrj", "MitCrdIpxAvail") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWdbePrj", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdPrj"), retrieveCi(srcdoc, "ContInfWdbePrj", "MtxCrdPrj"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagWdbePrj", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdIpx"), retrieveTi(srcdoc, "TagWdbePrj", "MitCrdIpx"));

	height -= setMitMspAvail("MspCrd1", MspCrd1Avail, 1);
	height -= setMitMspAvail("MitCrdNew", MitCrdNewAvail, 20);
	height -= setMitMspAvail("MitCrdIpx", MitCrdIpxAvail, 20);
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgWdbePrjImpex")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgimpex = retrieveSi(srcdoc, "StatShrWdbePrj", "scrJrefDlgimpex");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrWdbePrj", "scrJrefDlgnew");

	if (scrJrefDlgimpex != "") {
		if (scrJrefDlg != scrJrefDlgimpex) showDlg("DlgWdbePrjImpex", scrJrefDlgimpex);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgWdbePrjNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfWdbePrj", "MtxCrdPrj") + " - WhizniumDBE v1.0.3";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrjDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrjDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWdbePrj", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrjData", "ContInfWdbePrj", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrjData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWdbePrjData", "StatAppWdbePrj", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrjData", "StatShrWdbePrj", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrjData", "TagWdbePrj", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrjData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWdbePrj", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWdbePrj", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWdbePrj", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWdbePrjData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbePrj", "srefIxWdbeVReqitmode");

	if (srefIxWdbeVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWdbeVReqitmode = "notify";
		setSi(srcdoc, "StatAppWdbePrj", "srefIxWdbeVReqitmode", srefIxWdbeVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbePrj", "srefIxWdbeVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWdbePrj", "latency"));
