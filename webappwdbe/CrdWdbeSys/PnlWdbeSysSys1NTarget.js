/**
  * \file PnlWdbeSysSys1NTarget.js
  * web client functionality for panel PnlWdbeSysSys1NTarget
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeSysSys1NTarget", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeSysSys1NTarget", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Sys1NTarget", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Sys1NTarget_side").setAttribute("height", "30");
	doc.getElementById("Sys1NTarget_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Sys1NTarget_side").src = "./PnlWdbeSysSys1NTarget_aside.html";
	doc.getElementById("Sys1NTarget_cont").src = "./PnlWdbeSysSys1NTarget_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeSysSys1NTarget", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeSysSys1NTarget", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Sys1NTarget", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Sys1NTarget_side").setAttribute("height", "255");
	doc.getElementById("Sys1NTarget_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Sys1NTarget_side").src = "./PnlWdbeSysSys1NTarget_bside.html";
	doc.getElementById("Sys1NTarget_cont").src = "./PnlWdbeSysSys1NTarget_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sys1NTarget_side").contentDocument;
	contdoc = doc.getElementById("Sys1NTarget_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Sys1NTarget_side").contentDocument;
	contdoc = doc.getElementById("Sys1NTarget_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sys1NTarget_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sys1NTarget_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sys1NTarget_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSysSys1NTarget", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSysSys1NTarget", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeSysSys1NTarget", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSysSys1NTarget", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSysSys1NTarget", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSysSys1NTarget", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSysSys1NTarget", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSysSys1NTarget", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSysSys1NTarget", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSysSys1NTarget", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSysSys1NTarget", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSysSys1NTarget", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSysSys1NTarget", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSysSys1NTarget", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeSysSys1NTarget", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSysSys1NTarget", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSysSys1NTarget
		rownode = getRownode(srcdoc, "ListWdbeQSysSys1NTarget", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSysSys1NTarget", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSysSys1NTarget", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSysSys1NTarget", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sys1NTarget", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sys1NTarget_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sys1NTarget_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sys1NTarget_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sys1NTarget_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSysSys1NTarget", "ListWdbeQSysSys1NTarget", retrieveTi(srcdoc, "TagWdbeSysSys1NTarget", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSysSys1NTarget", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSysSys1NTarget", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSysSys1NTarget", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSysSys1NTarget", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSysSys1NTarget", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSysSys1NTarget", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSysSys1NTargetData", scrJref, "StgIacQryWdbeSysSys1NTarget");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeSysSys1NTarget", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSysSys1NTarget", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSysSys1NTarget", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeSysSys1NTarget", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSysSys1NTarget", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSysSys1NTargetDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "ContInfWdbeSysSys1NTarget", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "StatAppWdbeSysSys1NTarget", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "StatShrWdbeSysSys1NTarget", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "StgIacWdbeSysSys1NTarget", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "TagWdbeSysSys1NTarget", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "ListWdbeQSysSys1NTarget", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "StatAppQryWdbeSysSys1NTarget", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "StatShrQryWdbeSysSys1NTarget", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSysSys1NTargetData", "StgIacQryWdbeSysSys1NTarget", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeSysSys1NTargetData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSysSys1NTargetData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sys1NTarget");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSysSys1NTargetData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
