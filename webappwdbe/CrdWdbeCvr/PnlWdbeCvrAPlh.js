/**
  * \file PnlWdbeCvrAPlh.js
  * web client functionality for panel PnlWdbeCvrAPlh
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeCvrAPlh", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeCvrAPlh", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("APlh", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("APlh_side").setAttribute("height", "30");
	doc.getElementById("APlh_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("APlh_side").src = "./PnlWdbeCvrAPlh_aside.html";
	doc.getElementById("APlh_cont").src = "./PnlWdbeCvrAPlh_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeCvrAPlh", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeCvrAPlh", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("APlh", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("APlh_side").setAttribute("height", "255");
	doc.getElementById("APlh_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("APlh_side").src = "./PnlWdbeCvrAPlh_bside.html";
	doc.getElementById("APlh_cont").src = "./PnlWdbeCvrAPlh_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("APlh_side").contentDocument;
	contdoc = doc.getElementById("APlh_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("APlh_side").contentDocument;
	contdoc = doc.getElementById("APlh_cont").contentDocument;
	hdrdoc = contdoc.getElementById("APlh_hdr").contentDocument;
	tbldoc = contdoc.getElementById("APlh_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("APlh_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCvrAPlh", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCvrAPlh", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeCvrAPlh", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCvrAPlh", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCvrAPlh", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrAPlh", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAPlh", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAPlh", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrAPlh", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrAPlh", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrAPlh", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAPlh", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAPlh", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrAPlh", "TcoSrfWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrAPlh", "TcoCmtWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeCvrAPlh", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCvrAPlh", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCvrAPlh
		rownode = getRownode(srcdoc, "ListWdbeQCvrAPlh", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCvrAPlh", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCvrAPlh", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCvrAPlh", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("APlh", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("APlh_side").setAttribute("height", "" + (height+55));
	doc.getElementById("APlh_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("APlh_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("APlh_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCvrAPlh", "ListWdbeQCvrAPlh", retrieveTi(srcdoc, "TagWdbeCvrAPlh", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCvrAPlh", "TxtRecord2"),
				["Srf","Cmt"],
				["srf","cmt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrAPlh", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAPlh", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrAPlh", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCvrAPlh", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCvrAPlh", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCvrAPlhData", scrJref, "StgIacQryWdbeCvrAPlh");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeCvrAPlh", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAPlh", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCvrAPlh", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeCvrAPlh", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrAPlh", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCvrAPlhDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "ContInfWdbeCvrAPlh", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "StatAppWdbeCvrAPlh", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "StatShrWdbeCvrAPlh", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "StgIacWdbeCvrAPlh", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "TagWdbeCvrAPlh", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "ListWdbeQCvrAPlh", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "StatAppQryWdbeCvrAPlh", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "StatShrQryWdbeCvrAPlh", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAPlhData", "StgIacQryWdbeCvrAPlh", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeCvrAPlhData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCvrAPlhData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("APlh");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCvrAPlhData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
