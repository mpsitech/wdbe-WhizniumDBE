/**
  * \file PnlWdbeCvrHk1NModule.js
  * web client functionality for panel PnlWdbeCvrHk1NModule
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeCvrHk1NModule", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeCvrHk1NModule", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Hk1NModule_side").src = "./PnlWdbeCvrHk1NModule_aside.html";
	doc.getElementById("Hk1NModule_cont").src = "./PnlWdbeCvrHk1NModule_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeCvrHk1NModule", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeCvrHk1NModule", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Hk1NModule_side").src = "./PnlWdbeCvrHk1NModule_bside.html";
	doc.getElementById("Hk1NModule_cont").src = "./PnlWdbeCvrHk1NModule_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCvrHk1NModule", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCvrHk1NModule", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeCvrHk1NModule", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCvrHk1NModule", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCvrHk1NModule", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrHk1NModule", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrHk1NModule", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrHk1NModule", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrHk1NModule", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrHk1NModule", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrHk1NModule", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrHk1NModule", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrHk1NModule", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrHk1NModule", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeCvrHk1NModule", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCvrHk1NModule", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCvrHk1NModule
		rownode = getRownode(srcdoc, "ListWdbeQCvrHk1NModule", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCvrHk1NModule", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCvrHk1NModule", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCvrHk1NModule", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCvrHk1NModule", "ListWdbeQCvrHk1NModule", retrieveTi(srcdoc, "TagWdbeCvrHk1NModule", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCvrHk1NModule", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrHk1NModule", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrHk1NModule", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrHk1NModule", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCvrHk1NModule", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCvrHk1NModule", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCvrHk1NModuleData", scrJref, "StgIacQryWdbeCvrHk1NModule");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeCvrHk1NModule", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrHk1NModule", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCvrHk1NModule", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeCvrHk1NModule", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrHk1NModule", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCvrHk1NModuleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "ContInfWdbeCvrHk1NModule", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "StatAppWdbeCvrHk1NModule", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "StatShrWdbeCvrHk1NModule", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "StgIacWdbeCvrHk1NModule", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "TagWdbeCvrHk1NModule", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "ListWdbeQCvrHk1NModule", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "StatAppQryWdbeCvrHk1NModule", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "StatShrQryWdbeCvrHk1NModule", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCvrHk1NModuleData", "StgIacQryWdbeCvrHk1NModule", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeCvrHk1NModuleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCvrHk1NModuleData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCvrHk1NModuleData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
