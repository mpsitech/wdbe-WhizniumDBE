/**
  * \file PnlWdbeCvrList.js
  * web client functionality for panel PnlWdbeCvrList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeCvrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeCvrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeCvrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeCvrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCvrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCvrList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeCvrList", "srefIxWdbeVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWdbeCvrList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeCvrList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCvrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCvrList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCvrList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeCvrList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCvrList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeCvrList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeCvrList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrList", "TcoOwnWidth"));
		var TcoCprWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrList", "TcoCprWidth"));
		var TcoMajWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrList", "TcoMajWidth"));
		var TcoMinWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrList", "TcoMinWidth"));
		var TcoSubWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrList", "TcoSubWidth"));
		var TcoBcvWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrList", "TcoBcvWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrList", "TcoSteWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeCvrList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoGrp"); doc.widths.push(TcoGrpWidth); doc.fs.push("grp"); doc.tos.push(5); wtot += TcoGrpWidth;
			} else if (i == 2) {
				doc.cols.push("TcoOwn"); doc.widths.push(TcoOwnWidth); doc.fs.push("own"); doc.tos.push(4); wtot += TcoOwnWidth;
			} else if (i == 3) {
				doc.cols.push("TcoCpr"); doc.widths.push(TcoCprWidth); doc.fs.push("cpr"); doc.tos.push(3); wtot += TcoCprWidth;
			} else if (i == 4) {
				doc.cols.push("TcoMaj"); doc.widths.push(TcoMajWidth); doc.fs.push("maj"); doc.tos.push(0); wtot += TcoMajWidth;
			} else if (i == 5) {
				doc.cols.push("TcoMin"); doc.widths.push(TcoMinWidth); doc.fs.push("min"); doc.tos.push(0); wtot += TcoMinWidth;
			} else if (i == 6) {
				doc.cols.push("TcoSub"); doc.widths.push(TcoSubWidth); doc.fs.push("sub"); doc.tos.push(0); wtot += TcoSubWidth;
			} else if (i == 7) {
				doc.cols.push("TcoBcv"); doc.widths.push(TcoBcvWidth); doc.fs.push("bcv"); doc.tos.push(2); wtot += TcoBcvWidth;
			} else if (i == 8) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeCvrList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCvrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCvrList
		rownode = getRownode(srcdoc, "ListWdbeQCvrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCvrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCvrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCvrList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCvrList", "ListWdbeQCvrList", retrieveTi(srcdoc, "TagWdbeCvrList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCvrList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCvrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCvrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCvrListData", scrJref, "StgIacQryWdbeCvrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCvrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCvrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeCvrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeCvrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCvrListData", scrJref, "ContIacWdbeCvrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "ContIacWdbeCvrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "ContInfWdbeCvrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "StatShrWdbeCvrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "StgIacWdbeCvrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "TagWdbeCvrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "ListWdbeQCvrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "StatAppQryWdbeCvrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "StatShrQryWdbeCvrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCvrListData", "StgIacQryWdbeCvrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeCvrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCvrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCvrListData") {
				mask = mergeDpchEngData(dom);
