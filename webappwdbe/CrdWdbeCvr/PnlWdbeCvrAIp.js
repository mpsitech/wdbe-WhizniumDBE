/**
  * \file PnlWdbeCvrAIp.js
  * web client functionality for panel PnlWdbeCvrAIp
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeCvrAIp", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeCvrAIp", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("AIp", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("AIp_side").setAttribute("height", "30");
	doc.getElementById("AIp_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("AIp_side").src = "./PnlWdbeCvrAIp_aside.html";
	doc.getElementById("AIp_cont").src = "./PnlWdbeCvrAIp_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeCvrAIp", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeCvrAIp", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("AIp", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("AIp_side").setAttribute("height", "255");
	doc.getElementById("AIp_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("AIp_side").src = "./PnlWdbeCvrAIp_bside.html";
	doc.getElementById("AIp_cont").src = "./PnlWdbeCvrAIp_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AIp_side").contentDocument;
	contdoc = doc.getElementById("AIp_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("AIp_side").contentDocument;
	contdoc = doc.getElementById("AIp_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AIp_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AIp_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AIp_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCvrAIp", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCvrAIp", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeCvrAIp", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCvrAIp", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCvrAIp", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrAIp", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAIp", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAIp", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrAIp", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrAIp", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrAIp", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAIp", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAIp", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrAIp", "TcoSrfWidth"));
		var TcoTtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrAIp", "TcoTtyWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCvrAIp", "TcoCmtWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTty"); doc.widths.push(TcoTtyWidth); doc.fs.push("tty"); wtot += TcoTtyWidth;
			} else if (i == 3) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeCvrAIp", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCvrAIp", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCvrAIp
		rownode = getRownode(srcdoc, "ListWdbeQCvrAIp", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCvrAIp", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCvrAIp", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCvrAIp", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AIp", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AIp_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AIp_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AIp_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AIp_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCvrAIp", "ListWdbeQCvrAIp", retrieveTi(srcdoc, "TagWdbeCvrAIp", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCvrAIp", "TxtRecord2"),
				["Srf","Tty","","Cmt"],
				["srf","tty","tty2","cmt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrAIp", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAIp", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCvrAIp", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCvrAIp", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCvrAIp", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCvrAIpData", scrJref, "StgIacQryWdbeCvrAIp");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeCvrAIp", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCvrAIp", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCvrAIp", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeCvrAIp", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCvrAIp", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCvrAIpDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "ContInfWdbeCvrAIp", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "StatAppWdbeCvrAIp", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "StatShrWdbeCvrAIp", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "StgIacWdbeCvrAIp", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "TagWdbeCvrAIp", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "ListWdbeQCvrAIp", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "StatAppQryWdbeCvrAIp", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "StatShrQryWdbeCvrAIp", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCvrAIpData", "StgIacQryWdbeCvrAIp", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeCvrAIpData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCvrAIpData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AIp");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCvrAIpData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
