...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpRef1NVariable", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMtpRef1NVariable", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NVariable", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NVariable_side").setAttribute("height", "30");
	doc.getElementById("Ref1NVariable_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NVariable_side").src = "./PnlWdbeMtpRef1NVariable_aside.html";
	doc.getElementById("Ref1NVariable_cont").src = "./PnlWdbeMtpRef1NVariable_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpRef1NVariable", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMtpRef1NVariable", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NVariable", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NVariable_side").setAttribute("height", "255");
	doc.getElementById("Ref1NVariable_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NVariable_side").src = "./PnlWdbeMtpRef1NVariable_bside.html";
	doc.getElementById("Ref1NVariable_cont").src = "./PnlWdbeMtpRef1NVariable_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NVariable_side").contentDocument;
	contdoc = doc.getElementById("Ref1NVariable_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NVariable_side").contentDocument;
	contdoc = doc.getElementById("Ref1NVariable_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NVariable_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NVariable_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NVariable_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpRef1NVariable", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMtpRef1NVariable", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMtpRef1NVariable", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpRef1NVariable", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NVariable", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpRef1NVariable", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NVariable", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NVariable", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NVariable", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NVariable", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NVariable", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NVariable", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NVariable", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpRef1NVariable", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMtpRef1NVariable", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMtpRef1NVariable", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMtpRef1NVariable
		rownode = getRownode(srcdoc, "ListWdbeQMtpRef1NVariable", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMtpRef1NVariable", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMtpRef1NVariable", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMtpRef1NVariable", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NVariable", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NVariable_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NVariable_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NVariable_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NVariable_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMtpRef1NVariable", "ListWdbeQMtpRef1NVariable", retrieveTi(srcdoc, "TagWdbeMtpRef1NVariable", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMtpRef1NVariable", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpRef1NVariable", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NVariable", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NVariable", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMtpRef1NVariable", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMtpRef1NVariable", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMtpRef1NVariableData", scrJref, "StgIacQryWdbeMtpRef1NVariable");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpRef1NVariable", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NVariable", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMtpRef1NVariable", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpRef1NVariable", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpRef1NVariable", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMtpRef1NVariableDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "ContInfWdbeMtpRef1NVariable", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "StatAppWdbeMtpRef1NVariable", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "StatShrWdbeMtpRef1NVariable", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "StgIacWdbeMtpRef1NVariable", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "TagWdbeMtpRef1NVariable", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "ListWdbeQMtpRef1NVariable", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "StatAppQryWdbeMtpRef1NVariable", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "StatShrQryWdbeMtpRef1NVariable", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NVariableData", "StgIacQryWdbeMtpRef1NVariable", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMtpRef1NVariableData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMtpRef1NVariableData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NVariable");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMtpRef1NVariableData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
