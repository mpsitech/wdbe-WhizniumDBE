...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpHsm1NPipeline", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMtpHsm1NPipeline", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Hsm1NPipeline_side").src = "./PnlWdbeMtpHsm1NPipeline_aside.html";
	doc.getElementById("Hsm1NPipeline_cont").src = "./PnlWdbeMtpHsm1NPipeline_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpHsm1NPipeline", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMtpHsm1NPipeline", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Hsm1NPipeline_side").src = "./PnlWdbeMtpHsm1NPipeline_bside.html";
	doc.getElementById("Hsm1NPipeline_cont").src = "./PnlWdbeMtpHsm1NPipeline_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpHsm1NPipeline", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMtpHsm1NPipeline", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMtpHsm1NPipeline", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpHsm1NPipeline", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMtpHsm1NPipeline", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpHsm1NPipeline", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpHsm1NPipeline", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpHsm1NPipeline", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpHsm1NPipeline", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpHsm1NPipeline", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpHsm1NPipeline", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpHsm1NPipeline", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpHsm1NPipeline", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpHsm1NPipeline", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMtpHsm1NPipeline", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMtpHsm1NPipeline", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMtpHsm1NPipeline
		rownode = getRownode(srcdoc, "ListWdbeQMtpHsm1NPipeline", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMtpHsm1NPipeline", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMtpHsm1NPipeline", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMtpHsm1NPipeline", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMtpHsm1NPipeline", "ListWdbeQMtpHsm1NPipeline", retrieveTi(srcdoc, "TagWdbeMtpHsm1NPipeline", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMtpHsm1NPipeline", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpHsm1NPipeline", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpHsm1NPipeline", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpHsm1NPipeline", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMtpHsm1NPipeline", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMtpHsm1NPipeline", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMtpHsm1NPipelineData", scrJref, "StgIacQryWdbeMtpHsm1NPipeline");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpHsm1NPipeline", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpHsm1NPipeline", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMtpHsm1NPipeline", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpHsm1NPipeline", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpHsm1NPipeline", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMtpHsm1NPipelineDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "ContInfWdbeMtpHsm1NPipeline", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "StatAppWdbeMtpHsm1NPipeline", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "StatShrWdbeMtpHsm1NPipeline", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "StgIacWdbeMtpHsm1NPipeline", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "TagWdbeMtpHsm1NPipeline", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "ListWdbeQMtpHsm1NPipeline", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "StatAppQryWdbeMtpHsm1NPipeline", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "StatShrQryWdbeMtpHsm1NPipeline", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpHsm1NPipelineData", "StgIacQryWdbeMtpHsm1NPipeline", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMtpHsm1NPipelineData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMtpHsm1NPipelineData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMtpHsm1NPipelineData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
