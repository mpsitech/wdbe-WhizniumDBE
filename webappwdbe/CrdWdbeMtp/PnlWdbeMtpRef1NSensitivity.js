...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpRef1NSensitivity", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMtpRef1NSensitivity", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NSensitivity_side").src = "./PnlWdbeMtpRef1NSensitivity_aside.html";
	doc.getElementById("Ref1NSensitivity_cont").src = "./PnlWdbeMtpRef1NSensitivity_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpRef1NSensitivity", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMtpRef1NSensitivity", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NSensitivity_side").src = "./PnlWdbeMtpRef1NSensitivity_bside.html";
	doc.getElementById("Ref1NSensitivity_cont").src = "./PnlWdbeMtpRef1NSensitivity_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpRef1NSensitivity", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMtpRef1NSensitivity", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMtpRef1NSensitivity", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpRef1NSensitivity", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NSensitivity", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpRef1NSensitivity", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NSensitivity", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NSensitivity", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NSensitivity", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NSensitivity", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NSensitivity", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NSensitivity", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NSensitivity", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpRef1NSensitivity", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMtpRef1NSensitivity", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMtpRef1NSensitivity", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMtpRef1NSensitivity
		rownode = getRownode(srcdoc, "ListWdbeQMtpRef1NSensitivity", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMtpRef1NSensitivity", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMtpRef1NSensitivity", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMtpRef1NSensitivity", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMtpRef1NSensitivity", "ListWdbeQMtpRef1NSensitivity", retrieveTi(srcdoc, "TagWdbeMtpRef1NSensitivity", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMtpRef1NSensitivity", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpRef1NSensitivity", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NSensitivity", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NSensitivity", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMtpRef1NSensitivity", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMtpRef1NSensitivity", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMtpRef1NSensitivityData", scrJref, "StgIacQryWdbeMtpRef1NSensitivity");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpRef1NSensitivity", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NSensitivity", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMtpRef1NSensitivity", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpRef1NSensitivity", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpRef1NSensitivity", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMtpRef1NSensitivityDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "ContInfWdbeMtpRef1NSensitivity", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "StatAppWdbeMtpRef1NSensitivity", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "StatShrWdbeMtpRef1NSensitivity", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "StgIacWdbeMtpRef1NSensitivity", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "TagWdbeMtpRef1NSensitivity", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "ListWdbeQMtpRef1NSensitivity", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "StatAppQryWdbeMtpRef1NSensitivity", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "StatShrQryWdbeMtpRef1NSensitivity", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NSensitivityData", "StgIacQryWdbeMtpRef1NSensitivity", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMtpRef1NSensitivityData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMtpRef1NSensitivityData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMtpRef1NSensitivityData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
