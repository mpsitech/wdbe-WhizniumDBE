/**
  * \file PnlWdbeMtpRef1NFile.js
  * web client functionality for panel PnlWdbeMtpRef1NFile
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpRef1NFile", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMtpRef1NFile", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NFile", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NFile_side").setAttribute("height", "30");
	doc.getElementById("Ref1NFile_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlWdbeMtpRef1NFile_aside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlWdbeMtpRef1NFile_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpRef1NFile", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMtpRef1NFile", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NFile", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NFile_side").setAttribute("height", "255");
	doc.getElementById("Ref1NFile_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlWdbeMtpRef1NFile_bside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlWdbeMtpRef1NFile_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NFile_side").contentDocument;
	contdoc = doc.getElementById("Ref1NFile_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NFile_side").contentDocument;
	contdoc = doc.getElementById("Ref1NFile_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NFile_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NFile_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NFile_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpRef1NFile", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMtpRef1NFile", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMtpRef1NFile", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpRef1NFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NFile", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpRef1NFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NFile", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpRef1NFile", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMtpRef1NFile", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMtpRef1NFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMtpRef1NFile
		rownode = getRownode(srcdoc, "ListWdbeQMtpRef1NFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMtpRef1NFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMtpRef1NFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMtpRef1NFile", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NFile", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NFile_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NFile_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NFile_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NFile_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMtpRef1NFile", "ListWdbeQMtpRef1NFile", retrieveTi(srcdoc, "TagWdbeMtpRef1NFile", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMtpRef1NFile", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpRef1NFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpRef1NFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMtpRef1NFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMtpRef1NFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMtpRef1NFileData", scrJref, "StgIacQryWdbeMtpRef1NFile");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpRef1NFile", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpRef1NFile", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMtpRef1NFile", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpRef1NFile", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpRef1NFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMtpRef1NFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "ContInfWdbeMtpRef1NFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "StatAppWdbeMtpRef1NFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "StatShrWdbeMtpRef1NFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "StgIacWdbeMtpRef1NFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "TagWdbeMtpRef1NFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "ListWdbeQMtpRef1NFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "StatAppQryWdbeMtpRef1NFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "StatShrQryWdbeMtpRef1NFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpRef1NFileData", "StgIacQryWdbeMtpRef1NFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMtpRef1NFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMtpRef1NFileData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NFile");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMtpRef1NFileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
