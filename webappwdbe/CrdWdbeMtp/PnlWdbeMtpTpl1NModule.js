/**
  * \file PnlWdbeMtpTpl1NModule.js
  * web client functionality for panel PnlWdbeMtpTpl1NModule
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpTpl1NModule", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMtpTpl1NModule", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Tpl1NModule", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Tpl1NModule_side").setAttribute("height", "30");
	doc.getElementById("Tpl1NModule_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Tpl1NModule_side").src = "./PnlWdbeMtpTpl1NModule_aside.html";
	doc.getElementById("Tpl1NModule_cont").src = "./PnlWdbeMtpTpl1NModule_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpTpl1NModule", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMtpTpl1NModule", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Tpl1NModule", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Tpl1NModule_side").setAttribute("height", "255");
	doc.getElementById("Tpl1NModule_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Tpl1NModule_side").src = "./PnlWdbeMtpTpl1NModule_bside.html";
	doc.getElementById("Tpl1NModule_cont").src = "./PnlWdbeMtpTpl1NModule_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Tpl1NModule_side").contentDocument;
	contdoc = doc.getElementById("Tpl1NModule_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Tpl1NModule_side").contentDocument;
	contdoc = doc.getElementById("Tpl1NModule_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Tpl1NModule_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Tpl1NModule_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Tpl1NModule_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpTpl1NModule", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMtpTpl1NModule", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMtpTpl1NModule", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpTpl1NModule", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMtpTpl1NModule", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpTpl1NModule", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpTpl1NModule", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpTpl1NModule", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpTpl1NModule", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpTpl1NModule", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpTpl1NModule", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpTpl1NModule", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpTpl1NModule", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpTpl1NModule", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMtpTpl1NModule", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMtpTpl1NModule", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMtpTpl1NModule
		rownode = getRownode(srcdoc, "ListWdbeQMtpTpl1NModule", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMtpTpl1NModule", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMtpTpl1NModule", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMtpTpl1NModule", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Tpl1NModule", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Tpl1NModule_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Tpl1NModule_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Tpl1NModule_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Tpl1NModule_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMtpTpl1NModule", "ListWdbeQMtpTpl1NModule", retrieveTi(srcdoc, "TagWdbeMtpTpl1NModule", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMtpTpl1NModule", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpTpl1NModule", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpTpl1NModule", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpTpl1NModule", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMtpTpl1NModule", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMtpTpl1NModule", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMtpTpl1NModuleData", scrJref, "StgIacQryWdbeMtpTpl1NModule");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpTpl1NModule", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpTpl1NModule", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMtpTpl1NModule", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpTpl1NModule", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpTpl1NModule", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMtpTpl1NModuleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "ContInfWdbeMtpTpl1NModule", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "StatAppWdbeMtpTpl1NModule", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "StatShrWdbeMtpTpl1NModule", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "StgIacWdbeMtpTpl1NModule", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "TagWdbeMtpTpl1NModule", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "ListWdbeQMtpTpl1NModule", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "StatAppQryWdbeMtpTpl1NModule", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "StatShrQryWdbeMtpTpl1NModule", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpTpl1NModuleData", "StgIacQryWdbeMtpTpl1NModule", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMtpTpl1NModuleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMtpTpl1NModuleData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Tpl1NModule");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMtpTpl1NModuleData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
