/**
  * \file PnlWdbeMtpKParKey.js
  * web client functionality for panel PnlWdbeMtpKParKey
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpKParKey", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMtpKParKey", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("KParKey", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("KParKey_side").setAttribute("height", "30");
	doc.getElementById("KParKey_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("KParKey_side").src = "./PnlWdbeMtpKParKey_aside.html";
	doc.getElementById("KParKey_cont").src = "./PnlWdbeMtpKParKey_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMtpKParKey", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMtpKParKey", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("KParKey", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("KParKey_side").setAttribute("height", "255");
	doc.getElementById("KParKey_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("KParKey_side").src = "./PnlWdbeMtpKParKey_bside.html";
	doc.getElementById("KParKey_cont").src = "./PnlWdbeMtpKParKey_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KParKey_side").contentDocument;
	contdoc = doc.getElementById("KParKey_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("KParKey_side").contentDocument;
	contdoc = doc.getElementById("KParKey_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KParKey_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KParKey_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KParKey_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpKParKey", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMtpKParKey", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMtpKParKey", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMtpKParKey", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMtpKParKey", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpKParKey", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpKParKey", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpKParKey", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpKParKey", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpKParKey", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpKParKey", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpKParKey", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpKParKey", "ncol"));

		var TcoFixWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpKParKey", "TcoFixWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpKParKey", "TcoSrfWidth"));
		var TcoAvlWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpKParKey", "TcoAvlWidth"));
		var TcoImpWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpKParKey", "TcoImpWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpKParKey", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMtpKParKey", "TcoCmtWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMtpKParKey", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMtpKParKey", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMtpKParKey
		rownode = getRownode(srcdoc, "ListWdbeQMtpKParKey", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMtpKParKey", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMtpKParKey", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMtpKParKey", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KParKey", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KParKey_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KParKey_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KParKey_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KParKey_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMtpKParKey", "ListWdbeQMtpKParKey", retrieveTi(srcdoc, "TagWdbeMtpKParKey", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMtpKParKey", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpKParKey", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpKParKey", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMtpKParKey", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMtpKParKey", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMtpKParKey", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMtpKParKeyData", scrJref, "StgIacQryWdbeMtpKParKey");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpKParKey", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMtpKParKey", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMtpKParKey", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMtpKParKey", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMtpKParKey", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMtpKParKeyDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "ContInfWdbeMtpKParKey", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "StatAppWdbeMtpKParKey", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "StatShrWdbeMtpKParKey", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "StgIacWdbeMtpKParKey", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "TagWdbeMtpKParKey", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "ListWdbeQMtpKParKey", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "StatAppQryWdbeMtpKParKey", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "StatShrQryWdbeMtpKParKey", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMtpKParKeyData", "StgIacQryWdbeMtpKParKey", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMtpKParKeyData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMtpKParKeyData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KParKey");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMtpKParKeyData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
