...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePrtSrc1NSensitivity", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePrtSrc1NSensitivity", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Src1NSensitivity", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Src1NSensitivity_side").setAttribute("height", "30");
	doc.getElementById("Src1NSensitivity_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Src1NSensitivity_side").src = "./PnlWdbePrtSrc1NSensitivity_aside.html";
	doc.getElementById("Src1NSensitivity_cont").src = "./PnlWdbePrtSrc1NSensitivity_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePrtSrc1NSensitivity", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePrtSrc1NSensitivity", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Src1NSensitivity", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Src1NSensitivity_side").setAttribute("height", "255");
	doc.getElementById("Src1NSensitivity_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Src1NSensitivity_side").src = "./PnlWdbePrtSrc1NSensitivity_bside.html";
	doc.getElementById("Src1NSensitivity_cont").src = "./PnlWdbePrtSrc1NSensitivity_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Src1NSensitivity_side").contentDocument;
	contdoc = doc.getElementById("Src1NSensitivity_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Src1NSensitivity_side").contentDocument;
	contdoc = doc.getElementById("Src1NSensitivity_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Src1NSensitivity_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Src1NSensitivity_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Src1NSensitivity_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrtSrc1NSensitivity", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrtSrc1NSensitivity", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePrtSrc1NSensitivity", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrtSrc1NSensitivity", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrtSrc1NSensitivity", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrtSrc1NSensitivity", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtSrc1NSensitivity", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtSrc1NSensitivity", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrtSrc1NSensitivity", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrtSrc1NSensitivity", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrtSrc1NSensitivity", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtSrc1NSensitivity", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtSrc1NSensitivity", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrtSrc1NSensitivity", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePrtSrc1NSensitivity", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrtSrc1NSensitivity", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrtSrc1NSensitivity
		rownode = getRownode(srcdoc, "ListWdbeQPrtSrc1NSensitivity", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrtSrc1NSensitivity", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrtSrc1NSensitivity", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrtSrc1NSensitivity", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Src1NSensitivity", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Src1NSensitivity_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Src1NSensitivity_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Src1NSensitivity_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Src1NSensitivity_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrtSrc1NSensitivity", "ListWdbeQPrtSrc1NSensitivity", retrieveTi(srcdoc, "TagWdbePrtSrc1NSensitivity", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrtSrc1NSensitivity", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrtSrc1NSensitivity", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtSrc1NSensitivity", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrtSrc1NSensitivity", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrtSrc1NSensitivity", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrtSrc1NSensitivity", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrtSrc1NSensitivityData", scrJref, "StgIacQryWdbePrtSrc1NSensitivity");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePrtSrc1NSensitivity", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtSrc1NSensitivity", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrtSrc1NSensitivity", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePrtSrc1NSensitivity", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrtSrc1NSensitivity", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrtSrc1NSensitivityDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "ContInfWdbePrtSrc1NSensitivity", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "StatAppWdbePrtSrc1NSensitivity", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "StatShrWdbePrtSrc1NSensitivity", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "StgIacWdbePrtSrc1NSensitivity", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "TagWdbePrtSrc1NSensitivity", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "ListWdbeQPrtSrc1NSensitivity", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "StatAppQryWdbePrtSrc1NSensitivity", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "StatShrQryWdbePrtSrc1NSensitivity", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrtSrc1NSensitivityData", "StgIacQryWdbePrtSrc1NSensitivity", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePrtSrc1NSensitivityData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrtSrc1NSensitivityData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Src1NSensitivity");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrtSrc1NSensitivityData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
