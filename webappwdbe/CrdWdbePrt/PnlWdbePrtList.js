...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbePrtList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbePrtList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbePrtList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbePrtList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrtList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrtList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbePrtList", "srefIxWdbeVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWdbePrtList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbePrtList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrtList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrtList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrtList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbePrtList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrtList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrtList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrtList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrtList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrtList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbePrtList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbePrtList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrtList", "TcoSrfWidth"));
		var TcoMdlWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrtList", "TcoMdlWidth"));
		var TcoMdcWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrtList", "TcoMdcWidth"));
		var TcoDirWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrtList", "TcoDirWidth"));
		var TcoHtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrtList", "TcoHtyWidth"));
		var TcoWidWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrtList", "TcoWidWidth"));
		var TcoMmxWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrtList", "TcoMmxWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbePrtList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoDir"); doc.widths.push(TcoDirWidth); doc.fs.push("dir2"); doc.tos.push(0); wtot += TcoDirWidth;
			} else if (i == 5) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbePrtList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrtList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrtList
		rownode = getRownode(srcdoc, "ListWdbeQPrtList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrtList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrtList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrtList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrtList", "ListWdbeQPrtList", retrieveTi(srcdoc, "TagWdbePrtList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrtList", "TxtRecord2"),
				["Srf","Mdl","","Mdc","","Dir","","Hty","Wid","Mmx"],
				["srf","mdl","mdc","mdc2","dir","dir2","hty","hty2","wid","mmx"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrtList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrtList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrtList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrtList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrtListData", scrJref, "StgIacQryWdbePrtList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrtList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrtList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrtList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrtListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbePrtList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbePrtList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrtListData", scrJref, "ContIacWdbePrtList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "ContIacWdbePrtList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "ContInfWdbePrtList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "StatShrWdbePrtList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "StgIacWdbePrtList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "TagWdbePrtList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "ListWdbeQPrtList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "StatAppQryWdbePrtList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "StatShrQryWdbePrtList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrtListData", "StgIacQryWdbePrtList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbePrtListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrtListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrtListData") {
				mask = mergeDpchEngData(dom);
