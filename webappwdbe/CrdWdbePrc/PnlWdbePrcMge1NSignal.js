/**
  * \file PnlWdbePrcMge1NSignal.js
  * web client functionality for panel PnlWdbePrcMge1NSignal
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePrcMge1NSignal", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePrcMge1NSignal", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Mge1NSignal_side").src = "./PnlWdbePrcMge1NSignal_aside.html";
	doc.getElementById("Mge1NSignal_cont").src = "./PnlWdbePrcMge1NSignal_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePrcMge1NSignal", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePrcMge1NSignal", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Mge1NSignal_side").src = "./PnlWdbePrcMge1NSignal_bside.html";
	doc.getElementById("Mge1NSignal_cont").src = "./PnlWdbePrcMge1NSignal_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcMge1NSignal", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrcMge1NSignal", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePrcMge1NSignal", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcMge1NSignal", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrcMge1NSignal", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcMge1NSignal", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcMge1NSignal", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcMge1NSignal", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcMge1NSignal", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcMge1NSignal", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcMge1NSignal", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcMge1NSignal", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcMge1NSignal", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrcMge1NSignal", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePrcMge1NSignal", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrcMge1NSignal", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrcMge1NSignal
		rownode = getRownode(srcdoc, "ListWdbeQPrcMge1NSignal", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrcMge1NSignal", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrcMge1NSignal", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrcMge1NSignal", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrcMge1NSignal", "ListWdbeQPrcMge1NSignal", retrieveTi(srcdoc, "TagWdbePrcMge1NSignal", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrcMge1NSignal", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcMge1NSignal", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcMge1NSignal", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcMge1NSignal", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrcMge1NSignal", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrcMge1NSignal", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrcMge1NSignalData", scrJref, "StgIacQryWdbePrcMge1NSignal");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcMge1NSignal", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcMge1NSignal", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrcMge1NSignal", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcMge1NSignal", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcMge1NSignal", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrcMge1NSignalDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "ContInfWdbePrcMge1NSignal", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "StatAppWdbePrcMge1NSignal", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "StatShrWdbePrcMge1NSignal", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "StgIacWdbePrcMge1NSignal", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "TagWdbePrcMge1NSignal", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "ListWdbeQPrcMge1NSignal", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "StatAppQryWdbePrcMge1NSignal", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "StatShrQryWdbePrcMge1NSignal", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcMge1NSignalData", "StgIacQryWdbePrcMge1NSignal", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePrcMge1NSignalData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrcMge1NSignalData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrcMge1NSignalData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
