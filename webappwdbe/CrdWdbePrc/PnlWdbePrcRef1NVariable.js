...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePrcRef1NVariable", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePrcRef1NVariable", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NVariable_side").src = "./PnlWdbePrcRef1NVariable_aside.html";
	doc.getElementById("Ref1NVariable_cont").src = "./PnlWdbePrcRef1NVariable_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePrcRef1NVariable", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePrcRef1NVariable", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NVariable_side").src = "./PnlWdbePrcRef1NVariable_bside.html";
	doc.getElementById("Ref1NVariable_cont").src = "./PnlWdbePrcRef1NVariable_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcRef1NVariable", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrcRef1NVariable", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePrcRef1NVariable", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcRef1NVariable", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NVariable", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcRef1NVariable", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NVariable", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NVariable", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NVariable", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NVariable", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NVariable", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NVariable", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NVariable", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrcRef1NVariable", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePrcRef1NVariable", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrcRef1NVariable", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrcRef1NVariable
		rownode = getRownode(srcdoc, "ListWdbeQPrcRef1NVariable", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrcRef1NVariable", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrcRef1NVariable", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrcRef1NVariable", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrcRef1NVariable", "ListWdbeQPrcRef1NVariable", retrieveTi(srcdoc, "TagWdbePrcRef1NVariable", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrcRef1NVariable", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcRef1NVariable", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NVariable", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NVariable", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrcRef1NVariable", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrcRef1NVariable", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrcRef1NVariableData", scrJref, "StgIacQryWdbePrcRef1NVariable");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcRef1NVariable", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NVariable", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrcRef1NVariable", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcRef1NVariable", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcRef1NVariable", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrcRef1NVariableDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "ContInfWdbePrcRef1NVariable", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "StatAppWdbePrcRef1NVariable", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "StatShrWdbePrcRef1NVariable", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "StgIacWdbePrcRef1NVariable", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "TagWdbePrcRef1NVariable", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "ListWdbeQPrcRef1NVariable", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "StatAppQryWdbePrcRef1NVariable", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "StatShrQryWdbePrcRef1NVariable", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NVariableData", "StgIacQryWdbePrcRef1NVariable", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePrcRef1NVariableData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrcRef1NVariableData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrcRef1NVariableData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
