...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePrcRef1NSensitivity", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePrcRef1NSensitivity", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NSensitivity", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NSensitivity_side").setAttribute("height", "30");
	doc.getElementById("Ref1NSensitivity_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NSensitivity_side").src = "./PnlWdbePrcRef1NSensitivity_aside.html";
	doc.getElementById("Ref1NSensitivity_cont").src = "./PnlWdbePrcRef1NSensitivity_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePrcRef1NSensitivity", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePrcRef1NSensitivity", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NSensitivity", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NSensitivity_side").setAttribute("height", "255");
	doc.getElementById("Ref1NSensitivity_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NSensitivity_side").src = "./PnlWdbePrcRef1NSensitivity_bside.html";
	doc.getElementById("Ref1NSensitivity_cont").src = "./PnlWdbePrcRef1NSensitivity_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NSensitivity_side").contentDocument;
	contdoc = doc.getElementById("Ref1NSensitivity_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NSensitivity_side").contentDocument;
	contdoc = doc.getElementById("Ref1NSensitivity_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NSensitivity_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NSensitivity_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NSensitivity_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcRef1NSensitivity", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrcRef1NSensitivity", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePrcRef1NSensitivity", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcRef1NSensitivity", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NSensitivity", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcRef1NSensitivity", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NSensitivity", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NSensitivity", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NSensitivity", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NSensitivity", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NSensitivity", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NSensitivity", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NSensitivity", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrcRef1NSensitivity", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePrcRef1NSensitivity", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrcRef1NSensitivity", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrcRef1NSensitivity
		rownode = getRownode(srcdoc, "ListWdbeQPrcRef1NSensitivity", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrcRef1NSensitivity", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrcRef1NSensitivity", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrcRef1NSensitivity", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NSensitivity", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NSensitivity_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NSensitivity_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NSensitivity_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NSensitivity_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrcRef1NSensitivity", "ListWdbeQPrcRef1NSensitivity", retrieveTi(srcdoc, "TagWdbePrcRef1NSensitivity", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrcRef1NSensitivity", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcRef1NSensitivity", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NSensitivity", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcRef1NSensitivity", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrcRef1NSensitivity", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrcRef1NSensitivity", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrcRef1NSensitivityData", scrJref, "StgIacQryWdbePrcRef1NSensitivity");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcRef1NSensitivity", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcRef1NSensitivity", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrcRef1NSensitivity", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcRef1NSensitivity", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcRef1NSensitivity", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrcRef1NSensitivityDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "ContInfWdbePrcRef1NSensitivity", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "StatAppWdbePrcRef1NSensitivity", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "StatShrWdbePrcRef1NSensitivity", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "StgIacWdbePrcRef1NSensitivity", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "TagWdbePrcRef1NSensitivity", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "ListWdbeQPrcRef1NSensitivity", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "StatAppQryWdbePrcRef1NSensitivity", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "StatShrQryWdbePrcRef1NSensitivity", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcRef1NSensitivityData", "StgIacQryWdbePrcRef1NSensitivity", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePrcRef1NSensitivityData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrcRef1NSensitivityData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NSensitivity");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrcRef1NSensitivityData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
