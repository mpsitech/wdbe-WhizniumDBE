...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePrcFsmHk1NVector", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePrcFsmHk1NVector", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("FsmHk1NVector", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("FsmHk1NVector_side").setAttribute("height", "30");
	doc.getElementById("FsmHk1NVector_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("FsmHk1NVector_side").src = "./PnlWdbePrcFsmHk1NVector_aside.html";
	doc.getElementById("FsmHk1NVector_cont").src = "./PnlWdbePrcFsmHk1NVector_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePrcFsmHk1NVector", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePrcFsmHk1NVector", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("FsmHk1NVector", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("FsmHk1NVector_side").setAttribute("height", "255");
	doc.getElementById("FsmHk1NVector_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("FsmHk1NVector_side").src = "./PnlWdbePrcFsmHk1NVector_bside.html";
	doc.getElementById("FsmHk1NVector_cont").src = "./PnlWdbePrcFsmHk1NVector_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("FsmHk1NVector_side").contentDocument;
	contdoc = doc.getElementById("FsmHk1NVector_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("FsmHk1NVector_side").contentDocument;
	contdoc = doc.getElementById("FsmHk1NVector_cont").contentDocument;
	hdrdoc = contdoc.getElementById("FsmHk1NVector_hdr").contentDocument;
	tbldoc = contdoc.getElementById("FsmHk1NVector_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("FsmHk1NVector_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcFsmHk1NVector", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrcFsmHk1NVector", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePrcFsmHk1NVector", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcFsmHk1NVector", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrcFsmHk1NVector", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcFsmHk1NVector", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmHk1NVector", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmHk1NVector", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcFsmHk1NVector", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcFsmHk1NVector", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcFsmHk1NVector", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmHk1NVector", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmHk1NVector", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrcFsmHk1NVector", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePrcFsmHk1NVector", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrcFsmHk1NVector", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrcFsmHk1NVector
		rownode = getRownode(srcdoc, "ListWdbeQPrcFsmHk1NVector", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrcFsmHk1NVector", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrcFsmHk1NVector", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrcFsmHk1NVector", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("FsmHk1NVector", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("FsmHk1NVector_side").setAttribute("height", "" + (height+55));
	doc.getElementById("FsmHk1NVector_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("FsmHk1NVector_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("FsmHk1NVector_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrcFsmHk1NVector", "ListWdbeQPrcFsmHk1NVector", retrieveTi(srcdoc, "TagWdbePrcFsmHk1NVector", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrcFsmHk1NVector", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcFsmHk1NVector", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmHk1NVector", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcFsmHk1NVector", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrcFsmHk1NVector", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrcFsmHk1NVector", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrcFsmHk1NVectorData", scrJref, "StgIacQryWdbePrcFsmHk1NVector");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcFsmHk1NVector", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmHk1NVector", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrcFsmHk1NVector", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcFsmHk1NVector", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcFsmHk1NVector", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrcFsmHk1NVectorDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "ContInfWdbePrcFsmHk1NVector", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "StatAppWdbePrcFsmHk1NVector", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "StatShrWdbePrcFsmHk1NVector", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "StgIacWdbePrcFsmHk1NVector", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "TagWdbePrcFsmHk1NVector", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "ListWdbeQPrcFsmHk1NVector", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "StatAppQryWdbePrcFsmHk1NVector", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "StatShrQryWdbePrcFsmHk1NVector", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmHk1NVectorData", "StgIacQryWdbePrcFsmHk1NVector", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePrcFsmHk1NVectorData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrcFsmHk1NVectorData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("FsmHk1NVector");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrcFsmHk1NVectorData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
