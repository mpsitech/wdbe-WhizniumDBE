/**
  * \file PnlWdbePrcFsmFsm1NFsmstate.js
  * web client functionality for panel PnlWdbePrcFsmFsm1NFsmstate
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePrcFsmFsm1NFsmstate", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePrcFsmFsm1NFsmstate", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("FsmFsm1NFsmstate", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("FsmFsm1NFsmstate_side").setAttribute("height", "30");
	doc.getElementById("FsmFsm1NFsmstate_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("FsmFsm1NFsmstate_side").src = "./PnlWdbePrcFsmFsm1NFsmstate_aside.html";
	doc.getElementById("FsmFsm1NFsmstate_cont").src = "./PnlWdbePrcFsmFsm1NFsmstate_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePrcFsmFsm1NFsmstate", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePrcFsmFsm1NFsmstate", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("FsmFsm1NFsmstate", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("FsmFsm1NFsmstate_side").setAttribute("height", "255");
	doc.getElementById("FsmFsm1NFsmstate_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("FsmFsm1NFsmstate_side").src = "./PnlWdbePrcFsmFsm1NFsmstate_bside.html";
	doc.getElementById("FsmFsm1NFsmstate_cont").src = "./PnlWdbePrcFsmFsm1NFsmstate_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("FsmFsm1NFsmstate_side").contentDocument;
	contdoc = doc.getElementById("FsmFsm1NFsmstate_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("FsmFsm1NFsmstate_side").contentDocument;
	contdoc = doc.getElementById("FsmFsm1NFsmstate_cont").contentDocument;
	hdrdoc = contdoc.getElementById("FsmFsm1NFsmstate_hdr").contentDocument;
	tbldoc = contdoc.getElementById("FsmFsm1NFsmstate_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("FsmFsm1NFsmstate_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcFsmFsm1NFsmstate", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrcFsmFsm1NFsmstate", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePrcFsmFsm1NFsmstate", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrcFsmFsm1NFsmstate", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrcFsmFsm1NFsmstate", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcFsmFsm1NFsmstate", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmFsm1NFsmstate", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmFsm1NFsmstate", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcFsmFsm1NFsmstate", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcFsmFsm1NFsmstate", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcFsmFsm1NFsmstate", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmFsm1NFsmstate", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmFsm1NFsmstate", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrcFsmFsm1NFsmstate", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePrcFsmFsm1NFsmstate", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrcFsmFsm1NFsmstate", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrcFsmFsm1NFsmstate
		rownode = getRownode(srcdoc, "ListWdbeQPrcFsmFsm1NFsmstate", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrcFsmFsm1NFsmstate", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrcFsmFsm1NFsmstate", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrcFsmFsm1NFsmstate", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("FsmFsm1NFsmstate", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("FsmFsm1NFsmstate_side").setAttribute("height", "" + (height+55));
	doc.getElementById("FsmFsm1NFsmstate_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("FsmFsm1NFsmstate_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("FsmFsm1NFsmstate_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrcFsmFsm1NFsmstate", "ListWdbeQPrcFsmFsm1NFsmstate", retrieveTi(srcdoc, "TagWdbePrcFsmFsm1NFsmstate", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrcFsmFsm1NFsmstate", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcFsmFsm1NFsmstate", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmFsm1NFsmstate", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrcFsmFsm1NFsmstate", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrcFsmFsm1NFsmstate", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrcFsmFsm1NFsmstate", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrcFsmFsm1NFsmstateData", scrJref, "StgIacQryWdbePrcFsmFsm1NFsmstate");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcFsmFsm1NFsmstate", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrcFsmFsm1NFsmstate", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrcFsmFsm1NFsmstate", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePrcFsmFsm1NFsmstate", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrcFsmFsm1NFsmstate", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrcFsmFsm1NFsmstateDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "ContInfWdbePrcFsmFsm1NFsmstate", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "StatAppWdbePrcFsmFsm1NFsmstate", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "StatShrWdbePrcFsmFsm1NFsmstate", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "StgIacWdbePrcFsmFsm1NFsmstate", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "TagWdbePrcFsmFsm1NFsmstate", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "ListWdbeQPrcFsmFsm1NFsmstate", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "StatAppQryWdbePrcFsmFsm1NFsmstate", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "StatShrQryWdbePrcFsmFsm1NFsmstate", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrcFsmFsm1NFsmstateData", "StgIacQryWdbePrcFsmFsm1NFsmstate", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePrcFsmFsm1NFsmstateData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrcFsmFsm1NFsmstateData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("FsmFsm1NFsmstate");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrcFsmFsm1NFsmstateData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
