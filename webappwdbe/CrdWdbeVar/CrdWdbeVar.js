/**
  * \file CrdWdbeVar.js
  * web client functionality for card CrdWdbeVar
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWdbeVar", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWdbeVar", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWdbeVar", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWdbeVar", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWdbeVar", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWdbeVarHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWdbeVarList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWdbeVarRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWdbeVar", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWdbeVar", "shortMenu", short);
	setSi(srcdoc, "StatAppWdbeVar", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWdbeVar", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWdbeVar", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWdbeVar", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWdbeVar", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdVar"), retrieveCi(srcdoc, "ContInfWdbeVar", "MtxCrdVar"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWdbeVar", "MtxCrdVar") + " - WhizniumDBE v1.0.3";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeVarDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeVarDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWdbeVar", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeVarData", "ContInfWdbeVar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeVarData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWdbeVarData", "StatAppWdbeVar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeVarData", "StatShrWdbeVar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeVarData", "TagWdbeVar", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeVarData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWdbeVar", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWdbeVar", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWdbeVar", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWdbeVarData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeVar", "srefIxWdbeVReqitmode");

	if (srefIxWdbeVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWdbeVReqitmode = "notify";
		setSi(srcdoc, "StatAppWdbeVar", "srefIxWdbeVReqitmode", srefIxWdbeVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeVar", "srefIxWdbeVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWdbeVar", "latency"));
