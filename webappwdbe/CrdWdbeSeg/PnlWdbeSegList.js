...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeSegList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeSegList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeSegList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeSegList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSegList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeSegList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeSegList", "srefIxWdbeVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWdbeSegList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeSegList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSegList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSegList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeSegList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeSegList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeSegList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSegList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSegList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSegList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSegList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSegList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSegList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeSegList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeSegList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSegList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSegList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSegList", "TcoSrfWidth"));
		var TcoPplWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSegList", "TcoPplWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSegList", "TcoSupWidth"));
		var TcoIfmWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSegList", "TcoIfmWidth"));
		var TcoOfmWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSegList", "TcoOfmWidth"));
		var TcoLcyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeSegList", "TcoLcyWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeSegList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoOfm"); doc.widths.push(TcoOfmWidth); doc.fs.push("ofm"); doc.tos.push(0); wtot += TcoOfmWidth;
			} else if (i == 6) {
				doc.cols.push("TcoLcy"); doc.widths.push(TcoLcyWidth); doc.fs.push("lcy"); doc.tos.push(0); wtot += TcoLcyWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeSegList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeSegList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQSegList
		rownode = getRownode(srcdoc, "ListWdbeQSegList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeSegList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeSegList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeSegList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeSegList", "ListWdbeQSegList", retrieveTi(srcdoc, "TagWdbeSegList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeSegList", "TxtRecord2"),
				["Srf","Ppl","Sup","Ifm","Ofm","Lcy"],
				["srf","ppl","sup","ifm","ofm","lcy"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSegList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSegList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeSegList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeSegList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeSegList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSegListData", scrJref, "StgIacQryWdbeSegList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeSegList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeSegList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeSegList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeSegListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeSegList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeSegList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeSegListData", scrJref, "ContIacWdbeSegList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "ContIacWdbeSegList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "ContInfWdbeSegList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "StatShrWdbeSegList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "StgIacWdbeSegList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "TagWdbeSegList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "ListWdbeQSegList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "StatAppQryWdbeSegList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "StatShrQryWdbeSegList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeSegListData", "StgIacQryWdbeSegList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeSegListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeSegListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeSegListData") {
				mask = mergeDpchEngData(dom);
