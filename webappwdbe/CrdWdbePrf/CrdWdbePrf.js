function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWdbePrf", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWdbePrf", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneDaemon = (retrieveSi(srcdoc, "StatAppWdbePrf", "initdoneDaemon") == "true");
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWdbePrf", "initdoneHeadbar") == "true");
	var initdoneLicense = (retrieveSi(srcdoc, "StatAppWdbePrf", "initdoneLicense") == "true");

	if (!initdoneDaemon) {
		doc.getElementById("Daemon").src = "./PnlWdbePrfDaemon.html?scrJref=" + scrJrefDaemon;
	} else if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWdbePrfHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneLicense) {
		doc.getElementById("License").src = "./PnlWdbePrfLicense.html?scrJref=" + scrJrefLicense;
	} else {
		doc.initdone = true;
...
function getHeight() {
	...
	height += 10 + parseInt(doc.getElementById("tdLicense").getAttribute("height")) + 8;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWdbePrf", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWdbePrf", "shortMenu", short);
	setSi(srcdoc, "StatAppWdbePrf", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWdbePrf", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWdbePrf", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWdbePrf", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWdbePrf", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MitCrdRvr"), retrieveTi(srcdoc, "TagWdbePrf", "MitCrdRvr"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdSto"), retrieveTi(srcdoc, "TagWdbePrf", "MitCrdSto"));
};
...
function changeHeight(pnlshort, height) {
	...
	} else if (pnlshort == "License") {
		doc.getElementById("tdLicense").setAttribute("height", "" + height);
		doc.getElementById("License").setAttribute("height", "" + height);
	};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrfDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrfDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWdbePrf", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrfData", "ContInfWdbePrf", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrfData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWdbePrfData", "StatAppWdbePrf", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrfData", "StatShrWdbePrf", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrfData", "TagWdbePrf", srcdoc)) mask.push("tag");
...
function handleDpchEngSub(_scrJref, dom, dpch) {
	...
	} else if (_scrJref == scrJrefLicense) {
		doc.getElementById("License").contentWindow.handleDpchEng(dom, dpch);
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrfData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefDaemon = retrieveSi(srcdoc, "StatShrWdbePrf", "scrJrefDaemon");
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWdbePrf", "scrJrefHeadbar");
				scrJrefLicense = retrieveSi(srcdoc, "StatShrWdbePrf", "scrJrefLicense");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWdbePrfData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbePrf", "srefIxWdbeVReqitmode");

	if (srefIxWdbeVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWdbeVReqitmode = "notify";
		setSi(srcdoc, "StatAppWdbePrf", "srefIxWdbeVReqitmode", srefIxWdbeVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbePrf", "srefIxWdbeVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWdbePrf", "latency"));
