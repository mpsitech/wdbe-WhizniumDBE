/**
  * \file PnlWdbeUntHk1NModule.js
  * web client functionality for panel PnlWdbeUntHk1NModule
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUntHk1NModule", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUntHk1NModule", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Hk1NModule", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Hk1NModule_side").setAttribute("height", "30");
	doc.getElementById("Hk1NModule_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Hk1NModule_side").src = "./PnlWdbeUntHk1NModule_aside.html";
	doc.getElementById("Hk1NModule_cont").src = "./PnlWdbeUntHk1NModule_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUntHk1NModule", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUntHk1NModule", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Hk1NModule", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Hk1NModule_side").setAttribute("height", "255");
	doc.getElementById("Hk1NModule_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Hk1NModule_side").src = "./PnlWdbeUntHk1NModule_bside.html";
	doc.getElementById("Hk1NModule_cont").src = "./PnlWdbeUntHk1NModule_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Hk1NModule_side").contentDocument;
	contdoc = doc.getElementById("Hk1NModule_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Hk1NModule_side").contentDocument;
	contdoc = doc.getElementById("Hk1NModule_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Hk1NModule_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Hk1NModule_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Hk1NModule_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntHk1NModule", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUntHk1NModule", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUntHk1NModule", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntHk1NModule", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NModule", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntHk1NModule", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NModule", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NModule", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NModule", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NModule", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NModule", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NModule", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NModule", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntHk1NModule", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUntHk1NModule", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUntHk1NModule", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUntHk1NModule
		rownode = getRownode(srcdoc, "ListWdbeQUntHk1NModule", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUntHk1NModule", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUntHk1NModule", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUntHk1NModule", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Hk1NModule", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Hk1NModule_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Hk1NModule_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Hk1NModule_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Hk1NModule_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUntHk1NModule", "ListWdbeQUntHk1NModule", retrieveTi(srcdoc, "TagWdbeUntHk1NModule", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUntHk1NModule", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntHk1NModule", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NModule", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NModule", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUntHk1NModule", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUntHk1NModule", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUntHk1NModuleData", scrJref, "StgIacQryWdbeUntHk1NModule");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntHk1NModule", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NModule", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUntHk1NModule", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntHk1NModule", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntHk1NModule", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUntHk1NModuleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "ContInfWdbeUntHk1NModule", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "StatAppWdbeUntHk1NModule", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "StatShrWdbeUntHk1NModule", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "StgIacWdbeUntHk1NModule", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "TagWdbeUntHk1NModule", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "ListWdbeQUntHk1NModule", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "StatAppQryWdbeUntHk1NModule", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "StatShrQryWdbeUntHk1NModule", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NModuleData", "StgIacQryWdbeUntHk1NModule", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUntHk1NModuleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUntHk1NModuleData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Hk1NModule");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUntHk1NModuleData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
