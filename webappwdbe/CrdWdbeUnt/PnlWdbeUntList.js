...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeUntList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeUntList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeUntList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeUntList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUntList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeUntList", "srefIxWdbeVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWdbeUntList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeUntList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUntList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeUntList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUntList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeUntList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeUntList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoTitWidth"));
		var TcoFsrWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoFsrWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoTypWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoReuWidth"));
		var TcoSysWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoSysWidth"));
		var TcoMdlWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoMdlWidth"));
		var TcoPkgWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoPkgWidth"));
		var TcoEsyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoEsyWidth"));
		var TcoTchWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntList", "TcoTchWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeUntList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTit"); doc.widths.push(TcoTitWidth); doc.fs.push("tit"); doc.tos.push(4); wtot += TcoTitWidth;
			} else if (i == 3) {
				...
				doc.cols.push("TcoSys"); doc.widths.push(TcoSysWidth); doc.fs.push("sys"); doc.tos.push(3); wtot += TcoSysWidth;
			} else if (i == 8) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeUntList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUntList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUntList
		rownode = getRownode(srcdoc, "ListWdbeQUntList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUntList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUntList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUntList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUntList", "ListWdbeQUntList", retrieveTi(srcdoc, "TagWdbeUntList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUntList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUntList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUntList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUntListData", scrJref, "StgIacQryWdbeUntList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUntList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUntListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeUntList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeUntList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUntListData", scrJref, "ContIacWdbeUntList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "ContIacWdbeUntList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "ContInfWdbeUntList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "StatShrWdbeUntList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "StgIacWdbeUntList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "TagWdbeUntList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "ListWdbeQUntList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "StatAppQryWdbeUntList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "StatShrQryWdbeUntList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntListData", "StgIacQryWdbeUntList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeUntListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUntListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUntListData") {
				mask = mergeDpchEngData(dom);
