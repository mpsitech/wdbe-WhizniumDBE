/**
  * \file PnlWdbeUntHk1NVector.js
  * web client functionality for panel PnlWdbeUntHk1NVector
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUntHk1NVector", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUntHk1NVector", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Hk1NVector_side").src = "./PnlWdbeUntHk1NVector_aside.html";
	doc.getElementById("Hk1NVector_cont").src = "./PnlWdbeUntHk1NVector_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUntHk1NVector", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUntHk1NVector", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Hk1NVector_side").src = "./PnlWdbeUntHk1NVector_bside.html";
	doc.getElementById("Hk1NVector_cont").src = "./PnlWdbeUntHk1NVector_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntHk1NVector", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUntHk1NVector", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUntHk1NVector", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntHk1NVector", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NVector", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntHk1NVector", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NVector", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NVector", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NVector", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NVector", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NVector", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NVector", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NVector", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntHk1NVector", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUntHk1NVector", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUntHk1NVector", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUntHk1NVector
		rownode = getRownode(srcdoc, "ListWdbeQUntHk1NVector", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUntHk1NVector", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUntHk1NVector", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUntHk1NVector", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUntHk1NVector", "ListWdbeQUntHk1NVector", retrieveTi(srcdoc, "TagWdbeUntHk1NVector", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUntHk1NVector", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntHk1NVector", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NVector", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntHk1NVector", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUntHk1NVector", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUntHk1NVector", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUntHk1NVectorData", scrJref, "StgIacQryWdbeUntHk1NVector");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntHk1NVector", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntHk1NVector", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUntHk1NVector", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntHk1NVector", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntHk1NVector", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUntHk1NVectorDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "ContInfWdbeUntHk1NVector", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "StatAppWdbeUntHk1NVector", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "StatShrWdbeUntHk1NVector", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "StgIacWdbeUntHk1NVector", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "TagWdbeUntHk1NVector", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "ListWdbeQUntHk1NVector", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "StatAppQryWdbeUntHk1NVector", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "StatShrQryWdbeUntHk1NVector", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntHk1NVectorData", "StgIacQryWdbeUntHk1NVector", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUntHk1NVectorData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUntHk1NVectorData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUntHk1NVectorData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
