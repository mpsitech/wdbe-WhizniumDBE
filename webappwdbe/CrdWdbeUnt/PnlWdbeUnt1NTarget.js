/**
  * \file PnlWdbeUnt1NTarget.js
  * web client functionality for panel PnlWdbeUnt1NTarget
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUnt1NTarget", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUnt1NTarget", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NTarget_side").src = "./PnlWdbeUnt1NTarget_aside.html";
	doc.getElementById("1NTarget_cont").src = "./PnlWdbeUnt1NTarget_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUnt1NTarget", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUnt1NTarget", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NTarget_side").src = "./PnlWdbeUnt1NTarget_bside.html";
	doc.getElementById("1NTarget_cont").src = "./PnlWdbeUnt1NTarget_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUnt1NTarget", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUnt1NTarget", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUnt1NTarget", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUnt1NTarget", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUnt1NTarget", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NTarget", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NTarget", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NTarget", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NTarget", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NTarget", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NTarget", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NTarget", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NTarget", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUnt1NTarget", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUnt1NTarget", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUnt1NTarget", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUnt1NTarget
		rownode = getRownode(srcdoc, "ListWdbeQUnt1NTarget", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUnt1NTarget", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUnt1NTarget", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUnt1NTarget", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUnt1NTarget", "ListWdbeQUnt1NTarget", retrieveTi(srcdoc, "TagWdbeUnt1NTarget", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUnt1NTarget", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NTarget", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NTarget", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NTarget", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUnt1NTarget", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUnt1NTarget", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUnt1NTargetData", scrJref, "StgIacQryWdbeUnt1NTarget");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUnt1NTarget", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NTarget", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUnt1NTarget", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUnt1NTarget", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NTarget", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUnt1NTargetDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "ContInfWdbeUnt1NTarget", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "StatAppWdbeUnt1NTarget", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "StatShrWdbeUnt1NTarget", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "StgIacWdbeUnt1NTarget", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "TagWdbeUnt1NTarget", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "ListWdbeQUnt1NTarget", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "StatAppQryWdbeUnt1NTarget", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "StatShrQryWdbeUnt1NTarget", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NTargetData", "StgIacQryWdbeUnt1NTarget", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUnt1NTargetData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUnt1NTargetData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUnt1NTargetData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
