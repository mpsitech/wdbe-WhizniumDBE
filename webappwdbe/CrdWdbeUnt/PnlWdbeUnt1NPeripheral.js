/**
  * \file PnlWdbeUnt1NPeripheral.js
  * web client functionality for panel PnlWdbeUnt1NPeripheral
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUnt1NPeripheral", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUnt1NPeripheral", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NPeripheral_side").src = "./PnlWdbeUnt1NPeripheral_aside.html";
	doc.getElementById("1NPeripheral_cont").src = "./PnlWdbeUnt1NPeripheral_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUnt1NPeripheral", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUnt1NPeripheral", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NPeripheral_side").src = "./PnlWdbeUnt1NPeripheral_bside.html";
	doc.getElementById("1NPeripheral_cont").src = "./PnlWdbeUnt1NPeripheral_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUnt1NPeripheral", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUnt1NPeripheral", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUnt1NPeripheral", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUnt1NPeripheral", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUnt1NPeripheral", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NPeripheral", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NPeripheral", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NPeripheral", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NPeripheral", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NPeripheral", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NPeripheral", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NPeripheral", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NPeripheral", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUnt1NPeripheral", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUnt1NPeripheral", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUnt1NPeripheral", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUnt1NPeripheral
		rownode = getRownode(srcdoc, "ListWdbeQUnt1NPeripheral", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUnt1NPeripheral", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUnt1NPeripheral", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUnt1NPeripheral", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUnt1NPeripheral", "ListWdbeQUnt1NPeripheral", retrieveTi(srcdoc, "TagWdbeUnt1NPeripheral", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUnt1NPeripheral", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NPeripheral", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NPeripheral", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NPeripheral", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUnt1NPeripheral", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUnt1NPeripheral", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUnt1NPeripheralData", scrJref, "StgIacQryWdbeUnt1NPeripheral");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUnt1NPeripheral", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NPeripheral", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUnt1NPeripheral", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUnt1NPeripheral", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NPeripheral", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUnt1NPeripheralDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "ContInfWdbeUnt1NPeripheral", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "StatAppWdbeUnt1NPeripheral", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "StatShrWdbeUnt1NPeripheral", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "StgIacWdbeUnt1NPeripheral", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "TagWdbeUnt1NPeripheral", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "ListWdbeQUnt1NPeripheral", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "StatAppQryWdbeUnt1NPeripheral", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "StatShrQryWdbeUnt1NPeripheral", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NPeripheralData", "StgIacQryWdbeUnt1NPeripheral", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUnt1NPeripheralData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUnt1NPeripheralData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUnt1NPeripheralData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
