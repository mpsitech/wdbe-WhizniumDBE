/**
  * \file PnlWdbeUntRef1NError.js
  * web client functionality for panel PnlWdbeUntRef1NError
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUntRef1NError", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUntRef1NError", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NError", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NError_side").setAttribute("height", "30");
	doc.getElementById("Ref1NError_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NError_side").src = "./PnlWdbeUntRef1NError_aside.html";
	doc.getElementById("Ref1NError_cont").src = "./PnlWdbeUntRef1NError_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUntRef1NError", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUntRef1NError", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NError", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NError_side").setAttribute("height", "255");
	doc.getElementById("Ref1NError_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NError_side").src = "./PnlWdbeUntRef1NError_bside.html";
	doc.getElementById("Ref1NError_cont").src = "./PnlWdbeUntRef1NError_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NError_side").contentDocument;
	contdoc = doc.getElementById("Ref1NError_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NError_side").contentDocument;
	contdoc = doc.getElementById("Ref1NError_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NError_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NError_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NError_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntRef1NError", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUntRef1NError", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUntRef1NError", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntRef1NError", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NError", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntRef1NError", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NError", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NError", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NError", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NError", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NError", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NError", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NError", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntRef1NError", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUntRef1NError", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUntRef1NError", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUntRef1NError
		rownode = getRownode(srcdoc, "ListWdbeQUntRef1NError", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUntRef1NError", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUntRef1NError", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUntRef1NError", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NError", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NError_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NError_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NError_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NError_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUntRef1NError", "ListWdbeQUntRef1NError", retrieveTi(srcdoc, "TagWdbeUntRef1NError", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUntRef1NError", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntRef1NError", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NError", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NError", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUntRef1NError", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUntRef1NError", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUntRef1NErrorData", scrJref, "StgIacQryWdbeUntRef1NError");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntRef1NError", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NError", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUntRef1NError", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntRef1NError", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntRef1NError", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUntRef1NErrorDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "ContInfWdbeUntRef1NError", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "StatAppWdbeUntRef1NError", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "StatShrWdbeUntRef1NError", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "StgIacWdbeUntRef1NError", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "TagWdbeUntRef1NError", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "ListWdbeQUntRef1NError", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "StatAppQryWdbeUntRef1NError", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "StatShrQryWdbeUntRef1NError", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NErrorData", "StgIacQryWdbeUntRef1NError", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUntRef1NErrorData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUntRef1NErrorData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NError");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUntRef1NErrorData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
