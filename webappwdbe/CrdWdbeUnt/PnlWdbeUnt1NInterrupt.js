...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUnt1NInterrupt", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUnt1NInterrupt", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NInterrupt_side").src = "./PnlWdbeUnt1NInterrupt_aside.html";
	doc.getElementById("1NInterrupt_cont").src = "./PnlWdbeUnt1NInterrupt_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUnt1NInterrupt", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUnt1NInterrupt", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NInterrupt_side").src = "./PnlWdbeUnt1NInterrupt_bside.html";
	doc.getElementById("1NInterrupt_cont").src = "./PnlWdbeUnt1NInterrupt_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUnt1NInterrupt", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUnt1NInterrupt", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUnt1NInterrupt", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUnt1NInterrupt", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUnt1NInterrupt", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NInterrupt", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NInterrupt", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NInterrupt", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NInterrupt", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NInterrupt", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NInterrupt", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NInterrupt", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NInterrupt", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUnt1NInterrupt", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUnt1NInterrupt", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUnt1NInterrupt", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUnt1NInterrupt
		rownode = getRownode(srcdoc, "ListWdbeQUnt1NInterrupt", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUnt1NInterrupt", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUnt1NInterrupt", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUnt1NInterrupt", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUnt1NInterrupt", "ListWdbeQUnt1NInterrupt", retrieveTi(srcdoc, "TagWdbeUnt1NInterrupt", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUnt1NInterrupt", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NInterrupt", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NInterrupt", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NInterrupt", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUnt1NInterrupt", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUnt1NInterrupt", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUnt1NInterruptData", scrJref, "StgIacQryWdbeUnt1NInterrupt");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUnt1NInterrupt", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NInterrupt", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUnt1NInterrupt", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUnt1NInterrupt", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NInterrupt", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUnt1NInterruptDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "ContInfWdbeUnt1NInterrupt", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "StatAppWdbeUnt1NInterrupt", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "StatShrWdbeUnt1NInterrupt", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "StgIacWdbeUnt1NInterrupt", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "TagWdbeUnt1NInterrupt", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "ListWdbeQUnt1NInterrupt", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "StatAppQryWdbeUnt1NInterrupt", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "StatShrQryWdbeUnt1NInterrupt", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NInterruptData", "StgIacQryWdbeUnt1NInterrupt", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUnt1NInterruptData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUnt1NInterruptData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUnt1NInterruptData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
