/**
  * \file PnlWdbeUntFwd1NController.js
  * web client functionality for panel PnlWdbeUntFwd1NController
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUntFwd1NController", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUntFwd1NController", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Fwd1NController", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Fwd1NController_side").setAttribute("height", "30");
	doc.getElementById("Fwd1NController_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Fwd1NController_side").src = "./PnlWdbeUntFwd1NController_aside.html";
	doc.getElementById("Fwd1NController_cont").src = "./PnlWdbeUntFwd1NController_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUntFwd1NController", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUntFwd1NController", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Fwd1NController", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Fwd1NController_side").setAttribute("height", "255");
	doc.getElementById("Fwd1NController_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Fwd1NController_side").src = "./PnlWdbeUntFwd1NController_bside.html";
	doc.getElementById("Fwd1NController_cont").src = "./PnlWdbeUntFwd1NController_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Fwd1NController_side").contentDocument;
	contdoc = doc.getElementById("Fwd1NController_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Fwd1NController_side").contentDocument;
	contdoc = doc.getElementById("Fwd1NController_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Fwd1NController_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Fwd1NController_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Fwd1NController_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntFwd1NController", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUntFwd1NController", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUntFwd1NController", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntFwd1NController", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUntFwd1NController", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntFwd1NController", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntFwd1NController", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntFwd1NController", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntFwd1NController", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntFwd1NController", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntFwd1NController", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntFwd1NController", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntFwd1NController", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntFwd1NController", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUntFwd1NController", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUntFwd1NController", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUntFwd1NController
		rownode = getRownode(srcdoc, "ListWdbeQUntFwd1NController", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUntFwd1NController", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUntFwd1NController", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUntFwd1NController", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Fwd1NController", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Fwd1NController_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Fwd1NController_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Fwd1NController_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Fwd1NController_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUntFwd1NController", "ListWdbeQUntFwd1NController", retrieveTi(srcdoc, "TagWdbeUntFwd1NController", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUntFwd1NController", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntFwd1NController", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntFwd1NController", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntFwd1NController", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUntFwd1NController", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUntFwd1NController", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUntFwd1NControllerData", scrJref, "StgIacQryWdbeUntFwd1NController");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntFwd1NController", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntFwd1NController", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUntFwd1NController", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntFwd1NController", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntFwd1NController", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUntFwd1NControllerDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "ContInfWdbeUntFwd1NController", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "StatAppWdbeUntFwd1NController", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "StatShrWdbeUntFwd1NController", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "StgIacWdbeUntFwd1NController", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "TagWdbeUntFwd1NController", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "ListWdbeQUntFwd1NController", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "StatAppQryWdbeUntFwd1NController", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "StatShrQryWdbeUntFwd1NController", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntFwd1NControllerData", "StgIacQryWdbeUntFwd1NController", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUntFwd1NControllerData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUntFwd1NControllerData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Fwd1NController");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUntFwd1NControllerData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
