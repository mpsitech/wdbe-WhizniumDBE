/**
  * \file PnlWdbeUntRef1NCommand.js
  * web client functionality for panel PnlWdbeUntRef1NCommand
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUntRef1NCommand", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUntRef1NCommand", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NCommand_side").src = "./PnlWdbeUntRef1NCommand_aside.html";
	doc.getElementById("Ref1NCommand_cont").src = "./PnlWdbeUntRef1NCommand_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUntRef1NCommand", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUntRef1NCommand", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NCommand_side").src = "./PnlWdbeUntRef1NCommand_bside.html";
	doc.getElementById("Ref1NCommand_cont").src = "./PnlWdbeUntRef1NCommand_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntRef1NCommand", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUntRef1NCommand", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUntRef1NCommand", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUntRef1NCommand", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NCommand", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntRef1NCommand", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NCommand", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NCommand", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NCommand", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NCommand", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NCommand", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NCommand", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NCommand", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUntRef1NCommand", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUntRef1NCommand", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUntRef1NCommand", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUntRef1NCommand
		rownode = getRownode(srcdoc, "ListWdbeQUntRef1NCommand", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUntRef1NCommand", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUntRef1NCommand", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUntRef1NCommand", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUntRef1NCommand", "ListWdbeQUntRef1NCommand", retrieveTi(srcdoc, "TagWdbeUntRef1NCommand", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUntRef1NCommand", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntRef1NCommand", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NCommand", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUntRef1NCommand", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUntRef1NCommand", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUntRef1NCommand", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUntRef1NCommandData", scrJref, "StgIacQryWdbeUntRef1NCommand");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntRef1NCommand", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUntRef1NCommand", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUntRef1NCommand", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUntRef1NCommand", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUntRef1NCommand", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUntRef1NCommandDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "ContInfWdbeUntRef1NCommand", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "StatAppWdbeUntRef1NCommand", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "StatShrWdbeUntRef1NCommand", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "StgIacWdbeUntRef1NCommand", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "TagWdbeUntRef1NCommand", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "ListWdbeQUntRef1NCommand", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "StatAppQryWdbeUntRef1NCommand", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "StatShrQryWdbeUntRef1NCommand", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUntRef1NCommandData", "StgIacQryWdbeUntRef1NCommand", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUntRef1NCommandData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUntRef1NCommandData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUntRef1NCommandData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
