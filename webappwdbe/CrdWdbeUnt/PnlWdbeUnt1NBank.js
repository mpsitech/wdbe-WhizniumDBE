/**
  * \file PnlWdbeUnt1NBank.js
  * web client functionality for panel PnlWdbeUnt1NBank
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeUnt1NBank", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeUnt1NBank", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NBank", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NBank_side").setAttribute("height", "30");
	doc.getElementById("1NBank_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NBank_side").src = "./PnlWdbeUnt1NBank_aside.html";
	doc.getElementById("1NBank_cont").src = "./PnlWdbeUnt1NBank_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeUnt1NBank", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeUnt1NBank", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NBank", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NBank_side").setAttribute("height", "255");
	doc.getElementById("1NBank_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NBank_side").src = "./PnlWdbeUnt1NBank_bside.html";
	doc.getElementById("1NBank_cont").src = "./PnlWdbeUnt1NBank_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NBank_side").contentDocument;
	contdoc = doc.getElementById("1NBank_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NBank_side").contentDocument;
	contdoc = doc.getElementById("1NBank_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NBank_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NBank_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NBank_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUnt1NBank", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeUnt1NBank", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeUnt1NBank", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeUnt1NBank", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeUnt1NBank", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NBank", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NBank", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NBank", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NBank", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NBank", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NBank", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NBank", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NBank", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeUnt1NBank", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeUnt1NBank", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeUnt1NBank", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQUnt1NBank
		rownode = getRownode(srcdoc, "ListWdbeQUnt1NBank", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeUnt1NBank", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeUnt1NBank", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeUnt1NBank", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NBank", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NBank_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NBank_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NBank_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NBank_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeUnt1NBank", "ListWdbeQUnt1NBank", retrieveTi(srcdoc, "TagWdbeUnt1NBank", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeUnt1NBank", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NBank", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NBank", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeUnt1NBank", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeUnt1NBank", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeUnt1NBank", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeUnt1NBankData", scrJref, "StgIacQryWdbeUnt1NBank");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeUnt1NBank", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeUnt1NBank", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeUnt1NBank", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeUnt1NBank", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeUnt1NBank", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeUnt1NBankDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "ContInfWdbeUnt1NBank", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "StatAppWdbeUnt1NBank", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "StatShrWdbeUnt1NBank", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "StgIacWdbeUnt1NBank", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "TagWdbeUnt1NBank", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "ListWdbeQUnt1NBank", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "StatAppQryWdbeUnt1NBank", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "StatShrQryWdbeUnt1NBank", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeUnt1NBankData", "StgIacQryWdbeUnt1NBank", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeUnt1NBankData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeUnt1NBankData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NBank");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeUnt1NBankData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
