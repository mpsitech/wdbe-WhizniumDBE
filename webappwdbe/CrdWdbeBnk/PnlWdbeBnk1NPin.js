/**
  * \file PnlWdbeBnk1NPin.js
  * web client functionality for panel PnlWdbeBnk1NPin
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeBnk1NPin", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeBnk1NPin", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NPin", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NPin_side").setAttribute("height", "30");
	doc.getElementById("1NPin_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NPin_side").src = "./PnlWdbeBnk1NPin_aside.html";
	doc.getElementById("1NPin_cont").src = "./PnlWdbeBnk1NPin_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeBnk1NPin", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeBnk1NPin", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NPin", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NPin_side").setAttribute("height", "255");
	doc.getElementById("1NPin_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NPin_side").src = "./PnlWdbeBnk1NPin_bside.html";
	doc.getElementById("1NPin_cont").src = "./PnlWdbeBnk1NPin_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NPin_side").contentDocument;
	contdoc = doc.getElementById("1NPin_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NPin_side").contentDocument;
	contdoc = doc.getElementById("1NPin_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NPin_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NPin_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NPin_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeBnk1NPin", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeBnk1NPin", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeBnk1NPin", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeBnk1NPin", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeBnk1NPin", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeBnk1NPin", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeBnk1NPin", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeBnk1NPin", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeBnk1NPin", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeBnk1NPin", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeBnk1NPin", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeBnk1NPin", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeBnk1NPin", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeBnk1NPin", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeBnk1NPin", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeBnk1NPin", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQBnk1NPin
		rownode = getRownode(srcdoc, "ListWdbeQBnk1NPin", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeBnk1NPin", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeBnk1NPin", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeBnk1NPin", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NPin", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NPin_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NPin_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NPin_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NPin_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeBnk1NPin", "ListWdbeQBnk1NPin", retrieveTi(srcdoc, "TagWdbeBnk1NPin", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeBnk1NPin", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeBnk1NPin", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeBnk1NPin", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeBnk1NPin", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeBnk1NPin", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeBnk1NPin", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeBnk1NPinData", scrJref, "StgIacQryWdbeBnk1NPin");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeBnk1NPin", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeBnk1NPin", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeBnk1NPin", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeBnk1NPin", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeBnk1NPin", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeBnk1NPinDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "ContInfWdbeBnk1NPin", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "StatAppWdbeBnk1NPin", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "StatShrWdbeBnk1NPin", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "StgIacWdbeBnk1NPin", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "TagWdbeBnk1NPin", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "ListWdbeQBnk1NPin", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "StatAppQryWdbeBnk1NPin", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "StatShrQryWdbeBnk1NPin", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeBnk1NPinData", "StgIacQryWdbeBnk1NPin", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeBnk1NPinData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeBnk1NPinData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NPin");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeBnk1NPinData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
