...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWdbeLibAMakefile", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("AMakefile_side").src = "./PnlWdbeLibAMakefile_aside.html";
	doc.getElementById("AMakefile_cont").src = "./PnlWdbeLibAMakefile_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeLibAMakefile", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeLibAMakefile", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("AMakefile_side").src = "./PnlWdbeLibAMakefile_bside.html";
	doc.getElementById("AMakefile_cont").src = "./PnlWdbeLibAMakefile_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeLibAMakefile", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeLibAMakefile", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeLibAMakefile", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeLibAMakefile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeLibAMakefile", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeLibAMakefile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibAMakefile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibAMakefile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibAMakefile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibAMakefile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibAMakefile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibAMakefile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibAMakefile", "ncol"));

		var TcoMchWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeLibAMakefile", "TcoMchWidth"));
		var TcoTagWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeLibAMakefile", "TcoTagWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeLibAMakefile", "TcoValWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeLibAMakefile", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeLibAMakefile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQLibAMakefile
		rownode = getRownode(srcdoc, "ListWdbeQLibAMakefile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeLibAMakefile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeLibAMakefile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeLibAMakefile", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeLibAMakefile", "ListWdbeQLibAMakefile", retrieveTi(srcdoc, "TagWdbeLibAMakefile", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeLibAMakefile", "TxtRecord2"),
				["Mch","Tag","","Val"],
				["mch","tag","tag2","val"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeLibAMakefile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibAMakefile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibAMakefile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeLibAMakefile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeLibAMakefile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeLibAMakefileData", scrJref, "StgIacQryWdbeLibAMakefile");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeLibAMakefile", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibAMakefile", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeLibAMakefile", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeLibAMakefile", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeLibAMakefile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeLibAMakefileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "ContInfWdbeLibAMakefile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "StatAppWdbeLibAMakefile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "StatShrWdbeLibAMakefile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "StgIacWdbeLibAMakefile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "TagWdbeLibAMakefile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "ListWdbeQLibAMakefile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "StatAppQryWdbeLibAMakefile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "StatShrQryWdbeLibAMakefile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeLibAMakefileData", "StgIacQryWdbeLibAMakefile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeLibAMakefileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeLibAMakefileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeLibAMakefileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
