...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeLibList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeLibList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeLibList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeLibList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeLibList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeLibList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeLibList", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeLibList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeLibList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeLibList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeLibList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeLibList", "TcoTitWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeLibList", "TcoVerWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeLibList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoVer"); doc.widths.push(TcoVerWidth); doc.fs.push("ver"); doc.tos.push(0); wtot += TcoVerWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeLibList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeLibList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQLibList
		rownode = getRownode(srcdoc, "ListWdbeQLibList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeLibList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeLibList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeLibList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeLibList", "ListWdbeQLibList", retrieveTi(srcdoc, "TagWdbeLibList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeLibList", "TxtRecord2"),
				["Srf","Tit","Ver"],
				["srf","tit","ver"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeLibList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeLibList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeLibList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeLibListData", scrJref, "StgIacQryWdbeLibList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeLibList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeLibList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeLibListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeLibList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeLibList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeLibListData", scrJref, "ContIacWdbeLibList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "ContIacWdbeLibList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "ContInfWdbeLibList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "StatShrWdbeLibList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "StgIacWdbeLibList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "TagWdbeLibList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "ListWdbeQLibList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "StatAppQryWdbeLibList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "StatShrQryWdbeLibList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeLibListData", "StgIacQryWdbeLibList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeLibListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeLibListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeLibListData") {
				mask = mergeDpchEngData(dom);
