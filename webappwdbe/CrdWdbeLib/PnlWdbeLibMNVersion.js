/**
  * \file PnlWdbeLibMNVersion.js
  * web client functionality for panel PnlWdbeLibMNVersion
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeLibMNVersion", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeLibMNVersion", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("MNVersion", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNVersion_side").setAttribute("height", "30");
	doc.getElementById("MNVersion_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNVersion_side").src = "./PnlWdbeLibMNVersion_aside.html";
	doc.getElementById("MNVersion_cont").src = "./PnlWdbeLibMNVersion_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeLibMNVersion", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeLibMNVersion", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("MNVersion", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNVersion_side").setAttribute("height", "255");
	doc.getElementById("MNVersion_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNVersion_side").src = "./PnlWdbeLibMNVersion_bside.html";
	doc.getElementById("MNVersion_cont").src = "./PnlWdbeLibMNVersion_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNVersion_side").contentDocument;
	contdoc = doc.getElementById("MNVersion_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNVersion_side").contentDocument;
	contdoc = doc.getElementById("MNVersion_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNVersion_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNVersion_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNVersion_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeLibMNVersion", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeLibMNVersion", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeLibMNVersion", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeLibMNVersion", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeLibMNVersion", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeLibMNVersion", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibMNVersion", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibMNVersion", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibMNVersion", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibMNVersion", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibMNVersion", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibMNVersion", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibMNVersion", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeLibMNVersion", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeLibMNVersion", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeLibMNVersion", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQLibMNVersion
		rownode = getRownode(srcdoc, "ListWdbeQLibMNVersion", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeLibMNVersion", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeLibMNVersion", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeLibMNVersion", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNVersion", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNVersion_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNVersion_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNVersion_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNVersion_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeLibMNVersion", "ListWdbeQLibMNVersion", retrieveTi(srcdoc, "TagWdbeLibMNVersion", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeLibMNVersion", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeLibMNVersion", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibMNVersion", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeLibMNVersion", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeLibMNVersion", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeLibMNVersion", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeLibMNVersionData", scrJref, "StgIacQryWdbeLibMNVersion");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeLibMNVersion", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeLibMNVersion", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeLibMNVersion", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeLibMNVersion", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeLibMNVersion", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeLibMNVersionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "ContInfWdbeLibMNVersion", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "StatAppWdbeLibMNVersion", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "StatShrWdbeLibMNVersion", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "StgIacWdbeLibMNVersion", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "TagWdbeLibMNVersion", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "ListWdbeQLibMNVersion", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "StatAppQryWdbeLibMNVersion", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "StatShrQryWdbeLibMNVersion", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeLibMNVersionData", "StgIacQryWdbeLibMNVersion", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeLibMNVersionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeLibMNVersionData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNVersion");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeLibMNVersionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
