...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWdbeVecVec1NVectoritem", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Vec1NVectoritem", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Vec1NVectoritem_side").setAttribute("height", "30");
	doc.getElementById("Vec1NVectoritem_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Vec1NVectoritem_side").src = "./PnlWdbeVecVec1NVectoritem_aside.html";
	doc.getElementById("Vec1NVectoritem_cont").src = "./PnlWdbeVecVec1NVectoritem_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeVecVec1NVectoritem", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeVecVec1NVectoritem", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Vec1NVectoritem", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Vec1NVectoritem_side").setAttribute("height", "255");
	doc.getElementById("Vec1NVectoritem_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Vec1NVectoritem_side").src = "./PnlWdbeVecVec1NVectoritem_bside.html";
	doc.getElementById("Vec1NVectoritem_cont").src = "./PnlWdbeVecVec1NVectoritem_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Vec1NVectoritem_side").contentDocument;
	contdoc = doc.getElementById("Vec1NVectoritem_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Vec1NVectoritem_side").contentDocument;
	contdoc = doc.getElementById("Vec1NVectoritem_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Vec1NVectoritem_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Vec1NVectoritem_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Vec1NVectoritem_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVecVec1NVectoritem", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeVecVec1NVectoritem", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeVecVec1NVectoritem", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeVecVec1NVectoritem", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeVecVec1NVectoritem", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVecVec1NVectoritem", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVecVec1NVectoritem", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVecVec1NVectoritem", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVecVec1NVectoritem", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVecVec1NVectoritem", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVecVec1NVectoritem", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVecVec1NVectoritem", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVecVec1NVectoritem", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeVecVec1NVectoritem", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeVecVec1NVectoritem", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeVecVec1NVectoritem", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQVecVec1NVectoritem
		rownode = getRownode(srcdoc, "ListWdbeQVecVec1NVectoritem", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeVecVec1NVectoritem", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeVecVec1NVectoritem", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeVecVec1NVectoritem", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Vec1NVectoritem", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Vec1NVectoritem_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Vec1NVectoritem_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Vec1NVectoritem_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Vec1NVectoritem_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeVecVec1NVectoritem", "ListWdbeQVecVec1NVectoritem", retrieveTi(srcdoc, "TagWdbeVecVec1NVectoritem", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeVecVec1NVectoritem", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVecVec1NVectoritem", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVecVec1NVectoritem", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeVecVec1NVectoritem", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeVecVec1NVectoritem", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeVecVec1NVectoritem", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeVecVec1NVectoritemData", scrJref, "StgIacQryWdbeVecVec1NVectoritem");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeVecVec1NVectoritem", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeVecVec1NVectoritem", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeVecVec1NVectoritem", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeVecVec1NVectoritem", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeVecVec1NVectoritem", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeVecVec1NVectoritemDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "ContInfWdbeVecVec1NVectoritem", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "StatAppWdbeVecVec1NVectoritem", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "StatShrWdbeVecVec1NVectoritem", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "StgIacWdbeVecVec1NVectoritem", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "TagWdbeVecVec1NVectoritem", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "ListWdbeQVecVec1NVectoritem", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "StatAppQryWdbeVecVec1NVectoritem", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "StatShrQryWdbeVecVec1NVectoritem", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeVecVec1NVectoritemData", "StgIacQryWdbeVecVec1NVectoritem", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeVecVec1NVectoritemData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeVecVec1NVectoritemData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Vec1NVectoritem");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeVecVec1NVectoritemData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
