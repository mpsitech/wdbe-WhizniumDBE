...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeCprList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeCprList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeCprList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeCprList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCprList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCprList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeCprList", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCprList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCprList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCprList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCprList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCprList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCprList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCprList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCprList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCprList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCprList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCprList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCprList", "TcoOwnWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCprList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCprList", "TcoTitWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCprList", "TcoTypWidth"));
		var TcoCvrWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCprList", "TcoCvrWidth"));
		var TcoGrlWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCprList", "TcoGrlWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeCprList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTit"); doc.widths.push(TcoTitWidth); doc.fs.push("tit"); doc.tos.push(3); wtot += TcoTitWidth;
			} else if (i == 5) {
				doc.cols.push("TcoTyp"); doc.widths.push(TcoTypWidth); doc.fs.push("typ2"); doc.tos.push(0); wtot += TcoTypWidth;
			} else if (i == 6) {
				doc.cols.push("TcoCvr"); doc.widths.push(TcoCvrWidth); doc.fs.push("cvr"); doc.tos.push(1); wtot += TcoCvrWidth;
			} else if (i == 7) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeCprList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCprList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCprList
		rownode = getRownode(srcdoc, "ListWdbeQCprList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCprList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCprList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCprList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCprList", "ListWdbeQCprList", retrieveTi(srcdoc, "TagWdbeCprList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCprList", "TxtRecord2"),
				["Grp","Own","Srf","Tit","","Typ","Cvr","Grl"],
				["grp","own","srf","tit","typ","typ2","cvr","grl"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCprList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCprList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCprList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCprList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCprList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCprListData", scrJref, "StgIacQryWdbeCprList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCprList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCprList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCprList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCprListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeCprList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeCprList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCprListData", scrJref, "ContIacWdbeCprList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "ContIacWdbeCprList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "ContInfWdbeCprList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "StatShrWdbeCprList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "StgIacWdbeCprList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "TagWdbeCprList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "ListWdbeQCprList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "StatAppQryWdbeCprList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "StatShrQryWdbeCprList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCprListData", "StgIacQryWdbeCprList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeCprListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCprListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCprListData") {
				mask = mergeDpchEngData(dom);
