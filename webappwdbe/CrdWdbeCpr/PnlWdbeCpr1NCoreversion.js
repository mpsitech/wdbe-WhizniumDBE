/**
  * \file PnlWdbeCpr1NCoreversion.js
  * web client functionality for panel PnlWdbeCpr1NCoreversion
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeCpr1NCoreversion", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeCpr1NCoreversion", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NCoreversion", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NCoreversion_side").setAttribute("height", "30");
	doc.getElementById("1NCoreversion_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NCoreversion_side").src = "./PnlWdbeCpr1NCoreversion_aside.html";
	doc.getElementById("1NCoreversion_cont").src = "./PnlWdbeCpr1NCoreversion_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeCpr1NCoreversion", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeCpr1NCoreversion", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NCoreversion", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NCoreversion_side").setAttribute("height", "255");
	doc.getElementById("1NCoreversion_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NCoreversion_side").src = "./PnlWdbeCpr1NCoreversion_bside.html";
	doc.getElementById("1NCoreversion_cont").src = "./PnlWdbeCpr1NCoreversion_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NCoreversion_side").contentDocument;
	contdoc = doc.getElementById("1NCoreversion_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NCoreversion_side").contentDocument;
	contdoc = doc.getElementById("1NCoreversion_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NCoreversion_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NCoreversion_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NCoreversion_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCpr1NCoreversion", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeCpr1NCoreversion", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeCpr1NCoreversion", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeCpr1NCoreversion", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeCpr1NCoreversion", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCpr1NCoreversion", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCpr1NCoreversion", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCpr1NCoreversion", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCpr1NCoreversion", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCpr1NCoreversion", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCpr1NCoreversion", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCpr1NCoreversion", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCpr1NCoreversion", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeCpr1NCoreversion", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeCpr1NCoreversion", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeCpr1NCoreversion", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQCpr1NCoreversion
		rownode = getRownode(srcdoc, "ListWdbeQCpr1NCoreversion", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeCpr1NCoreversion", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeCpr1NCoreversion", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeCpr1NCoreversion", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NCoreversion", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCoreversion_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCoreversion_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NCoreversion_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NCoreversion_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeCpr1NCoreversion", "ListWdbeQCpr1NCoreversion", retrieveTi(srcdoc, "TagWdbeCpr1NCoreversion", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeCpr1NCoreversion", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCpr1NCoreversion", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCpr1NCoreversion", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeCpr1NCoreversion", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeCpr1NCoreversion", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeCpr1NCoreversion", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeCpr1NCoreversionData", scrJref, "StgIacQryWdbeCpr1NCoreversion");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeCpr1NCoreversion", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeCpr1NCoreversion", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeCpr1NCoreversion", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeCpr1NCoreversion", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeCpr1NCoreversion", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeCpr1NCoreversionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "ContInfWdbeCpr1NCoreversion", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "StatAppWdbeCpr1NCoreversion", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "StatShrWdbeCpr1NCoreversion", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "StgIacWdbeCpr1NCoreversion", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "TagWdbeCpr1NCoreversion", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "ListWdbeQCpr1NCoreversion", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "StatAppQryWdbeCpr1NCoreversion", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "StatShrQryWdbeCpr1NCoreversion", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeCpr1NCoreversionData", "StgIacQryWdbeCpr1NCoreversion", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeCpr1NCoreversionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeCpr1NCoreversionData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NCoreversion");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeCpr1NCoreversionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
