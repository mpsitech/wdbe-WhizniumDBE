/**
  * \file CrdWdbeRls.js
  * web client functionality for card CrdWdbeRls
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWdbeRls", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWdbeRls", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWdbeRls", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWdbeRls", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWdbeRls", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWdbeRlsHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWdbeRlsList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWdbeRlsRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWdbeRls", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWdbeRls", "shortMenu", short);
	setSi(srcdoc, "StatAppWdbeRls", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWdbeRls", "shortMenu", "");
...
function initMenApp() {
	...
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWdbeRls", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWdbeRls", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MitCrdCrtActive = (retrieveSi(srcdoc, "StatShrWdbeRls", "MitCrdCrtActive") == "true");
	MitCrdFrtAvail = (retrieveSi(srcdoc, "StatShrWdbeRls", "MitCrdFrtAvail") == "true");
	MitCrdFrtActive = (retrieveSi(srcdoc, "StatShrWdbeRls", "MitCrdFrtActive") == "true");
	MitCrdWcoAvail = (retrieveSi(srcdoc, "StatShrWdbeRls", "MitCrdWcoAvail") == "true");
	MitCrdWcoActive = (retrieveSi(srcdoc, "StatShrWdbeRls", "MitCrdWcoActive") == "true");
	...
	setMitActive("MitCrdCrt", MitCrdCrtActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdFrt"), retrieveTi(srcdoc, "TagWdbeRls", "MitCrdFrt"));
	setMitActive("MitCrdFrt", MitCrdFrtActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdWco"), retrieveTi(srcdoc, "TagWdbeRls", "MitCrdWco"));
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgWdbeRlsStareptr") || (sref == "DlgWdbeRlsFinreptr") || (sref == "DlgWdbeRlsWrite")) myif.setAttribute("height", "585");
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWdbeRls", "MtxCrdRls") + " - WhizniumDBE v1.0.3";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeRlsDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeRlsDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWdbeRls", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeRlsData", "ContInfWdbeRls", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeRlsData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWdbeRlsData", "StatAppWdbeRls", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeRlsData", "StatShrWdbeRls", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeRlsData", "TagWdbeRls", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeRlsData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWdbeRls", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWdbeRls", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWdbeRls", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWdbeRlsData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeRls", "srefIxWdbeVReqitmode");

	if (srefIxWdbeVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWdbeVReqitmode = "notify";
		setSi(srcdoc, "StatAppWdbeRls", "srefIxWdbeVReqitmode", srefIxWdbeVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWdbeVReqitmode = retrieveSi(srcdoc, "StatAppWdbeRls", "srefIxWdbeVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWdbeRls", "latency"));
