/**
  * \file PnlWdbePrsMNCoreproject.js
  * web client functionality for panel PnlWdbePrsMNCoreproject
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePrsMNCoreproject", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePrsMNCoreproject", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("MNCoreproject", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNCoreproject_side").setAttribute("height", "30");
	doc.getElementById("MNCoreproject_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNCoreproject_side").src = "./PnlWdbePrsMNCoreproject_aside.html";
	doc.getElementById("MNCoreproject_cont").src = "./PnlWdbePrsMNCoreproject_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePrsMNCoreproject", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePrsMNCoreproject", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("MNCoreproject", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNCoreproject_side").setAttribute("height", "255");
	doc.getElementById("MNCoreproject_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNCoreproject_side").src = "./PnlWdbePrsMNCoreproject_bside.html";
	doc.getElementById("MNCoreproject_cont").src = "./PnlWdbePrsMNCoreproject_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNCoreproject_side").contentDocument;
	contdoc = doc.getElementById("MNCoreproject_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNCoreproject_side").contentDocument;
	contdoc = doc.getElementById("MNCoreproject_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNCoreproject_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNCoreproject_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNCoreproject_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrsMNCoreproject", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrsMNCoreproject", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePrsMNCoreproject", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrsMNCoreproject", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrsMNCoreproject", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrsMNCoreproject", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNCoreproject", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNCoreproject", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsMNCoreproject", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsMNCoreproject", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsMNCoreproject", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNCoreproject", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNCoreproject", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsMNCoreproject", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsMNCoreproject", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsMNCoreproject", "TcoStoWidth"));
		var TcoFctWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsMNCoreproject", "TcoFctWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePrsMNCoreproject", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrsMNCoreproject", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrsMNCoreproject
		rownode = getRownode(srcdoc, "ListWdbeQPrsMNCoreproject", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrsMNCoreproject", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrsMNCoreproject", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrsMNCoreproject", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNCoreproject", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNCoreproject_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNCoreproject_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNCoreproject_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNCoreproject_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrsMNCoreproject", "ListWdbeQPrsMNCoreproject", retrieveTi(srcdoc, "TagWdbePrsMNCoreproject", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrsMNCoreproject", "TxtRecord2"),
				["Mref","Sta","Sto","Fct",""],
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrsMNCoreproject", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNCoreproject", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsMNCoreproject", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrsMNCoreproject", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrsMNCoreproject", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrsMNCoreprojectData", scrJref, "StgIacQryWdbePrsMNCoreproject");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePrsMNCoreproject", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNCoreproject", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrsMNCoreproject", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePrsMNCoreproject", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrsMNCoreproject", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrsMNCoreprojectDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "ContInfWdbePrsMNCoreproject", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "StatAppWdbePrsMNCoreproject", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "StatShrWdbePrsMNCoreproject", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "StgIacWdbePrsMNCoreproject", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "TagWdbePrsMNCoreproject", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "ListWdbeQPrsMNCoreproject", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "StatAppQryWdbePrsMNCoreproject", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "StatShrQryWdbePrsMNCoreproject", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNCoreprojectData", "StgIacQryWdbePrsMNCoreproject", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePrsMNCoreprojectData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrsMNCoreprojectData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNCoreproject");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrsMNCoreprojectData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
