/**
  * \file PnlWdbePrsADetail.js
  * web client functionality for panel PnlWdbePrsADetail
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePrsADetail", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePrsADetail", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("ADetail", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ADetail_side").setAttribute("height", "30");
	doc.getElementById("ADetail_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ADetail_side").src = "./PnlWdbePrsADetail_aside.html";
	doc.getElementById("ADetail_cont").src = "./PnlWdbePrsADetail_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePrsADetail", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePrsADetail", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("ADetail", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ADetail_side").setAttribute("height", "255");
	doc.getElementById("ADetail_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ADetail_side").src = "./PnlWdbePrsADetail_bside.html";
	doc.getElementById("ADetail_cont").src = "./PnlWdbePrsADetail_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ADetail_side").contentDocument;
	contdoc = doc.getElementById("ADetail_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("ADetail_side").contentDocument;
	contdoc = doc.getElementById("ADetail_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ADetail_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ADetail_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ADetail_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrsADetail", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrsADetail", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePrsADetail", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrsADetail", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrsADetail", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrsADetail", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsADetail", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsADetail", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsADetail", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsADetail", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsADetail", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsADetail", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsADetail", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsADetail", "TcoTypWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsADetail", "TcoValWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePrsADetail", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrsADetail", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrsADetail
		rownode = getRownode(srcdoc, "ListWdbeQPrsADetail", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrsADetail", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrsADetail", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrsADetail", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ADetail", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ADetail_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ADetail_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ADetail_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ADetail_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrsADetail", "ListWdbeQPrsADetail", retrieveTi(srcdoc, "TagWdbePrsADetail", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrsADetail", "TxtRecord2"),
				["Typ","","Val"],
				["typ","typ2","val"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrsADetail", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsADetail", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsADetail", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrsADetail", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrsADetail", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrsADetailData", scrJref, "StgIacQryWdbePrsADetail");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePrsADetail", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsADetail", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrsADetail", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePrsADetail", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrsADetail", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrsADetailDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "ContInfWdbePrsADetail", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "StatAppWdbePrsADetail", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "StatShrWdbePrsADetail", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "StgIacWdbePrsADetail", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "TagWdbePrsADetail", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "ListWdbeQPrsADetail", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "StatAppQryWdbePrsADetail", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "StatShrQryWdbePrsADetail", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrsADetailData", "StgIacQryWdbePrsADetail", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePrsADetailData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrsADetailData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ADetail");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrsADetailData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
