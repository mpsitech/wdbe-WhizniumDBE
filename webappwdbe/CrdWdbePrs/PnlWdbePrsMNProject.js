/**
  * \file PnlWdbePrsMNProject.js
  * web client functionality for panel PnlWdbePrsMNProject
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbePrsMNProject", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbePrsMNProject", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("MNProject", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNProject_side").setAttribute("height", "30");
	doc.getElementById("MNProject_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNProject_side").src = "./PnlWdbePrsMNProject_aside.html";
	doc.getElementById("MNProject_cont").src = "./PnlWdbePrsMNProject_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbePrsMNProject", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbePrsMNProject", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("MNProject", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNProject_side").setAttribute("height", "255");
	doc.getElementById("MNProject_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNProject_side").src = "./PnlWdbePrsMNProject_bside.html";
	doc.getElementById("MNProject_cont").src = "./PnlWdbePrsMNProject_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNProject_side").contentDocument;
	contdoc = doc.getElementById("MNProject_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNProject_side").contentDocument;
	contdoc = doc.getElementById("MNProject_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNProject_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNProject_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNProject_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrsMNProject", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbePrsMNProject", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbePrsMNProject", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbePrsMNProject", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbePrsMNProject", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrsMNProject", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNProject", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNProject", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsMNProject", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsMNProject", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsMNProject", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNProject", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNProject", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsMNProject", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsMNProject", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsMNProject", "TcoStoWidth"));
		var TcoFctWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbePrsMNProject", "TcoFctWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbePrsMNProject", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbePrsMNProject", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQPrsMNProject
		rownode = getRownode(srcdoc, "ListWdbeQPrsMNProject", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbePrsMNProject", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbePrsMNProject", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbePrsMNProject", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNProject", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNProject_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNProject_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNProject_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNProject_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbePrsMNProject", "ListWdbeQPrsMNProject", retrieveTi(srcdoc, "TagWdbePrsMNProject", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbePrsMNProject", "TxtRecord2"),
				["Mref","Sta","Sto","Fct",""],
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrsMNProject", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNProject", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbePrsMNProject", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbePrsMNProject", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbePrsMNProject", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbePrsMNProjectData", scrJref, "StgIacQryWdbePrsMNProject");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbePrsMNProject", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbePrsMNProject", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbePrsMNProject", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbePrsMNProject", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbePrsMNProject", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbePrsMNProjectDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "ContInfWdbePrsMNProject", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "StatAppWdbePrsMNProject", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "StatShrWdbePrsMNProject", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "StgIacWdbePrsMNProject", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "TagWdbePrsMNProject", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "ListWdbeQPrsMNProject", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "StatAppQryWdbePrsMNProject", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "StatShrQryWdbePrsMNProject", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbePrsMNProjectData", "StgIacQryWdbePrsMNProject", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbePrsMNProjectData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbePrsMNProjectData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNProject");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbePrsMNProjectData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
