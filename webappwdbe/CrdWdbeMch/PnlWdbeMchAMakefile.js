/**
  * \file PnlWdbeMchAMakefile.js
  * web client functionality for panel PnlWdbeMchAMakefile
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMchAMakefile", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMchAMakefile", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("AMakefile", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("AMakefile_side").setAttribute("height", "30");
	doc.getElementById("AMakefile_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("AMakefile_side").src = "./PnlWdbeMchAMakefile_aside.html";
	doc.getElementById("AMakefile_cont").src = "./PnlWdbeMchAMakefile_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMchAMakefile", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMchAMakefile", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("AMakefile", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("AMakefile_side").setAttribute("height", "255");
	doc.getElementById("AMakefile_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("AMakefile_side").src = "./PnlWdbeMchAMakefile_bside.html";
	doc.getElementById("AMakefile_cont").src = "./PnlWdbeMchAMakefile_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AMakefile_side").contentDocument;
	contdoc = doc.getElementById("AMakefile_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("AMakefile_side").contentDocument;
	contdoc = doc.getElementById("AMakefile_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AMakefile_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AMakefile_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AMakefile_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMchAMakefile", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMchAMakefile", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMchAMakefile", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMchAMakefile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMchAMakefile", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMchAMakefile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAMakefile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAMakefile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchAMakefile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchAMakefile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchAMakefile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAMakefile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAMakefile", "ncol"));

		var TcoTagWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMchAMakefile", "TcoTagWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMchAMakefile", "TcoValWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoTag"); doc.widths.push(TcoTagWidth); doc.fs.push("tag"); wtot += TcoTagWidth;
			} else if (i == 2) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMchAMakefile", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMchAMakefile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMchAMakefile
		rownode = getRownode(srcdoc, "ListWdbeQMchAMakefile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMchAMakefile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMchAMakefile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMchAMakefile", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AMakefile", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AMakefile_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AMakefile_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AMakefile_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AMakefile_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMchAMakefile", "ListWdbeQMchAMakefile", retrieveTi(srcdoc, "TagWdbeMchAMakefile", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMchAMakefile", "TxtRecord2"),
				["Tag","","Val"],
				["tag","tag2","val"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMchAMakefile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAMakefile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchAMakefile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMchAMakefile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMchAMakefile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMchAMakefileData", scrJref, "StgIacQryWdbeMchAMakefile");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMchAMakefile", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAMakefile", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMchAMakefile", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMchAMakefile", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMchAMakefile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMchAMakefileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "ContInfWdbeMchAMakefile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "StatAppWdbeMchAMakefile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "StatShrWdbeMchAMakefile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "StgIacWdbeMchAMakefile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "TagWdbeMchAMakefile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "ListWdbeQMchAMakefile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "StatAppQryWdbeMchAMakefile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "StatShrQryWdbeMchAMakefile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMchAMakefileData", "StgIacQryWdbeMchAMakefile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMchAMakefileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMchAMakefileData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AMakefile");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMchAMakefileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
