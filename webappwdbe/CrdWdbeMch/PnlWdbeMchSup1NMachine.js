...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMchSup1NMachine", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMchSup1NMachine", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Sup1NMachine", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Sup1NMachine_side").setAttribute("height", "30");
	doc.getElementById("Sup1NMachine_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Sup1NMachine_side").src = "./PnlWdbeMchSup1NMachine_aside.html";
	doc.getElementById("Sup1NMachine_cont").src = "./PnlWdbeMchSup1NMachine_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMchSup1NMachine", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMchSup1NMachine", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Sup1NMachine", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Sup1NMachine_side").setAttribute("height", "255");
	doc.getElementById("Sup1NMachine_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Sup1NMachine_side").src = "./PnlWdbeMchSup1NMachine_bside.html";
	doc.getElementById("Sup1NMachine_cont").src = "./PnlWdbeMchSup1NMachine_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sup1NMachine_side").contentDocument;
	contdoc = doc.getElementById("Sup1NMachine_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Sup1NMachine_side").contentDocument;
	contdoc = doc.getElementById("Sup1NMachine_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sup1NMachine_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sup1NMachine_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sup1NMachine_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMchSup1NMachine", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMchSup1NMachine", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMchSup1NMachine", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMchSup1NMachine", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMchSup1NMachine", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMchSup1NMachine", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchSup1NMachine", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchSup1NMachine", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchSup1NMachine", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchSup1NMachine", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchSup1NMachine", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchSup1NMachine", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchSup1NMachine", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMchSup1NMachine", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMchSup1NMachine", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMchSup1NMachine", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMchSup1NMachine
		rownode = getRownode(srcdoc, "ListWdbeQMchSup1NMachine", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMchSup1NMachine", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMchSup1NMachine", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMchSup1NMachine", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sup1NMachine", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NMachine_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NMachine_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sup1NMachine_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sup1NMachine_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMchSup1NMachine", "ListWdbeQMchSup1NMachine", retrieveTi(srcdoc, "TagWdbeMchSup1NMachine", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMchSup1NMachine", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMchSup1NMachine", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchSup1NMachine", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchSup1NMachine", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMchSup1NMachine", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMchSup1NMachine", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMchSup1NMachineData", scrJref, "StgIacQryWdbeMchSup1NMachine");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMchSup1NMachine", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchSup1NMachine", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMchSup1NMachine", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMchSup1NMachine", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMchSup1NMachine", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMchSup1NMachineDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "ContInfWdbeMchSup1NMachine", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "StatAppWdbeMchSup1NMachine", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "StatShrWdbeMchSup1NMachine", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "StgIacWdbeMchSup1NMachine", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "TagWdbeMchSup1NMachine", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "ListWdbeQMchSup1NMachine", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "StatAppQryWdbeMchSup1NMachine", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "StatShrQryWdbeMchSup1NMachine", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMchSup1NMachineData", "StgIacQryWdbeMchSup1NMachine", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMchSup1NMachineData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMchSup1NMachineData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sup1NMachine");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMchSup1NMachineData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
