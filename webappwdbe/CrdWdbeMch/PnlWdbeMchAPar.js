/**
  * \file PnlWdbeMchAPar.js
  * web client functionality for panel PnlWdbeMchAPar
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMchAPar", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMchAPar", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlWdbeMchAPar_aside.html";
	doc.getElementById("APar_cont").src = "./PnlWdbeMchAPar_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMchAPar", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMchAPar", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlWdbeMchAPar_bside.html";
	doc.getElementById("APar_cont").src = "./PnlWdbeMchAPar_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMchAPar", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMchAPar", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMchAPar", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMchAPar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMchAPar", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMchAPar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAPar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAPar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchAPar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchAPar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchAPar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAPar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAPar", "ncol"));

		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMchAPar", "TcoKeyWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMchAPar", "TcoValWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMchAPar", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMchAPar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMchAPar
		rownode = getRownode(srcdoc, "ListWdbeQMchAPar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMchAPar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMchAPar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMchAPar", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMchAPar", "ListWdbeQMchAPar", retrieveTi(srcdoc, "TagWdbeMchAPar", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMchAPar", "TxtRecord2"),
				["Key","","Val"],
				["key","key2","val"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMchAPar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAPar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMchAPar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMchAPar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMchAPar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMchAParData", scrJref, "StgIacQryWdbeMchAPar");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMchAPar", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMchAPar", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMchAPar", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMchAPar", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMchAPar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMchAParDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "ContInfWdbeMchAPar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "StatAppWdbeMchAPar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "StatShrWdbeMchAPar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "StgIacWdbeMchAPar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "TagWdbeMchAPar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "ListWdbeQMchAPar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "StatAppQryWdbeMchAPar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "StatShrQryWdbeMchAPar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMchAParData", "StgIacQryWdbeMchAPar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMchAParData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMchAParData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMchAParData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
