/**
  * \file PnlWdbeMch1NRelease.js
  * web client functionality for panel PnlWdbeMch1NRelease
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMch1NRelease", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMch1NRelease", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NRelease_side").src = "./PnlWdbeMch1NRelease_aside.html";
	doc.getElementById("1NRelease_cont").src = "./PnlWdbeMch1NRelease_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMch1NRelease", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMch1NRelease", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NRelease_side").src = "./PnlWdbeMch1NRelease_bside.html";
	doc.getElementById("1NRelease_cont").src = "./PnlWdbeMch1NRelease_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMch1NRelease", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMch1NRelease", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMch1NRelease", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMch1NRelease", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMch1NRelease", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMch1NRelease", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMch1NRelease", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMch1NRelease", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMch1NRelease", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMch1NRelease", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMch1NRelease", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMch1NRelease", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMch1NRelease", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMch1NRelease", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMch1NRelease", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMch1NRelease", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMch1NRelease
		rownode = getRownode(srcdoc, "ListWdbeQMch1NRelease", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMch1NRelease", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMch1NRelease", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMch1NRelease", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMch1NRelease", "ListWdbeQMch1NRelease", retrieveTi(srcdoc, "TagWdbeMch1NRelease", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMch1NRelease", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMch1NRelease", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMch1NRelease", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMch1NRelease", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMch1NRelease", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMch1NRelease", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMch1NReleaseData", scrJref, "StgIacQryWdbeMch1NRelease");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMch1NRelease", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMch1NRelease", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMch1NRelease", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMch1NRelease", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMch1NRelease", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMch1NReleaseDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "ContInfWdbeMch1NRelease", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "StatAppWdbeMch1NRelease", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "StatShrWdbeMch1NRelease", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "StgIacWdbeMch1NRelease", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "TagWdbeMch1NRelease", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "ListWdbeQMch1NRelease", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "StatAppQryWdbeMch1NRelease", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "StatShrQryWdbeMch1NRelease", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMch1NReleaseData", "StgIacQryWdbeMch1NRelease", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMch1NReleaseData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMch1NReleaseData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMch1NReleaseData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
