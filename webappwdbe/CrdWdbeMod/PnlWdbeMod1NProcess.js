/**
  * \file PnlWdbeMod1NProcess.js
  * web client functionality for panel PnlWdbeMod1NProcess
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMod1NProcess", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMod1NProcess", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NProcess", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NProcess_side").setAttribute("height", "30");
	doc.getElementById("1NProcess_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NProcess_side").src = "./PnlWdbeMod1NProcess_aside.html";
	doc.getElementById("1NProcess_cont").src = "./PnlWdbeMod1NProcess_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMod1NProcess", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMod1NProcess", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NProcess", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NProcess_side").setAttribute("height", "255");
	doc.getElementById("1NProcess_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NProcess_side").src = "./PnlWdbeMod1NProcess_bside.html";
	doc.getElementById("1NProcess_cont").src = "./PnlWdbeMod1NProcess_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NProcess_side").contentDocument;
	contdoc = doc.getElementById("1NProcess_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NProcess_side").contentDocument;
	contdoc = doc.getElementById("1NProcess_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NProcess_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NProcess_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NProcess_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMod1NProcess", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMod1NProcess", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMod1NProcess", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMod1NProcess", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMod1NProcess", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMod1NProcess", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NProcess", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NProcess", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMod1NProcess", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMod1NProcess", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMod1NProcess", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NProcess", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NProcess", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMod1NProcess", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMod1NProcess", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMod1NProcess", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMod1NProcess
		rownode = getRownode(srcdoc, "ListWdbeQMod1NProcess", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMod1NProcess", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMod1NProcess", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMod1NProcess", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NProcess", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NProcess_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NProcess_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NProcess_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NProcess_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMod1NProcess", "ListWdbeQMod1NProcess", retrieveTi(srcdoc, "TagWdbeMod1NProcess", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMod1NProcess", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMod1NProcess", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NProcess", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMod1NProcess", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMod1NProcess", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMod1NProcess", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMod1NProcessData", scrJref, "StgIacQryWdbeMod1NProcess");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMod1NProcess", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NProcess", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMod1NProcess", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMod1NProcess", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMod1NProcess", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMod1NProcessDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "ContInfWdbeMod1NProcess", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "StatAppWdbeMod1NProcess", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "StatShrWdbeMod1NProcess", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "StgIacWdbeMod1NProcess", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "TagWdbeMod1NProcess", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "ListWdbeQMod1NProcess", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "StatAppQryWdbeMod1NProcess", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "StatShrQryWdbeMod1NProcess", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NProcessData", "StgIacQryWdbeMod1NProcess", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMod1NProcessData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMod1NProcessData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NProcess");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMod1NProcessData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
