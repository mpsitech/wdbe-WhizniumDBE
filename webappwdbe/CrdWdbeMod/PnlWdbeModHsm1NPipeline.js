...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModHsm1NPipeline", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModHsm1NPipeline", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Hsm1NPipeline", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Hsm1NPipeline_side").setAttribute("height", "30");
	doc.getElementById("Hsm1NPipeline_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Hsm1NPipeline_side").src = "./PnlWdbeModHsm1NPipeline_aside.html";
	doc.getElementById("Hsm1NPipeline_cont").src = "./PnlWdbeModHsm1NPipeline_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModHsm1NPipeline", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModHsm1NPipeline", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Hsm1NPipeline", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Hsm1NPipeline_side").setAttribute("height", "255");
	doc.getElementById("Hsm1NPipeline_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Hsm1NPipeline_side").src = "./PnlWdbeModHsm1NPipeline_bside.html";
	doc.getElementById("Hsm1NPipeline_cont").src = "./PnlWdbeModHsm1NPipeline_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Hsm1NPipeline_side").contentDocument;
	contdoc = doc.getElementById("Hsm1NPipeline_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Hsm1NPipeline_side").contentDocument;
	contdoc = doc.getElementById("Hsm1NPipeline_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Hsm1NPipeline_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Hsm1NPipeline_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Hsm1NPipeline_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModHsm1NPipeline", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModHsm1NPipeline", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModHsm1NPipeline", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModHsm1NPipeline", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModHsm1NPipeline", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModHsm1NPipeline", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModHsm1NPipeline", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModHsm1NPipeline", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModHsm1NPipeline", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModHsm1NPipeline", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModHsm1NPipeline", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModHsm1NPipeline", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModHsm1NPipeline", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModHsm1NPipeline", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModHsm1NPipeline", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModHsm1NPipeline", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModHsm1NPipeline
		rownode = getRownode(srcdoc, "ListWdbeQModHsm1NPipeline", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModHsm1NPipeline", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModHsm1NPipeline", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModHsm1NPipeline", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Hsm1NPipeline", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Hsm1NPipeline_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Hsm1NPipeline_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Hsm1NPipeline_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Hsm1NPipeline_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModHsm1NPipeline", "ListWdbeQModHsm1NPipeline", retrieveTi(srcdoc, "TagWdbeModHsm1NPipeline", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModHsm1NPipeline", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModHsm1NPipeline", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModHsm1NPipeline", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModHsm1NPipeline", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModHsm1NPipeline", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModHsm1NPipeline", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModHsm1NPipelineData", scrJref, "StgIacQryWdbeModHsm1NPipeline");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModHsm1NPipeline", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModHsm1NPipeline", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModHsm1NPipeline", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModHsm1NPipeline", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModHsm1NPipeline", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModHsm1NPipelineDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "ContInfWdbeModHsm1NPipeline", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "StatAppWdbeModHsm1NPipeline", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "StatShrWdbeModHsm1NPipeline", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "StgIacWdbeModHsm1NPipeline", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "TagWdbeModHsm1NPipeline", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "ListWdbeQModHsm1NPipeline", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "StatAppQryWdbeModHsm1NPipeline", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "StatShrQryWdbeModHsm1NPipeline", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModHsm1NPipelineData", "StgIacQryWdbeModHsm1NPipeline", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModHsm1NPipelineData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModHsm1NPipelineData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Hsm1NPipeline");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModHsm1NPipelineData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
