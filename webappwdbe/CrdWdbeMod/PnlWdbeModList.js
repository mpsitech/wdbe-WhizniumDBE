...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWdbeModList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeModList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWdbeModList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWdbeModList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModList", "Trs"));
...
function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatShrWdbeModList", "srefIxWdbeVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWdbeModList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeModList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeModList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWdbeModList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWdbeModList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModList", "TcoSrfWidth"));
		var TcoVndWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModList", "TcoVndWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModList", "TcoTypWidth"));
		var TcoHktWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModList", "TcoHktWidth"));
		var TcoHkuWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModList", "TcoHkuWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModList", "TcoSupWidth"));
		var TcoTplWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModList", "TcoTplWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeModList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWdbeModList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModList
		rownode = getRownode(srcdoc, "ListWdbeQModList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModList", "ListWdbeQModList", retrieveTi(srcdoc, "TagWdbeModList", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModList", "TxtRecord2"),
				["Srf","","Vnd","","Typ","","Hkt","Hku","Sup","Tpl",""],
				["srf","vnd","vnd2","typ","typ2","hkt","hkt2","hku","sup","tpl","srr"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModListData", scrJref, "StgIacQryWdbeModList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModList", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWdbeModList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWdbeModList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModListData", scrJref, "ContIacWdbeModList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "ContIacWdbeModList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "ContInfWdbeModList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "StatShrWdbeModList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "StgIacWdbeModList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "TagWdbeModList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "ListWdbeQModList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "StatAppQryWdbeModList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "StatShrQryWdbeModList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModListData", "StgIacQryWdbeModList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWdbeModListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModListData") {
				mask = mergeDpchEngData(dom);
