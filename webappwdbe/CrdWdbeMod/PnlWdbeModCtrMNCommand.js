...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWdbeModCtrMNCommand", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("CtrMNCommand", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("CtrMNCommand_side").setAttribute("height", "30");
	doc.getElementById("CtrMNCommand_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("CtrMNCommand_side").src = "./PnlWdbeModCtrMNCommand_aside.html";
	doc.getElementById("CtrMNCommand_cont").src = "./PnlWdbeModCtrMNCommand_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModCtrMNCommand", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModCtrMNCommand", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("CtrMNCommand", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("CtrMNCommand_side").setAttribute("height", "255");
	doc.getElementById("CtrMNCommand_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("CtrMNCommand_side").src = "./PnlWdbeModCtrMNCommand_bside.html";
	doc.getElementById("CtrMNCommand_cont").src = "./PnlWdbeModCtrMNCommand_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("CtrMNCommand_side").contentDocument;
	contdoc = doc.getElementById("CtrMNCommand_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("CtrMNCommand_side").contentDocument;
	contdoc = doc.getElementById("CtrMNCommand_cont").contentDocument;
	hdrdoc = contdoc.getElementById("CtrMNCommand_hdr").contentDocument;
	tbldoc = contdoc.getElementById("CtrMNCommand_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("CtrMNCommand_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtrMNCommand", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModCtrMNCommand", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModCtrMNCommand", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtrMNCommand", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModCtrMNCommand", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrMNCommand", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrMNCommand", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrMNCommand", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrMNCommand", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrMNCommand", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrMNCommand", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrMNCommand", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrMNCommand", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCtrMNCommand", "TcoMrefWidth"));
		var TcoIvrWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCtrMNCommand", "TcoIvrWidth"));
		var TcoRvrWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCtrMNCommand", "TcoRvrWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModCtrMNCommand", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModCtrMNCommand", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModCtrMNCommand
		rownode = getRownode(srcdoc, "ListWdbeQModCtrMNCommand", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModCtrMNCommand", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModCtrMNCommand", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModCtrMNCommand", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("CtrMNCommand", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("CtrMNCommand_side").setAttribute("height", "" + (height+55));
	doc.getElementById("CtrMNCommand_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("CtrMNCommand_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("CtrMNCommand_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModCtrMNCommand", "ListWdbeQModCtrMNCommand", retrieveTi(srcdoc, "TagWdbeModCtrMNCommand", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModCtrMNCommand", "TxtRecord2"),
				["Mref","Ivr","Rvr"],
				["mref","ivr","rvr"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrMNCommand", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrMNCommand", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrMNCommand", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModCtrMNCommand", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModCtrMNCommand", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModCtrMNCommandData", scrJref, "StgIacQryWdbeModCtrMNCommand");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtrMNCommand", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrMNCommand", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModCtrMNCommand", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtrMNCommand", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrMNCommand", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModCtrMNCommandDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "ContInfWdbeModCtrMNCommand", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "StatAppWdbeModCtrMNCommand", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "StatShrWdbeModCtrMNCommand", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "StgIacWdbeModCtrMNCommand", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "TagWdbeModCtrMNCommand", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "ListWdbeQModCtrMNCommand", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "StatAppQryWdbeModCtrMNCommand", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "StatShrQryWdbeModCtrMNCommand", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrMNCommandData", "StgIacQryWdbeModCtrMNCommand", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModCtrMNCommandData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModCtrMNCommandData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("CtrMNCommand");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModCtrMNCommandData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
