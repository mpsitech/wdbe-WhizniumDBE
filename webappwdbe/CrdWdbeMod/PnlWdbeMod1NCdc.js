...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeMod1NCdc", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeMod1NCdc", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("1NCdc", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NCdc_side").setAttribute("height", "30");
	doc.getElementById("1NCdc_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NCdc_side").src = "./PnlWdbeMod1NCdc_aside.html";
	doc.getElementById("1NCdc_cont").src = "./PnlWdbeMod1NCdc_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeMod1NCdc", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeMod1NCdc", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("1NCdc", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NCdc_side").setAttribute("height", "255");
	doc.getElementById("1NCdc_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NCdc_side").src = "./PnlWdbeMod1NCdc_bside.html";
	doc.getElementById("1NCdc_cont").src = "./PnlWdbeMod1NCdc_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NCdc_side").contentDocument;
	contdoc = doc.getElementById("1NCdc_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NCdc_side").contentDocument;
	contdoc = doc.getElementById("1NCdc_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NCdc_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NCdc_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NCdc_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMod1NCdc", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeMod1NCdc", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeMod1NCdc", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeMod1NCdc", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeMod1NCdc", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMod1NCdc", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NCdc", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NCdc", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMod1NCdc", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMod1NCdc", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMod1NCdc", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NCdc", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NCdc", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeMod1NCdc", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeMod1NCdc", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeMod1NCdc", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQMod1NCdc
		rownode = getRownode(srcdoc, "ListWdbeQMod1NCdc", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeMod1NCdc", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeMod1NCdc", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeMod1NCdc", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NCdc", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCdc_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCdc_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NCdc_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NCdc_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeMod1NCdc", "ListWdbeQMod1NCdc", retrieveTi(srcdoc, "TagWdbeMod1NCdc", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeMod1NCdc", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMod1NCdc", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NCdc", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeMod1NCdc", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeMod1NCdc", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeMod1NCdc", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeMod1NCdcData", scrJref, "StgIacQryWdbeMod1NCdc");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeMod1NCdc", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeMod1NCdc", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeMod1NCdc", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeMod1NCdc", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeMod1NCdc", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeMod1NCdcDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "ContInfWdbeMod1NCdc", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "StatAppWdbeMod1NCdc", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "StatShrWdbeMod1NCdc", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "StgIacWdbeMod1NCdc", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "TagWdbeMod1NCdc", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "ListWdbeQMod1NCdc", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "StatAppQryWdbeMod1NCdc", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "StatShrQryWdbeMod1NCdc", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeMod1NCdcData", "StgIacQryWdbeMod1NCdc", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeMod1NCdcData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeMod1NCdcData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NCdc");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeMod1NCdcData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
