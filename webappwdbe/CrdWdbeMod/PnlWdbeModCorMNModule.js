...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModCorMNModule", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModCorMNModule", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("CorMNModule", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("CorMNModule_side").setAttribute("height", "30");
	doc.getElementById("CorMNModule_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("CorMNModule_side").src = "./PnlWdbeModCorMNModule_aside.html";
	doc.getElementById("CorMNModule_cont").src = "./PnlWdbeModCorMNModule_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModCorMNModule", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModCorMNModule", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("CorMNModule", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("CorMNModule_side").setAttribute("height", "255");
	doc.getElementById("CorMNModule_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("CorMNModule_side").src = "./PnlWdbeModCorMNModule_bside.html";
	doc.getElementById("CorMNModule_cont").src = "./PnlWdbeModCorMNModule_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("CorMNModule_side").contentDocument;
	contdoc = doc.getElementById("CorMNModule_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("CorMNModule_side").contentDocument;
	contdoc = doc.getElementById("CorMNModule_cont").contentDocument;
	hdrdoc = contdoc.getElementById("CorMNModule_hdr").contentDocument;
	tbldoc = contdoc.getElementById("CorMNModule_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("CorMNModule_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCorMNModule", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModCorMNModule", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModCorMNModule", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCorMNModule", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModCorMNModule", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCorMNModule", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCorMNModule", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCorMNModule", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCorMNModule", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCorMNModule", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCorMNModule", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCorMNModule", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCorMNModule", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCorMNModule", "TcoMrefWidth"));
		var TcoFctWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCorMNModule", "TcoFctWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModCorMNModule", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModCorMNModule", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModCorMNModule
		rownode = getRownode(srcdoc, "ListWdbeQModCorMNModule", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModCorMNModule", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModCorMNModule", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModCorMNModule", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("CorMNModule", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("CorMNModule_side").setAttribute("height", "" + (height+55));
	doc.getElementById("CorMNModule_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("CorMNModule_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("CorMNModule_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModCorMNModule", "ListWdbeQModCorMNModule", retrieveTi(srcdoc, "TagWdbeModCorMNModule", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModCorMNModule", "TxtRecord2"),
				["Mref","Fct",""],
				["mref","fct","fct2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCorMNModule", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCorMNModule", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCorMNModule", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModCorMNModule", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModCorMNModule", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModCorMNModuleData", scrJref, "StgIacQryWdbeModCorMNModule");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCorMNModule", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCorMNModule", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModCorMNModule", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCorMNModule", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCorMNModule", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModCorMNModuleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "ContInfWdbeModCorMNModule", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "StatAppWdbeModCorMNModule", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "StatShrWdbeModCorMNModule", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "StgIacWdbeModCorMNModule", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "TagWdbeModCorMNModule", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "ListWdbeQModCorMNModule", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "StatAppQryWdbeModCorMNModule", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "StatShrQryWdbeModCorMNModule", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCorMNModuleData", "StgIacQryWdbeModCorMNModule", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModCorMNModuleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModCorMNModuleData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("CorMNModule");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModCorMNModuleData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
