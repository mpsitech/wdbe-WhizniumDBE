/**
  * \file PnlWdbeModCtrHk1NVector.js
  * web client functionality for panel PnlWdbeModCtrHk1NVector
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModCtrHk1NVector", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModCtrHk1NVector", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("CtrHk1NVector", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("CtrHk1NVector_side").setAttribute("height", "30");
	doc.getElementById("CtrHk1NVector_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("CtrHk1NVector_side").src = "./PnlWdbeModCtrHk1NVector_aside.html";
	doc.getElementById("CtrHk1NVector_cont").src = "./PnlWdbeModCtrHk1NVector_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModCtrHk1NVector", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModCtrHk1NVector", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("CtrHk1NVector", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("CtrHk1NVector_side").setAttribute("height", "255");
	doc.getElementById("CtrHk1NVector_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("CtrHk1NVector_side").src = "./PnlWdbeModCtrHk1NVector_bside.html";
	doc.getElementById("CtrHk1NVector_cont").src = "./PnlWdbeModCtrHk1NVector_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("CtrHk1NVector_side").contentDocument;
	contdoc = doc.getElementById("CtrHk1NVector_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("CtrHk1NVector_side").contentDocument;
	contdoc = doc.getElementById("CtrHk1NVector_cont").contentDocument;
	hdrdoc = contdoc.getElementById("CtrHk1NVector_hdr").contentDocument;
	tbldoc = contdoc.getElementById("CtrHk1NVector_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("CtrHk1NVector_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtrHk1NVector", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModCtrHk1NVector", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModCtrHk1NVector", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtrHk1NVector", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModCtrHk1NVector", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrHk1NVector", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrHk1NVector", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrHk1NVector", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrHk1NVector", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrHk1NVector", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrHk1NVector", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrHk1NVector", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrHk1NVector", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCtrHk1NVector", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModCtrHk1NVector", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModCtrHk1NVector", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModCtrHk1NVector
		rownode = getRownode(srcdoc, "ListWdbeQModCtrHk1NVector", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModCtrHk1NVector", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModCtrHk1NVector", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModCtrHk1NVector", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("CtrHk1NVector", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("CtrHk1NVector_side").setAttribute("height", "" + (height+55));
	doc.getElementById("CtrHk1NVector_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("CtrHk1NVector_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("CtrHk1NVector_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModCtrHk1NVector", "ListWdbeQModCtrHk1NVector", retrieveTi(srcdoc, "TagWdbeModCtrHk1NVector", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModCtrHk1NVector", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrHk1NVector", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrHk1NVector", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrHk1NVector", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModCtrHk1NVector", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModCtrHk1NVector", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModCtrHk1NVectorData", scrJref, "StgIacQryWdbeModCtrHk1NVector");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtrHk1NVector", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrHk1NVector", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModCtrHk1NVector", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtrHk1NVector", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrHk1NVector", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModCtrHk1NVectorDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "ContInfWdbeModCtrHk1NVector", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "StatAppWdbeModCtrHk1NVector", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "StatShrWdbeModCtrHk1NVector", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "StgIacWdbeModCtrHk1NVector", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "TagWdbeModCtrHk1NVector", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "ListWdbeQModCtrHk1NVector", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "StatAppQryWdbeModCtrHk1NVector", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "StatShrQryWdbeModCtrHk1NVector", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrHk1NVectorData", "StgIacQryWdbeModCtrHk1NVector", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModCtrHk1NVectorData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModCtrHk1NVectorData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("CtrHk1NVector");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModCtrHk1NVectorData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
