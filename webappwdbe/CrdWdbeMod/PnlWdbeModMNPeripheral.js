...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModMNPeripheral", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModMNPeripheral", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("MNPeripheral", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNPeripheral_side").setAttribute("height", "30");
	doc.getElementById("MNPeripheral_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNPeripheral_side").src = "./PnlWdbeModMNPeripheral_aside.html";
	doc.getElementById("MNPeripheral_cont").src = "./PnlWdbeModMNPeripheral_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModMNPeripheral", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModMNPeripheral", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("MNPeripheral", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNPeripheral_side").setAttribute("height", "255");
	doc.getElementById("MNPeripheral_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNPeripheral_side").src = "./PnlWdbeModMNPeripheral_bside.html";
	doc.getElementById("MNPeripheral_cont").src = "./PnlWdbeModMNPeripheral_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNPeripheral_side").contentDocument;
	contdoc = doc.getElementById("MNPeripheral_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNPeripheral_side").contentDocument;
	contdoc = doc.getElementById("MNPeripheral_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNPeripheral_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNPeripheral_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNPeripheral_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModMNPeripheral", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModMNPeripheral", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModMNPeripheral", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModMNPeripheral", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModMNPeripheral", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModMNPeripheral", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMNPeripheral", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMNPeripheral", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMNPeripheral", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMNPeripheral", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMNPeripheral", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMNPeripheral", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMNPeripheral", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModMNPeripheral", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModMNPeripheral", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModMNPeripheral", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModMNPeripheral
		rownode = getRownode(srcdoc, "ListWdbeQModMNPeripheral", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModMNPeripheral", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModMNPeripheral", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModMNPeripheral", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNPeripheral", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNPeripheral_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNPeripheral_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNPeripheral_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNPeripheral_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModMNPeripheral", "ListWdbeQModMNPeripheral", retrieveTi(srcdoc, "TagWdbeModMNPeripheral", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModMNPeripheral", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModMNPeripheral", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMNPeripheral", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMNPeripheral", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModMNPeripheral", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModMNPeripheral", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModMNPeripheralData", scrJref, "StgIacQryWdbeModMNPeripheral");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModMNPeripheral", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMNPeripheral", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModMNPeripheral", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModMNPeripheral", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModMNPeripheral", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModMNPeripheralDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "ContInfWdbeModMNPeripheral", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "StatAppWdbeModMNPeripheral", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "StatShrWdbeModMNPeripheral", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "StgIacWdbeModMNPeripheral", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "TagWdbeModMNPeripheral", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "ListWdbeQModMNPeripheral", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "StatAppQryWdbeModMNPeripheral", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "StatShrQryWdbeModMNPeripheral", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModMNPeripheralData", "StgIacQryWdbeModMNPeripheral", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModMNPeripheralData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModMNPeripheralData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNPeripheral");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModMNPeripheralData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
