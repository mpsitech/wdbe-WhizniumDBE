/**
  * \file PnlWdbeModMge1NSignal.js
  * web client functionality for panel PnlWdbeModMge1NSignal
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModMge1NSignal", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModMge1NSignal", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Mge1NSignal_side").src = "./PnlWdbeModMge1NSignal_aside.html";
	doc.getElementById("Mge1NSignal_cont").src = "./PnlWdbeModMge1NSignal_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModMge1NSignal", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModMge1NSignal", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Mge1NSignal_side").src = "./PnlWdbeModMge1NSignal_bside.html";
	doc.getElementById("Mge1NSignal_cont").src = "./PnlWdbeModMge1NSignal_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModMge1NSignal", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModMge1NSignal", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModMge1NSignal", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModMge1NSignal", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModMge1NSignal", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModMge1NSignal", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMge1NSignal", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMge1NSignal", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMge1NSignal", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMge1NSignal", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMge1NSignal", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMge1NSignal", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMge1NSignal", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModMge1NSignal", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModMge1NSignal", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModMge1NSignal", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModMge1NSignal
		rownode = getRownode(srcdoc, "ListWdbeQModMge1NSignal", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModMge1NSignal", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModMge1NSignal", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModMge1NSignal", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModMge1NSignal", "ListWdbeQModMge1NSignal", retrieveTi(srcdoc, "TagWdbeModMge1NSignal", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModMge1NSignal", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModMge1NSignal", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMge1NSignal", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMge1NSignal", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModMge1NSignal", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModMge1NSignal", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModMge1NSignalData", scrJref, "StgIacQryWdbeModMge1NSignal");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModMge1NSignal", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMge1NSignal", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModMge1NSignal", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModMge1NSignal", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModMge1NSignal", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModMge1NSignalDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "ContInfWdbeModMge1NSignal", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "StatAppWdbeModMge1NSignal", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "StatShrWdbeModMge1NSignal", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "StgIacWdbeModMge1NSignal", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "TagWdbeModMge1NSignal", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "ListWdbeQModMge1NSignal", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "StatAppQryWdbeModMge1NSignal", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "StatShrQryWdbeModMge1NSignal", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModMge1NSignalData", "StgIacQryWdbeModMge1NSignal", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModMge1NSignalData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModMge1NSignalData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModMge1NSignalData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
