...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModRef1NVariable", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModRef1NVariable", "srefIxWdbeVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NVariable_side").src = "./PnlWdbeModRef1NVariable_aside.html";
	doc.getElementById("Ref1NVariable_cont").src = "./PnlWdbeModRef1NVariable_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModRef1NVariable", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModRef1NVariable", "srefIxWdbeVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NVariable_side").src = "./PnlWdbeModRef1NVariable_bside.html";
	doc.getElementById("Ref1NVariable_cont").src = "./PnlWdbeModRef1NVariable_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModRef1NVariable", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModRef1NVariable", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModRef1NVariable", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModRef1NVariable", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModRef1NVariable", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModRef1NVariable", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NVariable", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NVariable", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModRef1NVariable", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModRef1NVariable", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModRef1NVariable", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NVariable", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NVariable", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModRef1NVariable", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModRef1NVariable", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModRef1NVariable", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModRef1NVariable
		rownode = getRownode(srcdoc, "ListWdbeQModRef1NVariable", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModRef1NVariable", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModRef1NVariable", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModRef1NVariable", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModRef1NVariable", "ListWdbeQModRef1NVariable", retrieveTi(srcdoc, "TagWdbeModRef1NVariable", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModRef1NVariable", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModRef1NVariable", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NVariable", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModRef1NVariable", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModRef1NVariable", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModRef1NVariable", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModRef1NVariableData", scrJref, "StgIacQryWdbeModRef1NVariable");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModRef1NVariable", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NVariable", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModRef1NVariable", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModRef1NVariable", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModRef1NVariable", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModRef1NVariableDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "ContInfWdbeModRef1NVariable", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "StatAppWdbeModRef1NVariable", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "StatShrWdbeModRef1NVariable", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "StgIacWdbeModRef1NVariable", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "TagWdbeModRef1NVariable", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "ListWdbeQModRef1NVariable", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "StatAppQryWdbeModRef1NVariable", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "StatShrQryWdbeModRef1NVariable", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NVariableData", "StgIacQryWdbeModRef1NVariable", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModRef1NVariableData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModRef1NVariableData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModRef1NVariableData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
