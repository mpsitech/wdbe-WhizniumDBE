/**
  * \file PnlWdbeModKHdltype.js
  * web client functionality for panel PnlWdbeModKHdltype
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModKHdltype", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModKHdltype", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("KHdltype", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("KHdltype_side").setAttribute("height", "30");
	doc.getElementById("KHdltype_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("KHdltype_side").src = "./PnlWdbeModKHdltype_aside.html";
	doc.getElementById("KHdltype_cont").src = "./PnlWdbeModKHdltype_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModKHdltype", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModKHdltype", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("KHdltype", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("KHdltype_side").setAttribute("height", "255");
	doc.getElementById("KHdltype_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("KHdltype_side").src = "./PnlWdbeModKHdltype_bside.html";
	doc.getElementById("KHdltype_cont").src = "./PnlWdbeModKHdltype_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KHdltype_side").contentDocument;
	contdoc = doc.getElementById("KHdltype_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("KHdltype_side").contentDocument;
	contdoc = doc.getElementById("KHdltype_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KHdltype_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KHdltype_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KHdltype_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModKHdltype", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModKHdltype", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModKHdltype", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModKHdltype", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModKHdltype", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModKHdltype", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModKHdltype", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModKHdltype", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModKHdltype", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModKHdltype", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModKHdltype", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModKHdltype", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModKHdltype", "ncol"));

		var TcoFixWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModKHdltype", "TcoFixWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModKHdltype", "TcoSrfWidth"));
		var TcoAvlWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModKHdltype", "TcoAvlWidth"));
		var TcoImpWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModKHdltype", "TcoImpWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModKHdltype", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModKHdltype", "TcoCmtWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoFix"); doc.widths.push(TcoFixWidth); doc.fs.push("fix"); wtot += TcoFixWidth;
			} else if (i == 2) {
				doc.cols.push("TcoSrf"); doc.widths.push(TcoSrfWidth); doc.fs.push("srf"); wtot += TcoSrfWidth;
			} else if (i == 3) {
				doc.cols.push("TcoAvl"); doc.widths.push(TcoAvlWidth); doc.fs.push("avl"); wtot += TcoAvlWidth;
			} else if (i == 4) {
				doc.cols.push("TcoImp"); doc.widths.push(TcoImpWidth); doc.fs.push("imp"); wtot += TcoImpWidth;
			} else if (i == 5) {
				doc.cols.push("TcoTit"); doc.widths.push(TcoTitWidth); doc.fs.push("tit"); wtot += TcoTitWidth;
			} else if (i == 6) {
				doc.cols.push("TcoCmt"); doc.widths.push(TcoCmtWidth); doc.fs.push("cmt"); wtot += TcoCmtWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModKHdltype", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModKHdltype", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModKHdltype
		rownode = getRownode(srcdoc, "ListWdbeQModKHdltype", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModKHdltype", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModKHdltype", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModKHdltype", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KHdltype", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KHdltype_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KHdltype_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KHdltype_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KHdltype_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModKHdltype", "ListWdbeQModKHdltype", retrieveTi(srcdoc, "TagWdbeModKHdltype", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModKHdltype", "TxtRecord2"),
				["Fix","Srf","Avl","Imp","Tit","Cmt"],
				["fix","srf","avl","imp","tit","cmt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModKHdltype", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModKHdltype", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModKHdltype", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModKHdltype", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModKHdltype", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModKHdltypeData", scrJref, "StgIacQryWdbeModKHdltype");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModKHdltype", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModKHdltype", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModKHdltype", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModKHdltype", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModKHdltype", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModKHdltypeDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "ContInfWdbeModKHdltype", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "StatAppWdbeModKHdltype", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "StatShrWdbeModKHdltype", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "StgIacWdbeModKHdltype", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "TagWdbeModKHdltype", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "ListWdbeQModKHdltype", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "StatAppQryWdbeModKHdltype", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "StatShrQryWdbeModKHdltype", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModKHdltypeData", "StgIacQryWdbeModKHdltype", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModKHdltypeData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModKHdltypeData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KHdltype");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModKHdltypeData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
