/**
  * \file PnlWdbeModCtrRef1NCommand.js
  * web client functionality for panel PnlWdbeModCtrRef1NCommand
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NCommand", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModCtrRef1NCommand", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("CtrRef1NCommand", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("CtrRef1NCommand_side").setAttribute("height", "30");
	doc.getElementById("CtrRef1NCommand_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("CtrRef1NCommand_side").src = "./PnlWdbeModCtrRef1NCommand_aside.html";
	doc.getElementById("CtrRef1NCommand_cont").src = "./PnlWdbeModCtrRef1NCommand_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NCommand", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModCtrRef1NCommand", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("CtrRef1NCommand", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("CtrRef1NCommand_side").setAttribute("height", "255");
	doc.getElementById("CtrRef1NCommand_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("CtrRef1NCommand_side").src = "./PnlWdbeModCtrRef1NCommand_bside.html";
	doc.getElementById("CtrRef1NCommand_cont").src = "./PnlWdbeModCtrRef1NCommand_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("CtrRef1NCommand_side").contentDocument;
	contdoc = doc.getElementById("CtrRef1NCommand_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("CtrRef1NCommand_side").contentDocument;
	contdoc = doc.getElementById("CtrRef1NCommand_cont").contentDocument;
	hdrdoc = contdoc.getElementById("CtrRef1NCommand_hdr").contentDocument;
	tbldoc = contdoc.getElementById("CtrRef1NCommand_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("CtrRef1NCommand_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtrRef1NCommand", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModCtrRef1NCommand", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NCommand", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtrRef1NCommand", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NCommand", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrRef1NCommand", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NCommand", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NCommand", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NCommand", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NCommand", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NCommand", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NCommand", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NCommand", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCtrRef1NCommand", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModCtrRef1NCommand", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModCtrRef1NCommand", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModCtrRef1NCommand
		rownode = getRownode(srcdoc, "ListWdbeQModCtrRef1NCommand", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModCtrRef1NCommand", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModCtrRef1NCommand", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModCtrRef1NCommand", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("CtrRef1NCommand", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("CtrRef1NCommand_side").setAttribute("height", "" + (height+55));
	doc.getElementById("CtrRef1NCommand_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("CtrRef1NCommand_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("CtrRef1NCommand_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModCtrRef1NCommand", "ListWdbeQModCtrRef1NCommand", retrieveTi(srcdoc, "TagWdbeModCtrRef1NCommand", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModCtrRef1NCommand", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrRef1NCommand", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NCommand", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NCommand", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModCtrRef1NCommand", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModCtrRef1NCommand", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModCtrRef1NCommandData", scrJref, "StgIacQryWdbeModCtrRef1NCommand");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NCommand", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NCommand", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModCtrRef1NCommand", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NCommand", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrRef1NCommand", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModCtrRef1NCommandDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "ContInfWdbeModCtrRef1NCommand", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "StatAppWdbeModCtrRef1NCommand", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "StatShrWdbeModCtrRef1NCommand", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "StgIacWdbeModCtrRef1NCommand", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "TagWdbeModCtrRef1NCommand", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "ListWdbeQModCtrRef1NCommand", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "StatAppQryWdbeModCtrRef1NCommand", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "StatShrQryWdbeModCtrRef1NCommand", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NCommandData", "StgIacQryWdbeModCtrRef1NCommand", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModCtrRef1NCommandData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModCtrRef1NCommandData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("CtrRef1NCommand");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModCtrRef1NCommandData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
