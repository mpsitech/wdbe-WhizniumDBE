...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModRef1NSignal", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModRef1NSignal", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NSignal", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NSignal_side").setAttribute("height", "30");
	doc.getElementById("Ref1NSignal_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NSignal_side").src = "./PnlWdbeModRef1NSignal_aside.html";
	doc.getElementById("Ref1NSignal_cont").src = "./PnlWdbeModRef1NSignal_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModRef1NSignal", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModRef1NSignal", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NSignal", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NSignal_side").setAttribute("height", "255");
	doc.getElementById("Ref1NSignal_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NSignal_side").src = "./PnlWdbeModRef1NSignal_bside.html";
	doc.getElementById("Ref1NSignal_cont").src = "./PnlWdbeModRef1NSignal_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NSignal_side").contentDocument;
	contdoc = doc.getElementById("Ref1NSignal_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NSignal_side").contentDocument;
	contdoc = doc.getElementById("Ref1NSignal_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NSignal_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NSignal_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NSignal_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModRef1NSignal", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModRef1NSignal", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModRef1NSignal", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModRef1NSignal", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModRef1NSignal", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModRef1NSignal", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NSignal", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NSignal", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModRef1NSignal", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModRef1NSignal", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModRef1NSignal", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NSignal", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NSignal", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModRef1NSignal", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModRef1NSignal", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModRef1NSignal", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModRef1NSignal
		rownode = getRownode(srcdoc, "ListWdbeQModRef1NSignal", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModRef1NSignal", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModRef1NSignal", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModRef1NSignal", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NSignal", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NSignal_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NSignal_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NSignal_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NSignal_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModRef1NSignal", "ListWdbeQModRef1NSignal", retrieveTi(srcdoc, "TagWdbeModRef1NSignal", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModRef1NSignal", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModRef1NSignal", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NSignal", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModRef1NSignal", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModRef1NSignal", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModRef1NSignal", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModRef1NSignalData", scrJref, "StgIacQryWdbeModRef1NSignal");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModRef1NSignal", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModRef1NSignal", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModRef1NSignal", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModRef1NSignal", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModRef1NSignal", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModRef1NSignalDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "ContInfWdbeModRef1NSignal", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "StatAppWdbeModRef1NSignal", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "StatShrWdbeModRef1NSignal", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "StgIacWdbeModRef1NSignal", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "TagWdbeModRef1NSignal", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "ListWdbeQModRef1NSignal", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "StatAppQryWdbeModRef1NSignal", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "StatShrQryWdbeModRef1NSignal", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModRef1NSignalData", "StgIacQryWdbeModRef1NSignal", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModRef1NSignalData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModRef1NSignalData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NSignal");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModRef1NSignalData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
