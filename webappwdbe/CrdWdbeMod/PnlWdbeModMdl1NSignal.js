/**
  * \file PnlWdbeModMdl1NSignal.js
  * web client functionality for panel PnlWdbeModMdl1NSignal
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModMdl1NSignal", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModMdl1NSignal", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Mdl1NSignal", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Mdl1NSignal_side").setAttribute("height", "30");
	doc.getElementById("Mdl1NSignal_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Mdl1NSignal_side").src = "./PnlWdbeModMdl1NSignal_aside.html";
	doc.getElementById("Mdl1NSignal_cont").src = "./PnlWdbeModMdl1NSignal_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModMdl1NSignal", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModMdl1NSignal", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Mdl1NSignal", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Mdl1NSignal_side").setAttribute("height", "255");
	doc.getElementById("Mdl1NSignal_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Mdl1NSignal_side").src = "./PnlWdbeModMdl1NSignal_bside.html";
	doc.getElementById("Mdl1NSignal_cont").src = "./PnlWdbeModMdl1NSignal_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Mdl1NSignal_side").contentDocument;
	contdoc = doc.getElementById("Mdl1NSignal_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Mdl1NSignal_side").contentDocument;
	contdoc = doc.getElementById("Mdl1NSignal_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Mdl1NSignal_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Mdl1NSignal_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Mdl1NSignal_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModMdl1NSignal", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModMdl1NSignal", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModMdl1NSignal", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModMdl1NSignal", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModMdl1NSignal", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModMdl1NSignal", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMdl1NSignal", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMdl1NSignal", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMdl1NSignal", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMdl1NSignal", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMdl1NSignal", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMdl1NSignal", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMdl1NSignal", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModMdl1NSignal", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModMdl1NSignal", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModMdl1NSignal", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModMdl1NSignal
		rownode = getRownode(srcdoc, "ListWdbeQModMdl1NSignal", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModMdl1NSignal", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModMdl1NSignal", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModMdl1NSignal", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Mdl1NSignal", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Mdl1NSignal_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Mdl1NSignal_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Mdl1NSignal_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Mdl1NSignal_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModMdl1NSignal", "ListWdbeQModMdl1NSignal", retrieveTi(srcdoc, "TagWdbeModMdl1NSignal", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModMdl1NSignal", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModMdl1NSignal", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMdl1NSignal", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModMdl1NSignal", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModMdl1NSignal", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModMdl1NSignal", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModMdl1NSignalData", scrJref, "StgIacQryWdbeModMdl1NSignal");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModMdl1NSignal", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModMdl1NSignal", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModMdl1NSignal", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModMdl1NSignal", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModMdl1NSignal", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModMdl1NSignalDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "ContInfWdbeModMdl1NSignal", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "StatAppWdbeModMdl1NSignal", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "StatShrWdbeModMdl1NSignal", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "StgIacWdbeModMdl1NSignal", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "TagWdbeModMdl1NSignal", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "ListWdbeQModMdl1NSignal", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "StatAppQryWdbeModMdl1NSignal", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "StatShrQryWdbeModMdl1NSignal", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModMdl1NSignalData", "StgIacQryWdbeModMdl1NSignal", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModMdl1NSignalData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModMdl1NSignalData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Mdl1NSignal");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModMdl1NSignalData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
