/**
  * \file PnlWdbeModCtrRef1NError.js
  * web client functionality for panel PnlWdbeModCtrRef1NError
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NError", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModCtrRef1NError", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("CtrRef1NError", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("CtrRef1NError_side").setAttribute("height", "30");
	doc.getElementById("CtrRef1NError_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("CtrRef1NError_side").src = "./PnlWdbeModCtrRef1NError_aside.html";
	doc.getElementById("CtrRef1NError_cont").src = "./PnlWdbeModCtrRef1NError_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NError", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModCtrRef1NError", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("CtrRef1NError", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("CtrRef1NError_side").setAttribute("height", "255");
	doc.getElementById("CtrRef1NError_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("CtrRef1NError_side").src = "./PnlWdbeModCtrRef1NError_bside.html";
	doc.getElementById("CtrRef1NError_cont").src = "./PnlWdbeModCtrRef1NError_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("CtrRef1NError_side").contentDocument;
	contdoc = doc.getElementById("CtrRef1NError_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("CtrRef1NError_side").contentDocument;
	contdoc = doc.getElementById("CtrRef1NError_cont").contentDocument;
	hdrdoc = contdoc.getElementById("CtrRef1NError_hdr").contentDocument;
	tbldoc = contdoc.getElementById("CtrRef1NError_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("CtrRef1NError_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtrRef1NError", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModCtrRef1NError", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NError", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtrRef1NError", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NError", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrRef1NError", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NError", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NError", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NError", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NError", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NError", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NError", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NError", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCtrRef1NError", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModCtrRef1NError", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModCtrRef1NError", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModCtrRef1NError
		rownode = getRownode(srcdoc, "ListWdbeQModCtrRef1NError", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModCtrRef1NError", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModCtrRef1NError", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModCtrRef1NError", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("CtrRef1NError", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("CtrRef1NError_side").setAttribute("height", "" + (height+55));
	doc.getElementById("CtrRef1NError_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("CtrRef1NError_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("CtrRef1NError_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModCtrRef1NError", "ListWdbeQModCtrRef1NError", retrieveTi(srcdoc, "TagWdbeModCtrRef1NError", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModCtrRef1NError", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrRef1NError", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NError", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtrRef1NError", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModCtrRef1NError", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModCtrRef1NError", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModCtrRef1NErrorData", scrJref, "StgIacQryWdbeModCtrRef1NError");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NError", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtrRef1NError", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModCtrRef1NError", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtrRef1NError", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtrRef1NError", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModCtrRef1NErrorDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "ContInfWdbeModCtrRef1NError", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "StatAppWdbeModCtrRef1NError", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "StatShrWdbeModCtrRef1NError", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "StgIacWdbeModCtrRef1NError", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "TagWdbeModCtrRef1NError", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "ListWdbeQModCtrRef1NError", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "StatAppQryWdbeModCtrRef1NError", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "StatShrQryWdbeModCtrRef1NError", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtrRef1NErrorData", "StgIacQryWdbeModCtrRef1NError", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModCtrRef1NErrorData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModCtrRef1NErrorData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("CtrRef1NError");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModCtrRef1NErrorData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
