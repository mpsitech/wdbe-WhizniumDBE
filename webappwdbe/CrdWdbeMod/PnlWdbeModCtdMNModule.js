...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModCtdMNModule", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModCtdMNModule", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("CtdMNModule", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("CtdMNModule_side").setAttribute("height", "30");
	doc.getElementById("CtdMNModule_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("CtdMNModule_side").src = "./PnlWdbeModCtdMNModule_aside.html";
	doc.getElementById("CtdMNModule_cont").src = "./PnlWdbeModCtdMNModule_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModCtdMNModule", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModCtdMNModule", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("CtdMNModule", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("CtdMNModule_side").setAttribute("height", "255");
	doc.getElementById("CtdMNModule_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("CtdMNModule_side").src = "./PnlWdbeModCtdMNModule_bside.html";
	doc.getElementById("CtdMNModule_cont").src = "./PnlWdbeModCtdMNModule_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("CtdMNModule_side").contentDocument;
	contdoc = doc.getElementById("CtdMNModule_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("CtdMNModule_side").contentDocument;
	contdoc = doc.getElementById("CtdMNModule_cont").contentDocument;
	hdrdoc = contdoc.getElementById("CtdMNModule_hdr").contentDocument;
	tbldoc = contdoc.getElementById("CtdMNModule_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("CtdMNModule_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtdMNModule", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModCtdMNModule", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModCtdMNModule", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCtdMNModule", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModCtdMNModule", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtdMNModule", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtdMNModule", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtdMNModule", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtdMNModule", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtdMNModule", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtdMNModule", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtdMNModule", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtdMNModule", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCtdMNModule", "TcoMrefWidth"));
		var TcoFctWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCtdMNModule", "TcoFctWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModCtdMNModule", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModCtdMNModule", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModCtdMNModule
		rownode = getRownode(srcdoc, "ListWdbeQModCtdMNModule", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModCtdMNModule", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModCtdMNModule", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModCtdMNModule", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("CtdMNModule", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("CtdMNModule_side").setAttribute("height", "" + (height+55));
	doc.getElementById("CtdMNModule_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("CtdMNModule_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("CtdMNModule_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModCtdMNModule", "ListWdbeQModCtdMNModule", retrieveTi(srcdoc, "TagWdbeModCtdMNModule", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModCtdMNModule", "TxtRecord2"),
				["Mref","Fct",""],
				["mref","fct","fct2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtdMNModule", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtdMNModule", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCtdMNModule", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModCtdMNModule", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModCtdMNModule", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModCtdMNModuleData", scrJref, "StgIacQryWdbeModCtdMNModule");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtdMNModule", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCtdMNModule", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModCtdMNModule", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCtdMNModule", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCtdMNModule", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModCtdMNModuleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "ContInfWdbeModCtdMNModule", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "StatAppWdbeModCtdMNModule", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "StatShrWdbeModCtdMNModule", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "StgIacWdbeModCtdMNModule", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "TagWdbeModCtdMNModule", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "ListWdbeQModCtdMNModule", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "StatAppQryWdbeModCtdMNModule", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "StatShrQryWdbeModCtdMNModule", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCtdMNModuleData", "StgIacQryWdbeModCtdMNModule", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModCtdMNModuleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModCtdMNModuleData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("CtdMNModule");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModCtdMNModuleData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
