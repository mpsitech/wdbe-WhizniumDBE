/**
  * \file PnlWdbeModCor1NImbuf.js
  * web client functionality for panel PnlWdbeModCor1NImbuf
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWdbeModCor1NImbuf", "srefIxWdbeVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWdbeModCor1NImbuf", "srefIxWdbeVExpstate", "mind");

	getRecwnd().changeHeight("Cor1NImbuf", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Cor1NImbuf_side").setAttribute("height", "30");
	doc.getElementById("Cor1NImbuf_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Cor1NImbuf_side").src = "./PnlWdbeModCor1NImbuf_aside.html";
	doc.getElementById("Cor1NImbuf_cont").src = "./PnlWdbeModCor1NImbuf_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWdbeModCor1NImbuf", "srefIxWdbeVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWdbeModCor1NImbuf", "srefIxWdbeVExpstate", "regd");

	getRecwnd().changeHeight("Cor1NImbuf", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Cor1NImbuf_side").setAttribute("height", "255");
	doc.getElementById("Cor1NImbuf_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Cor1NImbuf_side").src = "./PnlWdbeModCor1NImbuf_bside.html";
	doc.getElementById("Cor1NImbuf_cont").src = "./PnlWdbeModCor1NImbuf_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Cor1NImbuf_side").contentDocument;
	contdoc = doc.getElementById("Cor1NImbuf_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Cor1NImbuf_side").contentDocument;
	contdoc = doc.getElementById("Cor1NImbuf_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Cor1NImbuf_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Cor1NImbuf_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Cor1NImbuf_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCor1NImbuf", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWdbeModCor1NImbuf", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWdbeVExpstate = retrieveSi(srcdoc, "StatAppWdbeModCor1NImbuf", "srefIxWdbeVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWdbeModCor1NImbuf", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWdbeModCor1NImbuf", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCor1NImbuf", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCor1NImbuf", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCor1NImbuf", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCor1NImbuf", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCor1NImbuf", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCor1NImbuf", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCor1NImbuf", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCor1NImbuf", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWdbeModCor1NImbuf", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWdbeModCor1NImbuf", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWdbeModCor1NImbuf", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWdbeQModCor1NImbuf
		rownode = getRownode(srcdoc, "ListWdbeQModCor1NImbuf", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWdbeModCor1NImbuf", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWdbeModCor1NImbuf", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWdbeModCor1NImbuf", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Cor1NImbuf", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Cor1NImbuf_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Cor1NImbuf_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Cor1NImbuf_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Cor1NImbuf_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WdbeModCor1NImbuf", "ListWdbeQModCor1NImbuf", retrieveTi(srcdoc, "TagWdbeModCor1NImbuf", "TxtRecord1"), retrieveTi(srcdoc, "TagWdbeModCor1NImbuf", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCor1NImbuf", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCor1NImbuf", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWdbeModCor1NImbuf", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWdbeModCor1NImbuf", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWdbeModCor1NImbuf", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWdbeModCor1NImbufData", scrJref, "StgIacQryWdbeModCor1NImbuf");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCor1NImbuf", "srefIxWdbeVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWdbeModCor1NImbuf", "firstcol"));

	setSi(srcdoc, "StatAppQryWdbeModCor1NImbuf", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWdbeModCor1NImbuf", "srefIxWdbeVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWdbeModCor1NImbuf", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWdbeModCor1NImbufDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "ContInfWdbeModCor1NImbuf", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "StatAppWdbeModCor1NImbuf", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "StatShrWdbeModCor1NImbuf", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "StgIacWdbeModCor1NImbuf", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "TagWdbeModCor1NImbuf", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "ListWdbeQModCor1NImbuf", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "StatAppQryWdbeModCor1NImbuf", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "StatShrQryWdbeModCor1NImbuf", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWdbeModCor1NImbufData", "StgIacQryWdbeModCor1NImbuf", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWdbeModCor1NImbufData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWdbeModCor1NImbufData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Cor1NImbuf");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWdbeModCor1NImbufData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
